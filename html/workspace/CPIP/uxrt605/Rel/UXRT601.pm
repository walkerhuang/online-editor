...
sub init_common {
    ...
    $rel->{vers}='6.0.500.0';
    $rel->{basevers}=[ '6.0.400','6.0.300','6.0.200','6.0.100' ]; # Decreasing order necessary
    $rel->{gavers}='6.0.100.000';
    $rel->{prior_rp_title}{UXRT603}="6.0.3";
    $rel->{titlevers}='6.0.5';
    $rel->{basetitlevers}='6.0.1';
    $rel->{pkgs}=1 if(Cfg::opt('base_path'));
    $rel->{nowebinstaller}=1;
    $rel->{patches} = 1;
    ...
    $rel->{type}="RP";
    $rel->{rp}=1;
    $rel->{patchtype}="rppatches";
    $rel->{base_releases}=[ qw(UXRT603) ];
    $rel->{currentrelease}="UXRT605";
    $rel->{license_ver}="6.0";
    ...
    my %base_menuprods = $cpic->base_support_prods();
    if ($cpic->{base_path_release} && $base_menuprods{$cpic->{base_path_release}}) {
        $rel->{menuprods} = $base_menuprods{$cpic->{base_path_release}};
    }

    $rel->{upgradeprods} = [ qw(SFRAC60 SFSYBASECE60 SFCFSHA60 SFHA60 SF60 VCS60 VM60 DMP60 FS60) ];
    $rel->{allpkgs}=[ qw(VRTSperl514 VRTSvlic32 VRTSsfcpi601 VRTSspt60) ];
    $rel->{minpkgs}=[ qw(VRTSperl514 VRTSvlic32 VRTSsfcpi601) ];
    $rel->{recpkgs}=[ qw(VRTSperl514 VRTSvlic32 VRTSsfcpi601 VRTSspt60) ];
    $rel->{cpipkg}='VRTSsfcpi601';

    $rel->{ru_prod}=[ qw(SFRAC60 SVS60 SFSYBASECE60 SFCFSHA60 SFHA60 VCS60) ];
    $rel->{ru_unkernelpkgs}=[ qw(VRTSvcs60 VRTSvcsag60 VRTSvcsea60 VRTScavf60 VRTSvbs60) ];
    #For some products, the mainpkg is kernelpkg, which will lead the product version detection inaccurate after RU phase1. Adding them in the ru_noupgrademsg_prod list, so the version message will not be printed in RU phase2.
    $rel->{ru_noupgrademsg_prod}=[ qw(SFRAC SVS) ];
    $rel->{ru_version}='5.1';
    $rel->{bucket_version}='5.1';
    $rel->{pkgset_version}='5.1';

    $rel->{installonupgradepkgs}=[ qw(VRTSsfcpi601) ];
    $rel->{obsoletedsfcpipkgs}=[ qw(VRTSsfcpi602 VRTSsfcpi604) ];
...
sub init_args {
    ...
    $rel->{arguments} = $arguments = {
        ...
        'jumpstart' => {
            ...
            'handler' => \&Rel::UXRT601::Common::process_jumpstart_arg,
        },
        'flash_archive' => {
            ...
            'handler' => \&Rel::UXRT601::Common::process_flash_archive_arg,
        },
        'ai' => {
            ...
            'handler' => \&Rel::UXRT601::Common::process_ai_arg,
        },
        'kickstart' => {
            ...
            'handler' => \&Rel::UXRT601::Common::process_kickstart_arg,
        },
        'yumgroupxml' => {
            ...
            'handler' => \&Rel::UXRT601::Common::process_yumgroupxml_arg,
        },
        'nim' => {
            ...
            'handler' => \&Rel::UXRT601::Common::process_nim_arg,
        },
        ...
        'ignite' => {
            ...
            'handler' => \&Rel::UXRT601::Common::process_ignite_arg,
        },
        ...
        'security' => {
            ...
            'handler' => \&Rel::UXRT601::Common::process_security_arg,
        },
        'securityonenode' => {
        ...
        'fencing' => {
            ...
            'handler' => \&Rel::UXRT601::Common::process_fencing_arg,
        },
        'configcps' => {
...
sub process_security_arg {
    ...
    if (Cfg::opt('securityonenode') && $cfg->{systems}) {
        $msg=Msg::new("The securityonenode option can only be used on the local system without specifying the systems.");
        $msg->die();
    }
    return 0;
}
...
sub config_without_reboot_message {
    ...
    if ((Cfg::opt(qw(install)) && !Cfg::opt('configure')) || (Cfg::opt(qw(patchupgrade)) && !$rel->is_patchupgrade_configured())) {
...
sub reboot_messages {
    ...
    $reboot_msg.=$msg->{msg};
    #e3365378:prompt user to start after patchupgrade and rollback.
...
sub rup2_prod_version_sys {
    ...
    if (Cfg::opt('upgrade_nonkernelpkgs')) {
        ...
        $index=EDRu::arrpos('VCS60',@{$sys->{upgradeprod}});
...
sub upgrade_sfha_prods_mix_check {
    ...
    my (@sys_prod_map,$cpic,$edr,$prod,$prev_prod,$prev_prodvers,$msg,$sys,$sys_prod_map,$prodname,
        ...
        $upgrade_status,$index,$lprod,$prodobj,$cprod_installed,$upgrade_from_vers,$cv,$install_cmd,$upgrade_cmd);
    ...
    for my $sys (@{$cpic->{systems}}) {
        ...
        if ($prodname) {
            ...
            if ($nocurrentvers_flag) {
                ...
                #if upgrade from 6.0.0XX to 6.0.X00, we should not do patchupgrade.
                $patchupgrade_flag = 1 if (!EDRu::compvers($upgrade_from_vers,$cmpvers,2) && (EDRu::compvers($upgrade_from_vers, '6.0.100.000', 3) != 2) && $rel->{patches}) 
            }
        ...
        if (($upgrade_status eq 'prohibited_upgrade') && $nocurrentvers_flag) {
            if (Cfg::opt('base_path')) {
                $upgrade_cmd = "installmr -base_path <path_to_base> -upgrade";
                $install_cmd = "installmr -base_path <path_to_base> -install";
            } else {
                $upgrade_cmd = "installer -upgrade";
    ...
    if (Cfg::opt('precheck')) {
        ...
        if (!$cprod_installed) {
            if (!$cprod) {
                $msg=Msg::new("Cannot upgrade because there is no SFHA product installed on your system.");
            } else {
...
sub upgrade_sfha_check_sys {
    ...
    if (!EDRu::compvers($sfvers,$cmpvers,4) && EDRu::compvers($vcsvers,$cmpvers,4)) {
        if ($rel->{noconfig_vcs}) {
            $msg = Msg::new("Entered systems have different versions of subsets of SFHA installed:\nSF $sfvers and VCS $vcsvers are installed on $sys->{sys}.\n$rel->{sfha_conflict_msg}Systems running with different versions of subsets of SFHA must be upgraded independently");
        $sys->push_error($msg);
            set_value_allsys('stop_checks',1);
            return 1;
        }
        ...
        $sys->push_warning($msg);
        $rel->{sfha_conflict_msg} .= Msg::new("SF $sfvers and VCS $vcsvers are installed on $sys->{sys}.\n")->{msg};
        return 1;
    }
...
sub applicationha_sys {
    ...
    if ($vers) {
        unless (EDRu::compvers($vers, '6.0.100.000', 3) == 1 && EDRu::compvers($vers, '6.1.000.000', 3) == 2) {
...
sub get_local_script_version_suffix {
    my $rel=shift;
    my $suffix;

    # check 'rollback' when get local script version
    if (Cfg::opt('rollback')) {
        $suffix = ($rel->{rp}) ? $rel->{basetitlevers} : $rel->{titlevers};
    } else {
        $suffix = $rel->{titlevers};
    }
    $suffix =~ s/\s//mxg;
    $suffix =~ s/\.//mxg;
    ...
    if ($apphavers) {
        ...
        if (Cfg::opt(qw(precheck install upgrade patchupgrade))) {
            ...
            if ($rel->has_vcs($prod->{prod})) {
                ...
                # Later decide to upgrade VRTSsfmh for e2760175.
                #push(@{$sys->{donotupgradepkgs}},$mhpkg->{pkgi});
...
sub default_systemnames {
    ...
    if(Cfg::opt(qw(rolling_upgrade upgrade_kernelpkgs upgrade_nonkernelpkgs ))){
        ...
        return $dfs;
    } elsif (Cfg::opt(qw(patchupgrade upgrade rollback precheck))) {
...
sub ru_offline_parallel_groups {
    ...
    $msg->right_done;
    $msg->display_right() if(Obj::webui());
    $msg=Msg::new("Waiting for service group(s) to come offline on the sub-cluster");
    $msg->left;
    $msg->display_left($msg) if(Obj::webui());
    ...
    if ($verifiedhash) {
        ...
        Msg::n();
        $msg=Msg::new("The following parallel service group(s) are not in OFFLINE state:");
        $msg->warning();
        for $sys (keys%{$parallelhash}) {
            $sys=Obj::sys($sys);
        ...
        Msg::n();
        $msg=Msg::new("There're still parallel service group(s) online in the sub-cluster. It is recommended to solve the issue and restart rolling upgrade.");
        $webmsg .="$msg->{msg}\n";
        $msg->printn();
...
sub basic_precheck_sys {
    ...
    my %base_support_prods = $cpic->base_support_prods();
    if (Cfg::opt('upgrade') && $cpic->{base_path_release} && $base_support_prods{$cpic->{base_path_release}}) {
        if(!EDRu::inarr($cprod, @{$base_support_prods{$cpic->{base_path_release}}})) {
            $msg=Msg::new("$prod->{abbr} upgrade from $cpic->{base_path_release} base release is not supported");
            $sys->push_error($msg);
            return 0;
        }
    }
...
sub prod_menu {
    ...
    for my $mprodi (@{$rel->{menuprods}}) {
        ...
        if (($prod->{proddir} && (-d "$cpic->{mediapath}/$prod->{proddir}")) ||
            ($cfg->{opt}{base_path} && $prod->{proddir} && (-d "$cfg->{opt}{base_path}/$prod->{proddir}")) ||
            ($rel->{rp} && !$cfg->{opt}{base_path})) {
            push(@mprods,$mprodi);
        }
    }
...
sub ru_prod_version_sys {
    ...
    } elsif (Cfg::opt("patchupgrade") && (!$rel->{pkgs})) {
...
sub ru_check_newnode {
    ...
    if (Obj::webui()) {
        while (1){
            ...
            my $sys=($Obj::pool{"Sys::$sysname"}) ? Obj::sys($sysname) : Sys->new($sysname);
            # the first machine need Release Compatibility check.
            next unless($rel->ru_precheck_sys($sys,1));
...
_EOF_
    ...
    for my $prodi ( sort(@{$prods}) ) {
        ...
        $rpmlist = join(' ',@pkgs);
        @pkgs = ();
        $ver=$rel->get_local_script_version_suffix();
...
sub ignite {
    ...
    if (EDR::cmdexit()) {
        ...
        # add post_load_cmd = "/opt/VRTS/install/bin/UXRT601/add_install_scripts" in core_media_cfg
        ...
        for my $line (@conts) {
            if ($line=~/sd_software_list/) {
                $filecont.="    ";
                $filecont.="post_load_cmd = \"/opt/VRTS/install/bin/UXRT601/add_install_scripts\"\n";
            }
            $filecont.=$line."\n";
        }
    ...
    my @os_media_list;
    if ($localsys->exists($indexfile) && (!$rel->{rp})) {
...
sub nim {
    ...
    my ($bff_dir,$bnd_file,$bnd_name,$lpp_source,$lpp_location,$lpplist,$master,$rp,$tmp_bnd_file,$vx_nim_path);
    ...
    @pkgs=();
    @patches=();
    # figure out the list of packages/patches for current product
    for my $pkgi (@{$prod->allpkgs}) {
        $pkg=$cpic->pkg($pkgi);
        if ($pkg->{file}) {
            $lpplist .= "I:$pkg->{pkg}\n";
...
@Rel::UXRT601::Linux::ISA = qw(Rel::UXRT601::Common);

sub init_plat {
    my ($rel) = @_;

    $rel->{upgradeprods} = [ qw(SFRAC60 SFCFSRAC60 SFSYBASECE60 SFCFSHA60 SFHA60
...
sub platvers_sys {
    ...
    my ($localsys,$mediapath,$tmpdir,$vmware,$testVMware,@vmware_tool_scripts,$vmware_script,$state);
    $sys=Obj::sys($sys) unless (ref($sys) eq 'Sys');
    return '' if ($sys->{stop_checks});
    return '' if (Cfg::opt('ignorechecks'));
    ...
    if ($distro eq 'RHEL') {
        ...
        #On RHEL6.x, install/upgrade VCS6.0.4 + VCS 6.0.5 is not allowed
        if ($cpic->{base_path_release} eq '6.0.4') {
            $msg = Msg::new("Kernel Release $kstring found on $sys->{sys} is not supported if base release version is 6.0.4. Only Oracle Linux 6.4 with UEK kernel is supported.");
            $sys->push_error($msg);
            return '';
        }
        if ($kstring =~ /el6/m) {
        ...
        if ($f[0]>2) {
            ...
            if ((defined($patchlevel)) && ($patchlevel>3)){
                $msg=Msg::new("SuSE 11 SP1, SP2 and SP3 are the recommended platforms for the release on $sys->{sys}.");
                $sys->push_warning($msg);
                return '';
            }
        } else {
        ...
        if ($kstring =~ /el6/m) {
            ...
            if ($prod->{prod} ne 'VCS' && $kstring =~ /uek/m && $cpic->{base_path_release}) {
                $msg = Msg::new("$prod->{prod} is not supported on OL6.x UEK2 kernel");
                $sys->push_error($msg);
                return '';
                # On OL6.4/6.5(Redhat Compatible Kernel), install/upgrade VCS6.0.4 + VCS 6.0.5 is not allowed
            } elsif ($kstring !~ /uek/m && $cpic->{base_path_release} eq '6.0.4' && $kstring =~ /^2\.6\.32-[34].*/m) {
                $msg = Msg::new("VCS 6.0.4 is not supported on $kstring kernel, please reboot into UEK2 kernel or use 6.0.1/6.0.2 base media then run installer again");
                $sys->push_error($msg);
                return '';
            }
            if ($kstring =~ /uek/m && $kstring !~ /^2\.6\.39-400.*el6uek/m) {
            ...
            if ($kstring !~ /uek/m && $cpic->{base_path_release}) {
                if ($kstring =~ /^2\.6\.32-[12].*/m) {
                    ...
                    if ($cpic->{base_path_release} ne '6.0.4') {
                        ...
                        #On OL6.1~6.3(Redhat Compatible Kernel), install/upgrade 6.0.4+6.0.5 should block 
                        $msg = Msg::new("VCS 6.0.4 is not supported on $kstring kernel, please use 6.0.1/6.0.2 base media then run installer again");
                        $sys->push_error($msg);
                        return '';
                    }    
...
sub has_vcsvmw {
    ...
    for my $sys(@{CPIC::get('systems')}) {
        return 0 if (!$sys->{pkgvers}{'VRTSvcsvmw'});
    }
    return 1;
}
...
@Rel::UXRT601::RHEL5x8664::ISA = qw(Rel::UXRT601::Linux);
...
sub init_padv {
    ...
    $rel->{ru_prod}=EDRu::arrdel($rel->{ru_prod}, qw(SFSYBASECE60));
    $rel->{platvers}=[ qw(2.6.18-*) ];
    $rel->{basevers}=[ '6.0.300','6.0.200','6.0.100' ];
...
VRTSdbed:154,0,0,0
VRTSperl:0,0,0,0
VRTSsfcpi601:325,0,0,0
VRTSspt:863,0,0,0
VRTSvcs:502,1723,0,3
VRTSvcsag:81232,0,0,1
VRTSvcsea:6,0,0,0
VRTSvxfen:11,0,0,7
VRTSvxfs:864,49,0,57
VRTSvxvm:12,258,0,305
VRTSacclib:0,0,0,0
VRTSmq:0,0,0,0
VRTSsapwebas:0,0,0,0
VRTSvcsvmw:0,0,0,0
_PATCHSPACE_
    return;
}
...
@Rel::UXRT601::RHEL5ppc64::ISA = qw(Rel::UXRT601::Linux);
...
@Rel::UXRT601::RHEL6x8664::ISA = qw(Rel::UXRT601::Linux);

sub init_padv {
    my ($rel) = @_;

    $rel->{upgradeprods} = [ qw(SFRAC60 SFCFSRAC60 SVS60 SFSYBASECE60 SFCFSHA60 SFHA60
                                SF60 VCS60 VM60 DMP60 FS60) ];
...
VRTSsfcpi601:325,0,0,0
VRTSspt:863,0,0,0
VRTSvcs:355,1715,0,3
VRTSvcsag:73747,0,0,1
VRTSvcsea:6,0,0,0
VRTSvxfen:18,0,0,3
VRTSvxfs:0,54,0,5431
VRTSvxvm:19,228,0,293
VRTSacclib:0,0,0,0
VRTSmq:0,0,0,0
VRTSsapwebas:0,0,0,0
VRTSvcsvmw:0,0,0,0
...
@Rel::UXRT601::RHEL6ppc64::ISA = qw(Rel::UXRT601::Linux);
...
@Rel::UXRT601::SLES10x8664::ISA = qw(Rel::UXRT601::Linux);
...
VRTSsfcpi601:325,0,0,0
VRTSspt:863,0,0,0
VRTSvcs:515,1724,0,3
VRTSvcsag:81329,0,0,1
VRTSvcsea:5,0,0,0
VRTSvxfen:65,0,0,7
VRTSvxfs:854,77,0,0
VRTSvxvm:12,75,0,303
VRTSacclib:0,0,0,0
VRTSmq:0,0,0,0
VRTSsapwebas:0,0,0,0
VRTSvcsvmw:0,0,0,0
...
@Rel::UXRT601::SLES10ppc64::ISA = qw(Rel::UXRT601::Linux);
...
@Rel::UXRT601::SLES11x8664::ISA = qw(Rel::UXRT601::Linux);
...
VRTSsfcpi601:325,0,0,0
VRTSspt:863,0,0,0
VRTSvcs:351,1725,0,3
VRTSvcsag:82960,0,0,1
VRTSvcsea:5,0,0,0
VRTSvxfen:25,0,0,2
VRTSvxfs:1226,0,0,109
VRTSvxvm:12,56,0,374
VRTSacclib:0,0,0,0
VRTSmq:0,0,0,0
VRTSsapwebas:0,0,0,0
VRTSvcsvmw:0,0,0,0
...
@Rel::UXRT601::SLES11ppc64::ISA = qw(Rel::UXRT601::Linux);
...
@Rel::UXRT601::AIX::ISA = qw(Rel::UXRT601::Common);
...
sub init_plat {
    ...
    $rel->{ru_prod}=EDRu::arrdel($rel->{ru_prod}, qw(SVS60 SFSYBASECE60));
    $rel->{menuprods}=EDRu::arrdel($rel->{menuprods}, qw(SVS60 SFSYBASECE60));
    $rel->{obsoleted_but_still_support_pkgs} = [qw(VRTSat50)];
    $rel->{pkgsspace}=<<"_PKGSPACE_";
VRTSamf:2824,8393,0,7
VRTSaslapm:0,875,0,856
VRTScavf:797,66,0,2
VRTScps:32436,0,0,2
VRTSdbac:3931,3022,0,515
VRTSdbed:135715,0,0,5
VRTSfsadv:35142,0,0,3
VRTSfssdk:3162,0,0,0
VRTSgab:633,3279,0,76
VRTSglm:32,394,0,16
VRTSgms:0,160,0,0
VRTSllt:1141,546,0,228
VRTSob:94483,0,0,297
VRTSodm:490,577,0,5
VRTSperl:139259,0,0,0
VRTSsfcpi601:4842,0,0,0
VRTSsfmh:131705,0,0,0
VRTSspt:25709,0,0,0
VRTSvbs:62587,0,0,1
VRTSvcs:361521,19968,0,248
VRTSvcsag:20860,0,0,45
VRTSvcsea:6821,0,0,57
VRTSveki:0,130,0,0
VRTSvlic:70,0,0,1398
VRTSvxfen:686,1693,0,994
VRTSvxfs:31770,11569,0,3057
VRTSvxvm:8100,207810,0,37197
_PKGSPACE_

    $rel->{patchesspace}=<<"_PATCHSPACE_";
VRTSamf:1710,13189,0,0
VRTScavf:10,1267,0,0
VRTSdbac:3973,3022,0,514
VRTSdbed:230,136892,0,0
VRTSgab:634,3313,0,75
VRTSglm:31,394,0,17
VRTSllt:1142,546,0,227
VRTSodm:529,581,0,4
VRTSperl:0,85492,0,0
VRTSsfcpi601:515,5407,0,0
VRTSvcs:2619,391089,0,0
VRTSvcsag:131,21876,0,0
VRTSvcsea:0,4668,0,0
VRTSvxfen:13,3674,0,4
VRTSvxfs:0,45934,0,35
VRTSvxvm:122,249088,0,5971
_PATCHSPACE_
...
# AIX
sub detect_prior_rp_sys {
    my($rel,$cpic,$sys,$patch,$patchi,$patch_vers,$release,$patchvers_absent,$pkg,$pkgi,$prod,$vers,$padv,$level_output,$patchname,$patchvers,$prodpkgs);
    ($rel,$sys)=@_;
    $cpic=Obj::cpic();
    $prod=$cpic->prod;
    $prodpkgs=$prod->allpkgs;
    $padv=$sys->padv;
    foreach $release (@{$rel->{base_releases}}) {
        $patchvers_absent=0;
        for $pkgi (@{$prodpkgs}) {
            $pkg = $sys->pkg($pkgi);
            next unless ($pkg->version_sys($sys));
            for $patchi (@{$pkg->patches_allvers($release)}) {
                $patch = Obj::patch($patchi,$rel->{padv});
                ($patchname,$patchvers)=split("-",$patch->{patchid},2);
                $patchvers =~ s/_/\./g;
                unless ($sys->cmd("_cmd_lslpp -lca $patchname 2>/dev/null | _cmd_grep $patchvers")) {
                    $patchvers_absent++;
                }
            }
        }
        return $release unless ($patchvers_absent);
    }
    return "";
}
...
sub platvers_sys {
    ...
    if ($sys->{platvers} eq '6.1') {
        if ($tl < 5) {
            $msg=Msg::new("Cannot perform any task for $cprodabbr on system $sys->{sys} since its oslevel is 6.1 TL $tl. Upgrade the system to 6.1 TL5 or later to install $cprodabbr");
            $sys->push_error($msg);
            return '';
        }
    }
...
@Rel::UXRT601::HPUX::ISA = qw(Rel::UXRT601::Common);
...
# HPUX
sub detect_prior_rp_sys {
    my($rel,$cpic,$sys,$patch,$patchi,$patch_vers,$release,$patchvers_absent,$pkg,$pkgi,$prod,$vers,$padv,$prodpkgs);
    ($rel,$sys)=@_;
    $cpic=Obj::cpic();
    $prod=$cpic->prod;
    $prodpkgs=$prod->allpkgs;
    ...
    foreach $release (@{$rel->{base_releases}}) {
        ...
        for $pkgi (@{$prodpkgs}) {
            ...
            for $patchi (@{$pkg->patches_allvers($release)}) {
                $patch = Obj::patch($patchi,$rel->{padv});
                # fix 2425892/2914029 VRTSperl patch PHCO_42213 is only for ia, should skip for pa machine.
                next if ($patch->{nopa} && ($sys->{arch}!~/ia/));
                unless ($padv->rp_patch_version_sys($sys,$patch)) {
                    $patchvers_absent++;
                }
            }
        }
...
@Rel::UXRT601::HPUX1131::ISA = qw(Rel::UXRT601::HPUX);
...
_PKGSPACE_
    ...
    $rel->{patchesspace}=<<"_PATCHSPACE_";
PVCO_03982:79249,0,0,0
PVCO_04004:10190,65942,0,61229
PVCO_04005:12135,302248,0,347500
PVCO_04007:254485,15469,0,249
PVCO_04008:5886,1,0,58
PVCO_04009:9549,0,0,52
PVCO_04020:87,0,0,0
PVCO_04023:32991,0,0,0
PVCO_04024:5373,0,0,0
PVCO_04032:827,66,0,7
PVKL_04003:0,27456,0,0
PVKL_04006:0,23231,0,198
PVKL_04031:6786,815,0,940
PVNE_04016:1221,1723,0,553
PVNE_04017:3213,1606,0,7
PVNE_04021:218,2154,0,192
PVNE_04022:911,1315,0,798
_PATCHSPACE_
    return;
}

package Rel::UXRT601::SunOS;
@Rel::UXRT601::SunOS::ISA = qw(Rel::UXRT601::Common);

sub init_plat {
    my ($rel) = @_;

    $rel->{driververs}='6.0.1';
...
sub detect_prior_rp_sys {
    ...
    foreach $release (@{$rel->{base_releases}}) {
        ...
        for $pkgi (@{$prodpkgs}) {
            ...
            for $patchi (@{$pkg->patches_allvers($release)}) {
                $patch = Obj::patch($patchi,$rel->{padv});
                unless ($padv->rp_patch_version_sys($sys,$patch->{patchid})) {
                    $patchvers_absent++;
                }
            }
        }
...
sub platvers_sys {
    ...
    if (Cfg::opt(qw/install upgrade patchupgrade precheck/)) {
        ...
        if ($sys->{platvers} eq '5.10') {
            ...
            if (!$osupdatelevel) {
                ...
                if ($sru_level == 0) {
                    $msg=Msg::new("Solaris 11 GA is installed on $sys->{sys}. It is strongly recommended to install Solaris 11 Support Repository Update(SRU) 1 or later");
                    $sys->push_warning($msg);
                }
            }
        }
    ...
    if (!Cfg::opt('rootpath') && $sys->{padv}=~/Sol10/ &&
        $sys->{zone} && Cfg::opt(qw(precheck install upgrade patchupgrade uninstall rollback))) {
...
sub completion {
    ...
    return unless (Cfg::opt(qw(precheck install upgrade patchupgrade uninstall configure)));
...
@Rel::UXRT601::SolSparc::ISA = qw(Rel::UXRT601::SunOS);
...
VRTSsfcpi601:7132,0,0,0
VRTSsfmh:127489,0,0,0
VRTSspt:22679,0,0,0
VRTSsvs:167186,0,0,0
VRTSvbs:60061,0,0,1
...
_PKGSPACE_
    ...
    $rel->{patchesspace}=<<"_PATCHSPACE_";
148481-03:0,74,43480,78
148490-07:66,4357,178450,1219
148492-02:180,2855,248612,1
148496-02:195,0,6199,0
148497-02:0,0,4372,0
148498-02:3276,41,2121,0
149691-02:10,0,673,0
149695-02:9,0,1208,11
149696-02:203,0,107294,0
149699-02:599,0,53810,0
149702-02:2050,0,3001,0
149816-02:52,0,1291,0
150706-01:0,0,803,29
150710-01:0,0,828,0
150720-01:0,0,911,0
_PATCHSPACE_
    ...
    # VRTSvxvm package size + 3*(wc -c /var/sadm/pkg/VRTSvxvm/save/148490-07/undo.Z)/1024 on /var
    $rel->{patchbackoutspace}=[ qw/7175 187182 243146 32218/ ];
    return;
}
...
@Rel::UXRT601::Solx64::ISA = qw(Rel::UXRT601::SolSparc);
...
_PKGSPACE_
    ...
    $rel->{patchesspace}=<<"_PATCHSPACE_";
148482-03:0,74,43480,78
148491-07:66,4357,178450,1219
148493-02:180,2855,248612,1
148494-02:195,0,6199,0
148495-02:0,0,4372,0
149692-02:3276,41,2121,0
149693-02:10,0,673,0
149694-02:9,0,1208,11
149697-02:203,0,107294,0
149698-02:599,0,53810,0
149703-02:2050,0,3001,0
149817-02:52,0,1291,0
150707-01:0,0,803,29
150718-01:0,0,828,0
150721-01:0,0,911,0
_PATCHSPACE_
    # Due to incident 3394711, we need to define patch backout space for Solaris
    ...
    # Just record the largest patch size required for rollback, which is VRTSvxvm
    # Algorithm of this calculation:
    # VRTSvxvm package size + 3*(wc -c /var/sadm/pkg/VRTSvxvm/save/148491-07/undo.Z)/1024 on /var
    $rel->{patchbackoutspace}=[ qw/6428 184795 246588 40919/ ];
    return;
}

package Rel::UXRT601::Sol11sparc;
@Rel::UXRT601::Sol11sparc::ISA = qw(Rel::UXRT601::SunOS);
...
sub init_padv {
    ...
    $rel->{langpkgs}=[  qw(VRTSjacav60 VRTSjacs60 VRTSjacse60 VRTSjadba60
                           VRTSjadbe60 VRTSjafs60 VRTSjaodm60 VRTSjavm60
                           VRTSzhvm60 VRTSmulic32) ];

    $rel->{platvers}=[ qw(5.11) ];
    my $msg = Msg::new("Solaris 11 Support Repository Update(SRU) 1 or later (64 bit only)");
    $rel->{platreqs}=[ $msg->{msg} ];
    $rel->{upgradevers}=[ qw(6.0.10) ];
    $rel->{prods}=EDRu::arrdel($rel->{prods}, qw(SFSYBASECE60));
    $rel->{menuprods}=EDRu::arrdel($rel->{menuprods}, qw(SFSYBASECE60));
    $rel->{pkgsetprods}=EDRu::arrdel($rel->{pkgsetprods}, qw(SFSYBASECE60));
    $rel->{ru_prod}=EDRu::arrdel($rel->{ru_prod}, qw(SFSYBASECE60));
    $rel->{upgradeprods}=EDRu::arrdel($rel->{upgradeprods}, qw(SFSYBASECE60));
    $rel->{pkgsspace}=<<"_PKGSPACE_";
...
VRTSamf:3306,0,0,3
VRTScavf:31,1,0,0
VRTSdbed:84597,0,0,2
VRTSgab:0,0,0,0
VRTSodm:203,0,0,0
VRTSperl:0,0,2,0
VRTSsfcpi601:0,0,3,0
VRTSvcs:64281,12190,0,27
VRTSvcsag:2490,1,3,5
VRTSvcsea:0,0,3,0
VRTSvxfen:14,799,0,0
VRTSvxfs:0,0,2,292
VRTSvxvm:0,19306,0,0
_PATCHSPACE_
    return;
}
...
sub preremove_sys {
    ...
    if (!$sys->{sru_level}) {
        ...
        if (EDRu::inarr('VRTSvxvm60', @{$sys->{uninstallpkgs}})) {
...
@Rel::UXRT601::Sol11x64::ISA = qw(Rel::UXRT601::Sol11sparc);
...
VRTSamf:3236,0,0,3
VRTScavf:23,1,0,0
VRTSdbed:90165,0,0,2
VRTSgab:0,0,0,0
VRTSodm:207,0,0,0
VRTSperl:0,0,2,0
VRTSsfcpi601:727,0,3,0
VRTSvcs:0,2728,0,27
VRTSvcsag:127,1,3,5
VRTSvcsea:0,0,3,0
VRTSvxfen:14,837,0,0
VRTSvxfs:2950,0,2,649
VRTSvxvm:0,16390,0,0
_PATCHSPACE_
    return;
}
