...
sub licensing {
    ...
    while(1) {
        ...
        if (Cfg::opt(qw(install license upgrade)) || (!$licensed && Cfg::opt(qw(start patchupgrade configure)))) {
            ...
            $rel->cli_license_option() unless ((Cfg::opt(qw(upgrade patchupgrade)) && !Cfg::opt(qw(base_path))) || 
                                               (($prod->{prod} eq 'SF') && (Cfg::opt('prodmode') eq 'SF Basic'))||
                                               ($licensed && Cfg::opt(qw(base_path))));
            $rt = '';
...
sub cli_ask_license_key {
    my ($rel,$sys) = @_;
    my ($edr,$cpic,$msg,$cfg,$prod,$key,$license_status,$help,$prodname,$registered_keys);
    ...
    if (!$prod->{basic}) {
         ...
         while (1) {
             ...
             $msg->printn;
             if (Cfg::opt('makeresponsefile') && (Cfg::opt('responsefile_keys') || $license_status->{all_licensed})) {
                 if ($rel->cli_ask_additional_license($prod) eq 'N') {
                     Cfg::unset_opt('responsefile_keys');
                     return $edr->{msg}{back};
                 }
             }
             ...
             if (Cfg::opt('makeresponsefile') && $key) {
                 ...
                 ($key ne $edr->{msg}{back}) ? Cfg::set_opt('responsefile_keys') : Cfg::unset_opt('responsefile_keys'); 
                 last;
             }
             last if ($key && (!EDRu::inarr($key, @{$cfg->{keys}{$sys->{sys}}})));
...
sub license_products_vxkeyless {
    ...
    if (Cfg::opt(qw(install configure patchupgrade upgrade start license))) {
...
sub vxkeyless_register_sys {
    ...
    $vxkeyless='';
    $vxkeyless=$sys->cmd('_cmd_vxkeyless display 2>/dev/null') if ($sys->exists("/opt/VRTSperl/bin/perl"));
    ...
    if (Cfg::opt(qw(upgrade patchupgrade)) && !Cfg::opt('base_path')) {
