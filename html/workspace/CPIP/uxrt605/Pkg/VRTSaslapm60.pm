...
# Due to incident 3390525
# VRTSvxvm is in 'installed' state and dependency for VRTSaslapm couldn't be reached.
# We must use -x enforce_dependencies=false in bundle install in 6.0.5
sub preinstall_sys {
    my ($pkg,$sys)=@_;
    $pkg->{nodeps}=1 if (Cfg::opt('base_path'));
    return;
}
