...
sub configure_fencing {
    ...
    while (1) {
        ...
        # add a message to prompt users to configure password-less rsh or ssh at the beginning of sever-based fencing configuration 
        if ($menu == 1) {
            $msg = Msg::new("Before configuring Coordination Point clients, make sure that the password-less rsh or ssh connections from clients to CP servers are configured.");
            $msg->printn;
...
sub establish_trust_cps_cpc {
    ...
    my ($syslist,$vcs,$eat_env,$msg,$cpsat,$cpspkg,$rtn,$clientnode0,$web,$cpsvip,$cpcvip);
    ...
    for my $clientnode (@$syslist) {
        for my $cpserver (@$cps) {
            ...
            # the right format of this command is: /opt/VRTScps/bin/cpsat setuptrust -b [ip]:port -s high
            $cpsvip = "[$cpserver->{sys}]";
            $rtn = $clientnode->cmd("_cmd_yes y | $eat_env $cpsat setuptrust -b $cpsvip:$cpserver->{cpsport} -s high");
    ...
    for my $cpserver (@$cps) {
        ...
        # the right format of this command is: /opt/VRTScps/bin/cpsat setuptrust -b [ip]:port -s high
        $cpcvip = "[$clientnode0->{sys}]";
        $rtn= $cpserver->cmd("_cmd_yes y | $eat_env $cpsat setuptrust -b $cpcvip:14149 -s high");
        if (EDR::cmdexit() && !$noprint) {
            Msg::right_failed();
            Msg::print("$rtn");
            Msg::prtc();
...
sub remove_cps {
    ...
    # not prompt warning messages like "Cannot invoke 'cpsadm' to"
    my $verbose = 0;
    $cpspkg = $pkg->pkg("VRTScps60");
    $sys0 = ${CPIC::get('systems')}[0];
