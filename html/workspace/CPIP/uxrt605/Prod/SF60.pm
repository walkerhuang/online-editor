...
sub init_common {
    ...
    if($edr && (-d "$edr->{mediapath}/storage_foundation_basic")) {
        ...
        $prod->{eula}='EULA_SF_Basic_Ux_6.0.1.pdf';
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSvxvm60 VRTSveki60 VRTSjre15.rte VRTSjre.rte VRTSsfcpi601 VRTSperl514
...
sub init_plat {
    ...
    $prod->{obsoleted_bundles}=[ qw(Base-VxTools-50 Base-VxVM-50 B3929FB Base-VxFS-50 Base-VxVM Base-VxTools-501 Base-VxVM-501 B3929GB Base-VxFS-501 Base-VxFS-51 Base-VxTools-51 Base-VxVM-51 B3929HB) ];
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSvxfs60 VRTSvxvm60 VRTSjre15 VRTSjre VRTSsfcpi601 VRTSperl514
        VRTSvlic32 VRTSwl
    ) ];
    return;
}
...
sub init_plat {
    ...
    $prod->{minpkgs}=[ qw() ];
    $prod->{recpkgs}=[ qw(VRTSob34 VRTSdbed60 VRTSodm60 ) ];
    $prod->{upgradevers}=[qw(5.0.30 5.1 6.0 6.0.1 6.0.3 6.0.4)];
    $prod->{zru_releases}=[qw(4.1.40 5.0 5.1 6.0 6.0.1 6.0.3 6.0.4)];
