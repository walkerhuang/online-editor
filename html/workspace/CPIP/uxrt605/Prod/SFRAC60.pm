...
sub init_common {
    ...
    $prod->{eula}='EULA_SF_Oracle_Rac_Ux_6.0.1.pdf';
...
sub completion_messages {
    ...
    if (Cfg::opt(qw(patchupgrade upgrade rollback)) && !Cfg::opt(qw(upgrade_nonkernelpkgs makeresponsefile rootpath))) {
...
sub config_menu {
    my ($cscript, $prod, $selection, $backopt, $vcsalreadyup, $rel);
    $prod = shift;
    $vcsalreadyup = shift;
    $backopt = 0;
    $rel = Obj::rel();
    ...
    # only support config sfrac sub-component for installmr
    if (($cscript =~ /installmr/imx) || (Obj::webui() && $rel->{patches})) {
        $prod->config_menu_sfrac_only();
        return;
    }
    if ($vcsalreadyup) {
...
sub config_menu_sfrac_only {
    my $prod = shift;
    my $sfcfs;
    $sfcfs = $prod->prod('SFCFSHA60');
    $sfcfs->{cfscluster_config_pending} = 1;
    $prod->config_sfrac_subcomponents();
    return;
}
    ...
    while (1) {
        if (Obj::webui()) {
            ...
            if (Cfg::opt('responsefile')) {
                $def = scalar(@menuopts) unless $cfg->{relink_oracle_database};
            }
        ...
        if ($prod->{"menu_$choice"}) {
            ...
            if ($choice eq 'exit_cleanly' || $choice eq 'continue_ru') {
                if (Cfg::opt(qw(upgrade patchupgrade rollback))) {
                    return;
                }
                my $cpic = Obj::cpic();
                $cpic->completion();
...
sub post_config_check_sfrac {
    ...
    for my $sys (@{CPIC::get('systems')}) {
        ...
        if (EDRu::arrpos("db_install", @{$prod->{post_config_check_items}}) != -1) {
            if ($prod->{db_home} && $sys->exists($prod->{db_home}.'/OPatch/opatch')) {
                Msg::log("Checking Oracle Database installation path $prod->{db_home} on $sys->{sys}");
                $prod->{orauser} = $sys->cmd("_cmd_ls -l ".$prod->{db_home}."/OPatch/opatch 2>/dev/null | _cmd_awk '{print \$3}' 2> /dev/null");
                if ($prod->{orauser}) {
                    $cmdout = $sys->cmd("cd /opt/VRTS/install; "."_cmd_su ".$prod->{orauser}." -c '_cmd_sh ".$prod->{db_home}."/OPatch/opatch lsinventory -oh ".$prod->{db_home}." 2> /dev/null' | _cmd_grep 'Oracle Database' | _cmd_tail -1");
                    if ( $cmdout) {
                    ...
                    if (!$lmx->check_sys($sys, 'start')) {
                        ...
                        if ($sys->aix()) {
                            if (!$sys->exists('/opt/VRTSvcs/rac/bin/lmxshow')) {
                                ...
                                $cmd_ret = $sys->cmd("_cmd_lsattr -E -l lmx | _cmd_grep lmx_async_upd | _cmd_awk '{print \$2}'");
                                if ($cmd_ret eq '1') {
    ...
    if (Obj::webui()) {
        ...
        if ($msg) {
            $webmsg = Msg::new("The following errors were discovered during post SFRAC configuration checks:");
            $webmsg->{msg} = $webmsg->{msg}."<br/>&nbsp;".$msg;
        }
        $msg = '';
        for my $sys (@{$edr->{systems}}) {
        ...
        if ($msg) {
            $webmsg = Msg::new("The following warnings were discovered during post SFRAC configuration checks:");
            $webmsg->{msg} = $webmsg->{msg}."<br/>&nbsp;".$msg;
        }
...
sub verify_responsefile_create_ocr_vote_storage {
    my $prod = shift;
    my $cfg = Obj::cfg();
    my ($msg,$incomplete_data, $grid_user, $oracle_user);
...
sub create_ocr_vote_storage {
    my $prod = shift;
    my ($ayn, $msg, $cfg, $sys, $errstr, $question, $answer, $repeat, $preinst, $ret, $out, $availdisks, @strlist, @diskname, $line, $temp_user);
    $cfg = Obj::cfg();
    my $web = Obj::web();
...
DNA_INSTR:
    ...
    eval {$vxdctl_master = $firstsys->cmd("_cmd_vxdctl -c mode | _cmd_grep 'master:' | _cmd_awk '{print \$2}'");};
    $errstr = $@;
    if ($errstr) {
        $msg = Msg::log("Problem running 'vxdctl' on $firstsys->{sys}. Error info: $errstr");
    ...
    # You might want to print the manual steps here
        ...
        if (!$vxdctl_master) {
            $msg = Msg::log("Cannot determine the MASTER node. Error info: please check the cluster status.");
            $msg = Msg::new("Cannot determine the MASTER node");
            $msg->print;
            $web->web_script_form("alert", $msg->{msg}) if (Obj::webui());
            # You might want to print the manual steps here
        ...
        if ($Obj::pool{"Sys::$vxdctl_master"}) {
            ...
            for my $syskey (keys %Obj::pool) {
                ...
                if ($sys && !$sys->{vcs_sysname}) {
                    $sys->{vcs_sysname} = $vcs->get_vcs_sysname_sys($sys);
                }
...
DNA_SELECT_OPTION:
    ...
    $availdisks = $vxdctl_master->cmd('_cmd_vxdisk -o alldgs list | _cmd_grep online | _cmd_grep -v LVM | _cmd_grep -v error');
    $errstr = $@;
    if ($errstr) {
...
DNA_OPTION_CREATE_DG:
        ...
        if ($index eq 1) {
            ...
            for my $item (@availdisks) {
                ...
                if ($strlist[3] eq '-'){
                    # my $disksize = $vxdctl_master->cmd("_cmd_vxdisk list $strlist[0] 2>/dev/null | _cmd_grep '^public:'");
                    # $disksize =~ s/^.*\slen=(\d+).*$/$1/mx;
                    # if ( $disksize !~ /^\d+$/mx) {
                    #     $disksize = 0;
                    # }
                    # $disksize /= 2048;
    ...
    for my $tmpdsk (@initdisks) {
        ...
        if ($errstr) {
            ...
            if (EDR::cmdexit()) {
                ...
                my $type = $vxdctl_master->cmd("_cmd_vxdisk list $tmpdsk 2>/dev/null | _cmd_grep '^type:'");
                if ( $type !~ /^type:\s*auto\s*$/) {
                    $vxdctl_master->cmd("_cmd_vxdisk rm $tmpdsk");
                    $vxdctl_master->cmd("_cmd_vxdisk scandisks");
                }
                $vxdctl_master->cmd("_cmd_vxdisksetup -if $tmpdsk format=cdsdisk");
                if (EDR::cmdexit()) {
                    Msg::right_failed();
                    $msg = Msg::new("Initializing disk $tmpdsk on Master ($vxdctl_master->{sys}) failed. Refer to the installer logs for more information.") if (Obj::webui()); 
                    $web->web_script_form("alert", $msg->{msg}) if (Obj::webui());
...
sub get_netresname {
    ...
    return '' if ( !defined $ret or @$ret == 0);
    my @resname = @{$ret};
    if ($mpnic) {
        my $vcs = $prod->prod('VCS60');
        for my $item (@resname) {
...
sub config_cssd_agent {
    my $prod = shift;
    my $cfg = Obj::cfg();
    my ($errstr, $ret, $msg, $ayn, $index, $bs1, $msg_type, $msg_resource, $msg_service, $bs, $msg_pip, $msg_ocr, $msg_vd);
    my $cssdres = 'cssd';
    my $web = Obj::web();
    my $vcs = $prod->prod('VCS60');
    my $sys = @{CPIC::get('systems')}[0];
...
DNA_CONFIRMCSSDRES:
    ...
    if (EDR::cmdexit()) {
        Msg::right_failed();
        $msg = Msg::new("(Failed to override OfflineWaitLimit attribute for CSSD agent. Check logs for more information.)");
        $web->web_script_form("alert", $msg) if (Obj::webui());
        $msg->print;
        Msg::prtc();
        return 1;
    ...
    if (EDR::cmdexit()) {
        Msg::right_failed();
        $msg = Msg::new("(Failed to set OfflineWaitLimit attribute to 3 for CSSD agent. Check logs for more information.)");
        $web->web_script_form("alert", $msg) if (Obj::webui());
        $msg->print;
        Msg::prtc();
        return 1;
...
sub invoke_oui_installer {
    ...
    if (Cfg::opt('responsefile') && $oraprod eq 'crs') {
        ...
        @line = grep(/orainstRoot.sh/, split(/\n/,$crslog));
        $exestr = '';
        for my $item (@line) {
...
#   my $optach_script = "$prod->{db_home}/OPatch/opatch";
    ...
    chdir('/tmp');
    my $line = $firstsys->cmd("cd /opt/VRTS/install; "."_cmd_su $orauser -c '_cmd_sh $optach_script lsinventory -oh $home 2> /dev/null' | _cmd_grep '$productstr' | _cmd_tail -1");
...
sub check_gabports_sys {
    ...
    my $syslistref = shift;
    my $is_sfcfsrac = shift;
    my ($overall_status, $vxdctl_master, $up_port_cnt, $msg);
    my $msg_check;
    ...
    my $stage = Msg::new("Installation and Configuration Checks");
    eval {$vxdctl_master = $temp_sys->cmd("_cmd_vxdctl -c mode | _cmd_grep 'master:' | _cmd_awk '{print \$2}'");};
    #just get the package information
    for my $sys (@{$syslistref}) {
        $status = 1;
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSjre15 VRTSjre VRTSperl514 VRTSvlic32 VRTSwl
    ) ];
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSjre VRTSperl514 VRTSvlic32
    ) ];
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSdsa VRTSsfmh41 VRTSob34 VRTSobc33 VRTSaslapm60
        VRTSat50 VRTSsmf VRTSpbx VRTSicsco VRTSvxfs60 VRTSvxvm60
        VRTSjre15 VRTSjre VRTSperl514 VRTSvlic32
    ) ];
    ...
    for my $system (@cpnodes) {
        ...
        if (EDR::cmdexit()) {
            ...
            if (EDR::cmdexit()) {
                ...
                if ($system->exists("$static_lib" . ".backup")) {
                    $system->cmd("_cmd_rm -f $static_lib");
                } elsif($system->exists("$static_lib")){
                    $system->cmd("_cmd_mv $static_lib $static_lib.backup");
                    $system->cmd("_cmd_chown $orauser:$oragrp $static_lib.backup");
                }
...
sub init_plat {
    ...
    $prod->{minpkgs}=[ qw(VRTSdbac60 VRTSgms60 VRTSodm60 VRTSdbed60 VRTSvcsea60) ];
    $prod->{recpkgs}=[ qw(VRTSob34 VRTSdbac60 VRTSgms60 VRTSodm60 VRTSdbed60 VRTSvcsea60) ];
    $prod->{upgradevers}=[qw(5.0.3 5.1 6.0 6.0.1 6.0.3)];
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
            ...
            VRTSjre.rte VRTSperl514 VRTSperl.rte VRTSvlic32
        ) ];
    return;
}
...
sub init_padv {
    my $prod=shift;
    $prod->{upgradevers}=[qw(6.0.10 6.0.3)];
    $prod->{zru_releases}=[qw(6.0.10 6.0.3)];
    $prod->{minimal_cpu_speed_requirment} = '1 GHz';
