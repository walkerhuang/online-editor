...
sub svs_auto_config {
    ...
    Cfg::set_opt('silent');
    $cfg->{reconfigurevcs}=1 if ($vcs_prod->check_config() && $vcs_prod->check_uuid());
...
sub check_config {
    my ($syslist,$maincf,$prod,$rootpath,$sf,$sf_config,$sys,$vcs,$vcs_config);
    $prod = shift;
    ...
    # e3412663: Do not check if cvm is configured since sometimes it's not added in main.cf after configuration
    #$syslist=CPIC::get('systems');
    #$sys=$$syslist[0];
...
sub upgrade_prestop_sys {
    ...
    if(Cfg::opt('upgrade_kernelpkgs')) {
        return if ($prod->ru_prestop_sys($sys));
    }
    if (!Cfg::opt('rootpath')) {
...
sub stop_agents_sys {
    ...
    return 1 if ($prod->stop_odm_sys($sys));
...
sub stop_cvm_cfs_agents {
    my ($prod,$sys) = @_;
    my ($padv,$port_f,$vcs,@deps,@cvmdeps,$syslist,$port_q,$offlinefailuregroups,$offlinefailuregrouplist);
    $syslist=CPIC::get('systems');
    $vcs = $prod->prod('VCS60');
    $padv=$sys->padv;
...
sub version_sys {
    ...
    my ($sfcfsha_licensed,$sfsybasece_licensed,$svs_licensed);
    $cpic=Obj::cpic();
    $rel=$cpic->rel;
    ...
    if ($prod->sfsybasece()) {
        if ($sys->{pkgvers}{'VRTSvcssy'} eq '5.0.1' && $prod->{padv} =~ /Sol/m) {
            $pkgvers = '5.0';
        }
        # Skip super product license check(last parameter=1)
        # because SFCFSHA has SFSYBASECE, SFRAC as it's super product
        # If SFSYBASECE is licensed, $rel->prod_licensed_sys($sys, 'SFCFSHA60') will return licensed
        $sfcfsha_licensed=$rel->prod_licensed_sys($sys, 'SFCFSHA60', 1);
        ...
        #3115659: check SVS version
        $cv=$prod->prod("SFCFSHA60");
...
sub upgrade_postinstall_sys {
    my ($prod,$sys) = @_;
    # Only perform add_drv for phase upgrade
    if (Obj::cfg('nostart')) {
        # add_drv for vxglm/vxgms secretly
        for my $proci (qw/vxglm vxgms/) {
            $sys->cmd("_cmd_adddrv -f -m '* 0640 root sys' $proci 2> /dev/null");
        }
    }
    return $prod->SUPER::upgrade_postinstall_sys($sys);
}
...
@Prod::SFCFSHA60::Sol11x64::ISA = qw(Prod::SFCFSHA60::Sol11sparc);
