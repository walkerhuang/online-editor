...
sub set_vcs_allowcomms_sys {
    ...
    $pids = $sys->proc_pids(' had -onenode') if(!(@$pids));
...
sub phased_upgrade_precheck {
    ...
    if ($csystems < $nsystems) {
        ...
        #e3402248:installer should prompt user no matter whether the cluster is security.
...
sub completion_message_rollingupgrade {
    ...
    my ($showrup1list,@rup1list,@rup2list,$success);
    $cpic=Obj::cpic();
    $rel=$cpic->rel;
    $cfg=Obj::cfg();
    $web = Obj::web();
    ...
    for my $line (split(/^/m,$str)) {
        ...
        if ($line=~/vxfen_mode\s*=\s*(\w+)/mx) {
            ...
            for my $item(split(/,/,$line)) {
                # split by ]\s*:, for IPv6 
                ($cpsvip, $port) = split(/]\s*:/, $item);
                $cpsvip = "$cpsvip]" if ($cpsvip && ($cpsvip !~ /\]\s*$/));
                if ($cpsvip =~ /\[(.+)\]/m) {
                    $cpsvip = $1;
...
sub configure_sys {
    ...
    my ($cpspkg,$localsys,$rootpath,$vxfenmodedisablefile,$vxfenmodefile,$vxfendgfile);
    $cpic=Obj::cpic();
    $cfg = Obj::cfg();
    $cprod=CPIC::get('prod');
...
sub ru_prestop_sys {
    ...
    if (Cfg::opt('upgrade_kernelpkgs')) {
        ...
        $proc='had60';
        $procobj=$sys->proc($proc);
        # Use had60::Common::check_sys because
        # on had60::SunOS::check_sys, had will be considered stopped when SMF service system/vcs not online
        if (Proc::had60::Common::check_sys($procobj,$sys,'stop')) {
...
sub dynupgrade_pre_upgrade_sys {
    my ($prod,$sys) = @_;
    my ($rootpath,$msg,$uuid,$upgradepath,$sourcefilebase,$line,$rtn,$sourcefile,$maincf,$typescfpath);
    ...
    #Fix incident 3067510: Manually correct the attribute value in types.cf
...
sub stop_vcs {
    ...
    if (EDRu::isverror($stop)) {
        for my $sys (@$syslist) {
            ...
            $pids=$sys->proc_pids('bin/had');
            push @pids, @{$pids} if (@{$pids});
            $pids=$sys->proc_pids(' had -onenode');
            push @pids, @{$pids} if (@{$pids});
            $pids=$sys->proc_pids('hashadow');
            push @pids, @{$pids} if (@{$pids});
            $sys->kill_pids(@pids);
...
sub get_cluster_system {
    ...
    while(1){
        ...
        #e3423726:setup rsh/ssh password-less connection
        if (-1 == $edr->check_and_setup_transport_sys($sys)) {
            next;
        }
...
sub postcheck_vxfen_sys {
    my ($prod,$sys) = @_;
    my ($msg,$line,$rtn,$proc,$conf,$file,$report,$output,$vxfenmode,$diskgroup,@m);
    $proc=$sys->proc("vxfen60");
    $file=$proc->{initconf};
...
sub init_plat {
    ...
    $prod->{zru_releases}=[qw(3.5 4.1 5.0 5.1 6.0 6.0.1 6.0.3)];
...
sub init_plat {
    ...
    $prod->{zru_releases}=[qw(4.1.40 5.0 5.1 6.0 6.0.1 6.0.3)];
    ...
    $prod->{platreqs} =  [ 'RHEL5 U5 (2.6.18-194.el5)', 'RHEL5 U6 (2.6.18-238.el5)', 'RHEL5 U7 (2.6.18-274.el5)', 'RHEL5 U8 (2.6.18-308.el5)',
                         'RHEL5 U9 (2.6.18-348.el5)', 'RHEL5 U10 (2.6.18-371.el5)',
                         'RHEL6 U1 (2.6.32-131.el6)', 'RHEL6 U2 (2.6.32-220.el6)', 'RHEL6 U3 (2.6.32-279.el6)', 'RHEL6 U4 (2.6.32-358.el6)',
                         'RHEL6 U5 (2.6.32-431.el6)',

                         'SLES10 SP4 (2.6.16.60)',
                         'SLES11 SP1 (2.6.32.12)', 'SLES11 SP2 (3.0.13)', 'SLES11 SP3 (3.0.76)',
                         'OEL5 U5 (2.6.18-194.el5)', 'OEL5 U6 (2.6.18-238.el5)', 'OEL5 U7 (2.6.18-274.el5)','OEL5 U8 (2.6.18-308.el5)',
                         'OEL5 U9 (2.6.18-348.el5)', 'OEL5 U10 (2.6.18-371.el5)',
                         'OEL6 U1 (2.6.32-131.el6)', 'OEL6 U2 (2.6.32-220.el6)', 'OEL6 U3 (2.6.32-279.el6)', 'OEL6 U4 (2.6.32-358.el6)',
                         'OEL6 U5 (2.6.32-431.el6)',
...
sub padv_maincf_upgrade_precheck_sys {
    ...
    my ($grp_container_type,$grp_container_names,$res_container_type,$res_container_names,$sys_container,$compvers_rtn,$zone_num,$multi_zone,$vcspkg);
        ...
        for my $grp_name (@grps) {
            ...
            for my $res_name (@reses) {
                ...
                if ($res_type eq 'Zone') {
                    ...
                    $res_container_names = Prod::VCS60::Common::dynupgrade_get_attrvalue_of_resname_array($treeref, $res_name, 'ZoneName');
                } else {
                    $res_container_type = Prod::VCS60::Common::dynupgrade_get_attrvalue_of_typename($treeref,$res_type,'ContainerType');
                    if ($res_container_type) {
                        $res_container_names = Prod::VCS60::Common::dynupgrade_get_attrvalue_of_resname_array($treeref, $res_name, 'ContainerName');
                    }
                }
...
sub init_padv {
    my $prod=shift;
    $prod->{upgradevers}=[qw(5.0.3 5.1 6.0 6.0.1 6.0.3)];
    $prod->{zru_releases}=[qw(4.1.2 5.0 5.1 6.0 6.0.1 6.0.3)];
    return;
}
...
sub init_padv {
    my $prod=shift;
    $prod->{upgradevers}=[qw(5.0.3 5.1 6.0 6.0.1 6.0.3)];
    $prod->{zru_releases}=[qw(5.0 5.1 6.0 6.0.1 6.0.3)];
    return;
}
