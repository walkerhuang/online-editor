...
sub init_common {
    ...
    if ($prod->{class} =~ /DMP/m) {
        ...
        $prod->{eula}='EULA_DMP_Ux_6.0.1.pdf';
...
sub need_unencap_upgrade_sys {
    ...
    if ((($vers =~ /5\.1\.13[3,4]/)||($vers =~ /6\.0\.00\d/)) && ($sys->{padv} =~ /RHEL6/mx)) {
        return 1;
...
sub encap_upgrade_precheck_sys {
    ...
    if ($sys->sunos()) {
        ...
        # e2740143: reminor to 1000 unconditionally to keep consistent of VxVM preinstall script
        if ($minor_num != 1000) {
        ...
        for my $vol (split(/\s+/m,$volumes)) {
            ...
            # e3330485: try to get bootdg minor number for RVG if above cmd fails
...
sub enable_dmp_osn_sys {
    my ($prod,$sys) = @_;
    my ($cprod,$msg,$osn_enabled,$out,$voldmode,$exitcode);
    $cprod = CPIC::get('prod');
    ...
    if (($cprod =~ /DMP/m) || $osn_enabled) {
        ...
        # check if $exitcode is -41 because in EDR $exitcode -= 256 if ($exitcode > 127);
        $exitcode = 0 if (($exitcode eq '-41') && $sys->linux());
        if ($exitcode) {
            if ($out =~ /\brootvg\b/m) {
                # error for AIX LVM
...
sub ask_patchupgrade_configure {
    my ($prod) = @_;
    my ($msg,$syslist,$sys,$ayn,$cfg,$task_msg);
    my (@dmp_enabled_syslist,$out,$help,$sysobj,$sunos);
    $cfg=Obj::cfg();
    ...
    if ($cfg->{opt}{makeresponsefile}) {
        for my $sys(@$syslist) {
            $out=$sys->cmd('_cmd_vxdmpadm gettune dmp_native_support 2> /dev/null');
            push (@dmp_enabled_syslist,$sys->{sys})
                if(!$cfg->{opt}{rootpath} && $out =~ /dmp_native_support\s+on/mx);
        }
    } else {
        for my $sys(@$syslist) {
    ...
    $task_msg=Msg::new("upgrade") if (Cfg::opt(qw(patchupgrade upgrade)));
    $task_msg=Msg::new("rollback") if (Cfg::opt('rollback'));
    ...
    $msg->print();
    if ($sunos) {
        $msg=Msg::new("Retaining this tunable setting may increase the $task_msg->{msg} time depending on the number of LUNs and native ZFS pools configured on the system.");
    } else {
        $msg=Msg::new("Retaining this tunable setting may increase the $task_msg->{msg} time depending on the number of LUNs and native LVM volume groups configured on the system.");
    }
    $msg->print();
...
sub ask_upgrade_configure {
    my ($prod) = @_;
    $prod->ask_patchupgrade_configure();
}
...
sub dmp_osn_stop_sys {
    ...
    if ($out =~ /dmp_native_support\s+on/mx) {
        ...
        $sys->cmd("_cmd_touch $prod->{migrate_native_file}") if ($prod->enable_dmp_needed_sys($sys));
    }
    return 1;
}

# Enable dmp_native_support is required under following circumstances:
# 1. In upgrade/patchupgrade/stop task.
# 2. In rollback, except rollback to 6.0.1 on AIX platform.(known issue in 6.0.1, turnning on dmp_native_support cause system crash after reboot)
sub enable_dmp_needed_sys {
    my ($prod,$sys)=@_;
    my $rel=Obj::rel();
    return 1 if (Cfg::opt(qw(upgrade patchupgrade stop)));
    # Unless rollback to 6.0.1 on AIX, we will enable dmp_native_support
    return 0 unless (Cfg::opt('rollback'));
    return 0 if ($sys->{padv} =~ /AIX/ && !$sys->{prior_rp} && EDRu::compvers($rel->{gavers},'6.0.100.000',4) == 0);
    return 1;
}
...
sub poststart_sys {
    ...
    $cfg=Obj::cfg();
    if ($cfg->{opt}{responsefile} && $cfg->{vcs_prestart_script} && $cfg->{opt}{upgrade_kernelpkgs}) {
       # if $cfg->{vcs_prestart_script} not exist, print a warning in the log.
       if (!$sys->exists($cfg->{vcs_prestart_script})) {
            $msg=Msg::new("Cannot find $cfg->{vcs_prestart_script} on $sys->{sys}, skip execution");
            $msg->log;
        } else {
            $sys->cmd("_cmd_chmod +x $cfg->{vcs_prestart_script} 2> /dev/null; $cfg->{vcs_prestart_script} 2>/dev/null");
        }
    }
...
sub require_start_after_reboot_sys {
    my ($prod, $sys)=@_;
    #e3365378:should not set start after reboot flag if product isn't configured and it's not VM or SF.
...
sub completion_messages {
    my ($cpic,$prod,$msg,$padv,$sys,$tmppath,$rootpath,$web);
    my ($aslapm,$aslapmver);
    $prod=shift;
    $cpic = Obj::cpic();
    $padv=$prod->padv();
    ...
    if (Cfg::opt("rollback")) {
        # Check on each system, if VRTSaslapm has version higher than/equal to 6.0.5's VRTSaslapm, then print the message.
        # during rollback, inform user VRTSaslapm will not change
        for $sys(@{$cpic->{systems}}) {
            $aslapm=$sys->pkg('VRTSaslapm60');
...
sub postcheck_vm_disabled_vol_sys{
    my ($prod,$sys) = @_;
    my ($dgs, $vols, $unstartablemsg, $disabledmsg, $notinfstabmsg,$msg,$vol,$state,$fs,$blockdevs,$vcs,$vcsbds);
    $unstartablemsg = '';
...
sub dg_conf_upgrade_precheck_sys {
    ...
    if ($sys->{dgs_mix_clone}) {
        ...
        $msg = Msg::new("The following disk group(s) have mix of regular and clone disk on $sys->{sys}:\n\t$dglist\nFollow the chapter 'Clearing the udid_mismatch flag for clone disks' in trouble shooting guide to unset clone flag before upgrade.");
        $sys->push_error($msg);
    }
    return;    
}
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSsfcpi601 VRTSperl514 VRTSperl.rte VRTSvlic32
    ) ];
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSjre VRTSsfcpi601 VRTSperl514 VRTSvlic32 VRTSwl
    ) ];
    $prod->{obsoleted_bundles}=[ qw(Base-VxTools-50 Base-VxVM-50 Base-VxVM Base-VxTools-501 Base-VxVM-501 Base-VxTools-51 Base-VxVM-51) ];
    $prod->{obsoleted_bundled_pkgs}=[ qw(AVXTOOL AVXVM) ];
    $prod->{prerequisite_bundles_IA}=[ qw(CommonIO FibrChanl-01 FibrChanl-02 FibrChanl-03) ];
    $prod->{prerequisite_bundles_PA}=[ qw(CommonIO FibrChanl-01) ];
...
sub install_precheck_sys {
    ...
    $prod->check_hp_mt_safe_bundles_sys($sys);
    return;
}
...
sub check_hp_mt_safe_bundles_sys {
    my ($prod, $sys) = @_;
    my ($bundle_ver,$msg,$bundle_list,@prerequisite,@bundles);

    my $padv=$sys->{padv};
    if ($padv=~/1131ia64$/) {
        @prerequisite=@{$prod->{prerequisite_bundles_IA}};
    } elsif ($padv=~/1131par$/) {
        @prerequisite=@{$prod->{prerequisite_bundles_PA}};
    }
    
    for my $bundle (@prerequisite) {
        $bundle_ver=$sys->cmd("_cmd_swlist -l bundle 2>/dev/null | _cmd_grep $bundle | _cmd_awk '{print \$2}' | _cmd_awk -F. '{print \$4}'");
        push(@bundles, $bundle) unless ($bundle_ver >= 1311); # 3409588, B.11.31.1311 is the mt safe fixes version
    }

    if ($#bundles >= 0) {
        $bundle_list=join(' ', @bundles);
        $msg=Msg::new("Please install following OS bundles with revision B.11.31.1311 or above on $sys->{sys} to avoid multi-thread safety issue in VxVM:\n\t$bundle_list");
        $sys->push_warning($msg);
    }
}
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSicsco VRTSvxvm60 VRTSjre15 VRTSjre VRTSsfcpi601 VRTSperl514
        VRTSvlic32
    ) ];
...
@Prod::VM60::Sol11x64::ISA = qw(Prod::VM60::Sol11sparc);
