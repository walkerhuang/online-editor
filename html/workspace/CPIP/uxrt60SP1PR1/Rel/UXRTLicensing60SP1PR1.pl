...
sub license_products {
    ...
    my ($cpic,$pkgdir,$msg,$sitelic,$cfg,$lictype,$web,$edr,$sys,$ayn,$plat,$key,$rt,$keytype,$nlsys,$prodname,$pkg,$lic_ver_update,$prod);
    $edr=Obj::edr();
    $web=Obj::web();
    $cpic=Obj::cpic();
    if (Obj::webui()) {
    ...
    while(1){
        ...
        for my $lsys (@{$cpic->{systems}}) {
            ...
            if(!$prod->licensed_sys($lsys)){
                $web->{invalid_license} = 1;
                $msg = Msg::new("$key is not a valid license key for $prod->{abbr}");
                $web->{lic_result}=$msg->{msg};
                $web->write_status();
                last;
            }
...
sub feature_licensed_sys {
    ...
    for my $lk (sort { $a cmp $b } keys(%{$sys->{keys}})) {
        for my $feat (sort { $a cmp $b } keys(%{$sys->{keys}{$lk}})) {
            ...
            $ver = $sys->{keys}{$lk}{Version} if (($fval eq 'Enabled') || ($fval eq 'YES'));
...
sub restore_keyless_keys {
    ...
    if ($old_ver) {
        $msg=Msg::new("Found vxkeyless key(s) for previous release(s). Do you wish to update them to the current version?");
        $ayn=$msg->aynn(undef,1);
        if ($ayn eq 'Y') {
            for my $sys (@{$cpic->{systems}}) {
                $oldkey=$sys->{vxkeyless};
