...
sub init_common {
    ...
    $prod->{prod}='VCS';
    $prod->{abbr}='VCS';
    $prod->{vers}='6.0.110.000';
    $prod->{proddir}='cluster_server';
    $prod->{eula}='EULA_VCS_Ux_6.0SP1.pdf';
...
sub get_vxfen_config_sys {
    ...
    for my $line (split(/^/m,$str)) {
        ...
        if ($line=~/vxfen_mode\s*=\s*(\w+)/mx) {
            ...
            # the fips_mode is new added during 60SP1, it could be 1 or 0
            $conf->{fips_mode}=$1;
...
sub maincf_result_upgrade_cpagent_sys {
    ...
    for my $res (@reslist) {
        ...
        # add LevelTwoMonitorFreq since 6.0SP1
...
sub eat_configure_from_eat_atutil_restore {
    my ($prod)=@_;
    my ($syslist,$fold,$backup);

    $prod->eat_initiate(1);
    $syslist = CPIC::get("systems");
    $fold = '/var/VRTSvcs/vcsauth';
    $backup = '/var/VRTSvcs/vcsauth_atutil_backup';

    for my $sys (@$syslist) {
        $prod->eat_install_sys($sys,$prod->{eat_setup},"NO RAB");
        #restore the conf files
        $sys->cmd("_cmd_rmr $fold");
        $sys->cmd("_cmd_mv $backup $fold");
        $prod->eat_start_sys($sys);
    }
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSsmf VRTSpbx VRTSicsco VRTSjre.rte VRTSsfcpi60SP1PR1 VRTSperl514 VRTSperl.rte
        VRTSvlic32
    ) ];
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSsfcpi60SP1PR1 VRTSperl514 VRTSvlic32 VRTSwl
    ) ];
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSjre VRTSsfcpi60SP1PR1 VRTSperl514 VRTSvlic32
    ) ];
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSat50 VRTSsmf VRTSpbx VRTSicsco VRTSjre VRTSsfcpi60SP1PR1 VRTSperl514
        VRTSvlic32
    ) ];
    # For UXRT 5.1SP1, VRTSacclib need to be fresh on AIX and Solaris.
    $prod->{obsoleted_but_need_refresh_when_upgrade_pkgs} = [ qw(VRTSacclib52) ];
