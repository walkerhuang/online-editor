...
sub init_common {
    ...
    if ($prod->sfcfsha) {
        ...
        $prod->{eula}='EULA_CFSHA_Ux_6.0SP1.pdf';
        ...
        $prod->{eula}='EULA_VirtualStore_Ux_6.0SP1.pdf';
        ...
        $prod->{eula}='EULA_SF_Sybase_CE_Ux_6.0SP1.pdf';
...
sub web_initiation_questions {
    ...
    return if (Cfg::opt( qw(addnode fencing security) ));
...
sub version_sys {
    ...
    ##return $pkgvers if(($prod->sfsybasece()) && $pkgvers && $rel->prod_licensed_sys($sys,$prod->{prodi}));
    #For sfsybasece, suppose it is installed only in case of there is a sfsybasece license
    if ($prod->sfsybasece()){
        return $pkgvers if($pkgvers && $rel->prod_licensed_sys($sys,$prod->{prodi}));
        return '';
    }
...
sub postcheck_gabconfig_sys {
    ...
    for my $port (qw(a b d f h u v w)) {
...
sub addnode_config_cvm_cfs {
    ...
    $asymmetry_key = '_volasym';
    $rtn = $firstnode->cmd("/sbin/vxdefault list $asymmetry_key");
    if ((!EDRu::isverror($rtn)) && ($rtn =~ /$asymmetry_key\s+(\w+)/)) {
        $asymmetry_value = $1;
        for my $sysi (@{$cfg->{newnodes}}) {
            $sys = Obj::sys($sysi);
            # next if the value is the same as nodeone
            $rtn = $sys->cmd("/sbin/vxdefault list $asymmetry_key");
            if ((!EDRu::isverror($rtn)) && ($rtn =~ /$asymmetry_key\s+(\w+)/)) {
            ...
            $rtn = $sys->cmd("/sbin/vxdefault set $asymmetry_key $asymmetry_value");
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSjre.rte VRTSsfcpi60SP1PR1 VRTSperl514 VRTSperl.rte VRTSvlic32
    ) ];
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSjre15 VRTSjre VRTSsfcpi60SP1PR1 VRTSperl514 VRTSvlic32 VRTSwl
    ) ];
    return;
}
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSvxvm-common VRTSjre15 VRTSjre VRTSsfcpi60SP1PR1 VRTSperl514 VRTSvlic32
    ) ];
    return;
}
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSdsa VRTSsfmh41 VRTSob34 VRTSobc33 VRTSaslapm60
        VRTSat50 VRTSsmf VRTSpbx VRTSicsco VRTSvxfs60 VRTSvxvm60
        VRTSjre15 VRTSjre VRTSsfcpi60SP1PR1 VRTSperl514 VRTSvlic32
    ) ];
    return;
}
