        ...
        if ($sys->exists($bd)) {
            ...
            if ($fstype =~ /^$prod->{fstyp_pattern}/mx) {
                ...
                if ($fsvers <= $prod->{eol_vxfs_version}) {
                    $disklayoutmsg .= "\n\t$bd version $fsvers";
                } elsif ($fsvers == $prod->{deprecated_vxfs_version}) {
                    $depmsg .= "\n\t$bd version $fsvers";
                }
            }
        }
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSpbx VRTSicsco VRTSvxfs60 VRTSveki60 VRTSsfcpi60SP1PR1 VRTSperl514
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSvxfs-common VRTSsfcpi60SP1PR1 VRTSperl514 VRTSvlic32
    ) ];
    my $padv=$prod->padv();
    $prod->{fstab}='/etc/fstab';
    $padv->{cmd}{fstyp}='/opt/VRTS/bin/fstyp';
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSicsco VRTSvxfs60 VRTSsfcpi60SP1PR1 VRTSperl514 VRTSvlic32
    ) ];
