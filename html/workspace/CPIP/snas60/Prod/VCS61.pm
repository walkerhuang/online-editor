...
sub init_common {
    ...
    $prod->{superprods} = [qw(SFRAC61 SVS61 SFSYBASECE61 SNAS60 SFCFSHA61)];
...
sub ask_fencing_enabled {
    ...
    $msg->bold;
    $msg = Msg::new("A split brain can occur if servers within the cluster become unable to communicate for any number of reasons. If I/O Fencing is not enabled, you run the risk of data corruption should a split brain occur. Therefore, to avoid data corruption due to split brain in CFS environments, I/O Fencing has to be enabled.\n");
    $msg->print;
    $description = $msg->{msg};
...
sub upgrade_ru_check_sys {
    ...
    if ($ru_running + $ru_idle != 1) {
        unless ($had->check_sys($sys) && $gab->check_sys($sys) && $llt->check_sys($sys)) {
            $msg = Msg::new("To perform a rolling upgrade, VCS/GAB/LLT must be running on $sys->{sys}.");
            $sys->push_error($msg);
            return 0;
        }
    }
...
sub verify_responsefile {
    ...
    if (($cfg->{vcs_clustername}) || ($cfg->{vcs_clusterid}) || ($cfg->{'vcs_lltlink1'})) {
        ...
        if ($cfg->{vcs_allowcomms} && ($cfg->{opt}{configure} || $cfg->{opt}{installconfig})) {
            ...
            if ( !Cfg::opt(qw(fencing configcps)) ) {
                ...
                #fix for snas, new systems is stored in newnodes
                @systems = @{$cfg->{newnodes}} if (CPIC::get('prod') =~ /SNAS/);
                for my $sys (@systems) {
    ...
    if ((Cfg::opt('configure')||Cfg::opt('upgrade')||Cfg::opt('start')) && !Cfg::opt(qw(fencing configcps confignic))) {
...
sub upgrade_precheck_sys {
    ...
    if (!$prod->check_prod_enabled_sys($sys)) {
        if (Cfg::opt(qw(rolling_upgrade upgrade_kernelpkgs upgrade_nonkernelpkgs))) {
            ...
            $msg = Msg::new("The rolling upgrade cannot proceed because VCS_START is not set to 1 on $sys->{sys}. To perform a rolling upgrade, VCS_START must be set to 1 in $vcsinitfile so that VCS processes can be started during the upgrade");
            $sys->push_error($msg);
        } else {
...
sub completion_message_rollingupgrade {
    ...
    if(Cfg::opt('upgrade_nonkernelpkgs')){
        $rel->update_status_file("RU",'','',"Done");
...
sub maincf_check_TriggerResStateChange_sys {
    ...
    if (scalar @msgs) {
        $sys->push_warning(@msgs) unless(Cfg::opt('upgrade'));
    }
...
sub prestart_config_common_questions {
    ...
    if ($cfg->{vcs_allowcomms} && $cprod!~/^SFRAC\d+/mx && $cprod!~/^SFSYBASECE\d+/ && $cprod!~/^SNAS\d+/mx) {
...
sub prestart_sys {
    my ($prod,$sys) = @_;
    my ($cfg,$cprod);
    my ($vxfen,$rootpath,$vxfenmodedisablefile,$vxfenmodefile,$vxfendgfile,$str);
    my $cpic=Obj::cpic();
    $cfg = Obj::cfg();
    $cprod=CPIC::get('prod');
    ...
    if (!$cfg->{donotreconfigurevcs}) {
        ...
        if ($cprod =~ /^(SVS|SFCFS|SFRAC|SFSYBASECE|SNAS)/mx) {
...
sub check_rdma_nic_ips_sys {
    ...
    $rdma_nics=$prod->rdmanics_sys($sys,$all_nics,$rdma_type);
    if (!@$rdma_nics) {
        # Case 1.
...
sub ask_hbnics {
    ...
    my($all,$ayn,%hbn,@en,$dsn,$hb,$hb2,$hb3,$hb4,$hbl,$ip,$port,$rpn,$rsn,$all_rsn,$rdma_rsn,$udp_port,$used_port,$rtn,$rdmas);
    return '' if (Cfg::opt('responsefile'));
    $prod=shift;
    ...
    for my $sys (@{CPIC::get('systems')}) {
        ...
        if ($all) {
            ...
            if (EDRu::inarr($sys,@{CPIC::get('systems')})) {
                ...
                $rsn=$all_rsn;
                if ($rdma_tag) {
                    $rdma_rsn=$prod->rdmanics_sys($sys,$rsn) if ($rdma_tag);
                    $rsn=$rdma_rsn;
                }
                $rpn=$padv->gatewaynics_sys($sys);
                EDRu::arruniq(@$rsn);
...
sub autocfg_hbnics {
    ...
    for my $n (0..$#{$rnics->{$sysi}}) {
        $nic = $rnics->{$sysi}->[$n];
        $last_sap++;
        if ($nic =~ /(sit|virbr|vsw)\d+/m){ # skip sit# virbr# on Linux, vsw# on Solaris
            $non_nic_num++;
...
sub ask_common_nics {
    ...
    if ($ayn eq 'N' && $cprod eq 'SFRAC61' ){
        $warn=Msg::new("Oracle Clusterware requires using the same NICs for the private heartbeat links on all the systems of your cluster.");
        $warn->warning();
        $ayn = $msg->ayny($help,$backopt);
    }
    return $ayn;
...
sub update_gabtab {
    ...
    $systems = cluster_systems();
    # For SNAS, keep the number as 1
    $n = (CPIC::get('prod') =~ /SNAS/) ? 1 : $#$systems+1;
...
sub dynupgrade_pre_customize_check_for_statistics_sys {
    my ($prod,$sys,$treeref) = @_;
    my (@grps,$attr,$attrname,$attrvalue,$syslist_ref);
...
sub maincf_upgrade_sys {
    ...
    for my $line (@{$lines_ref}) {
        ...
        # remove the WorkLoad= { }from the main.cf(incident:3252248)
        if ($line=~/hares\s+\-modify\s+\w+\s+WorkLoad\s+\-delete\s+\-keys/mx) {
            Msg::log("Modify main.cmd from: $line");
            $line=' ';
            Msg::log("                  To: $line");
        }
...
sub haconf_dumpmakero {
    ...
    $syslist=CPIC::get('systems');
    $sys = $$syslist[0];
    $prod->haconf_dumpmakero_sys($sys);
    return;
}

sub haconf_dumpmakero_sys {
    my ($prod,$sys) = @_;
    my ($rtn,$done,$count);
...
sub haconf_makerw {
    ...
    $syslist=CPIC::get('systems');
    $sys = $$syslist[0];
    $prod->haconf_makerw_sys($sys);
    return;
}

sub haconf_makerw_sys {
    my ($prod,$sys) = @_;
    my $rtn;
    $rtn = $sys->cmd("$prod->{bindir}/haclus -value ReadOnly");
    if ($rtn eq '1') {
...
sub opt_delnode {
    ...
    my $cprod=CPIC::get('prod');
    my $prod = $vcs->prod($cprod);
    for my $sub (qw(delnode_precheck delnode_completion))
    {
        if ($prod->can($sub)) {
            $prod->$sub();
        } else {
...
sub addnode_get_cluster {
    ...
    while(1) {
        ...
        if (Cfg::opt('responsefile') && $cprod!="SNAS") {
        ...
        next unless($done);
        # update_status_file copies status file among cluster nodes, so only after the communication
        # is checked can update_status_file be called
        $rel->update_status_file("ADD","Checking cluster/new node(s) status",$cfg->{newnodes});
        ...
        if ($done) {
            unless (Cfg::opt('responsefile') && $cprod!="SNAS") {
                $cfg->{vcs_clustername} = $clustername;
                $cfg->{vcs_clusterid} = $clusterid;
        ...
        if (Cfg::opt('responsefile')) {
            if (CPIC::get('prod') =~ /SNAS/) {
                $msg = Msg::new("Unable to add node(s) because checking the cluster information failed");
            } else {
...
sub addnode_get_newnode {
    ...
    while (1) {
        ...
        if (Cfg::opt('responsefile')) {
            if (CPIC::get('prod') =~ /SNAS/) {
                $msg = Msg::new("Unable to add node(s) because checking the new node(s) information failed");
            } else {
                $msg = Msg::new("Failed to add node(s) using response file");
            }
            $msg->die();
...
sub addnode_compare_systems {
    ...
    my $prod = $cpic->prod();
    my $padv = $cpic->{padv};
    my @prodlist = qw(SFRAC61 SFSYBASECE61 SVS61 SNAS60 SFCFSHA61 SFHA61 VCS61);
...
sub addnode_configure_cluster {
    ...
    $tmpdir=EDR::tmpdir();
    $firstnode = Obj::sys(${$cfg->{clustersystems}}[0]);
    $n = (CPIC::get('prod') =~ /SNAS/) ? 1 : scalar(@{$cfg->{newnodes}}) + scalar(@{$cfg->{clustersystems}});
    # create gabtab
    $gabtab = "/sbin/gabconfig -c -n$n\n";
...
sub addnode_start_cluster {
    ...
    $stage->log();
    $rel->update_status_file("ADD","Starting processes on new node(s)",$cfg->{newnodes});
...
sub addnode_configure_fencing {
    ...
    my($ret,$cps_aref,$ports_href,$conf,$uuid,$errmsg,$max_pri);
    ...
    for my $sysi (@{$cfg->{newnodes}}) {
        ...
        if ($vxfenmode eq 'scsi3' || (($vxfenmode eq 'customized') && ($vxfen_mechanism eq 'cps'))) {
            if ($vxfenmode ne 'scsi3') {
                $max_pri++;
                $ret = $vxfen_pkg->addnode_configure_cpc_sys($sys, $cfg->{vcs_clustername}, $max_pri,
...
sub addnode_completion {
    ...
    if ($#$errors < 0) {
        ...
        $msg->add_summary(1);
        $rel->update_status_file("ADD",'',$cfg->{newnodes},"DONE");
    } else {
...
sub delnode_precheck {
    my $prod = shift;
    my ($had,$edr,$cpic,$localsys,$conf,$cfg,$msg,$sys,$failed,$host,@systems,@nodes,$nodestr,@run_nodes,$rel); 
    ...
    $edr->{savelog} = 1;
    while(1) {
        $conf = $prod->get_config_sys($localsys);
        if ($conf){
        ...
        for my $sysi (@{$cfg->{systems}}) {
            $host = transform_system_name($sysi);
            push(@systems, $host);
        }
        # Check the given nodes are in the cluster
        @nodes = @{EDRu::arrdel(\@systems, @{$cfg->{clustersystems}})};
        if ($#nodes > -1) {
            $nodestr = join(' ',@nodes);
            Msg::n();
            $msg = Msg::new("Specified nodes $nodestr not in the cluster");
            $msg->error();
            $failed = 1;
            last;
        }
        ...
        $cpic->{systems} = $edr->init_sys_objects();
        # check communication for all nodes
        for my $sysi (@{$cfg->{clustersystems}}) {
            ...
            if ($sys->{islocal} && EDRu::inarr($sys->{sys},@{$cfg->{systems}})) {
                Msg::n();
                $msg = Msg::new("The node selected for deletion is the console node. Deleting the console node $sysi is not allowed.");
                $msg->error();
                $failed = 1;
                last;
            }
        ...
        # Check VCS running status
        for my $sysi (@{$conf->{systems}}) {
            $sys = ($Obj::pool{"Sys::$sysi"}) ? Obj::sys($sysi) : Sys->new($sysi);
            next if((defined $sys->{canssh}) && !$sys->{canssh});
            $had = $sys->proc('had61');
            $msg = Msg::new("Checking $prod->{abbr} running state on $sysi");
            $msg->left();
            if ($had->check_sys($sys, 'poststart')) {
        ...
        # Input nodes should not be all the running nodes in the cluster
        $rel->{ru_idle_list} = EDRu::arrdel(\@run_nodes, @systems);
        @nodes = @{$rel->{ru_idle_list}};
        if ($#nodes < 0) {
            Msg::n();
            $nodestr = join(' ',@run_nodes);
            $msg = Msg::new("Only $nodestr running in the cluster. Deleting the node(s) from the cluster would leave no running nodes.");
            $msg->error();
            $failed = 1;
        }
        last;
...
sub delnode_update_maincf {
    my $prod = shift;
    my ($sys0,$nodename,$output,@sglist);
    my $edr = Obj::edr();
    my $cfg = Obj::cfg();

    $sys0 = $edr->{localsys};
    
    $output = $sys0->cmd("_cmd_hagrp -list 2>/dev/null | _cmd_awk '{print \$1}'| _cmd_sort -u");
    @sglist = split(/\n/,$output) if ($output);
    return unless (@sglist);

    $prod->haconf_makerw_sys($sys0);
    for my $sys (@{CPIC::get('systems')}) {
        $prod->{uncfg_sg} = ();
        $nodename = $sys->{vcs_sysname};
        $nodename ||= $sys->{sys};
        $prod->delnode_sgs_sys($sys0, \@sglist, $nodename);
        $sys0->cmd("_cmd_hasys -delete $nodename");
    }
    delete $prod->{uncfg_sg};
    $prod->haconf_dumpmakero_sys($sys0);
    $sys0->cmd("_cmd_vxclustadm -m vcs reinit");
    return;
}

sub delnode_sgs_sys {
    my ($prod,$sys,$sglist,$node) = @_;
    my ($output,@deplist);

    for my $sg (@{$sglist}) {
        next if(defined $prod->{uncfg_sg}{$sg});
        $output = $sys->cmd("_cmd_hagrp -dep $sg 2>/dev/null | _cmd_grep -v ^# | _cmd_awk '{print \$1}' | _cmd_uniq");
        if ($output) {
            @deplist = split(/\n/,$output);
            @deplist = @{EDRu::arrdel(\@deplist, $sg)};
            $prod->delnode_sgs_sys($sys,\@deplist, $node);
        }
        $sys->cmd("_cmd_hagrp -modify $sg AutoStartList -delete $node");
        $sys->cmd("_cmd_hagrp -modify $sg SystemList -delete $node");
        $prod->{uncfg_sg}{$sg} = 1;
    }
    return;
}

sub delnode_update_lltgab {
    my $prod = shift;
    my ($sys0,@remain,$nodename,$n,$content,$sys);
    my $edr = Obj::edr();
    my $cfg = Obj::cfg();
    $sys0 = $edr->{localsys};
    @remain = @{EDRu::arrdel($cfg->{clustersystems}, @{$cfg->{systems}})};
    # Update llthosts/gabtab for all remain nodes
    for my $sys (@{CPIC::get('systems')}) {
        $nodename = $sys->{vcs_sysname};
        $nodename ||= $sys->{sys};
        $sys0->cmd("_cmd_sed -i '/$nodename/d' $prod->{llthosts}");
    }
    $n = (CPIC::get('prod') =~ /SNAS/) ? 1 : scalar(@remain);
    $content = "/sbin/gabconfig -c -n$n\n";
    $sys0->writefile($content, $prod->{gabtab});
    
    for my $sysi (@remain) {
        next if ($sysi eq $sys0->{sys});
        $sys = Obj::sys($sysi);
        $sys0->copy_to_sys($sys,$prod->{llthosts});
        $sys0->copy_to_sys($sys,$prod->{gabtab});
    }
}

sub delnode_conf_backup {
    my $prod = shift;
    my ($sys);
    my $cfg = Obj::cfg();
    for my $sysi (@{$cfg->{systems}}) {
...
#    my $web = Obj::web();
    ...
    $errors = $cpic->failures('stopfailmsg', 'startfailmsg', 'errors');
    if ($#$errors < 0) {
        $msg = Msg::new("Delete node completed successfully");
        $msg->add_summary(1);
        $rel->update_status_file("DEL",'','',"DONE");
    } else {
#        $prod->addnode_handle_errors($cpic);
#        $web->{complete_failed} = 1;
        $msg = Msg::new("Delete node did not complete successfully");
        $msg->add_summary(1);
        $rel->update_status_file("DEL",'',$cfg->{systems},"FAILED");
    }
    $msg->bold();
...
sub init_plat {
    ...
    $prod->{recpkgs}=[ qw(VRTSveki61 VRTSllt61 VRTSgab61 VRTSvxfen61 VRTSamf61 VRTSvcs61 VRTSvcsag61 VRTSvcsea61 VRTSsfmh60 VRTSvbs61 VRTSvcswiz61) ];
...
sub init_plat {
    ...
    $prod->{recpkgs}=[ qw(VRTSllt61 VRTSgab61 VRTSvxfen61 VRTSamf61 VRTSvcs61 VRTSvcsag61 VRTSvcsea61 VRTSsfmh60 VRTSvbs61) ];
...
sub init_plat {
    ...
    $prod->{recpkgs}=[ qw(VRTSllt61 VRTSgab61 VRTSvxfen61 VRTSamf61 VRTSvcs61 VRTSvcsag61 VRTSvcsdr61 VRTSvcsea61 VRTSsfmh60 VRTSvbs61 VRTSvcswiz61) ];
...
sub init_padv {
    ...
    # SNAS pkg list
    $prod->{allpkgs}=[ qw(VRTSllt61 VRTSgab61 VRTSvxfen61 VRTSamf61 VRTSvcs61 VRTScps61 VRTSvcsag61 VRTSvcsdr61 VRTSvcsea61 VRTSsfmh60 VRTSvbs61) ];
    $prod->{minpkgs}=[ qw(VRTSllt61 VRTSgab61 VRTSvxfen61 VRTSamf61 VRTSvcs61 VRTSvcsag61 VRTSsfmh60) ];
    $prod->{recpkgs}=[ qw(VRTSllt61 VRTSgab61 VRTSvxfen61 VRTSamf61 VRTSvcs61 VRTSvcsag61 VRTSvcsdr61 VRTSvcsea61 VRTSsfmh60 VRTSvbs61) ];
...
sub init_plat {
    ...
    $prod->{minpkgs}=[ qw(VRTSllt61 VRTSgab61 VRTSvxfen61 VRTSamf61 VRTSvcs61 VRTSvcsag61 VRTSsfmh60 VRTSvcswiz61) ];
    $prod->{recpkgs}=[ qw(VRTSllt61 VRTSgab61 VRTSvxfen61 VRTSamf61 VRTSvcs61 VRTSvcsag61 VRTSvcsea61 VRTSsfmh60 VRTSvbs61 VRTSvcswiz61) ];
...
sub padv_maincf_upgrade_precheck_sys {
    ...
    my ($grp_container_type,$grp_container_names,$res_container_type,$res_container_names,$sys_container);
        ...
        for my $grp_name (@grps) {
            ...
            for my $res_name (@reses) {
                ...
                if ($allsys_container || keys %sys_containers) {
                    ...
                    $sys->set_value("maincf_res_container_type,$res_name", $res_container_type);
                }
            }
...
sub padv_maincf_upgrade_sys {
    ...
    my (@reses,$res_name,$res_type,$res_container_type,$container_opts);
    ...
    for my $grp_name (sort keys %{$sys->{maincf_grp_container_name}}) {
        ...
        for my $res_name (@reses) {
            ...
            $res_container_type=$sys->{maincf_res_container_type}{$res_name};
            if($res_container_type) {
