...
sub upgrade_precheck_sys {
    ...
    #$prod->cache_areas_sys($sys) unless (Cfg::opt(qw(upgrade_nonkernelpkgs snas_upgrade upgradecv)));
    $prod->open_volumes_sys($sys, 'upgrade') unless (Cfg::opt(qw(upgrade_nonkernelpkgs snas_upgrade)));
...
sub encap_upgrade_precheck_sys {
    ...
    if (($vers =~ /6\.0\.00\d/)&&($sys->{padv} =~ /RHEL6/mx)) {
...
sub dmp_osn_stop_sys {
    ...
    if ($out =~ /dmp_native_support\s+on/mx) {
        ...
        $sys->cmd("_cmd_touch $prod->{migrate_native_file}") if (Cfg::opt(qw(upgrade patchupgrade hotfixupgrade stop)));
    } else {
...
sub postcheck_vm_disabled_vol_sys{
    ...
    for my $dg (split(/\n/,$dgs)) {
        ...
        $vols = $sys->cmd("_cmd_vxprint -g $dg -q 2>/dev/null | _cmd_awk '/^v / {print \$2,\$4}'");
        if ($vols) {
            for my $line (split(/\n/,$vols)) {
...
sub dg_conf_upgrade_precheck_sys {
    ...
    if ($sys->{dgs_need_backup}) {
        ...
        #donot print this warning message during snas upgrade
        $sys->push_warning($msg) unless(Cfg::opt('upgrade'));
    } elsif ($dglist) {
