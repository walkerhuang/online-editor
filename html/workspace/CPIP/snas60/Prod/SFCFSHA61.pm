...
sub postcheck_gabconfig_sys {
    ...
    for my $port (qw(a b d f h u v w y)) {
        next if (($port eq 'd') && (CPIC::get('prod') =~ /SNAS/ || $prod->sfsybasece()));
...
sub addnode_config_cvm_cfs {
    ...
    if ($cprod ne 'SFRAC61') {
        ...
        for my $sysi (@{$cfg->{newnodes}}) {
            ...
            $rtn = $firstnode->cmd("$vcs->{bindir}/hagrp -online -propagate cvm -sys $system");
            if (EDRu::isverror($rtn)) {
                Msg::log("Make cvm online failed on $sysi");
                $status = 0;
            }
            $rtn = $firstnode->cmd("$vcs->{bindir}/hagrp -wait cvm State ONLINE -sys $system -time 300");
            if (EDRu::isverror($rtn)||EDR::cmdexit()) {
...
sub delnode_update_maincf {
    my $prod = shift;
    my ($sys0,$nodename,$output,@reslist);
    my $edr = Obj::edr();
    my $vcs = $prod->prod('VCS61');
    $sys0 = $edr->{localsys};

    $vcs->haconf_makerw_sys($sys0);
    for my $sys (@{CPIC::get('systems')}) {
        $nodename = $sys->{vcs_sysname};
        $nodename ||= $sys->{sys};
        $sys0->cmd("_cmd_hares -modify cvm_clus CVMNodeId -delete $nodename");
        $output = $sys0->cmd("_cmd_hares -list Type=CFSMount 2>/dev/null | _cmd_awk '{print \$1}' | _cmd_sort -u");
        if ($output) {
            @reslist = split(/\n/, $output);
            for my $res (@reslist) {
                $sys0->cmd("_cmd_hares -modify $res NodeList -delete $nodename");
            }
        }
    }
    $vcs->delnode_update_maincf();
    $vcs->haconf_dumpmakero_sys($sys0);
    return;
}
