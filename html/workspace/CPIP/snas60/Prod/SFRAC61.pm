...
# Display choice for creating a new disk group or use an existing disk group
    ...
    if ($index == 1 || $index == 3) {
        if ($index == 1) {
            ...
            eval {$availdisks = $master->cmd('_cmd_vxdisk -o fullshared -o alldgs list | _cmd_grep online | _cmd_grep -v LVM | _cmd_grep -v error');};
            $errstr = $@;
            if ($errstr) {
    ...
    # Volume size for OCR
            if (Cfg::opt('responsefile')) {
                ...
                if (Obj::webui()) {
                    ...
                    $question = Msg::new("Enter the volume size for OCR (in MB, at least 600MB for Oracle 12c):");
            ...
            while (1) {
                ...
                if (Cfg::opt('responsefile')) {
                    ...
                    if (Obj::webui()) {
                        ...
                        $question = Msg::new("Enter the volume size for OCR and Voting disk (in MB, at least 920MB for Oracle 12c):");
                        $answer = $question->ask($defvolsize, $help, 1);
                        goto DNA_SELECT_STORAGE if (EDR::getmsgkey($answer,'back'));
                    }
                }
...
sub ask_haip_ips_sys {
    ...
    $cfg->{$sys->{sys}}->{$inf}->{haipip} = $priv_ip if (!Cfg::opt('responsefile'));
    return $priv_ip;
}
...
sub install_oracle_clusterware {
    ...
    if (Cfg::opt('responsefile')) {
        ...
        if ($prod->{crs_release} !~ /11.2/m && $prod->{crs_release} !~ /12.1/m) {
            $temp_user = $cfg->{oracle_user};
        } else {
            $temp_user = $cfg->{grid_user};
        }
...
sub invoke_oui_installer {
    ...
    if (Cfg::opt('responsefile')) {
        ...
        if ($release !~ /11.2/m && $release !~ /12.1/m) {
...
sub install_oracle_database {
    ...
    if (Cfg::opt('responsefile')) {
        ...
        if ($prod->{db_release} !~ /11\.2|12\.1/m) {
...
DNA_ORACLE_VERSION:
    ...
    while (1) {
        ...
        if (($release ne '10.2') && ($release ne '11.1') && ($release ne '11.2') && ($release ne '12.1')) {
...
sub relink_oracle_database {
    ...
    $cfg->{relink_oracle_database} = 0 if (Cfg::opt('responsefile') && !Cfg::opt('upgrade_kernelpkgs'));
...
sub check_sfracoracleintegration_sys {
    ...
    for my $sys (@{$syslistref}) {
        ...
        goto OPROCD_CHECK if (($is_sfcfsrac) || ($prod->{oraver} =~ /11.2/m) || ($prod->{oraver} =~ /12.1/m));
...
CRS_END:
        if ($prod->{crs_release} =~ /11\.2|12\.1/m){
    ...
    while (($crshome eq '') && (!$done)) {
        ...
        for my $sys (@{$syslistref}) {
            if (!$sys->is_dir($crshome)) {
                if ($prod->{oraver} =~ /11.2/m || $prod->{oraver} =~ /12.1/m){
...
sub check_liblink_sys {
    ...
    if (($oraversion =~ /12.1/m) ||
        ($oraversion =~ /12.2/m)) { # Keep adding 11.2, 11.3 here as they get realesed
        $oracle_num = '12';
    }
...
sub relink_oracle {
    ...
    my $orclodm = "$prod->{db_home}/lib/libodm$oracle_num.so";
...
sub plat_check_kernelparams_sys {
    ...
    if ($oraversion =~ /11.2/m || $oraversion =~ /12.1/m){
        $shmmni_value = 4096;
    } else {
        $shmmni_value = 2046;
    }
...
sub copy_libodm {
    ...
    for my $sys (@cpnodes) {
        ...
        $sys->cmd("_cmd_chmod 644 $homedir/lib/libodm$oracle_num.so");
...
sub set_install_env {
    ...
    if ((($release eq '10.2') && ($patch_level eq '0.1')) ||
        ...
        (($release eq '11.2')) || (($release eq '12.1'))) {
        if ($ENV{'SKIP_ROOTPRE'} eq 'TRUE') {
            $msg = Msg::new("SKIP_ROOTPRE environment variable is set. Will not invoke Oracle rootpre.sh script.");
            $msg->print;
        } else {
...
DNA_ROOTPRE:
    if (($release eq '11.1') || ($release eq '11.2') || ($release eq '12.1')) {
        for my $sys (@{CPIC::get('systems')}) {
    ...
    if ((($release eq '10.2') && ($patch_level eq '0.4')) ||
        ($release eq '11.1') || ($release eq '11.2') || ($release eq '12.1')) {
        for my $sys (@{CPIC::get('systems')}) {
...
sub do_relink_oracle {
    ...
    if ($prod->{db_release} =~ /11\.2|12\.1/mx) {
...
sub plat_check_kernelparams_sys {
    ...
    if ("$cmd_ret" eq '') {
        ...
        $msg->log;
        $param_list->[++$counter] = "maxuproc value is UNKNOWN on $sys->{sys}. It should be 16384 or more for oracle versions 11.2/12.1 or 2048 or more for oracle versions less than 11.2/12.1 ";
        $status = 1;
    } elsif ((0+$cmd_ret) < $maxuprocval) {
        $msg = Msg::new("maxuproc parameter's value is NOT as per the requirement on $sys->{sys}. It should be 16384 or more for Oracle versions 11.2/12.1 or 2048 or more for Oracle versions less than 11.2/12.1 ");
        $msg->log;
        $param_list->[++$counter] = "maxuproc value is NOT as per the requirement on $sys->{sys}. It should be 16384 or more for oracle versions 11.2/12.1 or 2048 or more for oracle versions less than 11.2/12.1.";
        $status = 1;
    } else {
