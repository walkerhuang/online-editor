...
sub init_common {
    ...
    $pkg->{vxfentab_file}='/etc/vxfentab';
...
sub configure_fencing {
    ...
    while (1) {
        ...
        if ((($menu == 1) || ($menu == 2)) && (CPIC::get('prod')!~/SNAS/m)) {
        ...
        if ($menu==2) {
            if (CPIC::get('prod') =~ /SNAS/m) {
                $fencing_failed=$pkg->configure_snas_disk_fencing($dgtype,$diskpolicyOptions);
                $mode = 'disk' if (!$fencing_failed);
            } else {
...
sub create_vxfencoorddg {
    my ($pkg,$disks,$dgname,$donotdeport) = @_;
    ...
    # Deport the DG
    $ret = $pkg->deport_diskgroup_on_sys($sys0, $dgname) if (!$donotdeport);
    return $ret;
}
...
sub configure_snas_disk_fencing {
    my ($pkg,$dgtype,$diskpolicyOptions) = @_;
    my ($backopt,$cfg,$choice,$dglist,$diskpolicy,$menuopt,$msg,$ret,$sys,$vxvmdisks,$initdone);
    my (@disks,@items,@multisels,@sel_disks,$dgname,$disk,$donotdeport);
    $cfg = Obj::cfg();
    $diskpolicy='dmp';
    $dgname = 'sfscoorddg';
    $initdone = 0;
    ...
    while (1) {
        ...
        $msg=Msg::new("\nList of available disks to create a fencing disk group");
        $msg->print;
        if(@disks<3) {
            my $numdisks = $#disks + 1;
            if ($initdone) {
                $msg=Msg::new("The fencing disk group cannot be created as the number of available free VxVM CDS disks is $numdisks which is less than three. Make sure the disks initialized are shared disks. Only shared disks can be used to create fencing disk group. The installer can help to initialize more disks as VxVM disks.");
            } else {
                $msg=Msg::new("The fencing disk group cannot be created as the number of available free VxVM CDS disks is $numdisks which is less than three. The installer can help to initialize more disks as VxVM disks.");
            }
            $msg->print;
            ($ret,$vxvmdisks) = $pkg->init_vxvm_disks();
            return $ret if ($ret);
            if (@{$vxvmdisks}) {
                ...
                $msg=Msg::new("Initializing more VxVM disks failed.");
                $msg->print;
                return 1;
            }
        }
        my @menu_disks = ();
        for my $disk (@disks) {
            my $size = $dglist->{disksize}{"$disk"};
            $size = int($size/2048);
            push @menu_disks, "$disk ($size MB)";
        }
        $menuopt=[ @menu_disks ];
        ...
        # enable multi select and paging
        $choice=$msg->menu($menuopt,'','','',1,1);
        if (EDR::getmsgkey($choice,'back')) {
            return -1;
        }
        @multisels=@{$choice};
        next if (!$pkg->validate_disks_selection(@multisels));
        $ret = 0;
        @sel_disks = ();
        for my $choice (@multisels) {
            $disk = $disks[$choice-1];
            if ($pkg->check_keys_on_disk_sys($sys,$disk)) {
                $ret = 1;
                last;
            }
            push (@sel_disks, $disk);
    ...
    if (Cfg::opt('responsefile')) {
        @sel_disks = @{$cfg->{fencing_newdg_disks}};
        for my $disk (@sel_disks) {
            return 1 if ($pkg->check_keys_on_disk_sys($sys,$disk));
        }
    }

    # check and destroy existing fencing dg
    if (!$pkg->validate_diskgroup($dgname, 1)) {
        Msg::log("fencing dg $dgname is already existed, destroy it before creating new fencing dg...");
        if (defined($dglist->{$dgname}{state}) && ($dglist->{$dgname}{state} eq 'deported')) {
            $sys->cmd("_cmd_vxdg -t import $dgname 2>/dev/null");
        }

        $sys->cmd("_cmd_vxdg -o coordinator destroy $dgname 2>/dev/null");
    }

    # create new fencing dg
    Msg::n();
    return 1 if ($pkg->create_vxfencoorddg(\@sel_disks,$dgname,$donotdeport));
    if (!$pkg->validate_diskgroup($dgname)) {
        $msg =Msg::new("Disk group $dgname is not available on all the systems");
        $msg->print;
        return 1;
    }
    return 1 if ((Cfg::opt('responsefile')) && !$pkg->validate_dgnum($dgname));

    $cfg->{fencing_newdg_disks}=[ @sel_disks ];

    Msg::n();
    $pkg->stop_vcsfen();
    $pkg->update_disk_fencing_file($dgname,$diskpolicy);

    $cfg->{fencing_dgname}=$dgname;
    $cfg->{fencing_scsi3_disk_policy}=$diskpolicy;

    $ret = $pkg->start_vxfen_on_all_systems();
    return $ret if ($ret);

    return 0;
}

sub check_vm_mode_sys {
    my ($pkg,$sys) = (@_);
    my ($msg,$vmmode,$web);
    $web=Obj::web();
...
sub check_keys_on_disk_sys {
    my ($pkg,$sys,$disk) = (@_);
    my ($devpath,$msg,$unused);
    $devpath = '/dev/vx/rdmp/';
...
sub update_disk_fencing_file {
    my ($pkg,$dgname,$diskpolicy) = (@_);
    my ($str);
...
sub unregister_keys_sys {
    my ($pkg,$sys) = @_;
    my ($nodename,$output,@lines,@keys,$msg,$failed);

    my $edr = Obj::edr();
    $failed = 0;
    $nodename = $sys->{vcs_sysname};
    $output = $sys->cmd("_cmd_vxfenadm -s all -f $pkg->{vxfentab_file} 2>/dev/null | _cmd_grep -B1 $nodename | _cmd_sort -u | _cmd_grep 'Character Format' | _cmd_awk -F': ' '{print \$2}'");
    unless ($output) {
        $msg = Msg::new("Found no registered keys");
        $msg->printn();
        return 1;
    }
    @keys = split(/\n/, $output);
    for my $key (@keys) {
        $sys->cmd("_cmd_vxfenadm -x -k $key -f $pkg->{vxfentab_file} 2>/dev/null");
        if (EDR::cmdexit()) {
            $msg = Msg::new("$edr->{cmdstderr}");
            $msg->error();
            $failed = 1;
        }
    }
...
sub get_dglist_sys {
    ...
    for my $line (split(/\n/,$list)) {
        ...
        if ($group eq '-') {
            ...
            my $disk_size = $sys->cmd("_cmd_vxdisk -p list $disk 2>/dev/null | _cmd_grep LUN_SIZE |_cmd_awk '{print \$3}'");
            $dglist->{disksize}{"$disk"} = $disk_size;
        } else {
            if ($group=~/[\(\)]/m) {
                $group=~s/[\(\)]//mg;
...
sub config_vxfen {
    ...
    if (($option =~ /^(cps|disk|sybase)$/m) && $done && (CPIC::get('prod')!~/SNAS/m)) {
