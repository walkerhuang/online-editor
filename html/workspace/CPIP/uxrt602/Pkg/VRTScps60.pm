...
sub ask_cps_info {
    ...
    my ($defopt,$sys0,$ret,$web,$result,$vcs,$sec_or_fips);
    $sys0 = ${CPIC::get('systems')}[0];
    $vcs = $pkg->prod("VCS60");
    $backopt = 1;
    $defport = $pkg->{defport};
...
sub ask_vol_sys {
    ...
    my ($answer,$backopt,$cfg,$choice,$menuopt,$msg,$ret,$vcs);
    my (@disks,$enable_mirror,$disklist,$fs,$min_volsize,$mkret);
    $cfg = Obj::cfg();
    $fs = $pkg->prod('FS60');
    ...
    if($choice == 1) {
        ...
        while (1) {
            ...
            $volsize = $answer;
            next if ($vcs->validate_vol($volname, $volsize, 0, $dgname,$sys,$enable_mirror)); 
            last;
        }
...
sub addto_vcs_config {
    ...
    my ($cfg,$defopt,$fs,$line,$nwhosts_same,$vxfen_pkg,$vcs);
    @systems = @{CPIC::get('systems')};
    $sys0 = $systems[0];
    $fs = $pkg->prod('FS60');
    $vcs = $pkg->prod('VCS60');
...
sub verify_responsefile {
    my $pkg = shift;
    my ($cfg,$msg,$num,$sys0,$vxfen_pkg);
    my (@nics,@nw_hosts,@ports,@systems,@vips,$vcs);
        ...
        if ($cfg->{cps_sfha_config}) {
            ...
            if (!$cfg->{cps_newdg_disks}) {
                ...
                if ($cfg->{cps_newvol_volsize}) {
                    ...
                    if ($vcs->validate_vol($cfg->{cps_volume}, $cfg->{cps_newvol_volsize}, 0, $cfg->{cps_diskgroup},$sys0,$cfg->{cps_enable_mirroring})) {
