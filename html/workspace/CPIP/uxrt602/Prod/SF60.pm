...
sub init_common {
    ...
    if($edr && (-d "$edr->{mediapath}/storage_foundation_basic")) {
        ...
        $prod->{eula}='EULA_SF_Basic_Ux_6.0.2.pdf';
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSvxvm60 VRTSveki60 VRTSjre15.rte VRTSjre.rte VRTSsfcpi602 VRTSperl514
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSvxfs60 VRTSvxvm60 VRTSjre15 VRTSjre VRTSsfcpi602 VRTSperl514
        VRTSvlic32 VRTSwl
    ) ];
    return;
}
