...
sub init_common {
    ...
    if ($prod->{class} =~ /DMP/m) {
        ...
        $prod->{eula}='EULA_DMP_Ux_6.0.2.pdf';
        ...
        $prod->{eula}='EULA_SF_Ux_6.0.2.pdf';
...
sub is_dg_mix_clone_sys {
    ...
    $disknum=0;
    $clonedisknum=0;
    $disks = $sys->cmd("_cmd_vxdisk list -q -g $dg 2>/dev/null");
    for my $disk (split(/\n/, $disks)) {
        $disknum++;
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSsfcpi602 VRTSperl514 VRTSperl.rte VRTSvlic32
    ) ];
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSjre VRTSsfcpi602 VRTSperl514 VRTSvlic32 VRTSwl
    ) ];
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSsfcpi602 VRTSperl514 VRTSvlic32
    ) ];
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSicsco VRTSvxvm60 VRTSjre15 VRTSjre VRTSsfcpi602 VRTSperl514
        VRTSvlic32
    ) ];
