...
sub postcheck_lltconfig_sys {
    ...
    if($downlinks && $sys->system1){
        ...
        foreach my $line(@{$downnics}){
            ...
            if ( $nic2 eq 'all' ) {
                $msg=Msg::new("\n\tThe NIC $nic1 on $sys1 can not communicated with any NIC configured for LLT on $sys2. It is recommended to check the configuration of the NIC");
            } else {
...
sub init_plat {
    ...
    $prod->{minpkgs}=[ qw(VRTSllt60 VRTSgab60 VRTSvxfen60 VRTSamf60 VRTSvcs60 VRTSvcsag60) ];
    $prod->{recpkgs}=[ qw(VRTSllt60 VRTSgab60 VRTSvxfen60 VRTSamf60 VRTSvcs60 VRTSvcsag60 VRTSvcsea60 VRTSsfmh41 VRTSvbs60) ];
    $prod->{deleted_agents} = [ qw(Disk DiskReservation NFSLock ServiceGroupHB
    ...
    if ($onenode) {
        ...
        if ($rootpath) {
            ...
            if ($sys->{padv} =~ /^Sol11/m) {
                ...
                $sys->cmd("echo /usr/bin/cp /var/svc/manifest/system/$manifest /var/tmp/temp_manifest >> $smfprofile_upgrade");
            ...
            if ($sys->{padv} =~ /^Sol11/m) {
                ...
                $sys->cmd("_cmd_cp /var/svc/manifest/system/$manifest /var/tmp/temp_manifest");
...
package Prod::VCS60::Sol11sparc;
@Prod::VCS60::Sol11sparc::ISA = qw(Prod::VCS60::SunOS);
...
package Prod::VCS60::Sol11x64;
@Prod::VCS60::Sol11x64::ISA = qw(Prod::VCS60::SunOS);
