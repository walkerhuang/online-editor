...
package Rel::UXRT60PR1::Common;
@Rel::UXRT60PR1::Common::ISA = qw(Rel);
...
sub init_common {
    ...
    $rel->{allpkgs}=[ qw(VRTSperl512 VRTSvlic32 VRTSsfcpi60 VRTSspt60) ];
    $rel->{minpkgs}=[ qw(VRTSperl512 VRTSvlic32 VRTSsfcpi60) ];
    $rel->{recpkgs}=[ qw(VRTSperl512 VRTSvlic32 VRTSsfcpi60 VRTSspt60) ];
...
sub init_args {
    ...
    $rel->{arguments} = $arguments = {
        ...
        'jumpstart' => {
            ...
            'handler' => \&Rel::UXRT60PR1::Common::process_jumpstart_arg,
        },
        'flash_archive' => {
            ...
            'handler' => \&Rel::UXRT60PR1::Common::process_flash_archive_arg,
        },
        'kickstart' => {
            ...
            'handler' => \&Rel::UXRT60PR1::Common::process_kickstart_arg,
        },
        'yumgroupxml' => {
            ...
            'handler' => \&Rel::UXRT60PR1::Common::process_yumgroupxml_arg,
        },
        'nim' => {
            ...
            'handler' => \&Rel::UXRT60PR1::Common::process_nim_arg,
        },

        # UXRT60PR1 option args
        'ignite' => {
            ...
            'handler' => \&Rel::UXRT60PR1::Common::process_ignite_arg,
        },
        ...
        'security' => {
            ...
            'handler' => \&Rel::UXRT60PR1::Common::process_security_arg,
        },
        'securityonenode' => {
...
sub ignite {
    ...
    unless(Cfg::opt('ignite') && $cpic->{padv} =~ /^HPUX/m) {
    ...
    if (EDR::cmdexit()) {
        ...
        # add post_load_cmd = "/opt/VRTS/install/bin/UXRT60PR1/add_install_scripts" in core_media_cfg
        ...
        for my $line (@conts) {
            if ($line=~/sd_software_list/) {
                $filecont.="    ";
                $filecont.="post_load_cmd = \"/opt/VRTS/install/bin/UXRT60PR1/add_install_scripts\"\n";
            }
            $filecont.=$line."\n";
        }
    ...
    if ($localsys->exists($indexfile)) {
        ...
        if (scalar(@os_media_list)>=1){
            ...
            if ($index != scalar(@os_media_list)) {
                ...
                if ($cpic->{prod} =~ /VCS/) {
                    $msg=Msg::new("If the installvcs and uninstallvcs scripts are not generated under the /opt/VRTS/install directory after VCS bundle installation, manually generate these scripts by running the command '/opt/VRTS/install/bin/UXRT60PR1/add_install_scripts'.\n");
                    $msg->nprint();
                } elsif ($cpic->{prod} =~ /DMP/) {
...
@Rel::UXRT60PR1::AIX::ISA = qw(Rel::UXRT60PR1::Common);
...
@Rel::UXRT60PR1::HPUX::ISA = qw(Rel::UXRT60PR1::Common);
...
@Rel::UXRT60PR1::HPUX1131::ISA = qw(Rel::UXRT60PR1::HPUX);
...
@Rel::UXRT60PR1::Linux::ISA = qw(Rel::UXRT60PR1::Common);
...
@Rel::UXRT60PR1::RHEL5x8664::ISA = qw(Rel::UXRT60PR1::Linux);
...
@Rel::UXRT60PR1::RHEL5ppc64::ISA = qw(Rel::UXRT60PR1::Linux);
...
@Rel::UXRT60PR1::RHEL6x8664::ISA = qw(Rel::UXRT60PR1::Linux);
...
@Rel::UXRT60PR1::RHEL6ppc64::ISA = qw(Rel::UXRT60PR1::Linux);
...
@Rel::UXRT60PR1::SLES10x8664::ISA = qw(Rel::UXRT60PR1::Linux);
...
@Rel::UXRT60PR1::SLES10ppc64::ISA = qw(Rel::UXRT60PR1::Linux);
...
@Rel::UXRT60PR1::SLES11x8664::ISA = qw(Rel::UXRT60PR1::Linux);
...
@Rel::UXRT60PR1::SLES11ppc64::ISA = qw(Rel::UXRT60PR1::Linux);
...
@Rel::UXRT60PR1::SunOS::ISA = qw(Rel::UXRT60PR1::Common);
...
@Rel::UXRT60PR1::SolSparc::ISA = qw(Rel::UXRT60PR1::SunOS);
...
@Rel::UXRT60PR1::Solx64::ISA = qw(Rel::UXRT60PR1::SunOS);
...
@Rel::UXRT60PR1::Sol11sparc::ISA = qw(Rel::UXRT60PR1::SunOS);
...
sub init_padv {
    ...
    $rel->{platreqs}=[ 'Solaris 11 (64 bit only)' ];
    $rel->{upgradevers}=[ qw() ];
    $rel->{prods}=EDRu::arrdel($rel->{prods}, qw(SFSYBASECE60));
...
sub preremove_sys {
    my ($rel,$sys) = @_;
    # To fix issue that uninstalling VRTSvlic hang due to some special pipe files
    $sys->cmd("_cmd_rm -rf /etc/vx/vold_* /etc/vx/vxesd 2>/dev/null");
}

package Rel::UXRT60PR1::Sol11x64;
@Rel::UXRT60PR1::Sol11x64::ISA = qw(Rel::UXRT60PR1::Sol11sparc);
...
sub init_padv {
    ...
    $rel->{platreqs}=[ 'Solaris 11 (64 bit only)' ];
    $rel->{upgradevers}=[ qw() ];
    $rel->{prods}=EDRu::arrdel($rel->{prods}, qw(SFSYBASECE60 SFRAC60));
    $rel->{menuprods}=EDRu::arrdel($rel->{menuprods}, qw(SFSYBASECE60 SFRAC60));
    $rel->{pkgsetprods}=EDRu::arrdel($rel->{pkgsetprods}, qw(SFSYBASECE60 SFRAC60));
    $rel->{upgradeprods}=EDRu::arrdel($rel->{upgradeprods}, qw(SFSYBASECE60 SFRAC60));
    $rel->{pkgsspace}=<<"_PKGSPACE_";
VRTSamf:321,2031,3,7
VRTSaslapm:0,1228,0,1926
VRTSat:79534,0,29,1307
