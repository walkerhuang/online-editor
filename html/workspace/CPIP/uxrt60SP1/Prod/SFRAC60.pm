...
sub init_common {
    ...
    $prod->{eula}='EULA_SF_Oracle_Rac_Ux_6.0SP1.pdf';
...
sub completion_messages {
    ...
    if (Cfg::opt(qw(patchupgrade upgrade)) && !Cfg::opt(qw(upgrade_nonkernelpkgs responsefile rootpath)) && (!Obj::webui())) {
...
sub config_sfrac_subcomponents {
    ...
    foreach my $a ("config_file","vcsmm","lmx") {
...
sub config_privnic_common {
    ...
    my ($nodes_ref, $priv_rsrc, $nic_config_complete);
...
sub config_multiprivnic_common {
    ...
    my ($nodes_ref, $mpriv_rsrc, $nic_config_complete);
    my ($cmdout, $device_global);
    my %systems;
    my $vcs = $prod->prod('VCS60');
    my $cfg = Obj::cfg();
...
sub install_oracle_database {
    ...
    foreach my $a ("config_file","vcsmm","lmx","fencing_mode","db_install","relink") {
        push @{$prod->{post_config_check_items}}, $a;
    }
    $prod->{post_config_check_items} = EDRu::arruniq(@{$prod->{post_config_check_items}});
...
sub addnode_menu_sfrac {
    ...
    my ($priv_rsrc_ref, $mpriv_rsrc_ref, $newsys, $nic_config_complete);
    my $web = Obj::web();
...
sub addnode_check_resource {
    ...
    if (EDR::cmdexit() == 0) {
        ...
        for my $rsrc (@tmp_rsrcs) {
            $cmd = "$vcs->{bindir}/hares -display $rsrc | _cmd_grep Device";
            $cmdout = $sys->cmd("$cmd");
            if ($cmdout =~ /global/) {
                next;
            }
...
sub addnode_create_oracle_user_group {
    ...
    if ($prod->{oracle_sgroups}) {
        $msg = Msg::new("Do you want to configure secondary group(s) for the Oracle user $prod->{oracle_user} on new nodes?");
        $ayn= $msg->ayny;
        if ($ret eq 'N') {
            return 0;
        }
