    ...
    for my $lk (sort { $a cmp $b } keys(%{$sys->{keys}})) {
        ...
        for my $lname (@{$prod->{lic_names}}) {
            ...
            if ($pname=~/$temp_name/m) {
                ...
                $vcs_lic = $rel->prod_licensed_sys($sys,'VCS61');
                $sf_lic = $rel->prod_licensed_sys($sys,'SF61',1);
...
sub get_vxkeyless_prod_level {
    ...
    if (!$prod_only) {
        ...
        if ($new_prod !~ /^VCS/) {
            ...
            $new_prod.='_VFR' if (Cfg::opt('vfr') && (!Cfg::opt('vr')) && ($new_prod !~ /^SVS/) && ($uname eq "Linux"));
        }
        ...
        if (($pkg->version_sys($sys,1)) && ($sys->is_dir($vxlic_path))){
            ...
            if($lics) {
                for my $lic (split (/,/m,$lics)) {
                    $vxlicrep.=$sys->cmd("chroot $rootpath _cmd_vxlicrep -k $lic 2>/dev/null");
                }
...
sub ready_to_enter_license_keys {
    ...
    $msg=Msg::new("Sorry! No proper license keys found on the target system(s) for the product. Do you wish to enter license keys? Without a valid license key, the product configure may not complete properly.");
