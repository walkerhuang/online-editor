...
package Prod::SFCFSHA61::Common;
@Prod::SFCFSHA61::Common::ISA = qw(Prod);

sub init_common {
    my $prod = shift;
    $prod->{vers}='6.1.1.000';
    $prod->{default_cluster_protocol_version} = 130;
    ...
    if ($prod->sfcfsha) {
        ...
        $prod->{eula}='EULA_VirtualStore_Ux_6.1.pdf';
        $prod->{mainpkg}='VRTSsvs61';
        ...
        $prod->{eula}='EULA_SF_Sybase_CE_Ux_6.1.pdf';

        $prod->{installscript_prod}='SFSYBASECE61';
    ...
    $prod->{subprods}=[qw(VCS61 SF61 SFHA61)];
...
sub set_pkgs {
    ...
    for my $category (@categories) {
        $prod->{$category}=EDRu::arruniq(@{$rel->{$category}},
                                        ...
                                        @{$prod->{$category}});
       if($prod->sfsybasece()){
            $prod->{$category}=EDRu::arrdel($prod->{$category},'VRTSgms61');
            $prod->{$category}=EDRu::arrdel($prod->{$category},'VRTSodm61');
            $prod->{$category}=EDRu::arrdel($prod->{$category},'VRTScps61');
            $prod->{$category}=EDRu::arrdel($prod->{$category},'VRTSdbed61');
       }
    }
...
sub stop_agents_sys {
    ...
    # If upgrade, patchupgrade, hotfixupgrade or rollback do not stop VCS.
    # Just stop the CVM and CFS Agents. (Offline group cvm)
    # Otherwise the service group frozen operation will fail in later steps(ET3436664 ET355565)
    if (Cfg::opt(qw(upgrade patchupgrade hotfixupgrade rollback))) {
        if ($sys->system1) {
...
sub offline_groups {
    ...
    for my $dep (@{$deps}) {
        ...
        for my $sys_master (@master_nodes) {
            push (@offlinefailuregroups,$grp) if ($prod->offline_group_sys($sys,$sys_master,$grp,$state));
        }
    }
...
sub stop_odm_sys {
    ...
    $gmspkg=$sys->pkg('VRTSgms61');
    if (defined $gmspkg) {
        $vxgms=$sys->proc('vxgms61');
...
sub version_sys {
    ...
    if ($prod->sfsybasece()) {
        ...
        # If SFSYBASECE is licensed, $rel->prod_licensed_sys($sys, 'SFCFSHA61') will return licensed
        $sfcfsha_licensed=$rel->prod_licensed_sys($sys, 'SFCFSHA61', 1);
        ...
        $svs_licensed=$rel->prod_licensed_sys($sys, 'SVS61', 1) if ($svspkg);
...
sub reorder_vxglm {
    my ($prod,$ref_procs) = @_;
    my ($v,@a);
    if((EDRu::inarr('vxglm61', @{$ref_procs})) && (EDRu::inarr('had61', @{$ref_procs}))) {
        $ref_procs=EDRu::arrdel($ref_procs, 'vxglm61');
        for my $v (@{$ref_procs}) {
            if ($v eq 'had61') {
                push (@a,'vxglm61',$v);
            } else {
...
sub postcheck_cvmconfig_sys{
    ...
    if($rtn !~/ONLINE/){
        ...
        $rtn1=$sys->cmd("/opt/VRTSvcs/bin/hagrp -value cvm AutoStartList 2>/dev/null | _cmd_grep $sys->{vcs_sysname}");
        unless($rtn1){
            $msg=Msg::new("\n\tThe attribute AutoStartList is not configured on $sys->{vcs_sysname}");
            push(@m,$msg);
        }
...
package Prod::SFCFSHA61::AIX;
@Prod::SFCFSHA61::AIX::ISA = qw(Prod::SFCFSHA61::Common);

sub init_plat {
    my $prod=shift;
    $prod->{allpkgs}=[ qw(VRTSob34 VRTSglm61 VRTScavf61 VRTSgms61 VRTSodm61) ];
    $prod->{minpkgs}=[ qw(VRTSglm61 VRTScavf61) ];
    $prod->{recpkgs}=[ qw(VRTSob34 VRTSglm61 VRTScavf61 VRTSgms61 VRTSodm61) ];
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        VRTScsocw.rte VRTSdbac61 VRTSdbac.rte VRTSvbs61 VRTSvcsea61 VRTScfsdc
...
package Prod::SFCFSHA61::HPUX;
@Prod::SFCFSHA61::HPUX::ISA = qw(Prod::SFCFSHA61::Common);
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        VRTSdbckp VRTSormap VRTScsocw VRTSdbac61 VRTSvbs61 VRTSvcsea61 VRTScfsdc VRTSodm61
        ...
        VRTSdbed61 VRTSdbcom VRTSsydoc VRTSsybed VRTSvcsApache
        ...
        VRTSvcsag61 VRTScps61 VRTSvcs61 VRTSamf61 VRTSvxfen61 VRTSgab61
        ...
        VRTSvmman SYMClma VRTSspt61 VRTSaa VRTSmh VRTSccg VRTSobgui
        VRTSfspro VRTSdsa VRTSsfmh60 VRTSob34 VRTSobc33 VRTSaslapm61
...
package Prod::SFCFSHA61::Linux;
@Prod::SFCFSHA61::Linux::ISA = qw(Prod::SFCFSHA61::Common);
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        VRTScsocw VRTSvbs61 VRTSvcsvmw61 VRTSvcsea61 VRTScfsdc VRTSodm61 VRTSodm-platform
        VRTSodm-common VRTSgms61 VRTSsvs61 VRTScavf61 VRTSglm61 VRTScpi
        ...
        VRTSvcsmg VRTSvcsdr61 VRTSvcsag61 VRTScps61 VRTSvcs61 VRTSamf61
        VRTSvxfen61 VRTSgab61 VRTSllt61 VRTSfsmnd VRTSfssdk61
        VRTSfsadv61 VRTSfsdoc VRTSfsman VRTSvrdoc VRTSvrw VRTSweb VRTSvcsvr
    ...
    $prod->{obsolete_scripts} = [ qw(installsfsybasece61 uninstallsfsybasece61) ];
    return;
}

package Prod::SFCFSHA61::RHEL5x8664;
@Prod::SFCFSHA61::RHEL5x8664::ISA = qw(Prod::SFCFSHA61::Linux);

sub init_padv {
    my $prod=shift;
    if ($prod->svs()) {
        $prod->{minpkgs}=[ qw(VRTSsfmh60 VRTSglm61 VRTScavf61 VRTSsvs61) ];
        $prod->{recpkgs}=[ qw(VRTSob34 VRTSglm61 VRTScavf61 VRTSsvs61 VRTSgms61 VRTSodm61) ];
        $prod->{allpkgs}=[ qw(VRTSob34 VRTSglm61 VRTScavf61 VRTSsvs61 VRTSgms61 VRTSodm61) ];
        $prod->{proddir}='virtualstore';
    } elsif ($prod->sfcfsha) {
        $prod->{licsuperprods}=[ qw(SFRAC61) ];
    }
...
package Prod::SFCFSHA61::RHEL6x8664;
@Prod::SFCFSHA61::RHEL6x8664::ISA = qw(Prod::SFCFSHA61::Linux);
...
sub init_padv {
    ...
    if ($prod->svs()) {
        ...
        $prod->{licsuperprods}=[ qw(SFRAC61 SFSYBASECE61) ];
    }
    return ;
}

package Prod::SFCFSHA61::RHEL6ppc64;
@Prod::SFCFSHA61::RHEL6ppc64::ISA = qw(Prod::SFCFSHA61::Linux);
...
package Prod::SFCFSHA61::SLES10x8664;
@Prod::SFCFSHA61::SLES10x8664::ISA = qw(Prod::SFCFSHA61::Linux);
...
package Prod::SFCFSHA61::SLES11x8664;
@Prod::SFCFSHA61::SLES11x8664::ISA = qw(Prod::SFCFSHA61::Linux);
...
package Prod::SFCFSHA61::SLES11ppc64;
@Prod::SFCFSHA61::SLES11ppc64::ISA = qw(Prod::SFCFSHA61::Linux);
...
package Prod::SFCFSHA61::SunOS;
@Prod::SFCFSHA61::SunOS::ISA = qw(Prod::SFCFSHA61::Common);
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        VRTScsocw VRTSdbac61 VRTSvbs61 VRTSvcsea61 VRTScfsdc VRTSodm61
        VRTSgms61 VRTSsvs61 VRTScavf61 VRTSglm61 VRTScpi VRTSd2doc VRTSordoc
...
package Prod::SFCFSHA61::SolSparc;
@Prod::SFCFSHA61::SolSparc::ISA = qw(Prod::SFCFSHA61::SunOS);
...
package Prod::SFCFSHA61::Sol11sparc;
@Prod::SFCFSHA61::Sol11sparc::ISA = qw(Prod::SFCFSHA61::SunOS);
...
sub init_padv {
    ...
    if ($prod->svs()) {
        $prod->{minpkgs}=[ qw(VRTSsfmh60 VRTSglm61 VRTScavf61 VRTSsvs61) ];
        $prod->{recpkgs}=[ qw(VRTSglm61 VRTScavf61 VRTSsvs61 VRTSgms61 VRTSodm61) ];
        $prod->{allpkgs}=[ qw(VRTSglm61 VRTScavf61 VRTSsvs61 VRTSgms61 VRTSodm61) ];
        $prod->{proddir}='virtualstore';
    } else {
        $prod->{minpkgs}=[ qw(VRTSglm61 VRTScavf61) ];
        $prod->{recpkgs}=[ qw(VRTSglm61 VRTScavf61 VRTSgms61 VRTSodm61) ];
        $prod->{allpkgs}=[ qw(VRTSglm61 VRTScavf61 VRTSgms61 VRTSodm61) ];
        $prod->{proddir}='storage_foundation_cluster_file_system_ha';
    }
...
package Prod::SFCFSHA61::Solx64;
@Prod::SFCFSHA61::Solx64::ISA = qw(Prod::SFCFSHA61::SunOS);
...
package Prod::SFCFSHA61::Sol11x64;
@Prod::SFCFSHA61::Sol11x64::ISA = qw(Prod::SFCFSHA61::SunOS);
