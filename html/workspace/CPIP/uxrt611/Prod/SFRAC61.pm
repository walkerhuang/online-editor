...
package Prod::SFRAC61::Common;
@Prod::SFRAC61::Common::ISA = qw(Prod);
...
sub version_sys {
    ...
    if ($pkgvers) {
        @mainpkgs = qw(VRTSvxvm61 VRTSvxfs61 VRTSvcs61);
...
sub completion_messages {
    ...
    if (Cfg::opt(qw(hotfixupgrade patchupgrade upgrade rollback)) && !Cfg::opt(qw(upgrade_nonkernelpkgs makeresponsefile rootpath))) {
...
sub poststart_configure_sys {
    ...
    if ( $sys->system1 ) {
        if ($rel->{nowebinstaller}) {
            ...
            #for 611, cannot use webinstaller to configure fencing, so update the message.
            $msg = Msg::new("\nIf you have not yet configured Symantec I/O fencing, you can run the command '$fencing_cmd' to configure it.");
        }
        $msg->print;
        Msg::prtc();
    ...
    while (1) {
        ...
        if ($prod->{"menu_$choice"}) {
            ...
            if ($choice eq 'exit_cleanly' || $choice eq 'continue_ru') {
                if (Cfg::opt(qw(upgrade patchupgrade rollback hotfixupgrade))) {
                    return;
                }
                my $cpic = Obj::cpic();
                $cpic->completion();
...
# Display choice for creating a new disk group or use an existing disk group
    ...
    if ($index == 1 || $index == 3) {
        if ($index == 1) {
            ...
            eval {$availdisks = $master->cmd('_cmd_vxdisk -o fullshared -o alldgs list 2>/dev/null | _cmd_grep online | _cmd_grep -v LVM | _cmd_grep -v error | _cmd_grep -v exported');};
            $errstr = $@;
            if ($errstr) {
            ...
            for my $local_sys (@systems) {
                eval {$localdisks = $local_sys->cmd('_cmd_vxdisk -o local -o alldgs list | _cmd_grep online | _cmd_grep -v LVM | _cmd_grep -v error');};
                $errstr = $@;
                if ($errstr) {
...
DNA_OPTION_CREATE_DG:
    ...
    if ($index == 1 || $index == 3) {
        ...
        if (Obj::webui()) {
            if ($create_fss_dg) {
                $msg = Msg::new("As you create the FSS disk group, it is recommanded that you choose at least one local disk for each node if possible");
                $web->web_script_form("alert", $msg->{msg});
            }
            ...
            $msg = Msg::new("As you create the FSS disk group, it is recommanded that you choose at least one local disk for each node if possible:") if ($create_fss_dg);
        ...
        if ($prod->{is_fss_enabled}) {
            for my $dg (@dgnamelist) {
                my $fss_status = $master->cmd("_cmd_vxdg list $dg | _cmd_awk '/fss:/ {print \$2}'");
                if ($fss_status eq 'off') {
                    push(@shareddg, $dg);
                } else {
                    push(@fssdg, $dg);
                }
...
DNA_CREATE_RAW_VOL:
    ...
    # Volume size for OCR
            ...
            if ($prod->validate_vol($ocrvolname, $ocrvolsize, $unavailsize, $dgname, $master, $enable_mirroring)) {
                return 1 if (Cfg::opt('responsefile'));
                if (Obj::webui()) {
                    goto DNA_CREATE_RAW_VOL ;
                }
    ...
    # Volume size for Vote
                ...
                if ($prod->validate_vol($votevolname, $votevolsize, $unavailsize, $dgname, $master , $enable_mirroring)) {
                    return 1 if (Cfg::opt('responsefile'));
                    if (Obj::webui()) {
                        goto DNA_CREATE_RAW_VOL ;
                    }
 ...
 DNA_CREATE_CFS:
        if ($enable_sep_filesys == 0){
            ...
            while (1) {
                ...
                if ($prod->validate_vol($volname, $volsize, $unavailsize, $dgname, $master, $enable_mirroring)) {
                    return 1 if (Cfg::opt('responsefile'));
                    if (Obj::webui()) {
                        goto DNA_CREATE_CFS;
                    }
...
sub make_online_system {
    ...
    my $ret = 0;
    my $out;
    my $sysname = Prod::VCS61::Common::transform_system_name($sys->{sys});
...
sub set_up_fs {
    ...
    my $sysname = '';
    for my $sys (@{CPIC::get('systems')}) {
        $sysname = Prod::VCS61::Common::transform_system_name($sys->{sys});
...
sub ask_privnic_alias {
    ...
    if ($def_alias eq '') {
        my $sysname = Prod::VCS61::Common::transform_system_name($sys_name);
...
sub ask_haip_aliases {
    ...
    if (Cfg::opt('responsefile')) {
        ...
        $alias = $cfg->{$sys_name}->{$inf}->{hostname_for_ip};
    } else {
        $question = Msg::new("Enter Hostname alias for the above IP address:");
        $alias =  $question->ask($def_alias, $help, $backopt);
...
sub check_cps_installed_for_quorum {
    ...
    $cpspkg = $firstnode->pkg('VRTScps61');
    $web = Obj::web();
...
DNA_ORACLE_VERSION:
    ...
    while (1) {
        if (Cfg::opt('responsefile')) {
            ...
            $question = Msg::new("Enter Oracle Version (e.g. 10.2.0.1, 11.2.0.3):");
...
#        print "\n====$sys->{sys}========\n";
    ...
    Msg::title();
    $msg = Msg::new("The following $prod->{abbr} specific operations need to be performed for the new nodes being added.\n");
    $msg = Msg::new("Some $prod->{abbr} specific operations need to be performed for the new nodes being added to cluster. Options for these operations will appear once you click ok button.\n") if (Obj::webui());
    $web->web_script_form("alert", $msg->{msg}) if(Obj::webui());
    $msg->print;
...
sub addnode_start_procs {
    ...
    for my $sys (@{$prod->{newnodes}}) {
        ...
        my $smallsys = Prod::VCS61::Common::transform_system_name($firstnode->{sys});
        my $vcs = $prod->prod('VCS61');
...
sub addnode_comp_message {
    ...
    my $smallsys = Prod::VCS61::Common::transform_system_name($sys->{sys});
    my $vcs = $prod->prod('VCS61');
    ...
    if ( $prod->{crs_release} eq '' ) {
        ...
        if ($ret) {
            ...
            if (Obj::webui()){
                ...
                $question = Msg::new("Enter Oracle Clusterware Version (e.g. 10.2.0.1, 11.2.0.3):");
                $answer = $question->ask($prod->{$oraprod.'_release'.'.'.$oraprod.'_patch_level'}, $help, $backopt);
            }
            chomp($answer);
...
package Prod::SFRAC61::HPUX;
@Prod::SFRAC61::HPUX::ISA = qw(Prod::SFRAC61::Common);

sub init_plat {
    my $prod=shift;
    $prod->{allpkgs}=[ qw(VRTSob34 VRTSdbac61 VRTSgms61 VRTSodm61 VRTSdbed61 VRTSvcsea61) ];
    $prod->{minpkgs}=[ qw(VRTSdbac61 VRTSgms61 VRTSodm61 VRTSdbed61 VRTSvcsea61) ];
    $prod->{recpkgs}=[ qw(VRTSob34 VRTSdbac61 VRTSgms61 VRTSodm61 VRTSdbed61 VRTSvcsea61) ];
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        VRTSdbckp VRTSormap VRTScsocw VRTSdbac61 VRTSvcsea61 VRTScfsdc VRTSodm61
        VRTSgms61 VRTScavf61 VRTSglm61 VRTScpi VRTSd2doc VRTSordoc
        VRTSfsnbl VRTSfppm VRTSap VRTStep VRTSspc VRTSspcq
        VRTSfasdc VRTSfasag VRTSfas VRTSgapms VRTSmapro VRTSvail
        VRTSd2gui VRTSorgui VRTSvxmsa VRTSvrdev VRTSdbdoc VRTSdb2ed VRTSdbed61
        ...
        VRTSfsmnd VRTSfssdk61 VRTSfsdoc VRTSfsman VRTSvrdoc
        ...
        VRTSjre15 VRTSjre VRTSperl516 VRTSvlic32 VRTSwl
    ) ];
...
package Prod::SFRAC61::Linux;
@Prod::SFRAC61::Linux::ISA = qw(Prod::SFRAC61::Common);
...
sub init_plat {
    ...
    $prod->{zru_releases}=[qw(5.0.30 5.1 6.0 6.1)];
    $prod->{menu_options}=['Symantec Volume Replicator','Symantec File Replicator Option','Global Cluster Option'];

    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        VRTScsocw VRTSdbac61 VRTSvcsvmw61 VRTSvcsea61 VRTScfsdc VRTSodm61
        VRTSodm-platform VRTSodm-common VRTSgms61 VRTScavf61
        VRTSglm61 VRTScpi VRTSd2doc VRTSordoc VRTSfsnbl VRTSfppm
        VRTSap VRTStep VRTSgapms VRTSmapro-common VRTSvail
        VRTSd2gui-common VRTSorgui-common VRTSvxmsa VRTSdbdoc
        VRTSdb2ed-common VRTSdbed61 VRTSdbed-common VRTSdbcom-common
        ...
        VRTSvcsdc VRTSvcsmn VRTSvcsmg VRTSvcsdr61 VRTSvcsag61
        VRTScps61 VRTSvcs61 VRTSamf61 VRTSvxfen61 VRTSgab61 VRTSllt61
        VRTSfsmnd VRTSfssdk61 VRTSfsdoc VRTSfsman VRTSvrdoc
        VRTSvrw VRTSweb VRTSvcsvr VRTSvrpro VRTSalloc VRTSdcli
        VRTSvsvc VRTSvmpro VRTSddlpr VRTSvdid VRTSlvmconv61
        VRTSvmdoc VRTSvmman SYMClma VRTSspt61 VRTSaa VRTSmh
        VRTSccg VRTSobgui VRTSfspro VRTSdsa VRTSsfmh60 VRTSob34
        VRTSobc33 VRTSaslapm61 VRTSat50 VRTSatClient50 VRTSsmf
        VRTSpbx VRTSicsco VRTSvxfs61 VRTSvxfs-platform VRTSvxfs-common
        VRTSvxvm61 VRTSvxvm-platform VRTSvxvm-common VRTSjre15
        VRTSjre VRTSperl516 VRTSvlic32
    ) ];
...
sub plat_check_kernelparams_sys {
    ...
    my ($oraversion,$shmall_value,$shmmni_value,$filemax,$ip_local_port_range_max,$rmemmax);
...
package Prod::SFRAC61::SunOS;
@Prod::SFRAC61::SunOS::ISA = qw(Prod::SFRAC61::Common);
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        VRTScsocw VRTSdbac61 VRTSvcsea61 VRTScfsdc VRTSodm61
        VRTSgms61 VRTScavf61 VRTSglm61 VRTScpi VRTSd2doc VRTSordoc
        VRTSfsnbl VRTSfppm VRTSap VRTStep VRTSspc VRTSspcq
        VRTSfasdc VRTSfasag VRTSfas VRTSgapms VRTSmapro VRTSvail
        VRTSd2gui VRTSorgui VRTSvxmsa VRTSdbdoc VRTSdb2ed VRTSdbed61
        VRTSdbcom VRTSsydoc VRTSsybed VRTSvcsApache VRTScmc
        VRTSccacm VRTSvcsw VRTScspro VRTSvcsdb VRTSvcsor VRTSvcssy
        ...
        VRTSdsa VRTSsfmh60 VRTSob34 VRTSobc33 VRTSaslapm61
        VRTSat50 VRTSsmf VRTSpbx VRTSicsco VRTSvxfs61 VRTSvxvm61
        VRTSjre15 VRTSjre VRTSperl516 VRTSvlic32
    ) ];
...
sub relink_oracle {
    ...
    my $oragrp = $prod->{oracle_group};
    my $cfsmnt = $prod->is_cfsmount($prod->{db_home});
    my $static_lib = "$prod->{db_home}/rdbms/lib/libodm10.a";
    my $static_lib_bk  = "$static_lib.backup";
    ...
    for my $system (@cpnodes) {
        ...
        if (EDR::cmdexit()) {
            ...
            if (EDR::cmdexit()) {
                ...
                if ($system->exists("$static_lib_bk")) {
                    $system->cmd("_cmd_rm -f $static_lib");
                } elsif($system->exists("$static_lib")){
                    $system->cmd("_cmd_mv $static_lib $static_lib_bk");
                    $system->cmd("_cmd_chown $orauser:$oragrp $static_lib_bk");
                }
...
package Prod::SFRAC61::AIX;
@Prod::SFRAC61::AIX::ISA = qw(Prod::SFRAC61::Common);
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
            VRTScsocw.rte VRTSdbac61 VRTSdbac.rte VRTSvcsea61 VRTScfsdc
            VRTSodm61 VRTSgms61 VRTScavf61 VRTSglm61 VRTScpi.rte
            ...
            VRTSjre.rte VRTSperl516 VRTSperl.rte VRTSvlic32
        ) ];
    return;
}
...
package Prod::SFRAC61::SolSparc;
@Prod::SFRAC61::SolSparc::ISA = qw(Prod::SFRAC61::SunOS);
...
package Prod::SFRAC61::Sol11sparc;
@Prod::SFRAC61::Sol11sparc::ISA = qw(Prod::SFRAC61::SunOS);
...
sub init_padv {
    ...
    $prod->{minimal_cpu_speed_requirment} = '1 GHz';
    $prod->{allpkgs}=[ qw(VRTSdbac61 VRTSgms61 VRTSodm61 VRTSdbed61 VRTSvcsea61) ];
    $prod->{minpkgs}=[ qw(VRTSdbac61 VRTSgms61 VRTSodm61 VRTSdbed61 VRTSvcsea61) ];
    $prod->{recpkgs}=[ qw(VRTSdbac61 VRTSgms61 VRTSodm61 VRTSdbed61 VRTSvcsea61) ];
    return;
}

package Prod::SFRAC61::Solx64;
@Prod::SFRAC61::Solx64::ISA = qw(Prod::SFRAC61::SunOS);
...
package Prod::SFRAC61::Sol11x64;
@Prod::SFRAC61::Sol11x64::ISA = qw(Prod::SFRAC61::SunOS);
