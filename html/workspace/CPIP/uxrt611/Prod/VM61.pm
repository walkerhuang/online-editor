...
package Prod::VM61::Common;
@Prod::VM61::Common::ISA = qw(Prod);
...
sub open_volumes_sys {
    ...
    if (scalar(@volumes) > 0) {
        ...
        if (Cfg::opt('upgrade_kernelpkgs') && !scalar(@notvcs_vol)) {
            $msg = Msg::new("Open volumes found on system $sys->{sys}. The upgrade can proceed because all the open volumes are under control of VCS.");
            $sys->push_note($msg);
        } else {
...
sub mounted_volume_notvcs_sys {
    my ($prod,$sys) = @_;
    my ($cprod,$rel,$vcs,@vols,$ret,$vol_list,$dgline,$vol_dg);
    $rel=$prod->rel();
    ...
    for my $vol (@$vol_list) {
        $dgline = $sys->cmd("_cmd_vxprint -v $vol 2>/dev/null | _cmd_grep 'Disk group:'");
        next unless ($dgline);
        $dgline =~ /^Disk group:\s*(\S+)/m;
        $vol_dg = $1;
        # Volume depends on DiskGroup of VCS agent. Volume can be failover if its based disk group is under VCS control
        push(@vols,$vol) unless($ret=~/DiskGroup\s+=\s+$vol_dg/m);
    }
    return \@vols;
}
...
sub rollback_precheck_sys {
    ...
    $prod->open_volumes_sys($sys, 'rollback') unless (Cfg::opt(qw(upgrade_kernelpkgs upgrade_nonkernelpkgs)));
}
...
sub upgrade_precheck_sys {
    ...
    $cprod=CPIC::get('prod');
    if (EDRu::inarr($cprod,qw/VM61 DMP61/)) {
    ...
    $prod->open_volumes_sys($sys, 'upgrade') unless (Cfg::opt(qw(upgrade_nonkernelpkgs)));
...
sub large_lun_check_sys {
    ...
    Msg::log("Checking large LUN: VM version: $ivers");
...
sub encap_upgrade_precheck_sys {
    ...
    # e3138925: on RHEL6:
    # Upgrade from VxVM-6.0GA/6.0RPx to any VxVM version: need unencapsulate
...
sub set_flag_encap_stop_sys {
    ...
    for my $proc (qw/vxdmp61 vxio61 vxspec61 vxconfigd61/) {
...
sub upgrade_postinstall_sys {
    ...
    if ($rootpath && ($sys->{platvers} eq '5.11')) {
        ...
        $cmd = "_cmd_cp -f $rootpath/etc/system $rootpath/etc/system.cpibak 2>/dev/null;_cmd_sed '$sedstr' $rootpath/etc/system.cpibak > $rootpath/etc/system 2>/dev/null";
        $sys->cmd($cmd);
    }
    return;
}
...
sub require_start_after_reboot_sys {
    my ($prod, $sys)=@_;
    #e3409595:should not set start after reboot flag if product isn't configured and it's not VM or SF.
...
sub verify_procs_list_sys {
    ...
    if ((defined $state && $state eq 'start') && $prod->vol_is_vm_disabled_sys($sys)) {
        $procs = EDRu::arrdel($procs, qw(vxrelocd61 vxcached61 vxconfigbackupd61 vxattachd61
                                        vvr61));
        return $procs;
    }

    $procs = EDRu::arrdel($procs, 'vvr61') unless ($prod->vr_licensed_sys($sys));
    if ((defined $state && $state eq 'start') && (!$rel->feature_licensed_sys($sys, 'VxVM'))) {
        $procs = EDRu::arrdel($procs, qw(vxrelocd61 vxconfigbackupd61));
    }
    unless ($rel->feature_licensed_sys($sys, 'FASTRESYNC') ||
            $rel->feature_licensed_sys($sys, 'FMR_DGSJ')) {
        $procs = EDRu::arrdel($procs, 'vxcached61');
        $procs = EDRu::arrdel($procs, 'vxattachd61');
    }
...
sub completion_messages {
    my $prod=shift;
    my ($cpic,$msg,$padv,$sys,$tmppath,$rootpath,$web,$alert_msg);
    ...
    if (Cfg::opt("rollback")) {
        # Check on each system, if VRTSaslapm has version higher than/equal to 6.1.1, then print the message.
        # during rollback, inform user VRTSaslapm will not change
        for $sys(@{$cpic->{systems}}) {
            $aslapm=$sys->pkg('VRTSaslapm61');
            $aslapmver=$aslapm->version_sys($sys) if($aslapm);
            if ($aslapmver && EDRu::compvers($aslapmver,$aslapm->{vers})<2) {
                $msg = Msg::new("VRTSaslapm package will not revert to older version during rollback process, if you insist on using VRTSaslapm package with older version, uninstall and reinstall VRTSaslapm package after rollback.");
                $msg->printn();
...
sub postcheck_vm_sys {
    ...
    unless($voldmode=~/mode: enabled/m){
        ...
        if($sys->exists($file)){
            $msg=Msg::new("\n\tThe file '$file' is found on $sys->{sys}. It is recommended to remove the file to start VM");
            push @m, $msg;
        }
        ...
        $rel=$prod->rel;
        unless($rel->prod_licensed_sys($sys,"VM61")){
...
sub is_fss_enabled {
    ...
    return 1 if ($prod->{padv}=~/(RHEL5|RHEL6|SLES11)/m);
    return 0;
}
...
sub is_fss_enabled_sys {
    ...
    return 1 if ($sys->{padv}=~/(RHEL5|RHEL6|SLES11)/m);
    return 0;
}

package Prod::VM61::AIX;
@Prod::VM61::AIX::ISA = qw(Prod::VM61::Common);

sub init_plat {
    my $prod=shift;
    my $padv=$prod->padv();
    if ($prod->{class} =~ /DMP/m) {
        $prod->{allpkgs}=[ qw(VRTSveki61 VRTSvxvm61 VRTSaslapm61 VRTSsfmh60) ];
        $prod->{minpkgs}=[ qw(VRTSveki61 VRTSvxvm61 VRTSaslapm61) ];
        $prod->{recpkgs}=[ qw(VRTSveki61 VRTSvxvm61 VRTSaslapm61 VRTSsfmh60) ];
    } else {
        $prod->{allpkgs}=[ qw(VRTSveki61 VRTSvxvm61 VRTSaslapm61 VRTSob34 VRTSsfmh60) ];
        $prod->{minpkgs}=[ qw(VRTSveki61 VRTSvxvm61 VRTSaslapm61) ];
        $prod->{recpkgs}=[ qw(VRTSveki61 VRTSvxvm61 VRTSaslapm61 VRTSob34 VRTSsfmh60) ];
    }
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSicsco VRTSvxvm61 VRTSveki61 VRTSjre15.rte VRTSjre.rte
        VRTSsfcpi61 VRTSperl516 VRTSperl.rte VRTSvlic32
    ) ];
...
sub preinstall_tasks {
    ...
    for my $sys (@$syslist){
        ...
        next unless ((EDRu::inarr('VRTSvxvm61',@{$sys->{installpkgs}})) ||
             (EDRu::inarr('VRTSvxvm61',@{$sys->{installpatches}})));
...
sub backup_vxvm_tunables_postremove_sys {
    ...
    if ($sys->exists('/etc/vx/vxvmtunables.cpiupgrade')) {
        $sys->cmd("_cmd_cp /etc/vx/vxvmtunables.cpiupgrade /etc/vx/vxvmtunables.save 2>/dev/null");
...
package Prod::VM61::HPUX;
@Prod::VM61::HPUX::ISA = qw(Prod::VM61::Common);

sub init_plat {
    my $prod=shift;
    my $padv=$prod->padv();
    if ($prod->{class} =~ /DMP/m) {
        $prod->{allpkgs}=[ qw(VRTSvxvm61 VRTSaslapm61 VRTSsfmh60) ];
        $prod->{minpkgs}=[ qw(VRTSvxvm61 VRTSaslapm61) ];
        $prod->{recpkgs}=[ qw(VRTSvxvm61 VRTSaslapm61 VRTSsfmh60) ];
        push @{$prod->{tunables}},
            {
        ...
        $prod->{allpkgs}=[ qw(VRTSvxvm61 VRTSaslapm61 VRTSob34 VRTSsfmh60) ];
        $prod->{minpkgs}=[ qw(VRTSvxvm61 VRTSaslapm61) ];
        $prod->{recpkgs}=[ qw(VRTSvxvm61 VRTSaslapm61 VRTSob34 VRTSsfmh60) ];
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        SYMClma VRTSspt61 VRTSaa VRTSmh VRTSccg VRTSobgui VRTSfspro
        VRTSdsa VRTSsfmh60 VRTSob34 VRTSobc33 VRTSaslapm61
        VRTSat50 VRTSsmf VRTSpbx VRTSicsco VRTSvxvm61 VRTSjre15
        VRTSjre VRTSsfcpi61 VRTSperl516 VRTSvlic32 VRTSwl
    ) ];
...
package Prod::VM61::Linux;
@Prod::VM61::Linux::ISA = qw(Prod::VM61::Common);
...
sub init_plat {
    ...
    if ($prod->{class} =~ /DMP/m) {
        ...
        $prod->{allpkgs}=[ qw(VRTSvxvm61 VRTSaslapm61 VRTSob34 VRTSlvmconv61 VRTSsfmh60) ];
        $prod->{minpkgs}=[ qw(VRTSvxvm61 VRTSaslapm61) ];
        $prod->{recpkgs}=[ qw(VRTSvxvm61 VRTSaslapm61 VRTSob34 VRTSsfmh60) ];
    }
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSvdid VRTSlvmconv61 VRTSvmdoc VRTSvmman VRTScpi
        SYMClma VRTSspt61 VRTSaa VRTSmh VRTSccg VRTSobgui VRTSfspro
        VRTSdsa VRTSsfmh60 VRTSob34 VRTSobc33 VRTSaslapm61
        VRTSat50 VRTSatClient50 VRTSsmf VRTSpbx VRTSicsco VRTSvxvm61
...
sub backup_vxvm_tunables_postremove_sys {
    ...
    if ($sys->exists('/etc/vx/vxtunables.rpmsave') && EDRu::compvers(${$sys->{prodvers}}[$index],'6.0') < 2) {
        ...
        $sys->cmd("_cmd_cp /etc/vx/vxtunables.rpmsave /etc/vx/vxtunables.cpiupgrade");
    }
    return;
}
...
package Prod::VM61::SunOS;
@Prod::VM61::SunOS::ISA = qw(Prod::VM61::Common);
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSdcli VRTSvmdoc VRTSvmman VRTScpi SYMClma VRTSspt61
        VRTSaa VRTSmh VRTSccg VRTSobgui VRTSfspro VRTSdsa VRTSsfmh60
        VRTSob34 VRTSobc33 VRTSaslapm61 VRTSat50 VRTSsmf VRTSpbx
        VRTSicsco VRTSvxvm61 VRTSjre15 VRTSjre VRTSsfcpi61 VRTSperl516
        VRTSvlic32
    ) ];
    ...
    if (scalar(@volumes) > 0) {
        ...
        if ($rc == 2 || $rc == 3 || $rc == 4) {
            ...
            if ($rc == 4 && Cfg::opt('upgrade_kernelpkgs')) {
                $vlist = join(' ', @vcs_vol);
                $msg=Msg::new("Cannot proceed because the following open volumes exist on system $sys->{sys}:\n\t$vlist\nSome local zones are based on those volumes and they are under control of VCS. The service group of local zones will be failover to other not upgrading nodes. This will cause the packages in local zones not to be sychronized because the local zones are not accessible to the nodes that are doing the upgrade.");
                $sys->push_error($msg);
...
package Prod::VM61::SolSparc;
@Prod::VM61::SolSparc::ISA = qw(Prod::VM61::SunOS);
...
package Prod::VM61::Sol11sparc;
@Prod::VM61::Sol11sparc::ISA = qw(Prod::VM61::SunOS);
...
package Prod::VM61::Solx64;
@Prod::VM61::Solx64::ISA = qw(Prod::VM61::SunOS);
...
package Prod::VM61::Sol11x64;
@Prod::VM61::Sol11x64::ISA = qw(Prod::VM61::SunOS);
