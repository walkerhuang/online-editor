...
package Prod::FS61::Common;
@Prod::FS61::Common::ISA = qw(Prod);
...
sub description {
    ...
    $msg=Msg::new("Veritas File System is a powerful, quick-recovery, journaling file system that provides high performance, and easy management required by mission critical applications. It delivers scalable performance, provides continuous availability, increased I/O, and up-to-date structural integrity.");
    $msg->print;
    return;
}
...
sub eol_disk_layout_sys {
    ...
    if (keys %eolfs || keys %depfs) {
        $msg=Msg::new("Use the vxupgrade command to upgrade mounted file systems, or the vxfsconvert command to upgrade unmounted file systems. See the Symantec Storage Foundation Installation Guide for more information on upgrading VxFS disk layout versions.");
        $sys->push_warning($msg);
        ...
        $msg=Msg::new("Notice: This version of VxFS supports only disk layout version $prod->{supported_vxfs_version_msg} and above.  Disk layout version $prod->{deprecated_vxfs_version} is deprecated and you can only mount it locally after upgrade. Only the VxFS file systems in the $prod->{fstab} file and mounted VxFS file systems were checked on the system $sys->{sys}. If you have any other VxFS file systems that are using unsupported or deprecated disk layout version, upgrade them to disk layout Version $prod->{supported_vxfs_version_msg} or above and restart the VxFS installation.");
        $sys->push_warning($msg);
        $msg=Msg::new("Use the vxupgrade(1M) command to upgrade mounted file systems, or the vxfsconvert(1M) command to upgrade unmounted file systems on the system $sys->{sys}. Use the fstyp -v command to determine the VxFS file system version. See the Symantec Storage Foundation Installation Guide for more information on upgrading VxFS disk layout versions.");
        $sys->push_warning($msg);
    }
    return;
}
...
sub check_vxfs_sys {
    ...
    my ($msg,$mps,$cpic,$ret,$mps_ref,$pkg,$rc,@notvcs_vxfs,@vcs_vxfs,$vlist);

    $mps_ref = $prod->mounted_vxfs_sys($sys);
    if (@$mps_ref) {
        @notvcs_vxfs = @{$prod->mounted_vxfs_notvcs_sys($sys)};
        @vcs_vxfs = @{EDRu::arrdel($mps_ref, @notvcs_vxfs)};
        ...
        if ($sys->sunos()) {
            ...
            if ($rc == 2 || $rc == 3 || $rc == 4) {
                ...
                if ($rc == 4 && Cfg::opt('upgrade_kernelpkgs')) {
                    $vlist = join(' ', @vcs_vxfs);
                    $msg=Msg::new("Cannot proceed because the following file system mounted on system $sys->{sys}:\n\t$vlist\nLocal zones are based on those file systems and they are under control of VCS. The service group of local zones will be failover to other not upgrading nodes. This will cause the packages in the local zones not to be sychronized because the local zones are not accessible to the nodes that are doing the upgrade.");
                    $sys->push_error($msg);
                } elsif (!$sys->{vxfs_vxvm_zone_check_done}) {
                    $pkg=$sys->pkg('VRTSvxvm61');
                    $pkg->set_value('stopprocs',undef);
                    $pkg=$sys->pkg('VRTSvxfs61');
                    $pkg->set_value('stopprocs',undef);
                ...
                if (Cfg::opt('upgrade_kernelpkgs') && !scalar(@notvcs_vxfs)) {
                    $msg=Msg::new("Rolling upgrade can proceed because all VxFS file systems mounted on node $sys->{sys} are under VCS control. And no local zone based on them.");
                    $sys->push_note($msg);
                    @$mps_ref = ();
                }
            }
            ...
            # If all VxFS are under VCS control, rolling upgrade can proceed
            if (Cfg::opt('upgrade_kernelpkgs') && !scalar(@notvcs_vxfs)) {
                $msg=Msg::new("Rolling upgrade can proceed because all VxFS file systems mounted on node $sys->{sys} are under VCS control.");
                $sys->push_note($msg);
                @$mps_ref = ();
            }
        }
...
package Prod::FS61::AIX;
@Prod::FS61::AIX::ISA = qw(Prod::FS61::Common);

sub init_plat {
    my $prod=shift;
    my $padv=$prod->padv();
    $prod->{allpkgs}=[ qw(VRTSob34 VRTSveki61 VRTSvxfs61 VRTSfsadv61 VRTSfssdk61 VRTSsfmh60) ];
    $prod->{minpkgs}=[ qw(VRTSveki61 VRTSvxfs61 VRTSfsadv61) ];
    $prod->{recpkgs}=[ qw(VRTSob34 VRTSveki61 VRTSvxfs61 VRTSfsadv61 VRTSsfmh60) ];
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        VRTSfppm VRTSap VRTStep VRTSsfmh60 VRTSfsmnd VRTSfssdk61
        VRTSfsadv61 VRTSfsdoc VRTSfsman VRTScpi.rte VRTSvsvc SYMClma VRTSspt61
        VRTSaa VRTSmh VRTSccg VRTSobgui VRTSfspro VRTSdsa VRTSob34
        VRTSobc33 VRTSat50 VRTSat.server VRTSat.client VRTSsmf
        VRTSpbx VRTSicsco VRTSvxfs61 VRTSveki61 VRTSsfcpi61 VRTSperl516
    ...
    # FS tunable list: run 'man vxtunefs', The FS tunables are not all changeable.
    # Get value of FS tunables: run 'modinfo vxfs' on linux, or 'vxtunefs -D print'
    $prod->{tunables} = [
        {
            'name' => "vxfs_ninode",
        ...
        {
            'name' => "read_pref_io",
            'desc' => Msg::new("The preferred read request size. The installer can only set the system default value of read_pref_io. Refer to the tunefstab(4) manual page for setting this tunable for the specified block device")->{msg},
            'define_object' => $prod,
            'reboot' => 0,
            'type' => 'range',
        ...
        {
            'name' => "read_nstream",
            'desc' => Msg::new("The number of parallel read requests of size read_pref_io that can be outstanding at one time. The installer can only set the system default value of read_nstream. Refer to the tunefstab(4) manual page for setting this tunable for the specified block device")->{msg},
            'define_object' => $prod,
            'reboot' => 0,
            'type' => 'range',
        ...
        {
            'name' => "write_pref_io",
            'desc' => Msg::new("The preferred write request size. The installer can only set the system default value of write_pref_io. Refer to the tunefstab(4) manual page for setting this tunable for the specified block device")->{msg},
            'define_object' => $prod,
            'reboot' => 0,
            'type' => 'range',
            'values' => [16384, undef, undef], # min, max, divisor
        ...
        {
            'name' => "write_nstream",
            'desc' => Msg::new("The number of parallel write requests of size write_pref_io that can be outstanding at one time. The installer can only set the system default value of write_nstream. Refer to the tunefstab(4) manual page for setting this tunable for the specified block device")->{msg},
            'define_object' => $prod,
            'reboot' => 0,
            'type' => 'range',
            'values' => [1, undef, undef], # min, max, divisor
        ...
        {
            'name' => "max_diskq",
            'desc' => Msg::new("Specifies the maximum disk queue generated by a single file. The installer can only set the system default value of max_diskq. Refer to the tunefstab(4) manual page for setting this tunable for the specified block device")->{msg},
            'define_object' => $prod,
            'reboot' => 0,
            'type' => 'range',
...
package Prod::FS61::HPUX;
@Prod::FS61::HPUX::ISA = qw(Prod::FS61::Common);

sub init_plat {
    my $prod=shift;
    $prod->{allpkgs}=[ qw(VRTSob34 VRTSvxfs61 VRTSfssdk61 VRTSsfmh60)];
    $prod->{minpkgs}=[ qw(VRTSvxfs61) ];
    $prod->{recpkgs}=[ qw(VRTSob34 VRTSvxfs61 VRTSsfmh60) ];
    $prod->{proddir}='storage_foundation';
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        VRTStep VRTSap VRTSsfmh60 VRTSfsmnd VRTSdcli VRTSfssdk61
        VRTSfsadv61 VRTSfsdoc VRTSfsman VRTScpi VRTSvsvc SYMClma VRTSspt61
        VRTSaa VRTSmh VRTSccg VRTSobgui VRTSfspro VRTSdsa VRTSob34
        VRTSobc33 VRTSat50 VRTSsmf VRTSpbx VRTSicsco VRTSvxfs61
...
package Prod::FS61::HPUX1131ia64;
@Prod::FS61::HPUX1131ia64::ISA = qw(Prod::FS61::HPUX);

sub init_padv {
    my $prod=shift;
    $prod->{allpkgs}=[ qw(VRTSob34 VRTSvxfs61 VRTSfsadv61 VRTSfssdk61 VRTSsfmh60)];
    $prod->{minpkgs}=[ qw(VRTSvxfs61 VRTSfsadv61) ];
    $prod->{recpkgs}=[ qw(VRTSob34 VRTSvxfs61 VRTSfsadv61 VRTSsfmh60) ];
    return;
}
...
package Prod::FS61::Linux;
@Prod::FS61::Linux::ISA = qw(Prod::FS61::Common);
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        VRTSfsnbl VRTSfppm VRTSap VRTStep VRTSsfmh60 VRTSdcli
        VRTSfsmnd VRTSfssdk61 VRTSfsadv61 VRTSfsdoc VRTSfsman VRTScpi VRTSvsvc
        SYMClma VRTSspt61 VRTSaa VRTSmh VRTSccg VRTSobgui VRTSfspro
        VRTSdsa VRTSob34 VRTSobc33 VRTSat50 VRTSatClient50
        VRTSsmf VRTSpbx VRTSicsco VRTSvxfs61 VRTSvxfs-platform
        VRTSvxfs-common VRTSsfcpi61 VRTSperl516 VRTSvlic32
    ) ];
    my $padv=$prod->padv();
    $prod->{fstab}='/etc/fstab';
    $padv->{cmd}{fstyp}='/opt/VRTS/bin/fstyp';
...
package Prod::FS61::SunOS;
@Prod::FS61::SunOS::ISA = qw(Prod::FS61::Common);
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSicsco VRTSvxfs61 VRTSsfcpi61 VRTSperl516 VRTSvlic32
    ) ];
...
sub show_error_zones_on_cfs_sys {
    ...
    $action=Msg::get("task_$action");
    $msg=Msg::new("The $action is not supported because there are shared volumes on the system $sys->{sys}, and there are zones with root path mounted on these shared volumes. Packages in the zones mounted on CFS are not able to $action correctly. To finish the procedure, you need to run 'zoneadm -z <zonename> uninstall' to uninstall the zone and re-run the $action. Use this command carefully because it will destroy the zone.");
    $sys->push_error($msg);
    return '';
}
...
#   $sys
# Output Parameters:
#   4   if any zones are mounted on vxvm open volume, not CFS/CVM and under VCS control.    exit upgrade since vcs control
...
sub zones_with_vxfs_vxvm_sys {
    my ($prod,$sys) = @_;
    my ($mntlist,$zonelist,$mount,@mounts,$zone,$zname,$zpath,$mntdevice,$mntpath,$mntfstype,$mntoption,$vcs,$mnt_maincf,@mnts_maincf);
    my ($zones_on_vxvm,$zones_on_vxfs,$zones_on_cfs,$zones_under_vcs);

    @mnts_maincf = ();
    $vcs=$prod->prod('VCS61');
    $mnt_maincf=$sys->cmd("_cmd_cat $vcs->{maincf} 2>/dev/null | _cmd_grep MountPoint | _cmd_awk -F\\\" '{print \$2}'");
    @mnts_maincf = split(/\n/,$mnt_maincf) unless ($mnt_maincf eq '');
    ...
    for my $zone (split(/\n/, $zonelist)) {
        if ($zone =~ /^\s*\S+\s+(\S+)\s+\S+\s+(\S+)/mx) {
            ...
            # The zone path can be the mount point itself or path under it
            for my $mnt_m (@mnts_maincf) {
                if($zpath =~ /$mnt_m(\/\w+)*\/?$/m) {
                    $zones_under_vcs = 1;
                    last;
                }
    ...
    return 4 if ($zones_under_vcs && ($zones_on_vxfs || $zones_on_vxvm));
...
package Prod::FS61::SolSparc;
@Prod::FS61::SolSparc::ISA = qw(Prod::FS61::SunOS);
...
package Prod::FS61::Sol11sparc;
@Prod::FS61::Sol11sparc::ISA = qw(Prod::FS61::SunOS);

sub init_padv {
    my $prod=shift;
    $prod->{upgradevers}=[qw(6.0.10 6.1)];
    $prod->{zru_releases}=[qw(6.0.10 6.1)];
    $prod->{allpkgs}=[ qw(VRTSvxfs61 VRTSfsadv61 VRTSfssdk61 VRTSsfmh60) ];
    $prod->{minpkgs}=[ qw(VRTSvxfs61 VRTSfsadv61) ];
    $prod->{recpkgs}=[ qw(VRTSvxfs61 VRTSfsadv61 VRTSsfmh60) ];
...
package Prod::FS61::Solx64;
@Prod::FS61::Solx64::ISA = qw(Prod::FS61::SunOS);
...
package Prod::FS61::Sol11x64;
@Prod::FS61::Sol11x64::ISA = qw(Prod::FS61::SunOS);

sub init_padv {
    my $prod=shift;
    $prod->{upgradevers}=[qw(6.0.10)];
    $prod->{zru_releases}=[qw(6.0.10)];
    $prod->{allpkgs}=[ qw(VRTSvxfs61 VRTSfssdk61 VRTSsfmh60) ];
    $prod->{minpkgs}=[ qw(VRTSvxfs61) ];
    $prod->{recpkgs}=[ qw(VRTSvxfs61 VRTSsfmh60) ];
