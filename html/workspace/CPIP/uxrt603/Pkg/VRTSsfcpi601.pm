...
sub donotinstall_sys {
    ...
    $cpic=Obj::cpic();
    $cprod=CPIC::get('prod');
    # Deal with 6.0.2 upgrade to 6.0.3
    # 1. Platform is Linux
    # 2. Installed with VCS/SFHA 602
    # 3. VRTSsfcpi602 installed
    # 4. Patch upgrade
    if (Cfg::opt('patchupgrade') && ($cprod =~ /VCS/ || $cprod =~ /SFHA/) && $sys->{plat} =~ /Linux/) {
        if ($sys->{pkgvers}{'VRTSvcsvmw'} =~ /^6\.0\.2/mx && $sys->{pkgvers}{'VRTSsfcpi602'} =~ /^6\.0\.2/mx) {
            $oldcpipkg=Pkg->new_pkg('VRTSsfcpi602',$cpic->{padv});
            $oldcpipkg=Pkg->new_pkg('VRTSsfcpi602',$sys->{padv});
            $oldcpipkg->set_uninstall_sys($sys);
            push @{$cpic->{uninstallpkgs}}, 'VRTSsfcpi602';
        }
    }
