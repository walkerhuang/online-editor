...
sub default_systemnames {
    ...
    if(Cfg::opt(qw(rolling_upgrade upgrade_kernelpkgs upgrade_nonkernelpkgs ))){
        ...
        return $dfs;
    } elsif (Cfg::opt(qw(patchupgrade upgrade rollback))) {
...
sub cli_installer_menu {
    ...
    while (1) {
        ...
        return "" if ($rel->{nopkgs});; # No product menu for RP release
...
sub prod_menu {
    ...
    for my $mprodi (@{$rel->{menuprods}}) {
        ...
        if (($rel->{rp}) || -d "$cpic->{mediapath}/$prod->{proddir}") {
            push(@mprods,$mprodi);
        }
    }
...
sub init_plat {
    ...
    $rel->{platreqs} = [ 'RHEL5 U5 (2.6.18-194.el5)', 'RHEL5 U6 (2.6.18-238.el5)', 'RHEL5 U7 (2.6.18-274.el5)', 'RHEL5 U8 (2.6.18-308.el5)',
                         'RHEL6 U1 (2.6.32-131.el6)', 'RHEL6 U2 (2.6.32-220.el6)', 'RHEL6 U3 (2.6.32-279.el6)',
                         ...
                         'OEL6 U1 (2.6.32-131.el6)','OEL6 U2 (2.6.32-220.el6)','OEL6 U3 (2.6.32-279.el6)'];
...
# on Linux, override get_local_script_version_suffix for terra nova
sub get_local_script_version_suffix {
    ...
    if ($rel->mr_for_602()) {
        ...
        $suffix = $rel->SUPER::get_local_script_version_suffix();
    }
    return $suffix;
}
...
sub mr_for_602 {
    ...
    my ($cprod);
    $cprod = CPIC::get('prod');
    if ($rel->{rp} && ($cprod =~ /^VCS/mx || $cprod =~ /^SFHA/mx)) {
...
_PKGSPACE_
    ...
    $rel->{patchesspace}=<<"_PATCHSPACE_";
VRTSamf:1705,13198,0,0
VRTScavf:9,1267,0,0
VRTSdbed:224,136892,0,0
VRTSperl:0,42482,0,0
VRTSsfcpi601:323,5407,0,0
VRTSvcs:1818,391081,0,0
VRTSvcsag:121,21872,0,0
VRTSvcsea:8,7238,0,0
VRTSvxfen:6,3669,0,2
VRTSvxfs:0,45898,0,32
VRTSvxvm:97,248650,0,5965
_PATCHSPACE_
...
sub detect_prior_rp_sys {
    my($rel,$cpic,$sys,$patch,$patchi,$patch_vers,$release,$patchvers_absent,$pkg,$pkgi,$prod,$vers,$padv,$level_output,$patchname,$patchvers);
    ($rel,$sys)=@_;
    $cpic=Obj::cpic();
    $prod=$cpic->prod;
    $padv=$sys->padv;
    foreach $release (@{$rel->{base_releases}}) {
        ...
        for $pkgi (@{$prod->pkgs}) {
            ...
            for $patchi (@{$pkg->patches_allvers($release)}) {
                ...
                $patchvers =~ s/_/\./g;
                unless (EDR::cmd_sys($sys, "_cmd_lslpp -lca $patchname 2>/dev/null | _cmd_grep $patchvers")) {
                    $patchvers_absent++;
                }
            }
        }
...
# HPUX
sub detect_prior_rp_sys {
    my($rel,$cpic,$sys,$patch,$patchi,$patch_vers,$release,$patchvers_absent,$pkg,$pkgi,$prod,$vers,$padv);
    ($rel,$sys)=@_;
    $cpic=Obj::cpic();
    $prod=$cpic->prod;
    $padv=$sys->padv;
    foreach $release (@{$rel->{base_releases}}) {
        ...
        for $pkgi (@{$prod->pkgs}) {
            ...
            for $patchi (@{$pkg->patches_allvers($release)}) {
                ...
                unless ($padv->tsub("rp_patch_version_sys",$sys,$patch)) {
                    $patchvers_absent++;
                }
            }
        }
...
_PKGSPACE_
    ...
    $rel->{patchesspace}=<<"_PATCHSPACE_";
PVCO_03970:5167,0,0,0
PVCO_03972:10191,65959,0,61236
PVCO_03973:785,66,0,7
PVCO_03974:26395,396654,0,492627
PVCO_03976:14687,0,0,0
PVCO_03977:54,0,0,0
PVCO_03978:63,0,0,0
PVCO_03981:244,0,0,0
PVCO_03982:79249,0,0,0
PVKL_03971:0,27549,0,0
PVKL_03975:0,15996,0,0
PVKL_03979:0,1467,0,0
PVKL_03980:0,985,0,0
_PATCHSPACE_
    return;
}
...
sub detect_prior_rp_sys {
    ...
    foreach $release (@{$rel->{base_releases}}) {
        ...
        for $pkgi (@{$prod->pkgs}) {
            $pkg = $sys->pkg($pkgi);
            next unless ($pkg->version_sys($sys));
            for $patchi (@{$pkg->patches_sys($sys,$release)}) {
                $patch = Obj::patch($patchi,$rel->{padv});
                unless ($padv->tsub("rp_patch_version_sys",$sys,$patch->{patchid})) {
                    $patchvers_absent++;
                }
            }
        }
...
sub platvers_sys {
    ...
    if (!Cfg::opt('rootpath') && $sys->{padv}=~/Sol10/ &&
        $sys->{zone} && Cfg::opt(qw(precheck install upgrade patchupgrade uninstall))) {
...
_PKGSPACE_
    ...
    $rel->{patchesspace}=<<"_PATCHSPACE_";
148481-02:0,35,0,41
148490-02:44,2205,0,224
148492-01:641,2857,0,1
148496-01:169,0,0,0
148497-01:3,0,0,0
148498-01:3277,41,0,0
149691-01:7,0,0,0
149695-01:6,0,0,0
149696-01:38,0,0,0
149699-01:0,0,0,0
149702-01:1669,0,0,0
_PATCHSPACE_
    return;
}

package Rel::UXRT601::Solx64;
@Rel::UXRT601::Solx64::ISA = qw(Rel::UXRT601::SunOS);
...
_PKGSPACE_
    ...
    $rel->{patchesspace}=<<"_PATCHSPACE_";
148482-02:431,69,36476,47
148491-02:32,2748,180967,1131
148493-01:209,0,5650,0
148494-01:0,24,238326,1
148495-01:3521,32,2212,0
149692-01:38,0,690,0
149693-01:6,0,7213,0
149694-01:6,0,1123,8
149697-01:216,0,106572,0
149698-01:0,0,93465,0
149703-01:323,0,3001,0
_PATCHSPACE_
    return;
}
