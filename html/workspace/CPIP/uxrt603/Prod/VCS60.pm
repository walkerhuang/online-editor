...
sub init_common {
    ...
    $prod->{prod}='VCS';
    $prod->{abbr}='VCS';
    $prod->{vers}='6.0.300.000';
    $prod->{proddir}='cluster_server';
    $prod->{eula}='EULA_VCS_Ux_6.0.1.pdf';
...
sub verify_responsefile {
    ...
    if ((Cfg::opt('configure')||Cfg::opt('upgrade')||Cfg::opt('start')) && !Cfg::opt(qw(fencing configcps))) {
    ...
    if (Cfg::opt("configure")) {
        ...
        if ($cfg->{fencingenabled} && (!$cfg->{donotreconfigurevcs}) && $prod->{fencing_config_pending}) {
            ...
            if ($rel->{nowebinstaller}) {
                $msg = Msg::new("I/O fencing configuration is not done. You can run the command '$installscript -fencing' to configure it.\n");
            } else {
...
sub completion_message_rollingupgrade {
    my ($prod)=@_;
    my ($sys,$rel,$cpic,$msg,@subs,@systems,$cfg,$ayn,$web,$webmsg);
    my ($showrup1list,@rup1list,@rup2list);
    $cpic=Obj::cpic();
    $rel=$cpic->rel;
    $cfg=Obj::cfg();
    $web = Obj::web();
    ...
    if(@rup1list && @rup2list) {
        ...
        if(@{$cpic->{reboot_systems}}){
            ...
            # if $cfg->{break_rolling_upgrade_phase} = 0 or undef, rolling upgrade will not be interrupted.
            # if $cfg->{break_rolling_upgrade_phase} = 1, rolling upgrade will break after first sub-cluster upgraded.
            # if $cfg->{break_rolling_upgrade_phase} = 2, rolling upgrade will break after second sub-cluster upgraded.
            if ($cfg->{opt}{responsefile} && $cfg->{break_rolling_upgrade_phase} && $cfg->{break_rolling_upgrade_phase} == 1) {
                $msg=Msg::new("It is recommended to perform rolling upgrade phase 1 on the systems $showrup1list in the next step. Rerun the installer to continue\n");
                $msg->bold;
                $web->web_script_form('alert', $msg) if (Obj::webui());
                $cpic->edr_completion();
            }
    ...
    if(!@rup1list && @rup2list) {
        if(@{$cpic->{reboot_systems}}){
            ...
            if ($cfg->{opt}{responsefile} && $cfg->{break_rolling_upgrade_phase} && $cfg->{break_rolling_upgrade_phase} == 2) {
                $msg=Msg::new("It is recommended to perform rolling upgrade phase 2 on all the cluster systems in the next step. Rerun the installer to continue\n");
                $msg->bold;
                $web->web_script_form('alert', $msg) if (Obj::webui());
                $cpic->edr_completion();
            }
...
sub maincf_precheck_if_update_vip_sys {
    ...
    $sys->set_value("upgrade_TriggersEnabled", 1) if ($vcsvers) && (EDRu::compvers($vcsvers,'6.0') == 2);
...
sub restore_vcs_lock_files_sys {
    ...
    $sys->cmd("_cmd_touch /var/VRTSvcs/lock/.habootfile");
    return;
}
...
sub prestart_config_common_questions {
    ...
    if ((($cprod eq 'VCS60')||($cprod eq 'SFHA60')) && (!$cfg->{fencingenabled})) {
        if (scalar(@$syslist) == 1) {
            my $addmsg = Msg::new("You can run $prod->{abbr} without GAB and LLT on a single node that is not intended to be part of a cluster. Note that if VCS engine hangs and GAB is disabled on the node, the engine cannot be restarted.");
            $addmsg->bold;
            Msg::n();
            $addmsg = Msg::new("Symantec recommends to  start GAB and LLT.");
            $addmsg->bold;
            Msg::n();
...
sub web_prestart_config_questions {
    my ($rtvalue,$prod,$web,$cfg,$cprod,$key,$msg,$help,$ayn,$multinode,$rpn,$alertmsg,$nic);
    $prod = shift;
...
sub update_maincf {
    my ($prod,$sys) = @_;
    my ($admins,$ddev_nwhosts,$sysi,$extype,$cfg,$ddev,$drid,$cdev_nwhosts,$extypes,$opers,$n,$drnd,$cdev,$csl);
    my ($users,$sys2,$prefix,$ddev_attr,$maincf,$tmpdir,$recp,$cdev_attr,$cons,$secure,$systems);
    $cfg = Obj::cfg();
...
sub dynupgrade_pre_upgrade_sys {
    ...
    unless ($sys->exists($prod->{maincf})) {
        $msg = Msg::new("main.cf does not exist on $sys->{sys}");
        $sys->push_warning($msg);
        return 0;
    }
...
sub maincf_result_upgrade_TriggersEnabled_sys {
    ...
    # get the sys list that already have TriggersEnabled
    for my $grp_name (@grps) {
        @grp_syslist = Prod::VCS60::Common::dynupgrade_get_systemlist_of_grpname($treeref,$grp_name);
...
sub haconf_dumpmakero {
    my ($syslist,$prod,$sys,$rtn,$done,$count);
    $prod = shift;
    $syslist=CPIC::get('systems');
    $sys = $$syslist[0];
...
sub eat_configure_from_eat_atutil_restore {
    ...
    for my $sys (@$syslist) {
        ...
        $sys->cmd("_cmd_cp -rf $backup/* $fold/");
...
sub eat_configure_from_eat_atutil_backup {
    ...
    for my $sys (@$syslist) {
        ...
        $sys->cmd("_cmd_cp -rf $fold $backup");
    }
    return 1;
}
...
sub securityonenode_menu {
    my ($prod)=@_;
    my ($edr,$cfg,$sys,$cpic);
    my ($msg,$one,$two,$rtn,$menuopt,$menu,$help,$done,$ayn,$fips,$withfips);
...
sub securitytrust {
    my ($prod)=@_;
    my ($cpic,$edr,$localsys,$sys0,$sys,$msg,$outerRB,$port,$ayn,$ping,$retry,$count);
...
sub security_menu {
    my ($prod)=@_;
    my ($cfg,$cpic,$edr,$msg,$ayn,$sys0,$web,$info,$sec_or_fips,$option);
    ...
    while(1){
        if ($failed) {
            ...
            if (Obj::webui()) {
                ...
                do {
                    if (Cfg::opt('configcps')) {
                        $msg=Msg::new("Enter the name of another system to configure CP Server:");
                    } else {
...
sub check_installed_ha_product_sys {
    ...
    $rel = Obj::rel();
    @prodlist = qw(SFRAC60 SFSYBASECE60 SVS60 SFCFSHA60 SFHA60 VCS60);
...
sub postcheck_pkg_vers_sys {
    ...
    $cpic = Obj::cpic();
    $rel = Obj::rel();
    # for RP release, don't do package check.
    return 1 if ($rel->{rp} == 1);
    $systems=CPIC::get("systems");
    $pkg = $sys->pkg($pkgname);
...
sub addnode_configure_heartbeat {
    ...
    unless ($clus_conf) {
        $msg = Msg::new("The configuration of $sys1->{sys} is not valid, please check the cluster.\n");
        $msg->die();
    }
...
sub addnode_update_llttab {
    ...
    if ($cfg->{lltoverudp}) {
        ...
        $prod->set_hb_nics($rhbn,CPIC::get('systems')) if(!Cfg::opt('responsefile'));
        $prod->update_llttab();
        for my $sys (@{CPIC::get('systems')}) {
            $sysi = $sys->{sys};
            $localsys->copy_to_sys($sys,"$edr->{tmpdir}/llttab.$sysi",$prod->{llttab});
...
sub addnode_configure_fencing {
    ...
    my($ret,$security,$cps_aref,$ports_href,$conf,$uuid,$lltconf,$nodeid,$system);
    ...
    for my $sysi (@{$cfg->{newnodes}}) {
        ...
        if ($vxfenmode eq 'scsi3' || (($vxfenmode eq 'customized') && ($vxfen_mechanism eq 'cps'))) {
            ...
            $ret = $vxfen_pkg->addnode_configure_cp_agent($firstnode, $sys, $nodeid);
            if ($ret) {
...
sub addnode_update_CSSG {
    my ($csgnic,$firstnode,$gconic,$global_csgnic,$global_gconic,$msg,$rtn,$sysi,$sys,$system,$lltconf,$nodeid);
    my $prod = shift;
    my $cfg = Obj::cfg();
    ...
    for my $sysi (@{$cfg->{newnodes}}) {
        ...
        $lltconf = $prod->get_llt_config_sys($sys);
        $nodeid = $lltconf->{hostsid}{$system};
        $firstnode->cmd("$prod->{bindir}/hagrp -modify ClusterService SystemList -add $system $nodeid");
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSsmf VRTSpbx VRTSicsco VRTSjre.rte VRTSsfcpi601 VRTSperl514 VRTSperl.rte
        VRTSvlic32
    ) ];
...
sub init_plat {
    ...
    $prod->{zru_releases}=[qw(3.5 4.1 5.0 5.1 6.0)];
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSsfcpi601 VRTSperl514 VRTSvlic32 VRTSwl
    ) ];
...
sub init_plat {
    my $prod=shift;
    $prod->{allpkgs}=[ qw(VRTSllt60 VRTSgab60 VRTSvxfen60 VRTSamf60 VRTSvcs60 VRTScps60 VRTSvcsag60 VRTSvcsdr60 VRTSvcsea60 VRTSsfmh41 VRTSvbs60 VRTSvcsvmw60 VRTSacclib52 VRTSmq651 VRTSsapwebas7160) ];
    $prod->{minpkgs}=[ qw(VRTSllt60 VRTSgab60 VRTSvxfen60 VRTSamf60 VRTSvcs60 VRTSvcsag60) ];
    $prod->{recpkgs}=[ qw(VRTSllt60 VRTSgab60 VRTSvxfen60 VRTSamf60 VRTSvcs60 VRTSvcsag60 VRTSvcsdr60 VRTSvcsea60 VRTSsfmh41 VRTSvbs60 VRTSvcsvmw60 VRTSacclib52 VRTSmq651 VRTSsapwebas7160) ];
    $prod->{installonupgradepkgs} = [ qw(VRTSamf VRTSsfmh VRTSvbs VRTSvcsvmw VRTSacclib VRTSmq6 VRTSsapwebas71) ];
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        VRTSvcsvmw60 VRTSacclib52 VRTSmq651 VRTSsapwebas7160 
        VRTScsocw VRTSvcsApache VRTScmc VRTSccacm VRTSvcsw
        VRTScspro VRTSvcsdb VRTSvcsor VRTSvcssy VRTSvcsvr VRTScmccc
        VRTScmcs VRTScscm VRTSweb VRTScscw VRTScssim VRTSvbs60 VRTSsfmh41
        VRTSvcsea60 VRTScutil VRTSjre15 VRTSvcsdc VRTSvcsmn
        ...
        VRTSjre VRTSsfcpi601 VRTSperl514 VRTSvlic32
    ) ];
...
sub appha_postinstall_sys {
    my ($prod,$sys) = @_; 
    my ($conf,$pid,@lines,$cfg,$edr,$msg,$exitcode);

    # config sfmh after appha installed
    if(Cfg::opt('installallpkgs') || Cfg::opt('installrecpkgs')){
        $cfg  = Obj::cfg();
        $edr  = Obj::edr();
        $conf =  $sys->cmd("_cmd_cat /etc/opt/VRTSsfmh/xprtld.conf");
        if($conf !~ /namespaces\s+vcs/){
            $pid= $sys->cmd("_cmd_ps -ef |_cmd_grep '/opt/VRTSsfmh/bin/xprtld'|_cmd_grep -v grep");
            $sys->cmd("/opt/VRTSsfmh/bin/xprtlc -l https://localhost/admin/lifecycle/stop?excludetree=1") if($pid);
            @lines = split /\n/,$conf;
            push @lines,'namespaces vcs=/opt/VRTSvcs/portal';
            $conf = join "\n",@lines;
            $conf .="\n\n";
            $sys->writefile($conf,'/etc/opt/VRTSsfmh/xprtld.conf');
        }     
        $pid= $sys->cmd("_cmd_ps -ef |_cmd_grep '/opt/VRTSsfmh/bin/xprtld'|_cmd_grep -v grep");
        $sys->cmd("/opt/VRTSsfmh/adm/xprtldctrl start")if(!$pid);
        ...
        if(Cfg::opt('upgrade')){
            $sys->cmd("/opt/VRTSvcs/portal/admin/synchronize_guest_config.pl CreateAppSGMap=Yes");
        }
    }
    return;
}

sub appha_postremove_sys{
    my ($prod,$sys) = @_; 
    my ($conf,@lines,@newlines,$cpic,$pid);

    $cpic = Obj::cpic();

    if(!EDRu::inarr('VRTSvcsvmw60', @{$sys->{uninstallpkgs}}) ||
        !EDRu::inarr('VRTSvcsvmw60', @{$cpic->{uninstallpkgs}})){
        return;
    }
    ...
    for my $line (@lines){
       push @newlines,$line if($line !~ /namespaces\s+vcs=/);
    }
    $conf = join "\n",@newlines;
    $conf .="\n\n";
    $sys->writefile($conf,'/etc/opt/VRTSsfmh/xprtld.conf');
    $sys->cmd("/opt/VRTSsfmh/adm/xprtldctrl start");
}

sub upgrade_postinstall_sys {
    my ($prod,$sys) = @_;
    appha_postinstall_sys($prod,$sys);
    $prod->SUPER::upgrade_postinstall_sys($sys);
    return;
}
...
sub postinstall_sys {
    my ($prod,$sys) = @_;
    appha_postinstall_sys($prod,$sys);
    $prod->SUPER::postinstall_sys($sys);
    return;
}
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSat50 VRTSsmf VRTSpbx VRTSicsco VRTSjre VRTSsfcpi601 VRTSperl514
        VRTSvlic32
    ) ];
    # For UXRT 5.1SP1, VRTSacclib need to be fresh on AIX and Solaris.
    $prod->{obsoleted_but_need_refresh_when_upgrade_pkgs} = [ qw(VRTSacclib52) ];
