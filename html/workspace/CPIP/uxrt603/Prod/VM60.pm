...
sub encap_upgrade_precheck_sys {
    ...
    $support_vers = ($sys->linux()) ? '5.1' :
                    ($sys->sunos()) ? '4.1' : '0';
    # e2832803: need to unencapsulate first before upgrade to 6.0.1 on Redhat6 
    if ((EDRu::compvers($vers,$support_vers,2)==2) || ($sys->{padv} =~ /RHEL6/mx)) {
...
sub enable_dmp_osn_sys {
    ...
    $cprod = CPIC::get('prod');
    if ($sys->{disable_dmp_native_support} == 1) {
        $msg=Msg::new("dmp_native_support is not enabled on $sys->{sys}. You need to run 'vxdmpadm settune dmp_native_support=on' manually to enable dmp_native_support in the future");
        $sys->push_warning($msg);
        return 1;
    }
...
sub ask_patchupgrade_configure {
    my ($prod) = @_;
    my ($msg,$syslist,$sys,$ayn);
    $syslist=CPIC::get('systems');

    # Ask user if he would like to enable_dmp_native_support after start.
    for my $sys (@$syslist) {
        if (Cfg::opt(qw(upgrade patchupgrade)) &&  # for upgrade
            $sys->{platvers} eq '5.10' &&
            $sys->exists($prod->{migrate_native_file})) { # previously dmp_native_support is enabled
            $msg=Msg::new("dmp_native_support is enabled before upgrade, do you want to turn it on after upgrade?");
            $ayn=$msg->aynn();
            if ($ayn eq 'N') {
                $sys->cmd("_cmd_rmr $prod->{migrate_native_file}");
                $sys->{disable_dmp_native_support}=1;
            }
        }
    }
}
...
# comment out: forceload: drv/vxdmp(vxio/vxspec)
...
# recover the comment entry: #forceload: drv/vxdmp(vxio/vxspec)
sub add_etc_system_entry_sys {
    ...
    if (@{$proc}) {
        for my $proci (@{$proc}) {
            $sedstr .= "s/^* *\\(forceload: drv\\/$proci\\)/\\1/;";
        }
        $cmd = "_cmd_cp -f /etc/system /etc/system.cpibak 2> /dev/null; _cmd_sed '$sedstr' /etc/system.cpibak > /etc/system 2> /dev/null";
        $sys->cmd($cmd);
    }
...
sub upgrade_poststart_sys {
    ...
    for my $proci (qw/vxdmp60 vxio60 vxspec60/) {
        $proc = $prod->proc($proci);
        if ($proc->check_sys($sys,"start")) {
            push (@procs, $proc->{proc});
        }
    }
    if (@procs) {
        $prod->add_etc_system_entry_sys($sys,\@procs);
    }
    return $prod->SUPER::upgrade_poststart_sys($sys);
}

sub upgrade_poststop_sys {
    my ($prod,$sys) = @_;
    my ($proc,@procs);
    for my $proci (qw/vxdmp60 vxio60 vxspec60/) {
        $proc = $prod->proc($proci);
    ...
    if (@procs) {
        $prod->rm_etc_system_entry_sys($sys,\@procs);
    }
    return;
}
...
sub poststart_sys {
    ...
    return $prod->SUPER::poststart_sys($sys);
}
