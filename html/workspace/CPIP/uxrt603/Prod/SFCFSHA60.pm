...
sub set_pkgs {
    my($prod,$rel,$vcs,$vcs1,$vm,$fs,$sf,$sf1,$category,@categories);
    $prod=shift;
    ...
    # Remove terra nova pkgs for SFCFSHA/SFCFSRAC/SVS/SFSYBASECE/SFRAC
    $vcs1->{allpkgs} = EDRu::arrdel($vcs->{allpkgs},qw(VRTSvcsvmw60 VRTSacclib52 VRTSmq651 VRTSsapwebas7160));
    $vcs1->{recpkgs} = EDRu::arrdel($vcs->{recpkgs},qw(VRTSvcsvmw60 VRTSacclib52 VRTSmq651 VRTSsapwebas7160));
    $vcs1->{minpkgs} = $vcs->{minpkgs};
...
sub upgrade_postinstall_sys {
    ...
    if (Obj::cfg('nostart')) {
        ...
        for my $proci (qw/vxglm vxgms/) {
            $sys->cmd("_cmd_adddrv $proci 2> /dev/null");
        }
    }
