...
package Prod::LP60::Common;
@Prod::LP60::Common::ISA = qw(Prod);
...
sub init_common {
    ...
    $prod->{name}=Msg::new("Veritas Language Pack")->{msg};
    $prod->{nolic}=1;
    $prod->{nocpionsys}=1;
    $prod->{lp}=1;
    return;
...
sub description {
    ...
    $msg=Msg::new("Veritas Language Pack Installer adds language packages for the installed Veritas Products");
    $msg->print;
    return;
}
...
package Prod::LP60::SunOS;
@Prod::LP60::SunOS::ISA = qw(Prod::LP60::Common);

package Prod::LP60::SolSparc;
@Prod::LP60::SolSparc::ISA = qw(Prod::LP60::SunOS);
...
sub init_padv {
    ...
    $prod->{allpkgs} = [ qw(VRTSmulic32 VRTSjacav60 VRTSjacs60 VRTSjacse60 VRTSjacsu60 VRTSjadba60 VRTSjafs60 VRTSjavm60 VRTSjadbe60 VRTSjaodm60 VRTSzhvm60) ];
...
sub getlangpkg {
    ...
    if ($prod->{lang} eq 'ja') {
        $prod->{allpkgs} = [ qw(VRTSmulic32 VRTSjacav60 VRTSjacs60 VRTSjacse60 VRTSjacsu60 VRTSjadba60 VRTSjafs60 VRTSjavm60 VRTSjadbe60 VRTSjaodm60) ];
