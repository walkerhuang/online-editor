...
package Pkg::VRTScps62::Common;
@Pkg::VRTScps62::Common::ISA = qw(Pkg);

sub init_common {
    my $pkg=shift;
    $pkg->{pkg}='VRTScps';
    $pkg->{name}=Msg::new("Cluster Server - Coordination Point Server")->{msg};
    $pkg->{stopprocs}=[ qw(vxcpserv62) ];
    ...
    $pkg->{cpskey} = 'AJZ9-IL33-2T7C-VT8C-PPRP-PPPP-PPPC-CVW4-P';
    $pkg->{obsolete_cpskey} = [qw(AJZU-WPDM-38ZY-U2T8-CP3P-PPPP-PPPP-C3GC-P AJZU-9IBB-R6XP-MZOS-I2XO-PPNP-PPPP-PPP3-P)];
...
sub cps_cleanup_sys {
    ...
    for my $cpsname (@{$vxfenconf->{cps}}) {
        ...
        if ($verbose && !(Cfg::opt(qw(upgrade patchupgrade hotfixupgrade)))) {
        ...
        if ($option != 3) {
            ...
            if ($config_flag->{sfha_config}) {
                ...
                for my $sys(@systems) {
                    ...
                    if ($vmmode !~ /enable/m) {
                        $msg=Msg::new("VxVM is not running on $sys->{sys}. Make sure VxVM is running before configuring Coordination Point Server on SFHA cluster.");
                        $msg->die();
                    }
                }
            }
            if ($config_flag->{singlenode_config}) {
                ...
                if ((CPIC::get('prod') =~ /VCS/m) && ($vcs->check_installed_ha_product_sys($sys0) eq 'VCS62')) {
                    if (!$rel->prod_permanent_licensed_sys($sys0, 'VCS62')) {
...
sub ask_https_cps_info {
    ...
    while (1) {
        ...
        for my $port(@ports) {
            if (!Pkg::VRTSvxfen62::Common::validate_cps_port($port,$default_https_port)) {
                $invalid = 1;
            }
        }
        next if ($invalid);
...
sub ask_ipm_cps_info {
    ...
    while (1) {
        ...
        while (1) {
            ...
            @ports = split(/\s+/, $answer);
            for my $port(@ports) {
                if (!Pkg::VRTSvxfen62::Common::validate_cps_port($port,$defport)) {
                    $invalid = 1;
                }
            }
            next if ($invalid);
...
sub import_dg_with_scsi3pr {
    ...
    my $sys0 = ${CPIC::get('systems')}[0];
    my $vxfen_pkg = $pkg->pkg('VRTSvxfen62');
...
sub delete_singlenode_cps_license_after_unconfig {
    my $pkg = shift;
    my ($keyfile,$lic_dir,$sys,$out,$rel);
    $sys = ${CPIC::get('systems')}[0];
    ...
    $sys->cmd("_cmd_rmr $keyfile");
    $rel = Obj::rel();
    return if ($rel->prod_permanent_licensed_sys($sys, 'VCS62'));
...
sub verify_responsefile {
    ...
    if ($cfg->{cps_singlenode_config} || $cfg->{cps_sfha_config}) {
        ...
        for my $port(@ports) {
            if (!Pkg::VRTSvxfen62::Common::validate_cps_port($port,$pkg->{defport},$pkg->{default_https_port})) {
...
package Pkg::VRTScps62::AIX;
@Pkg::VRTScps62::AIX::ISA = qw(Pkg::VRTScps62::Common);

package Pkg::VRTScps62::HPUX;
@Pkg::VRTScps62::HPUX::ISA = qw(Pkg::VRTScps62::Common);
...
package Pkg::VRTScps62::Linux;
@Pkg::VRTScps62::Linux::ISA = qw(Pkg::VRTScps62::Common);

package Pkg::VRTScps62::RHEL5x8664;
@Pkg::VRTScps62::RHEL5x8664::ISA = qw(Pkg::VRTScps62::Linux);
...
package Pkg::VRTScps62::RHEL6x8664;
@Pkg::VRTScps62::RHEL6x8664::ISA = qw(Pkg::VRTScps62::Linux);
...
sub init_padv {
    ...
    $pkg->{oslibs}{'RHEL6.6'}={
        "libcrypt.so.1"  =>  "glibc-2.12-1.149.el6.i686",
        "libcrypt.so.1(GLIBC_2.0)"  =>  "glibc-2.12-1.149.el6.i686",
        ...
        "libc.so.6(GLIBC_2.1.2)"  =>  "glibc-2.12-1.149.el6.i686",
        ...
        "libgcc_s.so.1(GCC_3.3)"  =>  "libgcc-4.4.7-11.el6.i686",
        "libgcc_s.so.1(GLIBC_2.0)"  =>  "libgcc-4.4.7-11.el6.i686",
        "libm.so.6"  =>  "glibc-2.12-1.149.el6.i686",
        "libm.so.6(GLIBC_2.0)"  =>  "glibc-2.12-1.149.el6.i686",
        "libm.so.6(GLIBC_2.1)"  =>  "glibc-2.12-1.149.el6.i686",
        "libm.so.6(GLIBC_2.2)"  =>  "glibc-2.12-1.149.el6.i686",
        ...
        "libresolv.so.2"  =>  "glibc-2.12-1.149.el6.i686",
        "librt.so.1"  =>  "glibc-2.12-1.149.el6.i686",
        "librt.so.1(GLIBC_2.2)"  =>  "glibc-2.12-1.149.el6.i686",
        ...
        "libstdc++.so.6(GLIBCXX_3.4.11)"  =>  "libstdc++-4.4.7-11.el6.i686",
        "libstdc++.so.6(GLIBCXX_3.4.9)"  =>  "libstdc++-4.4.7-11.el6.i686",
...
package Pkg::VRTScps62::RHEL7x8664;
@Pkg::VRTScps62::RHEL7x8664::ISA = qw(Pkg::VRTScps62::Linux);
...
sub init_padv {
    ...
    $pkg->{oslibs}{'RHEL7.0'}={
        ...
        "libc.so.6(GLIBC_2.4)"  =>  "glibc-2.17-55.el7.i686",
        ...
        "libgcc_s.so.1(GCC_3.3)"  =>  "libgcc-4.8.2-16.el7.i686",
        ...
        "libm.so.6(GLIBC_2.1)"  =>  "glibc-2.17-55.el7.i686",
        "libm.so.6(GLIBC_2.2)"  =>  "glibc-2.17-55.el7.i686",
        ...
        "libresolv.so.2"  =>  "glibc-2.17-55.el7.i686",
        ...
        "libstdc++.so.6(GLIBCXX_3.4.11)"  =>  "libstdc++-4.8.2-16.el7.i686",
        "libstdc++.so.6(GLIBCXX_3.4.15)"  =>  "libstdc++-4.8.2-16.el7.i686",
        "libstdc++.so.6(GLIBCXX_3.4.9)"  =>  "libstdc++-4.8.2-16.el7.i686",
...
package Pkg::VRTScps62::OL6x8664;
@Pkg::VRTScps62::OL6x8664::ISA = qw(Pkg::VRTScps62::RHEL6x8664);

package Pkg::VRTScps62::OL7x8664;
@Pkg::VRTScps62::OL7x8664::ISA = qw(Pkg::VRTScps62::RHEL7x8664);

package Pkg::VRTScps62::SLES10x8664;
@Pkg::VRTScps62::SLES10x8664::ISA = qw(Pkg::VRTScps62::Linux);
...
package Pkg::VRTScps62::SLES11x8664;
@Pkg::VRTScps62::SLES11x8664::ISA = qw(Pkg::VRTScps62::Linux);
...
package Pkg::VRTScps62::SunOS;
@Pkg::VRTScps62::SunOS::ISA = qw(Pkg::VRTScps62::Common);
