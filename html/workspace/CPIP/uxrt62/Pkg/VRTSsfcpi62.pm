...
package Pkg::VRTSsfcpi62::Common;
@Pkg::VRTSsfcpi62::Common::ISA = qw(Pkg);
        ...
        if ($oldpkg =~ /VRTSsfcpi/ && ($oldpkg ne $pkg->{pkg})) {
            ...
            if (!Cfg::opt('patchupgrade') && !Cfg::opt('rolling_upgrade')) {
                for my $pkgi (qw /VRTSvxvm VRTSvxfs VRTSvcs/) {
                    ...
                    if (!EDRu::inarr($pkgi.'62', @{$sys->{installpkgs}}) && !EDRu::compvers($sys->{pkgvers}{$oldpkg},$pkgver,3) ) {
...
sub postinstall_sys {
    ...
    my $scriptfile="$rootpath/opt/VRTS/install/bin/UXRT62/add_install_scripts";
...
package Pkg::VRTSsfcpi62::AIX;
@Pkg::VRTSsfcpi62::AIX::ISA = qw(Pkg::VRTSsfcpi62::Common);

package Pkg::VRTSsfcpi62::HPUX;
@Pkg::VRTSsfcpi62::HPUX::ISA = qw(Pkg::VRTSsfcpi62::Common);

package Pkg::VRTSsfcpi62::Linux;
@Pkg::VRTSsfcpi62::Linux::ISA = qw(Pkg::VRTSsfcpi62::Common);

package Pkg::VRTSsfcpi62::SunOS;
@Pkg::VRTSsfcpi62::SunOS::ISA = qw(Pkg::VRTSsfcpi62::Common);

package Pkg::VRTSsfcpi62::Sol11sparc;
@Pkg::VRTSsfcpi62::Sol11sparc::ISA = qw(Pkg::VRTSsfcpi62::SunOS);
...
sub postinstall_sys {
    my ($pkg,$sys)=@_;
    my $rootpath = Cfg::opt('rootpath');
    my $scriptfile="$rootpath/opt/VRTSsfcpi62/bin/run-once";
    if ($sys->exists($scriptfile)) {
        Msg::log("Running postinstall script of $pkg->{pkg}");
        $sys->cmd("BASEDIR=$rootpath VRTSSFCPISILENT=1 $scriptfile 2>/dev/null");
    }
    return $pkg->SUPER::postinstall_sys($sys);
}
