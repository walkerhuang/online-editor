...
package Pkg::VRTSvxfen62::Common;
@Pkg::VRTSvxfen62::Common::ISA = qw(Pkg);
...
sub init_common {
    ...
    $pkg->{startprocs}=[ qw(vxfen62) ];
    $pkg->{stopprocs}=[ qw(vxfen62) ];
    $pkg->{status}=0;
...
sub configure_disabled_mode {
    ...
    for my $sys (@{$cpic->{systems}}) {
        ...
        $proc=$pkg->proc('vxfen62');
        $proc->enable_sys($sys);
        # when configure fencing to disabled mode or majority-based mode, if PreferredFencingPolicy was set by users, need remove it first before starting process
        $vcs->set_preferredfencing_sys($sys);
...
sub configure_fencing {
    ...
    my ($help,$mode,$vxfenmode,$msg_cp_order,$msg_refresh_keys,$msg_majority,$autodisablegrps,$autodisablegrplist,$sys0,$cpic,$prodinstalled);
    ...
    $tmpdir=EDR::tmpdir();
    $cpic=Obj::cpic();
    $cpic->installed_prod_sys($sys0) unless (${$sys0->{upgradeprod}}[0]);
    $prodinstalled = ${$sys0->{upgradeprod}}[0];
    ...
    while (1) {
        ...
        if (Cfg::opt('responsefile')) {
            ...
            if (Obj::webui()){
                ...
                $msg_majority=Msg::new("Configure majority based fencing");

                $vxfenmode = $pkg->vxfen_mode_sys($sys0);
                # majority based fencing is not supported for SFSybaseCE and SFRAC
                if($prodinstalled =~ /SFSYBASECE/m){
                    if(Cfg::opt('fencing')){
                        if ($vxfenmode =~ /^disabled$/m) {
                            push (@{$menuopt},$msg_sybase->{msg});
                        } else {
                    ...
                    if ($prodinstalled =~ /SFRAC/m) {
                        ...
                        $help = Msg::new("I/O fencing that uses coordinator disks is referred to as disk-based I/O fencing. I/O fencing that uses at least one CP server system is referred to as server-based I/O fencing. Server-based fencing can include only CP servers, or a mix of CP servers and coordinator disks. In virtualized environments that do not support SCSI-3 PR, VCS supports non-SCSI-3 server-based I/O fencing. Majority based fencing is a fencing mode in which no coordination points are required.");
                        push (@{$menuopt},$msg_cpc->{msg},$msg_dsk->{msg},$msg_majority->{msg});
                    }

                    if (Cfg::opt('fencing')) {
                        # if vxfen is running in majority based mode or disabled mode, migration/refresh keys/set the cp order are not supported, so not list them
                        if ($vxfenmode !~ /^(majority|disabled)$/m) { 
                    ...
                    #  majority based fencing is not supported for SFSybaseCE and SFRAC
                    if ($prodinstalled !~ /SFRAC/m) {
                        # add a new fencing mode: majority based fencing
                        $menu = 8 if ($menu == 3);
                        $menu-- if ($menu >= 4);
                    }
                }
        ...
        # check whether openssl is installed except choosing 3(Configure fencing in disabled mode) or 7(majority based fencing)
        if (($menu != 3) && ($menu != 7)) {
        ...
        if (($menu == 1) || ($menu == 2)) {
            ...
            if (Cfg::opt('responsefile')) {
                ...
                if ($prodinstalled =~ /SFSYBASECE/m && !Cfg::opt('fencing')) {
                    $ayn = $msg->ayn('',$help);
                } else {
        ...
        if ($menu==2) {
            ...
            unless ($fencing_failed) {
                $mode = ($prodinstalled =~ /SFSYBASECE/m) ? 'sybase' : 'disk';
            }
        }
        if ($menu==4) {
        ...
        if ($menu==3) {
            ...
            $autodisablegrps = $pkg->get_autodisablenooffline_grplist_sys($sys);
            if (@{$autodisablegrps}) { 
                $autodisablegrplist = join ' ', @$autodisablegrps;
                Msg::n();
            ...
            Msg::n();
            if (($prodinstalled =~ /(VCS|SFHA)/m) && $pkg->{non_scsi3}) {
                $msg=Msg::new("HAD will be stopped without stopping the applications. Do you want to stop VCS and apply fencing configuration on all nodes at this point?");
            } else {
                $msg=Msg::new("HAD and all the applications will be stopped. Do you want to stop VCS and all its applications and apply fencing configuration on all nodes at this point?");
            }
        ...
        if ($menu == 7) {
            $fencing_failed=$pkg->configure_majority_fencing();
        }
...
sub validate_dgnum {
    my ($pkg,$dgname) = @_;
    my ($dgs,$msg,$web,$sys,$n,$prev,$syslist);
    $syslist=CPIC::get('systems');
    $web=Obj::web();
    for my $sys (@$syslist) {
        #e3626254:use regular expression to get the match items instead of grep -w which regard '-' as a non-word constituent.
        #The disk group name has parenthesis when it is deported, and fencing disk group is supposed to be in the deported state for default.So either dgname with parenthesis or dgname should be matched.
        $dgs=$sys->cmd("_cmd_vxdisk -o alldgs list 2>/dev/null| _cmd_awk '{print \$4}'");
        $n=0;
        for my $dg (split(/\n+/,$dgs)) {
                $n++ if ($dg eq $dgname || $dg eq "($dgname)");  
        }
        if ($sys->system1) {
            $prev=$n;
        } elsif ($n!=$prev) {
...
sub confirm_stop_vcsfen {
    ...
    my ($syslist,$sys0,$autodisablegrps,$autodisablegrplist,$cpic);
    $web = Obj::web();
    $syslist=CPIC::get('systems');
    $sys0 = $$syslist[0];
    $cpic = Obj::cpic();
    ...
    $webmsg->bold;
    # check if any group is set to AutoDisableNoOffline, and prompt users to offline manually 
    $autodisablegrps = $pkg->get_autodisablenooffline_grplist_sys($sys0);
    if (@{$autodisablegrps}) {
        $autodisablegrplist = join ' ', @$autodisablegrps;
        Msg::n();
        $msg = Msg::new("The value of the attribute 'SysDownPolicy' of the following service groups is 'AutoDisableNoOffline', these service groups need to be brought offline manually before stopping VCS:\n\t$autodisablegrplist");
    ...
    Msg::n();
    $cpic->installed_prod_sys($sys0) unless (${$sys0->{upgradeprod}}[0]);
    my $prodinstalled = ${$sys0->{upgradeprod}}[0];
    if (($prodinstalled =~ /(VCS|SFHA)/m) && $pkg->{non_scsi3}) {
        $msg=Msg::new("HAD will be stopped without stopping the applications. Do you want to stop VCS and apply fencing configuration on all nodes at this point?");
    } else {
        $msg=Msg::new("HAD and all the applications will be stopped. Do you want to stop VCS and all its applications and apply fencing configuration on all nodes at this point?");
...
# get the groups configured with SysDownPolicy = { AutoDisableNoOffline } attribute
sub get_autodisablenooffline_grplist_sys {
    my ($pkg,$sys) = @_;
    my ($vcs,$autodisablegrps,@autodisablegrp);

    $vcs = $pkg->prod('VCS62');
    @autodisablegrp=();
    $autodisablegrps = $sys->cmd("$vcs->{bindir}/hagrp -list State=~ONLINE SysDownPolicy=AutoDisableNoOffline 2>/dev/null | _cmd_awk '{print \$1}'");
    for my $grp (split(/\n/, $autodisablegrps)) {
        push @autodisablegrp, $grp if (!EDRu::inarr($grp,@autodisablegrp));
    }

    return \@autodisablegrp; 
}
...
sub configure_cpc {
    ...
    $cpsadm = $cpspkg->{cpsadm};
    $cpsat = $cpspkg->{cpsat};
    $defport = $cpspkg->{default_https_port};

    # Check if VRTScps is installed on all the client cluster nodes or not
    return 1 if (!$pkg->check_pkg_installed('VRTScps62'));
    ...
    if ($ndisks) {
        ...
        if ($vmmode !~ /enabled/m) {
            $msg=Msg::new("Configure VxVM before configuring Coordination Point server based fencing in the customized mode");
            $msg->print;
            $web->web_script_form('alert',$msg) if (Obj::webui());
            return 1;
        }
    ...
    Msg::n();
    $pkg->start_vxfen_on_all_systems();
    # vxfen fail to start, CPS clean up
    if (-3 == $pkg->{status}) {
        Msg::prtc();
        $pkg->stop_vcsfen();
...
sub cps_transport_sys {
    ...
    if ($sys->{islocal}) {
        ...
        # e3589028, not use ping to check if the input ip/hostname is up, use ssh to check, the timeout is 10 seconds
        $edr->{ssh_time_out}=10;
        $rtn=$edr->rsh_sys($sys);
        undef($edr->{ssh_time_out});
...
sub check_ssh_passwdless_sys0_sys {
    ...
    #e3612831:remove the stale key of sys on sys0.
    $rel=Obj::rel();
    $rel->remove_stale_key_from_knownhosts($sys0,$sys);
...
sub addnode_configure_nonscsi3_sys {
    ...
    my $vxfen_proc = $sys->proc('vxfen62');
...
# configure majority based fencing
sub configure_majority_fencing {
    my $pkg = shift;
    my ($cpic,$sys,$cfg,$msg,$vxfen,$ret,$str,$suffix,$oldvxfenconf,$vcs,$rtn,$ayn,$help,$loser_exit_delay,$vxfen_script_timeout,$notice);

    # add a disclaimer when users choose majority based fencing
    $cpic=Obj::cpic();
    $cfg=Obj::cfg();
    if (Cfg::opt('responsefile')) { 
        $rtn = $cfg->{config_majority_based_fencing} ? 'Y' : 'N'; 
    } else {
        $notice = Msg::new("Majority-based I/O fencing has limited functionality. In case of a split brain, where none of the sub-clusters have a majority or where the sub-cluster having majority is hung, all nodes in the cluster may panic. Refer to VCS Administrator's Guide for more details.");
        $notice->printn();
        $msg = Msg::new("Do you want to continue?");
        $rtn = $msg->aynn('','',$notice);
    }
    if ($rtn eq 'N') {
        $cfg->{config_majority_based_fencing} = 0 if (!Cfg::opt('responsefile'));
        $cpic->completion();
    } else {
        $cfg->{config_majority_based_fencing} = 1 if (!Cfg::opt('responsefile'));
    }
    ...
    # e3512539, configure NSF with majority based fencing
    unless (Cfg::opt('responsefile')) {
        Msg::n();
        $msg = Msg::new("Does your storage environment support SCSI3 PR?");
        $help = Msg::new("SCSI-3 Persistent Reservation (SCSI-3 PR) supports device access from multiple systems, or from multiple paths from a single system. At the same time it blocks access to the device from other systems, or other paths.\nUse of SCSI 3 PR protects against all elements in the IT environment that might be trying to write illegally to storage, not only VCS related elements.");
    ...
    $ret = $pkg->confirm_stop_vcsfen();
    return $ret if ($ret);
    # before stopping vxfen, remove cp agent if any
    $pkg->remove_cpagent();
    $pkg->stop_vcsfen();
    ...
    # generate /etc/vxfenmode file 
    $str = "vxfen_mode=majority\n";
    # add parameters in vxfenmode for non-scsi3 fencing
    if ($pkg->{non_scsi3}) {
        $str .= "loser_exit_delay=$loser_exit_delay\n";
        $str .= "vxfen_script_timeout=$vxfen_script_timeout\n";
    }
    for my $sys (@{$cpic->{systems}}) {
        $msg = Msg::new("Updating $pkg->{vxfenmode_file} file on $sys->{sys}");
        $msg->left;
...
sub start_vxfen_on_all_systems {
    my $pkg = shift;
    my ($cpic,$msg,$vxfen,$web);
...
sub migrate_fencing_precheck {
    ...
    for my $sys(@{$syslist}) {
        ...
        if ($vxfen_mode !~ /(disk|cps|sybase|nonscsi3)/im) {
            $msg = Msg::new("vxfen is not running in enabled mode or the operation is not supported based on current mode on $sys->{sys}");
            $msg->print;
            $web->web_script_form('alert',$msg->{msg})if (Obj::webui());
            Msg::prtc();
            return -1;
...
sub replace_cp {
    ...
    my ($dgname,$newconf,$scsi3,$web,$fips_mode);
    ...
    Msg::title();
    ($ret,$added_cp_href) = $pkg->ask_coord_points_to_add($conf_href,$removed_cp_href);
    return $ret if ($ret);
...
sub ask_coord_points_to_add {
    my ($pkg,$conf_href,$removed_cp_href) = @_;
    ...
    $msg->printn;
    $oldncp = $conf_href->{ncp} - $removed_cp_href->{ncp};
    ...
    while (1) {
        if (Obj::webui()){
            ...
            ($ret,$ncp,$ndisks)=$pkg->ask_new_coord_points_num($oldncp,$conf_href);
            return $ret if ($ret);
        }

        # if the reserved coordination points include disk, the number of coordination points cannot be 1 
        if (($conf_href->{ndisks} != $removed_cp_href->{ndisks}) && ($oldncp + $ncp == 1)) {
            $msg=Msg::new("Total number of coordination points should be odd and no less than three when the coordination points include disks. Input again");
            $msg->print;
            next;
        }
...
sub verify_responsefile_for_fencing {
    ...
    } elsif (!EDRu::inarr($cfg->{fencing_option},(3,4,5,6,7))) {
...
sub responsefile_comments_for_fencing {
    ...
    if ($cfg->{fencing_option}) {
        $cmt=Msg::new("This variable defines the I/O fencing configuration mode. (1: cp client-based fencing; 2: disk-based fencing or Sybase mode fencing for SFSYBASECE; 3: disabled mode; 4: online fencing migration; 5: Refresh keys/registrations on the existing coordination points; 6: Change the order of existing coordination points; 7: Majority based fencing)");
    ...
    if ($cfg->{fencing_option} == 2) {
        ...
        } elsif ($cfg->{fencing_option} == 7) {
            $cmt=Msg::new("This variable defines if the majority based fencing is configured");
            $edr->{rfc}{config_majority_based_fencing}=[$cmt->{msg},0,0,0];
        }
    }
    return;
}
...
sub verify_responsefile_for_dskfenc {
    ...
    $pkg = shift;
    $cfg = Obj::cfg();
    unless ($cfg->{fencing_dgname}) {
        $msg = Msg::new("fencing_dgname must be set in responsefile");
        $msg->die;
    }
    if ($cfg->{fencing_option} != 2) {
        $msg = Msg::new("fencing_option in response file is not a valid fencing configuration option");
...
sub stop_vcsfen_sys {
    my ($pkg,$sys) = @_;
    my ($msg,$port_h,$cpic,$proc,$had,$sysname,$vcs,$prodinstalled);
    $cpic=Obj::cpic();
    $had=$pkg->proc('had62');
    $proc=$pkg->proc('vxfen62');
    $vcs = $pkg->prod('VCS62');
    $cpic->installed_prod_sys($sys) unless (${$sys->{upgradeprod}}[0]);
    $prodinstalled = ${$sys->{upgradeprod}}[0];
    if ($had->check_sys($sys,'stop')) {
        ...
        $sysname = $sys->{vcs_sysname};
        if (($prodinstalled =~ /(VCS|SFHA)/m) && $pkg->{non_scsi3}) {
            $sys->cmd("/opt/VRTSvcs/bin/hastop -sys $sysname -force");
            #e3573631:hastop -force won't stop CmdServer, then product will give error when '/etc/init.d/vcs start' start CmdServer which has been running.
...
sub config_vxfen {
    my ($ayn,$cpic,$done,$edr,$msg,$option,$pids,$pkg,$port_h,$ret,$sys,$had,$vm);
    ...
    if (!$pkg->check_pkg_installed('VRTSvxfen62')) {
    ...
    for my $sys (@{$cpic->{systems}}) {
        ...
        #e3573631:vcs start will start CmdServer, check if it's successfully started for debug.
        $pids = $sys->proc_pids('CmdServer');
        if (-1 == $#$pids) {
            Msg::log("CmdServer haven't be started successfully during starting VCS!");
        }
    }
...
sub vxfen_mode_sys {
    ...
    if (defined($vxfen_config{fencing_mode})) {
        if ($vxfen_config{fencing_mode} =~ /SCSI3/im) {
            ...
            return 'sybase';
        } elsif ($vxfen_config{fencing_mode} =~ /majority/im) {
            return 'majority';
        }
    } else {
        return 'error';
...
package Pkg::VRTSvxfen62::AIX;
@Pkg::VRTSvxfen62::AIX::ISA = qw(Pkg::VRTSvxfen62::Common);
...
package Pkg::VRTSvxfen62::HPUX;
@Pkg::VRTSvxfen62::HPUX::ISA = qw(Pkg::VRTSvxfen62::Common);

package Pkg::VRTSvxfen62::Linux;
@Pkg::VRTSvxfen62::Linux::ISA = qw(Pkg::VRTSvxfen62::Common);

package Pkg::VRTSvxfen62::RHEL5x8664;
@Pkg::VRTSvxfen62::RHEL5x8664::ISA = qw(Pkg::VRTSvxfen62::Linux);
...
package Pkg::VRTSvxfen62::RHEL6x8664;
@Pkg::VRTSvxfen62::RHEL6x8664::ISA = qw(Pkg::VRTSvxfen62::Linux);
...
sub init_padv {
    ...
    $pkg->{oslibs}{'RHEL6.3'}={
        ...
        "perl(Exporter)"  =>  "perl-5.10.1-127.el6.x86_64",
        "perl(strict)"  =>  "perl-5.10.1-127.el6.x86_64",
        "perl(warnings)"  =>  "perl-5.10.1-127.el6.x86_64",
    ...
    $pkg->{oslibs}{'RHEL6.4'}={
        ...
        "perl(Exporter)"  =>  "perl-5.10.1-129.el6.x86_64",
        "perl(strict)"  =>  "perl-5.10.1-129.el6.x86_64",
        "perl(warnings)"  =>  "perl-5.10.1-129.el6.x86_64",
    ...
    $pkg->{oslibs}{'RHEL6.6'}={
        ...
        "libgcc_s.so.1"  =>  "libgcc-4.4.7-11.el6.i686",
        "libgcc_s.so.1(GCC_3.0)"  =>  "libgcc-4.4.7-11.el6.i686",
        ...
        "librt.so.1"  =>  "glibc-2.12-1.149.el6.i686",
...
package Pkg::VRTSvxfen62::RHEL7x8664;
@Pkg::VRTSvxfen62::RHEL7x8664::ISA = qw(Pkg::VRTSvxfen62::Linux);
...
sub init_padv {
    ...
    $pkg->{oslibs}{'RHEL7.0'}={
        ...
        "perl(Exporter)"  =>  "perl-Exporter-5.68-3.el7.noarch",
        "perl(strict)"  =>  "perl-5.16.3-283.el7.x86_64",
        "perl(warnings)"  =>  "perl-5.16.3-283.el7.x86_64",
...
package Pkg::VRTSvxfen62::OL6x8664;
@Pkg::VRTSvxfen62::OL6x8664::ISA = qw(Pkg::VRTSvxfen62::RHEL6x8664);

package Pkg::VRTSvxfen62::OL7x8664;
@Pkg::VRTSvxfen62::OL7x8664::ISA = qw(Pkg::VRTSvxfen62::RHEL7x8664);

package Pkg::VRTSvxfen62::SLES10x8664;
@Pkg::VRTSvxfen62::SLES10x8664::ISA = qw(Pkg::VRTSvxfen62::Linux);
...
package Pkg::VRTSvxfen62::SLES11x8664;
@Pkg::VRTSvxfen62::SLES11x8664::ISA = qw(Pkg::VRTSvxfen62::Linux);
...
package Pkg::VRTSvxfen62::SunOS;
@Pkg::VRTSvxfen62::SunOS::ISA = qw(Pkg::VRTSvxfen62::Common);
