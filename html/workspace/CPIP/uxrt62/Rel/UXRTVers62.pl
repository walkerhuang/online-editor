...
sub update_matrix_data {
    ...
    foreach my $level (qw(ga patch hotfix)) {
        ...
        $url .= "?padv=$padv&level=$level&pretty=1&product=SFHA";
        $url .= "&pseudo=1" if ($pseudo_hotfix);
        Msg::log("Downloading $url");
        $response = EDRu::download_page($url);
        # No need to adjust the result since 'pretty=1' provided in the $url
        # $response =~ s/[\n\r\t\f]+//mg;
        # $response =~ s/^.*<pre>//mg;
        # $response =~ s/<\/pre>.*$//mg;
        # $response =~ s/(.*)}.*$/$1}/mg;
...
sub get_padv_for_vc {
    ...
    if ( $padv =~ /^(RHEL[0-9]+)x8664/) {
        $sort_padv = $1."_x86_64";
    } elsif ( $padv =~ /^OL([0-9]+)x8664/) {
        $sort_padv = "RHEL".$1."_x86_64";
...
# this is temporary workaround
    ...
    $release = $mpvers;
    my $padv = $rel->{padv_managed_host_for_vc};
    if ($padv =~/aix71/ && $mpvers =~/^5.1SP1RP/){
        push @releases, "5.1SP1PR1";
    }
    ...
    @releases=@{EDRu::arruniq(@releases)};
    return @releases;
}
...
sub get_prod_required_pkgs {
    ...
    if (exists($rel->{vc_release_matrix_data}{$padv}{$rel_id})
        ...
        @requiredpkgs = @{$rel->{vc_release_matrix_data}{$padv}{$rel_id}{prod}{$prod}{rpkgs}} if (exists($rel->{vc_release_matrix_data}{$padv}{$rel_id}{prod}{$prod}{rpkgs}));
        ...
        @requiredpkgs = @{$rel->{vc_patch_matrix_data}{$padv}{$rel_id}{prod}{$prod}{rpkgs}} if (exists($rel->{vc_patch_matrix_data}{$padv}{$rel_id}{prod}{$prod}{rpkgs}));
    } else {
...
sub get_prod_all_pkgs {
    ...
    if (exists($rel->{vc_release_matrix_data}{$padv}{$rel_id})
        && exists($rel->{vc_release_matrix_data}{$padv}{$rel_id}{prod})
        && exists($rel->{vc_release_matrix_data}{$padv}{$rel_id}{prod}{$prod})) {
        push @allpkgs, @{$rel->{vc_release_matrix_data}{$padv}{$rel_id}{prod}{$prod}{rpkgs}} if (exists($rel->{vc_release_matrix_data}{$padv}{$rel_id}{prod}{$prod}{rpkgs}));
        ...
        && exists($rel->{vc_patch_matrix_data}{$padv}{$rel_id}{prod})
        && exists($rel->{vc_patch_matrix_data}{$padv}{$rel_id}{prod}{$prod})) {
        push @allpkgs, @{$rel->{vc_patch_matrix_data}{$padv}{$rel_id}{prod}{$prod}{rpkgs}} if (exists($rel->{vc_patch_matrix_data}{$padv}{$rel_id}{prod}{$prod}{rpkgs}));
...
sub check_prods_install_status_sys {
    ...
    my ($padv, $hotfix_t, $current_version, $ihotfixes, @items, $alreadyin, $cksum, $uploadfilename, $type, $dupMainPkg);
    ...
    for my $prod (sort keys %{$sys->{iprod}}) {
        ...
        $dupMainPkg = EDRu::duparr(@mainpkgs);
            ...
            for my $relvers (@reversed_releases) {
                ...
                for my $patch (@relpatches) {
                    if (EDRu::inarr($patch, @{$sys->{patches}})){
                        ...
                        #check if it is a patch of mainpkg
                        for my $tmpPkg(@{$rel->{vc_patch_matrix_data}{$padv}{$rel->{relname_to_relid}{$relvers}{relid}}{patch}{$patch}{pkgs}}){
                            if(EDRu::inarr($tmpPkg,@{$dupMainPkg})){
                                $dupMainPkg = EDRu::arrdel($dupMainPkg,$tmpPkg);
        ...
        $prod_lic = $rel->prod_licensed_sys($sys,uc($prod) ."62");
...
sub check_extra_pkgs {
    my ($rel, $sys)= @_;
    my (@allprodpkgs,@installedpkgs);

    for my $prod (sort keys %{$sys->{iprod}}) {
        push (@allprodpkgs,@{$rel->get_prod_all_pkgs($prod, $sys->{iprod}{$prod}{defined_vers})});
    }
    @allprodpkgs=@{EDRu::arruniq(@allprodpkgs)};
    @installedpkgs=keys%{$sys->{pkgvers}};
    $sys->{extrapkgs}=EDRu::arrdel(\@installedpkgs,@allprodpkgs);
    return '';
}

sub output_extra_pkgs {
    my ($rel, $sys)= @_;
    my ($msg,$rootpath,$pstamp,$pkgvers);
    return unless ($sys->{extrapkgs} && @{$sys->{extrapkgs}});
    Msg::n();
    $msg=Msg::new("Extra packages installed on $sys->{sys}:");
    $msg->bold();
    Msg::n();
...
sub check_available_update_status_sys {
    ...
    for my $prod (sort keys %{$sys->{iprod}}) {
        ...
        if($padv =~/aix71/ && $current_version=~/^5.1SP1RP/){
            $current_version =~ s/^5.1SP1RP/5.1SP1PR1RP/;
        }
    ...
    for my $id (@{$rel->{vc_update_status}{$sys->{sys}}{available_ga_update}{id_set}}) {
        ...
        if ($rel->{repos_set_for_vc} == 1){
            if ($rel->{vc_release_matrix_data}{$padv}{$id}{upload_location}){
                $uploadfilename = $rel->{vc_release_matrix_data}{$padv}{$id}{filename};
                $type = "ga";
...
sub output_upgrade_options_sys {
    my ($rel,$sys) = @_;
    my ($contents, $titles, $msg, $padv, $prodname, $relvers, $msgnone);
    ...
    for my $prod (sort keys %{$sys->{iprod}}) {
        ...
        Msg::n();
        $msg = Msg::new("Available Patches for $prodname $relvers:");
        $msg->bold();
...
sub output_prods_install_status_sys {
    ...
    Msg::n();
    $msg = Msg::new("Platform of $sys->{sys}:");
    $msg->bold();
    $msg = Msg::new("\t$sys->{osversion}");
    $msg->print();
    Msg::n();
    ...
    for my $prod (sort keys %{$sys->{iprod}}) {
        ...
        $prodvers = $sys->{iprod}{$prod}{license};
        $prodvers ||= Msg::new("none")->{msg};
        if ($sys->{partial_upgrade}) {
            $msg = Msg::new("\t$sys->{iprod}{$prod}{name} - $sys->{iprod}{$prod}{mprpvers} Partial upgraded - license $prodvers");
        } else {
            $msg = Msg::new("\t$sys->{iprod}{$prod}{name} - $sys->{iprod}{$prod}{mprpvers} - license $prodvers");
        }
        $msg->print();
    }
    Msg::n();
    ...
    for my $prod (sort keys %{$sys->{iprod}}) {
        ...
        if (exists($sys->{iprod}{$prod}{ipatches}) || exists($sys->{iprod}{$prod}{mpatches})) {
            ...
            for my $relvers (reverse @{$rel->{releases}}) {
                next if ($relvers lt $sys->{iprod}{$prod}{mprpvers});
        ...
        $count{$prod}{pkgs}{irpkgs} ||=0;
        $count{$prod}{pkgs}{arpkgs} ||=0;
        ...
        $relvers = $sys->{iprod}{$prod}{mprpvers};
        Msg::n();
        $msg = Msg::new("Installed Patches for $prodname $relvers:");
        $msg->bold();
...
sub get_download_releases {
    ...
    if ($download_go == 0) {
        $msg = Msg::new("For $ident_systems, there are no available Patches and Maintenance releases to be downloaded.");
        $msg->print;
        Msg::n();
    }
    return $download_go;
...
sub vc_download_patches {
    ...
    my ($no_release, $ident_systems, $deploy);

    require Deploy;
    if(Obj::created("Deploy")){
        $deploy=Obj::deploy();
    } else {
        $deploy=Deploy->new();
        $deploy->init_preference_hash();
    }
    ...
    if ($flag == 0) {
        $msg = Msg::new("No available Maintenance or Patch releases to be downloaded.");
        $msg->print;
        return '';
    }
    ...
    $backopt = 0;
    Msg::n();
    $msg = Msg::new("Would you like to download the available Maintenance or Patch releases that cannot be found in the repository?");
    $ayn = $msg->aynn();
    Msg::n();
    if ($ayn eq 'Y') {
        if (defined $deploy->{preference}{Repository}{value} && $deploy->{preference}{Repository}{value}){
            $rel->{vc_download_path} = $rel->{repository_path_for_vc};
        } else {
            $msg = Msg::new("\nInstaller detects that the repository directory is not set\n");
            $msg->print;
            if ($deploy->repository_set_home()) {
                $rel->{repository_path_for_vc} = $deploy->{preference}{Repository}{value};
                $rel->{vc_download_path} = $rel->{repository_path_for_vc};
        ...
        $msg->print;
        Msg::n();
        $msg = Msg::new("Public Maintenance and Patch releases will be downloaded into $rel->{vc_download_path} on the local system");
        $msg->print;
        ...
        for my $prodinfo (keys %{$rel->{prodsys}}) {
            ...
            while(1) {
                ...
                $msg_option_hotfix = Msg::new("Patch");
                ...
                if ($fix eq "Maintenance") {
                    if (@available_patch) {
                        if (!$rel->select_releases($sysname, $ident_systems, 'maintenance', \@download_ids, @available_patch)) {
                            next;
                        }
                    } else {
                ...
                if ($fix eq "Patch") {
                    if (@available_hotfix) {
                        if (!$rel->select_releases($sysname, $ident_systems, 'patch', \@download_ids, @available_hotfix)) {
                            next;
                        }
                    } else {
                        $msg = Msg::new("For $ident_systems, there are no available Patch releases to be downloaded.");
                        $msg->print;
                        Msg::n();
                        $no_release = 1;
                    }
                ...
                $msg = Msg::new("Do you want to download another available Maintenance or Patch release for $ident_systems?");
                $ayn = $msg->aynn();
                Msg::n();
                if ($ayn eq 'Y') {
                    next;
...
sub get_new_mprpvers{
    ...
    if(($padv =~ /aix/ && $mprpvers eq "5.1SP1" && $pr eq "1" && $rp)){
        $mprpvers .= "RP$rp";
        if ($p && ! $noP) {
            $mprpvers .= "P$p";
        }
