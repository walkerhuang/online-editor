...
sub init_common {
    ...
    $rel->{pkgsetprods}=[ qw(SFRAC62 SFSYBASECE62 SFCFSHA62 SFHA62 SF62 APPLICATIONHA62 VCS62 DMP62) ];
...
sub init_obsolete_packages_description {
    ...
    $rel->{package_descriptions} = {
        ...
        'VRTSlvmconv' => 'Linux LVM to VxVM Converter',
...
sub init_args {
    ...
    $rel->{arguments} = $arguments = {
        ...
        'args_opt' => [ qw(security securityonenode securitytrust fips addnode fencing configcps
                           vxunroot rolling_upgrade rollingupgrade_phase1 rollingupgrade_phase2 pseudo_hotfix disable_dmp_native_support online_upgrade) ],
        ...
        'online_upgrade' => {
            'description' => Msg::new("The -online_upgrade option is used to perform online upgrade. Using this option, installer will upgrade the whole cluster and support customer's application zero down time during the upgrade procedure. Now this option only support VCS and ApplicationHA."),
            'handler' => \&Rel::UXRT62::Common::process_online_upgrade_arg,
        },
...
sub get_platreqs {
    my ($rel,$prod)=@_;
    return $prod->{platreqs} || $rel->{platreqs};
}

sub filter_args {
    my ($rel,$args_hash,$prod)=@_;
    my ($cpic,$script,@vcs_args,@non_appha_args,@dmp_args,@unused_args);
    ...
    @vcs_args = qw(security securityonenode securitytrust fips addnode fencing configcps
                   ...
                   rolling_upgrade rollingupgrade_phase1 rollingupgrade_phase2 online_upgrade);
    ...
    if ($prod) {
        ...
        if ($prod =~ /^APPLICATIONHA/) {
            ...
            push @{$args_hash->{args_opt}}, "fips";
            push @{$args_hash->{args_opt}}, "security";
            $args_hash->{fips}->{undocumented}=1 if ($args_hash->{fips});
            $args_hash->{security}->{undocumented}=1 if ($args_hash->{security});
        }
        $args_hash->{args_def}=EDRu::arrdel($args_hash->{args_def}, qw(prod));

        #e3566673:add online_upgrade option for VCS and ApplicationHA
        if ($rel->support_online_upgrade($prod) && !Obj::cpic()->{fromdisk}) { 
            push @{$args_hash->{args_opt}}, "online_upgrade" if (!EDRu::inarr("online_upgrade", @{$args_hash->{args_opt}}));
        } else {
            $args_hash->{args_opt}=EDRu::arrdel($args_hash->{args_opt}, "online_upgrade");
        }
        ...
        if ($script eq 'installer') {
            ...
            $args_hash->{args_def}=EDRu::arrdel($args_hash->{args_def}, @non_appha_args);
            $args_hash->{args_task}=EDRu::arrdel($args_hash->{args_task}, @non_appha_args);
            push @{$args_hash->{args_opt}}, "online_upgrade";
            push @{$args_hash->{args_opt}}, "fips";
            push @{$args_hash->{args_opt}}, "security";
            $args_hash->{fips}->{undocumented}=1 if ($args_hash->{fips});
            $args_hash->{security}->{undocumented}=1 if ($args_hash->{security});
        ...
        if ($cpic->{fromdisk}) {
            ...
            $args_hash->{args_opt}=EDRu::arrdel($args_hash->{args_opt}, qw(upgrade_kernelpkgs upgrade_nonkernelpkgs rolling_upgrade rollingupgrade_phase1 rollingupgrade_phase2 installallpkgs installminpkgs installrecpkgs disable_dmp_native_support online_upgrade));
            $args_hash->{args_def}=EDRu::arrdel($args_hash->{args_def}, qw(pkgpath patch_path patch2_path patch3_path patch4_path patch5_path prod kickstart yumgroupxml ignite nim jumpstart flash_archive));
        }
    }

    #e3601142: remove options which is not valid for uninstall script
    if (Obj::cpic()->{script} =~ /uninstall/m) {
        @unused_args=qw(online_upgrade security fips);
        $args_hash->{args_opt}=EDRu::arrdel($args_hash->{args_opt}, @unused_args);
    }

    #e3604084:remove configure option for applicationha
    $args_hash->{args_task}=EDRu::arrdel($args_hash->{args_task}, "configure") if (Obj::cpic()->{script} =~ /applicationha/mi);

    return 1;
}

sub support_online_upgrade {
    my ($rel,$prod)=@_;
    $prod=~s/\d+$//;
    return 1 if (EDRu::inarr($prod, qw(VCS APPLICATIONHA)));
    return 0;
}
...
sub has_cvm {
    my ($rel,$prod)=@_;
    $prod=~s/\d+$//;
    return 1 if (EDRu::inarr($prod, qw(SFCFSHA SVS SFSYBASECE SFRAC)));
    return 0;
}
...
sub process_security_arg {
    ...
    for my $opt (qw(security securityonenode securitytrust fips)) {
        ...
        if (Cfg::opt($opt)) {
            ...
            #ApplicationHA also support security and fips options.
            if (!$rel->has_vcs($prod) && (($opt ne 'security' && $opt ne 'fips') || $prod !~ /ApplicationHA/mi)) {
...
sub process_online_upgrade_arg {
    my ($rel) = @_;
    my ($cpic, $prodname,$msg);
    $cpic=Obj::cpic();

    #3559322: block online upgrade immediately when the prod provided by customer is not supported.
    #online support vcs/appha only.
    $prodname = Cfg::opt("prod");
    if ($prodname && ($prodname !~ /VCS/i) && ($prodname !~ /ApplicationHA/i)) {
        $msg=Msg::new("Online upgrade doesn't support $prodname, choose VCS or ApplicationHA to perform online upgrade.\n");
        $msg->die();
    }

    if (Cfg::opt("online_upgrade")) {
...
sub process_ru_arg {
    my ($rel) = @_;
    my ($edr,$msg,$opt,$cpic,@options,$args_hash,@valid_args,@invalid_args,$args_list);
    $cpic=Obj::cpic();
    $edr=Obj::edr();
    ...
    if (!Cfg::opt("responsefile")) {
        #e3573498: define the options white list for rolling upgrade
        @valid_args = qw(base_path hotfix_path hotfix2_path hotfix3_path hotfix4_path hotfix5_path
                        ignorepatchreqs logpath noipc patch_path patch2_path patch3_path patch4_path patch5_path
                        patchpath pkgpath redirect require rsh serial timeout tmppath upgrade nolic tunablesfile
                        makeresponsefile keyfile disable_dmp_native_support
                        rolling_upgrade rollingupgrade_phase1 rollingupgrade_phase2
                        upgrade_kernelpkgs upgrade_nonkernelpkgs);
        @invalid_args = @{$args_hash->{options}};
        @invalid_args = @{EDRu::arrdel(\@invalid_args,@valid_args)};
        if (scalar @invalid_args) {
            for my $opt (qw(rolling_upgrade rollingupgrade_phase1 rollingupgrade_phase2)) {
                next unless (Cfg::opt($opt));
                $args_list = join(", ", @invalid_args);
                $msg=Msg::new("The $opt option could not be used with $args_list option.");
                $msg->die();
            }
        }
    }
...
sub upgrade_sfha_prods_mix_check {
    ...
    for my $sys (@{$cpic->{systems}}) {
        ...
        if ($sys->{sys} ne ${$cpic->{systems}}[0]->{sys}) {
            if ((!$prev_prod && $sys->{upgradeprod}[0] && EDRu::compvers($sys->{prodvers}[0], $relvers, 4)) || ($prev_prod && !$sys->{upgradeprod}[0] && EDRu::compvers($prev_prodvers, $relvers, 4))) {
        ...
        if ($cpic->{prod} && $prev_prod && $prev_prodvers) {
            ...
            if (!$nocurrentvers_flag && Cfg::opt('upgrade') && $cpic->{prod} ne $prev_prod) {
                set_value_allsys('stop_checks',1);
                $lprod=lc($prod->{prod});
                #3566473:when it's a partial_upgrade but an invalid upgrade path, it must be the big product has been installed. 
                if ($upgrade_status eq 'partial_upgrade') {
                    $msg = Msg::new("$sys_prod_map\n$cprod_name is a subset of $upgrade_prod which has been installed.");
                } else {
        ...
        if (($upgrade_status eq 'partial_upgrade') && $nocurrentvers_flag) {
            ...
            if ($mix_partial_upgrade) {
                ...
                if (Obj::webui()){
                    $msg = Msg::new("$cprod_name is a subset of $upgrade_prod. Upgrading only $cprod_name will partially upgrade the installed packages on the systems.\nRun 'Upgrade a product' on webinstaller for a complete upgrade.");
                }
            } else {
                $prod_value_join="$upgrade_prod".' '."${$sys->{prodvers}}[0]";
...
#e3612831:remove the key of sys in the file '.ssh/known_hosts' on sys0.
#both hostname and ip address should be removed from known_hosts.
sub remove_stale_key_from_knownhosts {
    my ($rel,$sys0,$sys) = @_;
    my ($root_path,$known_hosts_file,$hostname,$ip,$ips,$padv);
    return 1 if ($sys eq $sys0);

    $root_path=$sys0->{root_home} || $sys0->get_root_home();
    $known_hosts_file="$root_path/.ssh/known_hosts";
    ($hostname,$ip) = $sys->nslookup($sys->{sys});

    $sys0->cmd("_cmd_cp -f $known_hosts_file '$known_hosts_file.cpibak' 2> /dev/null; _cmd_sed '/$sys->{sys}/d' '$known_hosts_file.cpibak' > $known_hosts_file 2> /dev/null");

    if ($ip) {
        $sys0->cmd("_cmd_cp -f $known_hosts_file '$known_hosts_file.cpibak' 2> /dev/null; _cmd_sed '/$ip/d' '$known_hosts_file.cpibak' > $known_hosts_file 2> /dev/null");
    } else {
        #nslookup couldn't find ip by hostname, then try padv->ips_sys to get it.
        $padv=$sys->padv;
        $ips=$padv->ips_sys($sys);
        for my $ip (@{$ips}) {
            $sys0->cmd("_cmd_cp -f $known_hosts_file '$known_hosts_file.cpibak' 2> /dev/null; _cmd_sed '/$ip/d' '$known_hosts_file.cpibak' > $known_hosts_file 2> /dev/null");
        }
    }
        ...
        if ($mhvers && $mhcv==2 && Cfg::opt(qw(precheck install upgrade patchupgrade hotfixupgrade))) {
            if (EDRu::inarr($mhpkg->{pkgi}, @{$sys->{donotupgradepkgs}})) {
                ...
                if ($rtn) {
                    ...
                    if($cmsLowerThan61){
                        $msg->{msg} .= Msg::new("\nThe following management servers must be upgraded to 6.1 to ensure proper product functionality:\n\t$cmsLowerThan61")->{msg};
                    }
                    $sys->push_warning($msg);
                }
            }
...
sub default_systemnames {
    ...
    if(Cfg::opt(qw(rolling_upgrade upgrade_kernelpkgs upgrade_nonkernelpkgs ))){
        ...
        return $dfs;
    } elsif (Cfg::opt(qw(online_upgrade))) {
        $dfs=$rel->online_upgrade_get_systems();
        $dfs=~ s/^\s+|\s+$//g;
        @systems=split(/\s+/,$dfs);
        $cfg->{systems}=\@systems;
        return $dfs;
...
sub ru_switch_groups {
    ...
    if ($way == 0) {
        ...
        Msg::n();
        $msg=Msg::new("You may experience application down time while switching failover service group(s). Are you sure you want to continue?");
        $ayn=$msg->ayny('','',$webmsg);
        if ($ayn eq 'Y') {
    ...
    while (1) {
        ...
        if (Cfg::opt('upgrade') && (!$task) && (Cfg::opt('prodmode') ne 'SF Basic')) {
            ...
            if(EDR::getmsgkey($upgrade_task,'back')){
                ...
                $mtask='';
                next;
            } elsif($upgrade_task eq "rolling_upgrade"){
                ...
                $rel->process_ru_arg() if(Cfg::opt('rolling_upgrade'));
            } elsif($upgrade_task eq "online_upgrade") {
                #check the validation of online upgrade 
                Cfg::set_opt($upgrade_task);
                Msg::n();
                $rel->process_online_upgrade_arg();
            }
            last;
        }
        ...
        return "" if (!$rel->{pkgs} && ($cpic->{script} =~ /^install(mr|rp)/||$cpic->{script} =~ /^uninstall(mr|rp|\w+\d+P\d+$)/ ||
                                       ($cpic->{task} =~ /patchupgrade|upgrade|uninstall|rollback/)));
...
sub upgrade_menu {
    my ($rel)=@_;
    my ($cpic,$task,$msg,$menu,@dopts);

    $cpic=Obj::cpic();
    $cpic->cli_set_title();
    Msg::title();
    return '' if Cfg::opt('rootpath');
    $cpic->{menu}=[Msg::get('menu_fullupgrade'), Msg::get('menu_rollingupgrade'), Msg::get('menu_onlineupgrade')];
    @dopts=('','rolling_upgrade','online_upgrade');
...
sub ru_precheck_sys {
    ...
    undef ($sys->{errors});
    unless ($rel->check_installed_pkgs_version_sys($sys) && $rel->ru_prod_version_sys($sys,$firstnode)) {
        $msg->right_failed;
        Msg::n();
        for my $errmsg (@{$sys->{errors}}) {
            if(Obj::webui()) {
    ...
    $msg->right_done;
    if ($sys->{warnings} && @{$sys->{warnings}}) {
        Msg::n();
        for my $notemsg (@{$sys->{warnings}}) {
            if(Obj::webui()) {
                $msg->addWarning($notemsg);
            }
            Msg::print($notemsg);
        }
        Msg::n();
...
sub ru_prod_version_sys {
    ...
    if ($firstnode && (${$sys->{upgradeprod_abbr}}[0] ne $prodname)) {
        $cpic->set_value('prod',${$sys->{upgradeprod}}[0]);
        $msg=Msg::new("The detected product is ${$sys->{upgradeprod_abbr}}[0], but the specified product is $prodname, using ${$sys->{upgradeprod_abbr}}[0] as the product to be upgraded.");
        $sys->push_warning($msg);
    }
    ...
    if ($cv==2 || !EDRu::inarr($cprod,@{$rel->{ru_prod}})) {
        $cprod=~s/\d+//g;
        $msg=Msg::new("The system $sys->{sys} is installed with $cprod version $ivers. This release only supports rolling upgrade from version $ruvers or later for the following products: $prodlist");
        $sys->push_error($msg);
        return '';
    }
...
sub ru_confirm_cluster {
    ...
    if ($ayn eq 'Y') {
        ...
        if (!Obj::webui()) {
            ...
            do {
                $msg=Msg::new("Enter any system name of another cluster that you would like to perform rolling upgrade:");
                $sysname=$msg->ask;
...
sub ru_check_newnode {
    ...
    do {
        ...
        my $msg = Msg::new("Enter the system name of the cluster on which you would like to perform rolling upgrade");
...
sub online_upgrade_get_systems {
    my ($rel) = @_;
    my ($edr,$cfg,$cpic,$helpmsg,$msg,$web,$sysname,@systems,$sys,$check_ret,$prodname);
    $edr = Obj::edr();
    $cfg = Obj::cfg();
    $cpic = Obj::cpic();
    $web = Obj::web();
    ...
    #online support vcs/appha only.
    if ($cpic->{prod}) {
        $prodname = $cpic->prod->{abbr};
        if ($prodname !~ /VCS/i && $prodname !~ /ApplicationHA/i) {
            $msg=Msg::new("Online upgrade doesn't support $prodname, choose VCS or ApplicationHA to perform online upgrade.\n");
            Msg::n();
            $msg->error();
            Msg::n();
    ...
    do {
        #There has no cluster for ApplicationHA.
        if (defined $prodname && $prodname =~ /ApplicationHA/i) {
            $msg = Msg::new("Enter the system name on which you would like to perform online upgrade");
            $helpmsg=Msg::new("You are required to provide one system, both hostname and IP address is Ok.");
        } else {
            $msg = Msg::new("Enter the system name of the cluster on which you would like to perform online upgrade");
            $helpmsg=Msg::new("You are required to provide one system of the cluster. Installer will detect the other systems of the cluster automatically.");
        }
        if (Obj::webui()) {
            $cfg->{systems} = $web->web_script_form('selectCluster', $msg);
            $sysname=${$cfg->{systems}}[0];
        ...
        $check_ret = $rel->online_upgrade_precheck_sys($sys,1);
        if (1 == $check_ret) {
            my $syslist=$rel->online_upgrade_confirm_system($sys);
            if($syslist) {
                return $syslist;
            }
        }
        #precheck error, retry
        if (Cfg::opt('responsefile')) {
            $cpic->edr_completion(); 
        }
        undef $cfg->{systems};
        delete $Obj::pool{"Sys::$sysname"};
        undef $sys;
        if (!defined $prodname) {
            undef ($cpic->{prod});
        }
...
# -1, commuicate check error
# 0, other check error
# 1, success
sub online_upgrade_precheck_sys {
    my ($rel,$sys,$firstnode) = @_;
    my ($edr,$cpic,$msg, $web);
    $edr=Obj::edr();
    $cpic=Obj::cpic();
...
# my $msglist = $edr->{msglist};
    $edr->set_progress_steps(3);
#   $edr->{msglist} = $msglist if ($web->{perform_online_upgrade});
    ...
    # 3. check all other stuffs
    $msg=Msg::new("Checking online upgrade prerequisites on $sys->{sys}");
    $msg->left;
    $msg->display_left($msg) if (Obj::webui());
    $cpic->installed_prod_sys($sys);
    $msg->display_right() if (Obj::webui());
    ...
    # clean up error messages
    undef ($sys->{errors});
    unless ($rel->online_upgrade_prod_version_sys($sys)) {
        $msg->right_failed;
        Msg::n();
        for my $errmsg (@{$sys->{errors}}) {
            if(Obj::webui()) {
...
sub online_upgrade_prod_version_sys {
    my ($rel,$sys)=@_;
    my ($cpic,$prod,$prodname,$msg);
    $cpic=Obj::cpic();
    $sys=Obj::sys($sys) unless (ref($sys) eq 'Sys');
    return 1 if ($rel->{lp});
    if(!$sys->{upgradeprod_abbr}) {
    ...
    if ($prodname ne ${$sys->{upgradeprod_abbr}}[0]) {
        ...
        if(!($prodname =~ /VCS/ && ${$sys->{upgradeprod_abbr}}[0] =~/SFHA/)) {
            $msg=Msg::new("${$sys->{upgradeprod_abbr}}[0] is installed on $sys->{sys}, online upgrading to $prodname is not supported.\n");
            $sys->push_error($msg);
            return '';
        }
    }
    #online support vcs/appha only.
    if ($prodname !~ /VCS/ && $prodname !~ /ApplicationHA/) {
        $msg=Msg::new("$prodname is installed on $sys->{sys}, online upgrade only supports VCS or ApplicationHA.\n");
        $sys->push_error($msg);
        return '';
    }
    return 1;
...
sub online_upgrade_confirm_system {
    my ($rel,$sys) = @_;
    # All systems must be running on the cluster
    # Only needs to be checked one time.
    my ($cpic,$conf,$vcs);
    my ($msg,$webmsg);
    my ($clustername,@systems,$syslist,$web);
    $cpic=Obj::cpic();
    $vcs=$cpic->prod("VCS62");
    $conf=$vcs->get_config_sys($sys);
    if (!$conf) {
        $msg = Msg::new("Installer couldn't get correct configuration from main.cf, you may check it or try other system.");
        Msg::n();
        $msg->error();
        Msg::n();
        if (Obj::webui()) {
    ...
    if (@systems > 0) {
        if ($sys->prod->{abbr} =~ /VCS/) {
            $msg=Msg::new("Online upgrade supports application zero downtime for monitored applications. Would you like to perform online upgrade on the whole cluster?");
        } else {
            $msg=Msg::new("Online upgrade supports application zero downtime for monitored applications. Would you like to perform online upgrade on the system?");
        }
        $ayn=$msg->ayny('','',$webmsg);
        Msg::n();
    } else {
        $msg = Msg::new("No system is defined in main.cf.");
        Msg::n();
        $msg->error();
        Msg::n();
        if (Obj::webui()) {
...
sub set_menu_msgs {
    ...
    $msg=Msg::new("Online Upgrade [VCS/ApplicationHA only]");
    $msg->msg("menu_onlineupgrade");
...
sub new_pkg_init_common {
    ...
    if ($pkg->{pkg} eq 'VRTSdcli') {
        push(@{$pkg->{stopprocs}}, 'vxdclid50', 'xprtld61');
...
_FINISH_LINES_
    ...
    if ($ayn eq 'Y') {
        ...
        close($fh);
        EDR::cmd_local("_cmd_chmod +x $basename");
        $msg=Msg::new("The encapsulation boot disk script for VxVM is generated at $basename");
        $msg->bold;
    }
    return;
}
...
sub yumgroupxml {
    my ($rel) = @_;
    my ($basename,$msg,$pkg,@pkgs,$prod,$prods,$savepath,$ver,$fd,$group_xml,$all_xml,$num,$pkgpath,$padv);
    ...
    for my $prodi ( sort(@{$prods}) ) {
        ...
        $group_xml ="\t<group>\n";
        $group_xml.="\t\t<id>".uc($prod->{prod}).$ver."</id>\n";
        $group_xml.="\t\t<name>".uc($prod->{prod}).$ver."</name>\n";
        $group_xml.="\t\t<default>true</default>\n";
        $group_xml.="\t\t<description>RPMs of ".$prod->{name}.' '.$prod->{vers}."</description>\n";
        $group_xml.="\t\t<uservisible>true</uservisible>\n";
        $group_xml.="\t\t<packagelist>\n";
        for my $pkg (sort @pkgs) {
            $group_xml.="\t\t\t<packagereq type=\"default\">".$pkg."</packagereq>\n";
        }
        $group_xml.="\t\t</packagelist>\n";
        $group_xml.="\t</group>\n";
    ...
    if ($num > 1) {
        ...
        $msg = Msg::new("The yum group XML for all products is generated at $basename");
        $msg->bold;
    }
...
sub ignite {
    ...
    $mprod = '';
    if ($cpic->{prod} =~/VM/mx) {
        $msg = Msg::new("The -ignite option is not supported with VxVM product\n");
        $msg->warning();
        $edr->exit_noexitfile();
    }
    while (1) {
...
sub nim {
    ...
    #e3604099:nim option doesn't support any to any, so the local plat should be checked.
    unless(Cfg::opt('nim') && $cpic->{padv} =~ /^AIX/m && Obj::localplat() =~ /AIX/m) {
...
sub init_plat {
    ...
    $rel->{platreqs}=[ 'AIX 6.1 TL8', 'AIX 6.1 TL9',
                       'AIX 7.1 TL2', 'AIX 7.1 TL3' ];
...
VRTSamf:4660,8467,0,9
VRTSaslapm:0,1099,0,1068
VRTScavf:817,71,0,2
VRTScps:52179,0,0,6
VRTSdbac:4052,3042,0,513
VRTSdbed:136716,0,0,0
VRTSfsadv:35300,0,0,3
VRTSfssdk:3207,0,0,0
VRTSgab:643,3441,0,77
VRTSglm:32,395,0,18
VRTSgms:0,161,0,0
VRTSllt:1204,693,0,233
VRTSob:94748,0,0,297
VRTSodm:522,586,0,6
VRTSperl:82043,0,0,0
VRTSsfcpi62:13551,0,0,0
VRTSsfmh:147843,0,0,0
VRTSspt:28583,0,0,0
VRTSvbs:51696,0,0,1
VRTSvcs:398790,21578,0,272
VRTSvcsag:24228,0,0,46
VRTSvcsea:1809,0,0,56
VRTSvcswiz:7237,0,0,0
VRTSvcsvmw:11260,0,0,1
VRTSveki:0,130,0,0
VRTSvlic:88,0,0,1440
VRTSvxfen:741,1723,0,1004
VRTSvxfs:32255,12696,0,3156
VRTSvxvm:8960,210500,0,29793
_PKGSPACE_
...
sub platvers_sys {
    my ($rel,$sys) = @_;
    my ($msg,$iosl,$cprodabbr,$ostl,$tl,$min_tl,$platvers);
    $sys=Obj::sys($sys) unless (ref($sys) eq 'Sys');
    return '' if ($sys->{stop_checks});
    return '' if (Cfg::opt('ignorechecks'));
    ...
    if ($min_tl) {
        if ($tl < $min_tl) {
            ...
            $newlevel = $sys->cmd("_cmd_oslevel -qs 2>/dev/null | _cmd_head -1");
            $newtl = $1 if ($newlevel =~ /\d+-(\d+)/);
            if ($newtl < $min_tl) {
                $platvers = join "\n\t", @{$rel->{platreqs}};
                $msg=Msg::new("Cannot perform any task for $cprodabbr on system $sys->{sys} since its oslevel is $sys->{platvers} TL $tl. The following oslevels are supported to perform tasks for $cprodabbr :\n\t$platvers");
                $sys->push_error($msg);
                return '';
            } else {
...
sub define_patches {
    my $rel=shift;
    $rel->define_base_patches("VRTSsfmh61", "all", "VRTSsfmh_6_1_0_100");
    return 1;
}

package Patch::VRTSsfmh_6_1_0_100::AIX;
@Patch::VRTSsfmh_6_1_0_100::AIX::ISA = qw(Patch);

sub postinstall_sys {
    my ($patch, $sys)=@_;
    my ($vxdclid);
    $vxdclid = $sys->proc('vxdclid61');
    if ($vxdclid->check_sys($sys)) {
        $sys->cmd("/opt/VRTSsfmh/etc/vxdcli.sh stop"); 
    }
    return;
}
...
sub get_platreqs {
    my ($rel,$prod) = @_;
    my ($platreqs,$non_uekreqs);
    #e3535822:remove uek kernel from the product kernel list except vcs
    $platreqs = $prod->{platreqs} || $rel->{platreqs};
    if ($prod->{prod} !~ /vcs/i) {
        for my $req (@{$platreqs}) {
            next if ($req =~ /uek/i);
            push @{$non_uekreqs}, $req;
        }
        return $non_uekreqs;
    } else {
        return $platreqs;
...
sub osvers_sys {
    ...
    #e3533793:Get osversion through kernel version,
    if (defined $rel->{platreqs}
        && $sys->{platvers} =~ /(\d+\.\d+\.\d+-\d+)/s) {
        $ker_vers = $1;
        #e3581031:OEL kernel should have different regular expression.
        $ker_vers = $1 if ($sys->{platvers} =~ /(\d+\.\d+\.\d+-\d+.*uek)/s);
        for my $plat (@{$rel->{platreqs}}) {
            if ($plat =~ /\w+?(\d+) \w+?(\d+) \(\Q$ker_vers\E/
                || $plat =~ /\w+?(\d+) GA \(\Q$ker_vers\E/) {
                $2||=0;
                $minorvers = $sys->{distro}.$1.'.'.$2;
...
sub platvers_sys {
    ...
    my ($service,@vmtoolsd_scripts,$check_vmware_tools_command,$start_vmware_tools_command,$not_running);
    $sys=Obj::sys($sys) unless (ref($sys) eq 'Sys');
    return '' if ($sys->{stop_checks});
    return '' if (Cfg::opt('ignorechecks'));
    ...
    #e3515708:there is no need to check vmware-tools for uninstall
    if ($prod->{prod} =~ /ApplicationHA|VCS/ && !Cfg::opt('uninstall')) {
        ...
        if ($localsys->exists($testVMware)) {
            ...
            if (EDR::cmdexit() == 0) {
                ...
                $vmware = 1;
                @vmtoolsd_scripts=('/usr/lib/systemd/system/vmtoolsd.service');
                for my $script (@vmtoolsd_scripts) {
                    if ($sys->exists($script)) {
                        $service=EDRu::basename($script);
                        $check_vmware_tools_command="/usr/bin/systemctl is-active $service 2>/dev/null";
                        $start_vmware_tools_command="systemctl enable $service 2>/dev/null; systemctl start $service 2>/dev/null";
                        $not_running="inactive";
                        last;
                    }
                }
                # If vmtoolsd not exists
                if (!$service) {
                    ...
                    for my $script (@vmware_tool_scripts) {
                        if($sys->exists($script)) {
                            $check_vmware_tools_command="$script status 2>/dev/null";
                            $start_vmware_tools_command="$script start 2>/dev/null";
                            $not_running="not";
                            last;
                        }
                    }
                }
                if ($check_vmware_tools_command && $start_vmware_tools_command && $not_running) {
                    $state = $sys->cmd("$check_vmware_tools_command");
                    # vmware-tools not running
                    if ($state =~ /$not_running/i) {
                        # try to start it
                        $sys->cmd("$start_vmware_tools_command");
                        sleep 5;
                        $state = $sys->cmd("$check_vmware_tools_command");
                        if ($state =~ /$not_running/i) {
...
sub init_padv {
    ...
    # If install on RHEL 6.7, fallback to check OS libraries of RHEL 6.6
    $rel->{minorvers_for_fallback}='RHEL6.6';

    $rel->{pkgsspace}=<<"_PKGSPACE_";
VRTSacclib:504,0,0,0
VRTSamf:18405,221,0,17
VRTSaslapm:19661,0,0,1020
VRTScavf:803,88,0,0
VRTScps:29710,0,0,6
VRTSdbac:13190,66,0,346
VRTSdbed:112141,1,0,5
VRTSfsadv:14797,1,0,9
VRTSfssdk:1202,3,0,0
VRTSgab:20083,374,0,61
VRTSglm:61,0,0,396
VRTSgms:5,0,0,84
VRTSllt:42760,59,0,350
VRTSob:72126,0,0,206
VRTSodm:206,456,0,1189
VRTSperl:75321,0,0,0
VRTSsfcpi62:13583,0,0,0
VRTSsfmh:114018,0,0,0
VRTSspt:34124,0,0,0
VRTSvbs:37454,0,0,1
VRTSvcs:137863,8274,0,276
VRTSvcsag:77394,0,0,71
VRTSvcsdr:3865,0,0,1
VRTSvcsea:1197,0,0,51
VRTSvcsvmw:13915,0,0,1
VRTSvcswiz:11195,0,0,0
VRTSvlic:75,0,0,1265
VRTSvxfen:19616,45,0,678
VRTSvxfs:2646,24409,0,27891
VRTSvxvm:118090,58365,0,38538
_PKGSPACE_
    return;
}
...
sub init_padv {
    ...
    $rel->{pkgsetprods}=EDRu::arrdel($rel->{pkgsetprods}, qw(SFSYBASECE62));
    $rel->{menuprods}=EDRu::arrdel($rel->{menuprods}, qw(SFSYBASECE62));
    $rel->{platreqs} = ['RHEL7 GA (3.10.0-123.el7)',
                       ];

    # If install on RHEL 7.1, fallback to check OS libraries of RHEL 7.0
    $rel->{minorvers_for_fallback}='RHEL7.0';
    $rel->{oslibs}{'RHEL7.0'}={
        "/usr/bin/netstat"  =>  "net-tools-2.0-0.17.20131004git.el7.x86_64",
    };

    $rel->{commands_need_be_checked}=[ qw(arch ethtool getconf hostid modprobe modunload nm) ];
...
package Rel::UXRT62::OL6x8664;
@Rel::UXRT62::OL6x8664::ISA = qw(Rel::UXRT62::RHEL6x8664);

package Rel::UXRT62::OL7x8664;
@Rel::UXRT62::OL7x8664::ISA = qw(Rel::UXRT62::RHEL7x8664);
...
sub init_padv {
    ...
    $rel->{pkgsspace}=<<"_PKGSPACE_";
VRTSacclib:501,0,0,0
VRTSamf:10207,230,0,18
VRTSaslapm:80394,0,0,1839
VRTScavf:805,89,0,0
VRTScps:30763,0,0,6
VRTSdbac:1575,74,0,348
VRTSdbed:112778,1,0,5
VRTSfsadv:14944,1,0,9
VRTSfssdk:1331,3,0,0
VRTSgab:9836,381,0,80
VRTSglm:72,0,0,730
VRTSgms:8,0,0,143
VRTSllt:21995,62,0,367
VRTSob:72127,0,0,206
VRTSodm:218,450,0,598
VRTSperl:75692,0,0,0
VRTSsfcpi62:13584,0,0,0
VRTSsfmh:114019,0,0,0
VRTSspt:34125,0,0,0
VRTSvbs:37455,0,0,1
VRTSvcs:170591,8134,0,277
VRTSvcsag:86539,0,0,71
VRTSvcsdr:215,0,0,1
VRTSvcsea:1220,0,0,57
VRTSvcsvmw:13887,0,0,1
VRTSvcswiz:11257,0,0,0
VRTSvlic:86,0,0,1231
VRTSvxfen:7552,48,0,725
VRTSvxfs:4773,24279,0,15348
VRTSvxvm:36997,59505,0,58319
_PKGSPACE_
    return;
}
...
sub init_padv {
    ...
    $rel->{pkgsspace}=<<"_PKGSPACE_";
VRTSacclib:501,0,0,0
VRTSamf:3814,1490,3,8
VRTSaslapm:0,525,0,1512
VRTScavf:871,71,0,2
VRTScps:56868,0,0,6
VRTSdbac:4315,406,5,2494
VRTSdbed:126035,0,2,5
VRTSfsadv:16081,0,3,2
VRTSfssdk:2302,0,0,0
VRTSgab:133,504,3,1121
VRTSglm:43,0,0,514
VRTSgms:1,0,0,105
VRTSllt:469,84,3,1214
VRTSob:93904,0,0,205
VRTSodm:1433,1,3,4
VRTSperl:68921,0,0,205
VRTSsfcpi62:18511,0,0,0
VRTSsfmh:144240,0,0,0
VRTSspt:27573,0,0,0
VRTSvbs:68653,0,0,1
VRTSvcs:258760,19388,0,276
VRTSvcsag:8048,1,0,60
VRTSvcsea:2040,0,0,80
VRTSvcsvmw:13254,0,0,1
VRTSvcswiz:7247,0,0,0
VRTSvlic:2601,0,0,2674
VRTSvxfen:357,165,3,2194
VRTSvxfs:45854,5740,4,6638
VRTSvxvm:7499,195784,20,33105
_PKGSPACE_
    return;
}
...
sub define_patches {
    my $rel=shift;
    $rel->define_base_patches("VRTSsfmh61", "all", "151218_01");
    return 1;
}

package Patch::151218_01::SolSparc;
@Patch::151218_01::SolSparc::ISA = qw(Patch);
...
sub init_padv {
    ...
    $rel->{pkgsspace}=<<"_PKGSPACE_";
VRTSamf:3778,1108,2,10
VRTSaslapm:0,435,0,1302
VRTScavf:860,70,0,2
VRTScps:57273,0,0,2
VRTSdbac:1752,220,5,1750
VRTSdbed:127440,0,2,3
VRTSfsadv:15679,0,0,0
VRTSfssdk:2457,0,0,0
VRTSgab:127,543,3,965
VRTSglm:37,23,1,358
VRTSgms:0,5,1,60
VRTSllt:437,504,7,708
VRTSodm:1589,1,3,3
VRTSperl:93263,0,0,0
VRTSsfcpi62:18512,0,0,0
VRTSsfmh:144193,0,0,0
VRTSspt:27574,0,0,0
VRTSvbs:68534,0,0,3
VRTSvcs:253902,19156,5,228
VRTSvcsag:7950,0,0,58
VRTSvcsea:2024,0,0,71
VRTSvcswiz:7246,0,0,0
VRTSvlic:2106,2015,1,150
VRTSvxfen:355,1015,7,1175
VRTSvxfs:41677,4629,5,6526
VRTSvxvm:6098,191427,0,12913
_PKGSPACE_
    return;
}
