...
sub licensing {
    ...
    my ($cpic,$msg,$web,$edr,$licensed,$rt,$license_status,$prod);

    $cpic=Obj::cpic();
    # Make change for E2852406
    # $cpic->completion() if (Cfg::opt('nolic'));
    if (Cfg::opt('nolic')) {
       if (Cfg::opt('configure')) {
          $msg = Msg::new("Configuration, startup, or installation of license-based features are skipped when using the -nolic option.\n");
          $msg->bold();
       }
       $cpic->completion();
    }
    ...
    while(1) {
        ...
        return '' if ($#{$license_status->{vlic_uninstalled}} >= 0);
        $licensed = $license_status->{all_licensed} if (!Cfg::opt('makeresponsefile'));

        Cfg::unset_opt('vxkeyless') if ($back_from_key_menu);
        if (Cfg::opt(qw(upgrade patchupgrade hotfixupgrade license))) {
            if (Cfg::opt(qw(makeresponsefile))) {
                $rel->ask_for_responsefile_update_keyless_keys();
            } else {
                $rel->restore_keyless_keys();
            }
        }
        if (Cfg::opt(qw(install license upgrade)) || (!$licensed && Cfg::opt(qw(start patchupgrade hotfixupgrade configure)))) {
            if ($licensed) {
                if (($prod->{prod} eq 'SF') && (Cfg::opt('prodmode') eq 'SF Basic')) {
                    $msg=Msg::new("SF Basic is licensed on the systems\n");
                    $msg->print();
                    last;
                }
                if ((!$back_from_key_menu) && Cfg::opt('install')) {
                    return '' if ($rel->ask_additional_license($prod) eq 'N');
                }
            } elsif (Cfg::opt(qw(upgrade patchupgrade))) {
                last unless ($rel->ready_to_enter_license_keys());
            }
            $rel->license_option() unless (($licensed && Cfg::opt(qw(upgrade patchupgrade))) ||
...
sub licensing_status {
    my ($rel,$donotquery) = @_;
    my ($cpic,%license_status,$edr,$unlicensed_sys,$licensed,$key,$licensed_sys,$status,$pkg,$systems,$prod);
...
sub license_products {
    ...
    $prod=$cpic->prod;
#    if (Obj::webui()) {
#        if ($licensed && Cfg::opt('install')){
#            $prodname=$prod->{abbr};
#            $msg=Msg::new("$prodname is licensed on all systems. Do you wish to enter additional licenses?");
#            $msg->{msg}.=Msg::new("Do you wish to enter additional licenses?")->{msg};
#            $ayn=$msg->aynn(undef,0);
#            return '' if ($ayn eq 'N');
#        }
#        $web->{licensed} = $licensed;
#        if ($web->{updatekeys_license} && ($ayn ne 'Y')){
##            delete $web->{updatekeys_license};
#            return '';
#        } else {
...
#    }
    ...
    # unlicensed product
    return '' if (!$prod->licensed && Cfg::opt('upgrade'));
    return '' if (Cfg::opt('rootpath')||$prod->{nolic});
    ...
    unless (Cfg::opt('makeresponsefile')) {
        ...
        $msg->bold();
        if (Cfg::opt(qw(precheck upgrade patchupgrade hotfixupgrade uninstall configure stop start))) {
            for my $sys (@{$cpic->{systems}}) {
                # the following two lines are for the fix of 3120385
                # 1. move the $prod up above the read_licenses_sys
                # 2. unset $sys->{donotquerykeys} for AppHA
                $prod=$sys->prod($cpic->{prod});
                delete $sys->{donotquerykeys} if ( Cfg::opt(qw(upgrade patchupgrade hotfixupgrade)) &&
                                                  ($prod->{prod} eq "ApplicationHA") );
                # end of the changes for 3120385
                delete $sys->{donotquerykeys} if (Cfg::opt(qw(upgrade)));
        ...
        if (Cfg::opt(qw(responsefile precheck))) {
            for my $sys (@{$cpic->{systems}}) {
                ...
                $rt =~s/\s+//g;
                $sys->{iprod}{$prod->{abbr}}{license}=$rt;
                if (!$rt) {
                ...
                } elsif ($rt =~ /(Unused|Un-used)/i) {
                    $msg=Msg::new("Unrecognizable $prodname license version '$rt' on $sys->{sys}. Double check the keys installed under /etc/vx/licenses/lic. It is strongly recommended to upgrade to a $rel->{license_ver} key.\n");
                    $msg->warning;
                    $web->web_script_form('alert', $msg->{msg}) if(Obj::webui());
                }
           }
    ...
    $rel->add_additional_license_key($sys) if ((!Cfg::opt('upgrade')) || $lic_ver_update);
    for $sys (@{$cpic->{systems}}) {
        $prod=$sys->prod($cpic->{prod});
        $rt = $prod->licensed_sys($sys);
        $rt =~s/\s+//g;
        $msg=Msg::new("$prod->{abbr} license version is $rt on $sys->{sys}.");
        if (($rt =~ /^\d+/) && (EDRu::compvers($rt,$rel->{license_ver}) < 2) &&
                               (EDRu::compvers($rt,$sys->{iprod}{$prod->{abbr}}{license}) <= 1)) {
            $msg=Msg::new("$prod->{abbr} license version is updated to $rt on $sys->{sys}.") if ($lic_ver_update);
            $msg->printn;
        } elsif ($rt =~ /^\d+/) {
            $msg=Msg::new("$prod->{abbr} license version $rt is lower than $rel->{license_ver} on $sys->{sys}. It's recommended to update to $rel->{license_ver} in order to fully utilize the new features in this release.") if (($rt =~ /^\d+/) && (EDRu::compvers($rt,$rel->{license_ver}) == 2));
            $msg->printn;
        }
    }
    $prod->update_prod();
...
sub add_additional_license_key {
    my ($rel,$sys) = @_;
...
#sub web_license_key {
#    my ($rel) = @_;
#    my ($cpic,$web,$sys1,$msg,$key,$prod,$rt);
#    $cpic=Obj::cpic();
#    $web=Obj::web();
#    $sys1=$cpic->{systems}[0];
...
#        $web->purge('submit');
#
#        if (EDRu::inarr($key, @{$web->{registered_keys}})) {
#            $web->{lic_result}='duplicate';
#            $web->write_status();
#            next;
#        }
#
...
#            } elsif (($rt =~ /^\d+/) && (EDRu::compvers($rt,$rel->{license_ver}) == 2)) {
#                $web->{invalid_license} = 1;
#                $web->{lic_result} = Msg::new("$prod->{abbr} license version $rt is not updated to $rel->{license_ver} on $lsys->{sys}. It's recommended to upgrade to a $rel->{license_ver} key.\n")->{msg};
#                $web->write_status();
#                delete $web->{licensed_prodname};
#            } elsif ($rt =~ /(Unused|Un-used)/i) {
#                $web->{invalid_license} = 1;
#                $web->{lic_result} = Msg::new("Unrecognizable $prod->{abbr} license version '$rt' on $lsys->{sys}. Please double check the keys installed under /etc/vx/licenses/lic. It's strongly recommended to upgrade to a $rel->{license_ver} key.\n")->{msg};
#                $web->write_status();
#                delete $web->{licensed_prodname};
#            }
...
sub ask_license_key {
    my ($rel,$sys) = @_;
    my ($cpic,$msg,$cfg,$prod,$key,$license_status,$help,$prodname,$registered_keys,$web);

    $cpic=Obj::cpic();
    $prod=$cpic->prod;
    $cfg=Obj::cfg();
    $web = Obj::web() if (Obj::webui());
    ...
    if (!$prod->{basic}) {
         ...
         while (1) {
             ...
             if (!Cfg::opt('makeresponsefile')) {
                 ...
                 $msg->printn;
                 $web->web_script_form('alert', $msg->{msg}) if(Obj::webui());
             } elsif (Cfg::opt(qw(upgrade patchupgrade hotfixupgrade))) {
                 # skip licensing part for makeresponsefile and upgrade
                 return;
             }
             $msg=Msg::new("Enter a $prodname license key:");
             if (Obj::webui()){
                 $key=$web->web_script_form('license_key');
             } else {
...
sub license_products_vxkeyless {
    my ($rel,$prod_level) = @_;
    my ($cpic,$cfg,$msg,$edr,$sys,$mtask,$prodname,$prod,$web);
    ...
    $msg=Msg::new("Checking system licensing");
    $msg->bold();
    $rel->restore_keyless_keys() if (Cfg::opt(qw(upgrade patchupgrade hotfixupgrade license)));
    $prod_level=~s/\s+//mg;
    $prod_level||=$rel->get_installed_prod_vxkeyless_highest_key();
    if (Cfg::opt(qw(install configure patchupgrade hotfixupgrade start license upgrade_obsolete_keyless_keys))) {
        $prod=$cpic->prod;
        if (Obj::webui()){
            $web = Obj::web();
            if ($prod->{menu_modes} ||($prod->{menu_options} && $web->get_prod_option())){
                $mtask=$web->web_script_form('license_keyless');
            }
        } else {
...
#        $web->write_status();
#
#        delete $web->{register_result};
#        delete $web->{register_detail};
#    }
...
sub prod_licensed_sys {
    ...
    return 1 if (Cfg::opt(qw(makeresponsefile)) && !Cfg::opt('upgrade'));
    $cpic=Obj::cpic();
    $ha_status = 0;
    $skip_superset ||= 0;
    $sys=Obj::sys($sys) unless (ref($sys) eq 'Sys');
    if ($prodname) {
        # the following line is appha fix for 3541080
        # due to the inconsistency between $sys->{iprod} and $rel->{prods} 
        # $sys->{iprod} and JSON files use 'appha' 
        # $rel->{prods} uses APPLICATIONHA 
        $prodname=~s/^APPHA/APPLICATIONHA/i;
        # The following lines are for sfcfs fix in 3564266
        # older releases may have sfcfs which is already gone in 6.2.
        # therefore need to take it as SFCFSHA for license version check
        # the same to SVS
        if ($prodname=~/^(SFCFS|SVS)\d+/i) {
            $prodname=~s/^(SFCFS|SVS)/SFCFSHA/i;
        }
        return '' unless (EDRu::inarr($prodname,@{$rel->{prods}}));
        $prod=$rel->prod($prodname);
        #for fix of 3570023
        #even if a product is obsoleted, we still should get its license version
        #return '' if ($prod->{obsoleted});
    }
    $prod = ($prodname) ? $sys->prod($prodname) : $cpic->prod;
    $prodname||=$prod->{abbr};
    $rel->read_licenses_sys($sys) unless ($sys->{keys});
	...
	$vxkeyless="SF" if ($prod->{abbr} eq 'SF');
        $lk='';
        for my $vxk (split /,/m,$sys->{vxkeyless}) {
            ...
            if ($vxk =~ /^$vxkeyless(STD|STD_|ENT|ENT_|_|$)/mx) {
                $lk='vxkeyless';
                last;
            }
        }
        if ($lk) {
            $vxk=$rel->get_vxkeyless_key_version($sys,$prod);
            #return "$vxk $lk";
            return $lk;
        }
    }
        ...
        for my $lname (@{$prod->{lic_names}}) {
            ...
            if ($pname=~/$temp_name/m) {
                if($prodname =~ /APPLICATIONHA/mi){
                    if($sys->{keys}{$lk}{ApplicationHA} =~ /Enabled/mi) {
                        return ($sys->{keys}{$lk}{Version}) ? $sys->{keys}{$lk}{Version} : 1;
                    }
                    next;
                }
                ...
                $vcs_lic = $rel->prod_licensed_sys($sys,'VCS62');
                $sf_lic = $rel->prod_licensed_sys($sys,'SF62',1);
    ...
    if ($prod->{abbr} eq 'SF') {
        # need to check whether the SFBasic license is the only key exists on the system for ETrack 3131566
        for my $lk (sort { $a cmp $b } keys(%{$sys->{keys}})) {
            $pname=$sys->{keys}{$lk}{'Product Name'};
            $pname=~s/^VERITAS //mi;
            next unless ($pname =~ /^Storage Foundation/);
            if ($pname eq "Storage Foundation Basic") {
                $sf_lic=1;
            } else {
                $sf_lic=0;
                last;
...
sub prod_permanent_licensed_sys {
    ...
    $cpic=Obj::cpic();
    $sys=Obj::sys($sys) unless (ref($sys) eq 'Sys');
    $prod = ($prodi) ? $sys->prod($prodi) : $cpic->prod;
    $prodi ||= $prod->{abbr};
    $rel->read_licenses_sys($sys) unless ($sys->{keys});
    for my $lk (sort { $a cmp $b } keys(%{$sys->{keys}})) {
        # skip APPHA key or CPS key for vcs product
        next if (($prodi =~ /VCS/) && (($sys->{keys}{$lk}{'ApplicationHA'} eq 'Enabled') ||
            ($sys->{keys}{$lk}{'Coordination Point Server'} eq 'Enabled')));
...
sub register_key_sys {
    ...
    $rel->create_vxlic_license_key_dir($sys);
...
sub vxkeyless_register_sys {
    ...
    my ($cpic,$encap_keyfile,$pdfr,$vxkeyless,$prodname,$pkg,$prod,$msg,$web);
    return '' if (Cfg::opt('makeresponsefile'));
    $sys=Obj::sys($sys) unless (ref($sys) eq 'Sys');
    ...
    if (Cfg::opt(qw(upgrade patchupgrade hotfixupgrade)) && ($vxkeyless ne $sys->{vxkeyless})) {
    ...
    if (Cfg::opt('upgrade') && (!Cfg::opt('upgrade_obsolete_keyless_keys'))) {
    ...
    if ($pkg->version_sys($sys)) {
        ...
        if ($sys->{vxkeyless_valid} !~ /(^$prod_level$|^$prod_level,|,$prod_level,|,$prod_level$)/mx) {
            ...
            $msg->print;
            $web->web_script_form('alert', $msg->{msg}) if(Obj::webui());
#            $web->{register_result} = "fail";
#            $web->{register_detail} .= $msg->{msg} . "\n";
            return;
        }

        # do not register if the key is already-registered
        if ($sys->{vxkeyless} =~ /(^$prod_level$|^$prod_level,|,$prod_level,|,$prod_level$)/mx) {
            $msg=Msg::new("$prod_level license keys already exists on $sys->{sys}, skipping registration");
            $msg->print;
            $web->web_script_form('alert', $msg->{msg}) if(Obj::webui());
#            $web->{register_detail} .= $msg->{msg} . "\n";
            return;
...
# return the prod vxkeyless level version
# the version should be $rel->{vers} if the key doesn't have _\d+\w+ suffix
# otherwise the version is \d+\w+
sub get_vxkeyless_key_version {
    my ($rel,$sys,$prod) = @_;
    my ($version,$key_stem);

    $key_stem=$prod->{prod};
    $key_stem="SF" if ($key_stem =~ /^(FS|VM)$/m) ;

    for my $key (split /,/, $sys->{vxkeyless_new}) {
        next unless ($rel->vxkeyless_keys_inclusion($key,$key_stem));
        $version=$rel->{vers};
        last unless ($key =~ /_\d+/);
        # assuming all the old release keys are like VCS_5.1SP1
        $version=(split /_/, $key)[-1];
    }
...
# determine whether $key1 enables $key2
sub vxkeyless_keys_inclusion {
    my ($rel,$key1,$key2)=@_;

    if ($key2 eq 'DMP') {
        # All SF-related keys enable DMP
        return 1 if ($key1 =~ /^(SF|DMP|SVS)/);
    } elsif ($key2 eq 'VCS') {
        return 1 if ($key1 =~ /^(SFHA|VCS|SFCFS|SFSYB|SFRAC|SVS)/);
    } elsif ($key2 eq 'SF') {
        return 1 if ($key1 =~ /^(SF|SVS)/);
    } elsif ($key2 eq 'SFHA') {
        return 1 if ($key1 =~ /^(SFHA|SFCFS|SFSYB|SFRAC|SVS)/);
    } elsif ($key2 eq 'SFCFS') {
        return 1 if ($key1 =~ /^(SFCFS|SFSYB|SFRAC|SVS)/);
    } elsif ($key2 eq 'SFCFSHA') {
        return 1 if ($key1 =~ /^(SFCFSHA|SFSYB|SFRAC|SVS)/);
    } elsif ($key2 eq 'SVS') {
        # SVS is merged into SFCFSHA in 6.1
        return 1 if ($key1 =~ /^(SFCFSHA|SVS)/);
    } elsif ($key2 =~ /^SFSYB/) {
        return 1 if ($key1 =~ /^SFSYB/);
    } elsif ($key2 eq 'SFRAC') {
        return 1 if ($key1 =~ /^SFRAC/);
    } elsif ($key2 eq 'APPLICATIONHA') {
        # Right now 6.1 VCS keyless key doesn't enable ApplicationHA
        # return 1 if ($key1 =~ /^(SFHA|VCS|SFCFS|SFSYB|SFRAC|SVS)/);
        return 1 if ($key1 =~ /^APPLICATIONHA/);
    }
    return 0;
}
...
sub get_vxkeyless_prod_level {
    ...
    if ($prod->{prod} eq 'SF') {
        ...
        $new_prod='SFSTD';
    } elsif($prod->{prod} =~ /^ApplicationHA$/m) {
        $new_prod='APPLICATIONHA';
    } else {
        $new_prod=$prod->{prod};
    }
    ...
    if (!$prod_only) {
        ...
        # _VFR doesn't apply to non-Linux
        if ($new_prod !~ /^VCS/) {
            $new_prod.='_VR' if (Cfg::opt('vr') || Cfg::opt('vvr'));
            # _VR includes _VFR, and they don't show up together
            $new_prod.='_VFR' if (Cfg::opt('vfr') && (!Cfg::opt('vr')) && ($new_prod !~ /^SVS/) && (Padv::plat($sys->{padv}) eq "Linux"));
        }
        $new_prod.='_GCO' if (Cfg::opt('gco') && ($new_prod =~ /^(VCS|SFHAENT|SFCFSHAENT|SFRACENT|SVS|SFSYBASECE)/mx));
        $new_prod='SFBASIC' if ($new_prod =~ /^SFBASIC/i);
        $new_prod.='_EVAL' if (Cfg::opt('eval'));
        # DMP has no '_<blah>' at all. Etrack 3265969
        $new_prod='DMP' if ($new_prod =~ /^DMP/);
    }
...
sub vxlicrep_sys {
    my ($rel,$sys) = @_;
    my ($cpic,$msg,$pdfr,$vxlicrep,$pkg,$result);
    ...
    if ($rootpath && ($rootpath ne '/')) {
        ...
        if (($pkg->version_sys($sys,1)) && ($sys->is_dir($vxlic_path))){
            my $lics = $sys->cmd("cd $vxlic_path; _cmd_ls *.vxlic 2>/dev/null");
            $lics=~s/\.vxlic\s*/,/mgs;
            $lics=~s/,$//m;
            if($lics) {
                for my $lic (split (/,/m,$lics)) {
                    $result=$sys->cmd("chroot $rootpath _cmd_vxlicrep -k $lic 2>/dev/null");
                    # Due to incident 3598716
                    # On Solaris 11 GA upgrade Solaris 11 Ux BE, the above command will fail
                    # We assume Solaris 11 GA 99% has product installed, hence use local vxlicrep to try again
                    if (EDR::cmdexit()) {
                        $result=$sys->cmd("_cmd_vxlicrep -k $lic 2>/dev/null");
                    }
                    $vxlicrep.=$result;
                }
    ...
    if ($pkg->version_sys($sys)) {
        ...
        $sys->set_value('vxlicrep', $vxlicrep);
    } elsif ($cpic->{task} && (Cfg::opt(qw(upgrade patchupgrade hotfixupgrade license)))) {
        $pdfr=Msg::get('pdfr');
        $msg=Msg::new("$pkg->{pkg} $pdfr not installed on $sys->{sys}");
...
# Etrack 3046613
# due to SVS merging into SFCFSHA in uxRT-6.1.0
# we have to migrate the stale SVS keys to valid SFCFSHA ones
#  SVS_GCO          ==> SFCFSHAENT_VFR_GCO
#  SVS_GCO_EVAL     ==> SFCFSHAENT_VFR_GCO_EVAL
#  SVS_VR_GCO       ==> SFCFSHAENT_VR_GCO
#  SVS_VR_GCO_EVAL  ==> SFCFSHAENT_VR_GCO_EVAL
#
...
sub restore_keyless_keys {
    my ($rel) = @_;
    my ($cpic,$oldkey,$newkey,@keys,$old_ver,$edr,$sys,$msg,$ayn,$key,$key_stem,$obsoleted,$prod);
    ...
    for my $sys (@{$cpic->{systems}}) {
        ...
        $rel->get_all_valid_vxkeyless_keys($sys) unless ($sys->{vxkeyless_valid});
        if ($oldkey) {
            # assuming vxkeyless command is still valid
            $newkey=$sys->cmd('_cmd_vxkeyless display 2>/dev/null');
            $newkey=~s/no keys installed.//mgi;
            $old_ver=1 if ($newkey=~/(SVS_|_\d+)/);
            $key="$newkey,$oldkey";
            @keys=(split /,/, $key);
            @keys=sort @{EDRu::arruniq(@keys)};
            for ($key=0;$key<=$#keys;$key++) {
                $key_stem=(split /\_\d+/, $keys[$key])[0];
                $key_stem=~s/_VVR/_VR/mgi;
                if ($sys->{vxkeyless_valid} =~ /(^$key_stem$|^$key_stem,|,$key_stem,|,$key_stem$)/mx) {
                    $keys[$key]=$key_stem;
                } elsif ($newkey !~ /(^$keys[$key]$|^$keys[$key],|,$keys[$key],|,$keys[$key]$)/mx) {
                    delete $keys[$key];
                }
            }
            $newkey=join ",", @keys;
            $newkey=~s/SVS_GCO/SFCFSHAENT_VFR_GCO/mgi;
            $newkey=~s/SVS_VR_GCO/SFCFSHAENT_VR_GCO/mgi;
            $newkey=~s/SFCFS/SFCFSHA/mgi if ($newkey !~ /(SFCFSHA|SFCFSRAC)/);
        }
        $sys->{vxkeyless_new}=$newkey;
        $obsoleted=$rel->get_vxkeyless_key_version($sys,$prod);
        $obsoleted=($obsoleted ne $rel->{vers}) ? 1 : 0;
    }
    ...
    if ($old_ver) {
        if (Cfg::opt('updatekeys')) {
            ...
            $msg=Msg::new("Found vxkeyless key(s) for previous release(s). It is recommended to update them to fully utilize the new features in the current release. Do you wish to update them to the current version?");
            $ayn=$msg->ayny(undef,0);
            ...
            } elsif (($ayn eq 'N') && !Cfg::opt('responsefile')) {
                Msg::n();
                $msg=Msg::new("You chose not to update the vxkeyless key(s) from previous release(s). It is highly recommended to update them to fully utilize the new features in the current release. You can do this manaully after the current session completes");
                $msg->print();
                Msg::prtc();
                $sys =$cpic->{systems}[0];
                $rel->add_additional_license_key($sys);
            }
        } elsif (!Cfg::opt('upgrade_obsolete_keyless_keys')) {
            $msg=Msg::new("Found obsoleted vxkeyless key(s) for previous release(s), which cannot be updated to the current release. Do you wish to enable $prod->{prod} keyless key?");
            $ayn=$msg->ayny(undef,0);
            if ($ayn eq 'Y') {
                Cfg::set_opt('upgrade_obsolete_keyless_keys');
                $rel->license_products_vxkeyless();
                Cfg::unset_opt('upgrade_obsolete_keyless_keys');
            } elsif (($ayn eq 'N') && !Cfg::opt('responsefile')) {
                Msg::n();
                $msg=Msg::new("You chose not to enable $prod->{prod} vxkeyless key. It is highly recommended to update the existing obsoleted ones to fully utilize the new features in the current release. You can do this manaully after the current session completes");
                $msg->print();
                Msg::prtc();
            }
        }
        if (Cfg::opt('updatekeys')) {
            for my $sys (@{$cpic->{systems}}) {
                ...
                if ($oldkey && $newkey && ($oldkey ne $newkey)) {
                    ...
                    delete $sys->{donotquerykeys};
                    $rel->read_licenses_sys($sys);
                    $msg=Msg::new("Keyless keys are updated to version $rel->{vers} on $sys->{sys} ($newkey)");
                    $msg->print();
                    my $web = Obj::web();
                    $web->{updatekeys_license} = "1" if(Obj::webui());
                }
            }
...
sub ready_to_enter_license_keys {
    my $rel=shift;
    my ($ayn,$msg);

    $msg=Msg::new("Sorry! No proper license keys found on the target system(s) for the product. Do you want to enter license keys? Without a valid license key, the product configure may not complete properly.");
    # fix of 3542516
    # for makeresponsefile option, we don't know the license status 
    # on the target, therefore we shouldn't say it's unlicensed
    $msg=Msg::new("Do you want to enter license keys? Without a valid license key, the product configure may not complete properly.") if (Cfg::opt('makeresponsefile'));
    $ayn=$msg->ayny(undef,1);
    return ($ayn eq 'Y') ? 1 : 0;
}

sub ask_for_responsefile_update_keyless_keys {
    my $rel=shift;
    my ($ayn,$msg);

    $msg=Msg::new("In case vxkeyless keys are found on the target system(s), do you wish to automatically update them to the current version?");
    $ayn=$msg->ayny(undef,1);
    ($ayn eq 'Y') ? Cfg::set_opt('updatekeys') : Cfg::unset_opt('updatekeys');

    return '';
}

sub create_vxlic_license_key_dir {
    my ($rel,$sys) = @_;
    my ($key_dir,$rootpath);

    $rootpath = Cfg::opt('rootpath')||'';
    $key_dir="$rootpath/etc/vx/licenses/lic";

    unless ($sys->is_dir($key_dir)) {
        if ($sys->mkdir($key_dir)) {
            $sys->cmd("_cmd_chmod 755 $key_dir");
            $sys->cmd("_cmd_chown root:root $key_dir");
        }
    }
