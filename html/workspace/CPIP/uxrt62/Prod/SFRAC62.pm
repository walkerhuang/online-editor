...
package Prod::SFRAC62::Common;
@Prod::SFRAC62::Common::ISA = qw(Prod);
...
sub init_common {
    ...
    $prod->{name}=Msg::new("Symantec Storage Foundation for Oracle RAC")->{msg};
    $prod->{mainpkg}='VRTSdbac62';

    $prod->{extra_mainpkgs}=[qw(VRTSvxvm62 VRTSvxfs62 VRTSvcs62 VRTSllt62 VRTSgab62 VRTSvxfen62 VRTSvcsag62)];

    $prod->{lic_names}=['Veritas Storage Foundation for Oracle RAC'];
    $prod->{responsefileupgradeok}=1;
    $prod->{subprods}=[qw(VCS62 SF62)];
    ...
    if (Obj::webui()) {
        ...
        $prod->{db_home} = $localsys->get_env_value('ORACLE_HOME');
        $prod->{oracle_base} = $localsys->get_env_value('ORACLE_BASE');
        $prod->{crs_home} = $localsys->get_env_value('ORA_CRS_HOME');
    } else {
        $prod->{oracle_base} = $ENV{ORACLE_BASE};
        $prod->{db_home} = $ENV{ORACLE_HOME};
        $prod->{crs_home} = $ENV{ORA_CRS_HOME};
    }
    #Msg::log("db_home = $prod->{db_home}; oracle_base = $prod->{oracle_base}; crs_home = $prod->{crs_home}");

    $prod->{display} = $ENV{DISPLAY};
#    $prod->{oui_args} = '-ignoreInternalDriverError';
    $prod->{oui_args} = '';
    $prod->{oracle_install_script} = 'runInstaller';
    $prod->{oracle_bits} = '64'; # Oracle supports only 64-bit on both Sparc and X86 archs
    ...
    $prod->{cvmvoldg_ocr_res_name} = 'ocr_voldg';
    $prod->{cvmvoldg_vote_res_name} = 'vote_voldg';
    $prod->{cvmvoldg_res_name} = 'ocrvote_voldg';
    $prod->{ocrvote_mntpt} = '/ocrvote';

    $prod->{is_fss_enabled} = 0;
    $prod->{is_fss_enabled} = 1 if ($vm->is_fss_enabled());
    ...
    $prod->{dg_opts}=[];
    if ($prod->{is_fss_enabled}) {
        $msg=Msg::new("Create a disk group (Shared Storage)");
        push @{$prod->{dg_opts}}, $msg->{msg};
        $msg=Msg::new("Use an existing disk group (Shared Storage)");
        push @{$prod->{dg_opts}}, $msg->{msg};
        $msg=Msg::new("Create a disk group (FSS)");
        push @{$prod->{dg_opts}}, $msg->{msg};
        $msg=Msg::new("Use an existing disk group (FSS)");
        push @{$prod->{dg_opts}}, $msg->{msg};
    } else {
    ...
    $prod->{crs_offline_msg} ="Could not contact Oracle High Availability Services";
    ...
    $prod->{menu_sfrac} = ['config_sfrac_subcomponents', 'preinstall_oracle', 'install_oracle', 'postinstall_oracle', 'exit_cleanly'];
    ...
    $prod->{menu_create_private_network} = ['config_haip', 'config_privnic', 'config_multiprivnic', 'exit_cleanly'];
...
#    $msg = Msg::new("$prod->{abbr} Configuration Checks");
#    $msg->msg('sfrac_post_config_check');

    $msg = Msg::new("Exit $prod->{abbr} Configuration");
    $msg->msg('sfrac_exit_cleanly');
    $msg = Msg::new("Continue Rolling Upgrade");
    $msg->msg('sfrac_continue_ru');
    $msg = Msg::new("Create Oracle Users and Groups");
    ...
    $msg = Msg::new("Configure private IP addresses (PrivNIC Configuration) - For Oracle RAC prior to 11.2.0.2");
    $msg->msg('sfrac_config_privnic');

    $msg = Msg::new("Configure private IP addresses (MultiPrivNIC Configuration) - For Oracle RAC prior to 11.2.0.2");
    $msg->msg('sfrac_config_multiprivnic');

    $msg = Msg::new("Configure private IP addresses (HAIP Configuration) - For Oracle RAC 11.2.0.2 and later");
    $msg->msg('sfrac_config_haip');
    ...
    $prod->{post_config_check_items} = [];
    $prod->{nic_config_complete} = 1;
    return;
}

sub default_systemnames {
    my $prod=shift;
    my $vcs=$prod->prod('VCS62');
...
sub version_sys {
    ...
    my ($rootpath,$pkgvers,$tmppkgvers,$mpvers,$cpic,$rel,$cv,$pkg,$tmppkg,@mainpkgs);
    $cpic=Obj::cpic();
    $rel=$cpic->rel;
    ...
    if ($pkgvers) {
        @mainpkgs = qw(VRTSvxvm62 VRTSvxfs62 VRTSvcs62);
        for my $pkgname (@mainpkgs) {
            $tmppkg = $sys->pkg($pkgname);
            $tmppkgvers=$tmppkg->version_sys($sys,$force_flag);
            $cv = EDRu::compvers($tmppkgvers, $pkgvers, 4);
            if ($cv == 1) {
                $pkgvers = $tmppkgvers;
                $pkg = $tmppkg;
            }
...
sub verify_responsefile_option {
    my ($prod, $menuopt) = @_;
    my $cfg = Obj::cfg();
    my ($incomplete_data, $msg, $menu_verify_method);
...
sub responsefile_comments {
    ...
    $cmt=Msg::new("This variable defines whether to create the Oracle user/group after addnode");
    $edr->{rfc}{addnode_create_oracle_user_group}=[$cmt->{msg},0,0,0];
    $cmt=Msg::new("This variable defines the Oracle user name to create after addnode(Effective only if 'addnode_create_oracle_user_group' is defined as an non-zero value, e.g. 1)");
    $edr->{rfc}{oracle_user}=[$cmt->{msg},0,0,0];
    $cmt=Msg::new("This variable defines whether to configure private IP addresses (PrivNIC Configuration) - For Oracle RAC prior to 11.2.0.2 after addnode");
    $edr->{rfc}{addnode_config_privnic}=[$cmt->{msg},0,0,0];
    $cmt=Msg::new("This variable defines whether to configure private IP addresses (MultiPrivNIC Configuration) - For Oracle RAC prior to 11.2.0.2 after addnode");
    $edr->{rfc}{addnode_config_multiprivnic}=[$cmt->{msg},0,0,0];
    $cmt=Msg::new("This variable defines whether to configure private IP addresses (HAIP Configuration) - For Oracle RAC 11.2.0.2 and later after addnode");
    $edr->{rfc}{addnode_config_haip}=[$cmt->{msg},0,0,0];
    return;
}
...
sub precheck_task {
    ...
    for my $sys (@{$edr->{systems}}) {
        ...
        unless ($prod->any2any_precheck_sys($sys)) {
            $any2any=1;
            $msg=Msg::new("Since $sys->{sys} and the local system do not have the same platform architecture distribution version, none of the tasks are supported for $prod->{prod}");
            $sys->push_stop_checks_error($msg);
        }
    }
    return if($any2any);
    return $prod->perform_task('precheck_task');
}

sub any2any_precheck_sys {
    my ($prod,$sys) = @_;
    my $edr=Obj::edr();
    my $local_padv = $edr->{localsys}{padv};
    Msg::log("any2any check $sys->{hostname} padv=$sys->{padv} local=$local_padv");
    return 0 if ($sys->{padv} ne $local_padv);
    return 1;
}
...
sub prestart_sys {
    my ($prod,$sys)=@_;
    return $prod->perform_task_sys($sys, 'prestart_sys');
}
...
sub configure_sys {
    ...
    EDRu::writefile('/sbin/vcsmmconfig -c >/var/VRTSvcs/log/vcsmmconfig.log 2>&1 &', "$tmpdir/vcsmmtab");
    my $localsys=$prod->localsys;
    for my $othersys (@{CPIC::get('systems')}) {
        $localsys->copy_to_sys($othersys,"$tmpdir/vcsmmtab",$prod->{vcsmmtab});
    }
    return;
}
...
sub completion_messages {
    my $prod=shift;
    my ($backopt, $selection, $msg, $ret, $cpic, $rel, $suffix, $rac_cmd);

    # At the end of normal upgrade, except live upgrade(rootpath)
    # During rolling upgrade, at the end of upgrade of kernel packages
    # Provide option to relink oracle
    if (Cfg::opt(qw(hotfixupgrade patchupgrade upgrade)) && !Cfg::opt(qw(upgrade_nonkernelpkgs makeresponsefile rootpath))) {
        my @menu = @{$prod->{menu_postinstall_oracle}};
        shift(@menu); # remove the cssd agent configuration menu option
        # for rolling upgrade, change the menu option
        # Exit SF Oracle RAC Configuration --> Continue Rolling Upgrade Process
        if(Cfg::opt("rolling_upgrade")) {
            pop(@menu);
            push(@menu,'continue_ru');
        }
        $selection = $prod->display_product_menu_and_act($backopt, @menu);
    }
    if (Cfg::opt(qw(rootpath))){
        $cpic = Obj::cpic();
        $rel = $cpic->rel();
        $suffix = $rel->get_local_script_version_suffix();
        $rac_cmd = "/opt/VRTS/install/installsfrac$suffix -configure";
        Msg::n();
        $msg = Msg::new("Run the '$rac_cmd' command to do Oracle Database Binary Relink when the new boot environment is booted.");
        $msg->print;
        Msg::n();
    }
    if ($prod->{config_menu_displayed}) {
        if (@{$prod->{post_config_check_items}} == 0) {
            $prod->{post_config_check_items} = ["config_file","vcsmm","lmx","fencing_mode","db_install","crs_install","relink","misscount"];
        }
        $msg = Msg::new("Would you like to do post SFRAC configuration checks?");
        $ret = $msg->ayny;
        if ($ret eq 'Y') {
            $prod->post_config_check_sfrac();
        } else {
            Msg::n();
        }
    }
...
sub startprocs_sys {
    ...
    $ref_procs=$sfcfs->reorder_vxglm($ref_procs);
    $sf = $prod->prod('SF62');
    $ref_procs = $sf->adjust_vxdbd_nostart_sys($sys,$ref_procs);
    return $ref_procs;
}
...
sub poststart_configure_sys {
    my ($prod, $sys) = @_;
    my ($cpic,$rel,$suffix,$fencing_cmd,$msg,$backopt);
    $cpic = Obj::cpic();
    $rel = $cpic->rel();
    $suffix = $rel->get_local_script_version_suffix();
    $fencing_cmd="/opt/VRTS/install/installsfrac$suffix -fencing";
    ...
    if ( $sys->system1 ) {
        if ($rel->{nowebinstaller}) {
            ...
            $msg = Msg::new("If you have not yet configured Symantec I/O fencing, you have two ways to configure it:\n    1. Run the command '$fencing_cmd' \n    2. Use the I/O fencing configuration task on webinstaller.");
        }
        $msg->print;
        Msg::prtc();
        Msg::n();
...
sub display_product_menu_and_act {
    ...
    my ($msg,$sfcfs,$ii,$edr,$menukey,$menulist,$ret,$help,$choice,$menu,$def,$cfg);
    ...
    if(Cfg::opt("makeresponsefile")) {
        $msg = Msg::new("Currently SFRAC installer for configuration does not support -makeresponsefile option. \n");
        $msg->print;
        Msg::prtc();
        $sfcfs->{cfscluster_config_pending} = 0;
        my $cpic = Obj::cpic();
    ...
    while (1) {
        if (Obj::webui()) {
            ...
            $choice = $web->web_script_form('sfract_select_task', $prod, \@menuopts);
            if ($choice eq 'back') {
                return;
            }
        } else {
            ...
            # Exit in case of responsefile and no 'relink_oracle_database' configured, or relink has been done
            if (Cfg::opt('responsefile')) {
                if (!$cfg->{relink_oracle_database} || $prod->{relink_done_for_subcluster}) {
                    $def = scalar(@menuopts);
                    $prod->{relink_done_for_subcluster} = 0;
                }
            }
        ...
        if ($prod->{"menu_$choice"}) {
            ...
            if ($choice eq 'exit_cleanly' || $choice eq 'continue_ru') {
            ...
            $ret = $prod->$choice();
            if ((($choice eq 'config_sfrac_subcomponents') || Cfg::opt('responsefile')) && ($ret == 1)) {
                return;
            }
        }
    }
...
sub post_config_check_sfrac {
    my $prod = shift;
    my ($edr,$summary,$cmd_ret,$usefence,$crsup,$misscount,$msg_done,$msg_error_warning,$web,$webmsg,$msg,$lmx,$vcs,$vcs_sysname,$cmdout, $issue_found, $typesfile);
    $edr = Obj::edr();
    if (Cfg::opt('makeresponsefile')) {
        return 0;
    }
    for my $sys (@{CPIC::get('systems')}) {
        if (EDRu::arrpos("config_file", @{$prod->{post_config_check_items}}) != -1) {
            # check main.cf file
            $summary = undef;
            $cmd_ret = $sys->cmd("/opt/VRTSvcs/bin/hacf -verify /etc/VRTSvcs/conf/config/");
            if (EDR::cmdexit()!=0||$cmd_ret=~/WARNING/) {
                #./OracleTypes.cf:
                $typesfile=$1 if($cmd_ret=~/\/(.*):/);
                if($typesfile){
                    $summary=Msg::new("VCS configuration is incorrect on $sys->{sys}. It is recommended to check VCS configuration /etc/VRTSvcs/conf/config/$typesfile.");
                } else {
                    $summary=Msg::new("VCS configuration is incorrect on $sys->{sys}. It is recommended to check VCS configuration /etc/VRTSvcs/conf/config/main.cf.");
                }
            }
            $sys->push_error($summary) if ($summary);
            $issue_found = 1 if ($summary);
        }
        # check fencing in enabled state
        if (EDRu::arrpos("fencing_mode", @{$prod->{post_config_check_items}}) != -1) {
            $summary = undef;
            $cmd_ret = $sys->cmd('_cmd_vxfenadm -d 2>/dev/null');
            if ("$cmd_ret" eq '') {
                $summary = Msg::new("Fencing module is not configured on $sys->{sys}");
            } elsif ($cmd_ret =~ /Disabled/m) {
                $summary = Msg::new("Fencing module is configured in disabled mode on $sys->{sys}. It should be configured in enabled mode.");
            } else {
                $usefence = $sys->cmd("_cmd_cat /etc/VRTSvcs/conf/config/main.cf | _cmd_grep 'UseFence' | _cmd_grep 'SCSI3' ");
                if ( $usefence !~ /^\s*UseFence\s*=\s*SCSI3\s*$/m) {
                    if ( $usefence =~ /\/\/.*UseFence.*SCSI3/m) {
                        $summary = Msg::new("The UseFence variable is set to SCSI3 but is commented in /etc/VRTSvcs/conf/config/main.cf file on $sys->{sys}. It should be UseFence = SCSI3");
                    } else {
                        $summary = Msg::new("The UseFence variable does not exist or its value is empty or it is not set to SCSI3 in /etc/VRTSvcs/conf/config/main.cf file on $sys->{sys}. It should be UseFence = SCSI3");
                    }
                }
            }
            $sys->push_warning($summary) if ($summary);
            $issue_found = 1 if ($summary);
        }
        # check vcsmm
        if (EDRu::arrpos("vcsmm", @{$prod->{post_config_check_items}}) != -1) {
            $summary = undef;
            $cmd_ret = $sys->cmd("_cmd_gabconfig -a 2>/dev/null | _cmd_grep 'Port o' | _cmd_grep 'membership'");
            if ($cmd_ret eq '') {
                $summary = Msg::new("VCSMM is not configured on $sys->{sys}");
            }
            $sys->push_warning($summary) if ($summary);
            $issue_found = 1 if ($summary);
        }
        # check Oracle DB installation location
        if (EDRu::arrpos("db_install", @{$prod->{post_config_check_items}}) != -1) {
            if (!$prod->{db_home} || !$sys->exists($prod->{db_home})) {
                $prod->{db_home} = $sys->cmd("echo \$ORACLE_HOME 2>/dev/null");
                if (!$prod->{db_home} || !$sys->exists($prod->{db_home})) {
                    # get oracle home from VCS cfg.
                    $vcs=$prod->prod('VCS62');
                    $vcs_sysname = $vcs->get_vcs_sysname_sys($sys);
                    $cmdout = $sys->cmd("/opt/VRTSvcs/bin/hares -display -attribute Home -type Oracle 2>/dev/null");
                    for my $line (split(/\n/,$cmdout)) {
                        my @fields = split(/\s+/, $line);
                        if ( $fields[0] =~ /^#Resource/m ) {
                            next;
                        }
                        if ( $fields[2] eq 'global' || $fields[2] eq $vcs_sysname) {
                            $prod->{db_home} = $fields[3];
                            if (!$sys->exists($prod->{db_home})) {
                                $summary=Msg::new("Oracle Database installation path is not configured correctly in VCS on $sys->{sys}.");
                                $sys->push_warning($summary);
                                $issue_found = 1 if ($summary);
                            }
                            last;
                        }
        ...
        # check Oracle DB user and version
        if (EDRu::arrpos("db_install", @{$prod->{post_config_check_items}}) != -1) {
            if ($prod->{db_home}){
                my $opatch_file = $prod->{db_home}.'/OPatch/opatch';
                if ($sys->exists($opatch_file)) {
                    Msg::log("Checking Oracle Database installation path $prod->{db_home} on $sys->{sys}");
                    $prod->{orauser} = $sys->cmd("_cmd_ls -l ".$opatch_file ." 2>/dev/null | _cmd_awk '{print \$3}' 2> /dev/null");
                    if ($prod->{orauser}) {
                        $cmdout = $sys->cmd("cd /opt/VRTS/install; "."_cmd_su ".$prod->{orauser}." -c '_cmd_sh ".$opatch_file ." lsinventory -oh ".$prod->{db_home}." 2> /dev/null' | _cmd_grep 'Oracle Database' | _cmd_tail -1");
                        if ( $cmdout) {
                             my @fields = split(/\s+/, $cmdout);
                             $prod->{oraver} = $fields[-1];
                        }
                        if (!$prod->{oraver}) {
                            $summary=Msg::new("Failed to detect version of the Oracle Database installed in $prod->{db_home} on $sys->{sys}.");
                            $sys->push_warning($summary);
                            $issue_found = 1 if ($summary);
                        }
                    }else{
                        $summary=Msg::new("Failed to detect owner of the Oracle Database installed in $prod->{db_home} on $sys->{sys}.");
                        $sys->push_warning($summary);
                        $issue_found = 1 if ($summary);
                    }
                    Msg::log("Oracle Database owner: $prod->{orauser} on $sys->{sys}");
                    Msg::log("Oracle Database version: $prod->{oraver} on $sys->{sys}");
                }else{
                    $summary=Msg::new("$prod->{db_home} is not a valid Oracle Database installation path on $sys->{sys}.");
                    $sys->push_warning($summary);
                    $prod->{oraver} = '';
                    $issue_found = 1 if ($summary);
                }
            }else{
                $summary=Msg::new("Failed to detect Oracle Database installation on $sys->{sys} because ORACLE_HOME environment variable is not set correctly and Oracle resource is not configured correctly in VCS.");
                $sys->push_warning($summary);
                $issue_found = 1 if ($summary);
            }
        }
        # check Oracle CRS installation
        if (EDRu::arrpos("crs_install", @{$prod->{post_config_check_items}}) != -1) {
            if (!$prod->{crs_home}) {
                if ($sys->exists($prod->{initdir}."/init.ohasd") ) {
                    $prod->{crs_home} = $sys->cmd("_cmd_grep '^ORA_CRS_HOME=' ".$prod->{initdir}."/init.ohasd 2>/dev/null | _cmd_sed -e 's/^ORA_CRS_HOME=//'");
                } elsif ($sys->exists($prod->{initdir}."/init.crs")) {
                    $prod->{crs_home} = $sys->cmd($prod->{initdir}."/init.crs home");
                }
            }
        }
        # check Oracle CRS user and version
        if (EDRu::arrpos("crs_install", @{$prod->{post_config_check_items}}) != -1) {
            if ($prod->{crs_home} && $sys->exists($prod->{crs_home}.'/OPatch/opatch')) {
                Msg::log("Checking Oracle CRS installation path $prod->{crs_home} on $sys->{sys}");
                $prod->{crsuser} = $sys->cmd("_cmd_ls -l ".$prod->{crs_home}."/OPatch/opatch | _cmd_awk '{print \$3}' 2> /dev/null");
                if ($prod->{crsuser}) {
                    $cmdout = $sys->cmd("cd /opt/VRTS/install; "."_cmd_su ".$prod->{crsuser}." -c '_cmd_sh ".$prod->{crs_home}."/OPatch/opatch lsinventory -oh ".$prod->{crs_home}." 2> /dev/null' | _cmd_grep 'Oracle Clusterware' 2>/dev/null | _cmd_tail -1");
                    if (!$cmdout) {
                        $cmdout = $sys->cmd("cd /opt/VRTS/install; "."_cmd_su ".$prod->{crsuser}." -c '_cmd_sh ".$prod->{crs_home}."/OPatch/opatch lsinventory -oh ".$prod->{crs_home}." 2> /dev/null' | _cmd_grep 'Oracle Grid Infrastructure' 2>/dev/null | _cmd_tail -1");
                    }
                    if ( $cmdout) {
                        my @fields = split(/\s+/, $cmdout);
                        $prod->{crsver} = $fields[-1];
                    }
                    if (!$prod->{crsver}) {
                        $summary=Msg::new("Failed to detect version of the Oracle CRS installed in $prod->{crs_home} on $sys->{sys}.");
                        $sys->push_warning($summary);
                        $issue_found = 1 if ($summary);
                    }
                } else {
                    $summary=Msg::new("Failed to detect owner of the Oracle CRS installed in $prod->{crs_home} on $sys->{sys}.");
                    $sys->push_warning($summary);
                    $issue_found = 1 if ($summary);
                }
                Msg::log("Oracle CRS owner: $prod->{crsuser} on $sys->{sys}");
                Msg::log("Oracle CRS version: $prod->{crsver} on $sys->{sys}");
            } elsif ($prod->{crs_home}) {
                $summary=Msg::new("$prod->{crs_home} is not a valid Oracle CRS installation path on $sys->{sys}.");
                $sys->push_warning($summary);
                $prod->{crsver} = '';
                $issue_found = 1 if ($summary);
            } else {
                $summary=Msg::new("Failed to detect Oracle CRS installation on $sys->{sys}");
                $sys->push_warning($summary);
                $issue_found = 1 if ($summary);
            }
        }
        # check lmx on AIX/HP-UX/Solaris
        if (EDRu::arrpos("lmx", @{$prod->{post_config_check_items}}) != -1) {
            if (!($sys->linux()||$sys->sunos_sol11())) {
                if ( $prod->{crsver} =~ /^11/) {
                    $summary=Msg::new("LMX is not required on $sys->{sys} as Oracle CRS $prod->{crsver} is installed.");
                    $sys->push_note($summary);
                    $issue_found = 1 if ($summary);
                } else {
                    $summary = undef;
                    $lmx = $sys->proc('lmx62');
                    if (!$lmx->check_sys($sys, 'start')) {
                        if ( $prod->{crsver} =~ /^10/) {
                            $summary = Msg::new("LMX module is not loaded and Oracle CRS $prod->{crsver} is installed on $sys->{sys}. LMX should be running for Oracle CRS 10g releases.");
                            $sys->push_warning($summary);
                            $issue_found = 1 if ($summary);
                        } else {
                            $summary = Msg::new("LMX module is not loaded and Oracle CRS is not detected on $sys->{sys}. LMX should be running for Oracle CRS 10g releases.");
                            $sys->push_warning($summary);
                            $issue_found = 1 if ($summary);
                        }
                    } else {
                        if ($sys->aix()) {
                            if (!$sys->exists('/opt/VRTSvcs/rac/bin/lmxshow')) {
                                $summary = Msg::new("Could not run LMX helper thread check on $sys->{sys} due to missing /opt/VRTSvcs/rac/bin/lmxshow file. LMX does not seem to be installed on $sys->{sys}.");
                            } else {
                                $cmd_ret = $sys->cmd("_cmd_lsattr -E -l lmx | _cmd_awk '/lmx_async_upd/ {print \$2}'");
                                if ($cmd_ret eq '1') {
                                    $summary = Msg::new("LMX is running with helper thread ENABLED on node: $sys->{sys}. LMX helper thread should be disabled.");
                                }
                            }
                        } else {
                            if (!$sys->exists('/opt/VRTSvcs/rac/bin/lmxshow')) {
                                ...
                                $cmd_ret = $sys->cmd("/opt/VRTSvcs/rac/bin/lmxshow -t 2>/dev/null | _cmd_grep update_enabled | _cmd_cut -d ':' -f2 | _cmd_cut -d ' ' -f2");
                                if ($cmd_ret eq '1') {
                                    $summary = Msg::new("LMX is running with helper thread ENABLED on node: $sys->{sys}. LMX helper thread should be disabled.");
                                }
                            }
        ...
        #check relinking setting
        if (EDRu::arrpos("relink", @{$prod->{post_config_check_items}}) != -1) {
            if ($prod->{crs_home} && $prod->{crsver} && $prod->{db_home} && $prod->{oraver} && $prod->{orauser}) {
                $summary = undef;
                $cmd_ret = $prod->check_liblink_sys($sys, "$prod->{crs_home}", "$prod->{db_home}", "$prod->{oraver}", "$prod->{orauser}");
                if ($cmd_ret eq '1') {
                    $summary = Msg::new("Libraries are not linked properly on $sys->{sys}.");
                } elsif ($cmd_ret eq '2') {
                    $summary = Msg::new("No Oracle version was found. Library linking check was skipped on $sys->{sys}.");
                } elsif ($cmd_ret eq '3') {
                    $summary = Msg::new("Could not discover IPC library version. Library linking check was skipped on $sys->{sys}.");
                }
                $sys->push_warning($summary) if ($summary);
                $issue_found = 1 if ($summary);
            } else {
                if (!$prod->{crsver} && !$prod->{oraver}) {
                    $summary = Msg::new("No Oracle CRS/Database versions were found. Library linking check was skipped on $sys->{sys}.");
                } elsif (!$prod->{crsver}) {
                    $summary = Msg::new("No Oracle CRS version was found. Library linking check was skipped on $sys->{sys}.");
                } else {
                    $summary = Msg::new("No Oracle Database version was found. Library linking check was skipped on $sys->{sys}.");
                }
                $sys->push_warning($summary);
                $issue_found = 1 if ($summary);
            }
        }
        # check misscount setting
        if (EDRu::arrpos("misscount", @{$prod->{post_config_check_items}}) != -1) {
            if ($sys->linux() && $prod->{crs_home} && $prod->{crsver}) {
                $summary = undef;
                $misscount = $sys->cmd("$prod->{crs_home}/bin/crsctl get css misscount");
                if (!EDR::cmdexit()) {
                    if ($misscount !~ /^\d+$/) {
                        $misscount =~ s/^.*misscount\s+(\d+)\s+.*$/$1/;
                    }
                    if ( $misscount && $misscount =~ /^\d+$/) {
                        if ($misscount < 600) {
                            $summary = Msg::new("The current css misscount value ($misscount) is less than 600 on $sys->{sys}");
                        }
                    } else {
                        $summary = Msg::new("Failed to get css misscount value on $sys->{sys}.");
                    }
                } else {
                    $summary = Msg::new("Failed to get css misscount value on $sys->{sys}. Make sure Cluster Synchronization Services daemon is running.");
                }
                $sys->push_warning($summary) if ($summary);
                $issue_found = 1 if ($summary);
            } elsif ($sys->linux() && !$prod->{crsver}) {
                $summary = Msg::new("No Oracle CRS version was found. Misscount check was skipped on $sys->{sys}.");
                $sys->push_warning($summary);
                $issue_found = 1 if ($summary);
            }
        }
    }
    $msg_done = Msg::new("Post SFRAC configuration checks completed successfully");
    $msg_error_warning = Msg::new("Post SFRAC configuration checks completed");
    $edr->display_errors_warnings($msg_done,$msg_error_warning);
    if (!$issue_found) {
        $msg = Msg::new("No errors, warnings or notes have been found.\n");
        if (Obj::webui()) {
            $web = Obj::web();
            $web->web_script_form('alert', $msg);
        } else {
    ...
    if (Obj::webui()) {
        ...
        if ($msg) {
            $webmsg = Msg::new("<B>The following errors were discovered during post SFRAC configuration checks:</B>");
            $webmsg->{msg} = $webmsg->{msg}."<br/>".$msg."<br/>";
        }

        #print warnings
        $msg = '';
        for my $sys (@{$edr->{systems}}) {
            for my $warnmsg (@{$sys->{warnings}}) {
                $msg = $msg.$warnmsg."<br/>";
            }
        }
        if ($msg) {
            $webmsg->{msg} .= Msg::new("<B>The following warnings were discovered during post SFRAC configuration checks:</B>")->{msg};
            $webmsg->{msg} = $webmsg->{msg}."<br/>".$msg."<br/>";
        }
        ...
        $msg = '';
        for my $sys (@{$edr->{systems}}) {
            for my $notemsg (@{$sys->{notes}}) {
                $msg = $msg.$notemsg."<br/>";
            }
        }
        if ($msg) {
            $webmsg->{msg} .= Msg::new("<B>The following notes were discovered during post SFRAC configuration checks:</B>")->{msg};
            $webmsg->{msg} = $webmsg->{msg}."<br/>".$msg."<br/>";
        }

        $web = Obj::web();
        $msg = $webmsg->{msg}?$webmsg->{msg}:$msg_done->{msg};
        $web->web_script_form('alert',$msg);
    }
    return 1;
}
...
sub config_sfrac_subcomponents {
    ...
    foreach my $a ("config_file","vcsmm") {
        push @{$prod->{post_config_check_items}}, $a;
    }
    $prod->{post_config_check_items} = EDRu::arruniq(@{$prod->{post_config_check_items}});
...
sub create_oracle_user_group {
    ...
    my $range = 1000;
    my $web = Obj::web();
    my (@userhomenotdirsys, $invalidsystems);
...
DNA_CREATE_ORACLE_USER_GROUP:
    ...
    if (Obj::webui())
    {
        $web->web_script_form("create_oracle_user_group");
    ...
    while(1) {
        ...
        for my $sys (@{CPIC::get('systems')}) {
            if ( $sys->exists($answer) && !$sys->is_dir($answer)) {
                push @userhomenotdirsys, $sys->{sys};
            }
        }
        if (@userhomenotdirsys) {
            $invalidsystems = join(" \n\t",@userhomenotdirsys);
            $msg = Msg::new("$answer is not a valid directory on the following system(s). Input again \n\t$invalidsystems");
            $msg->print;
            if (Obj::webui()){
                $web->web_script_form("alert", $msg->{msg}) ;
                goto DNA_CREATE_ORACLE_USER_GROUP;
...
DNA_CREATE_SEC_GROUP:
        ...
        if (Obj::webui())
        {
            $web->web_script_form("create_oracle_sec_group");
            if ($web->param("back") eq "back") {
                goto DNA_CREATE_ORACLE_USER_GROUP;
            }
        }
...
DNA_SECGRP:
        if (Cfg::opt('responsefile')) {
            last if ((!defined($cfg->{oracle_secondary_group}) || ((scalar @{$cfg->{oracle_secondary_group}}) < 1)) ||
                     (!defined($cfg->{oracle_secondary_gid})   || ((scalar @{$cfg->{oracle_secondary_gid}}) < 1)));
...
sub verify_responsefile_create_ocr_vote_storage {
    my $prod = shift;
    my ($cfg,$msg,$incomplete_data);
    $cfg = Obj::cfg();

    if (($cfg->{ocrvotedgoption} != 0) &&
        ($cfg->{ocrvotedgoption} != 1) &&
        ($cfg->{ocrvotedgoption} != 2) &&
        ($cfg->{ocrvotedgoption} != 3)) {
...
sub validate_disk_space{
    my ($prod, $master, $dgname, $off_size, $indices, @disks) = @_;
    my ($ret, $msg, @lines, $total_free_size, @checkdisks, $free_size, $first_appear, $size, @items);
    my $web = Obj::web();

    foreach my $index (@{$indices}){
        push (@checkdisks, $disks[$index-1]);
    }

    # Check if the volume name is non-existent, if so check size available
    $ret = $master->cmd("_cmd_vxprint -g $dgname -u m");
    if (EDR::cmdexit()) {
        if ($ret =~ /No such disk group/){
        ...
        foreach my $disk (@checkdisks){
            ...
            foreach my $line (@lines){
                if($line =~ /\s+$disk\s+/m){
                    ...
                    $free_size = $free_size - $size - 0;
                }
            }
            $total_free_size = $total_free_size + $free_size + 0;
        }
    }

    if ($total_free_size < $off_size + 0){
        return 1;
    }
    return 0;
}
...
sub create_ocr_vote_storage {
    my $prod = shift;
    my ($web,$ayn,$msg,$cfg,$sys,$errstr,$question,$answer,$repeat,$preinst,$ret,$out,$availdisks,@strlist,@diskname,$line,$temp_user);
    $cfg = Obj::cfg();
    $web = Obj::web();

    return 0 if (Cfg::opt('responsefile') && $cfg->{create_ocr_vote_storage} == 0);

    for my $check_item ("config_file","vcsmm") {
        push @{$prod->{post_config_check_items}}, $check_item;
    }
    $prod->{post_config_check_items} = EDRu::arruniq(@{$prod->{post_config_check_items}});
...
DNA_INSTR:
    my ($master,$indices,$index,$def,$help,$cmd);
    my ($ocrdgname, $votedgname, $dgname);
    my ($ocrvolname, $votevolname, $volname, $ocrvolsize, $votevolsize, $volsize);
    my (@availdisks, @seldisks, @storeopts, $storeopt, $localdisksys, $localdisk_udid, $disk_status, @sel_disk_nodes, @sys_names, @systems);
    ...
    my $edr = Obj::edr();
    # If create fss dg is chosen, the chosen local disks should be initiated from local node
    my $create_fss_dg = 0;
    # Whenever FSS dg is used for OCR and VOTE dg, the volume to be created is recommended to have number of mirrors equal to number of nodes in cluster and each node contributing one of its local disk to the volume mirrors. If this condition is not met, we need to warn the user.
    my $use_fss_dg = 0;

    my $localsys = $prod->localsys;
    my $firstsys = Obj::cpic()->{systems}[0];
    ...
    my $vcs = $prod->prod('VCS62');
    $master = $vcs->cluster_master_sys($firstsys);
    return 1 if ($master == 1);

    # Get all systems in the cluster of $master
    @sys_names = ();
    my $conf = $vcs->get_config_sys($master);
    if ($conf) {
        @sys_names = @{$conf->{systems}};
    } else {
        $msg=Msg::new("Cluster configuration information checking failed on $master->{sys}");
        $msg->print;
        if (Obj::webui()) {
            $web->web_script_form('alert', $msg);
        }
...
DNA_SELECT_OPTION:
    if (Cfg::opt('responsefile')){
        ...
        if (!$cfg->{ocrvotedgoption}) {
            $create_fss_dg = 0;
            $use_fss_dg = 0;
            goto DNA_DISKS;
        } elsif ($cfg->{ocrvotedgoption} == 2) {
            $create_fss_dg = 1;
            $use_fss_dg = 1;
            goto DNA_DISKS;
        } elsif ($cfg->{ocrvotedgoption} == 1) {
            $create_fss_dg = 0;
            $use_fss_dg = 0;
            goto DNA_VOLTYPE;
        } elsif ($cfg->{ocrvotedgoption} == 3) {
            $create_fss_dg = 0;
            $use_fss_dg = 1;
            goto DNA_VOLTYPE;
        }
    ...
    if (Obj::webui()) {
        ...
        $webresult=$web->web_script_form("selectocroptions", $prod);
        if ($web->param("back") eq "back") {
...
# Display choice for creating a new disk group or use an existing disk group
        # Hide FSS items in case of single node
        if (scalar(@sys_names) == 1) {
            splice(@dgopts, scalar(@dgopts) - 2);
        }
        $index = $msg->menu(\@dgopts, $def, $help, 1);
        return 0 if (EDR::getmsgkey($index,'back'));
    }

    # Choose to create new disk group. 1 - Shared Storage  3 - FSS
    if ($index == 1 || $index == 3) {
        if ($index == 1) {
            $create_fss_dg = 0;
            $use_fss_dg = 0;
            $msg = Msg::new("\nDo you want to enable mirroring?");
            eval {$availdisks = $master->cmd('_cmd_vxdisk -o fullshared -o alldgs list 2>/dev/null');};
            $errstr = $@;
            if ($errstr) {
                $msg = Msg::log("Fail to find eligible disks on $master->{sys}. Error info: $errstr");
                ...
                if ($availdisks) {
                    my @tmpdisks = split(/\n/, $availdisks);
                    @tmpdisks = grep(/online/, @tmpdisks);
                    @tmpdisks = grep(!/(LVM|error|exported)/, @tmpdisks);
                    $availdisks = join("\n", @tmpdisks);
                }
            }
            ...
            $msg = Msg::new("Listing local disks for nodes in cluster $conf->{clustername}. Please wait a moment...");
            $msg->print;
            @systems = ();
            for my $sys_name (@sys_names) {
                my $tmp_sys;
                if ($Obj::pool{"Sys::$sys_name"}) {
                    $tmp_sys = Obj::sys($sys_name);
                } else {
                    $tmp_sys = Sys->new($sys_name);
                    if (!$edr->transport_sys($tmp_sys)) {
                        $msg = Msg::new("The CVM node $tmp_sys->{sys} cannot be accessed as root user via either SSH or RSH without entering a password from the local system. Make sure SSH or RSH communication is configured in password-less mode on $tmp_sys->{sys}.");
                        $msg->print;
                        $web->web_script_form("alert", $msg->{msg}) if (Obj::webui());
                        next;
                    }
                }
                push(@systems, $tmp_sys);
            }

            for my $local_sys (@systems) {
                eval {$localdisks = $local_sys->cmd('_cmd_vxdisk -o local -o alldgs list 2>/dev/null');};
                $errstr = $@;
                if ($errstr) {
                    $msg = Msg::log("Fail to find eligible disks on $local_sys->{sys}. Error info: $errstr");
                    # You might want to print the manual steps here
                    $msg = Msg::new("Fail to find eligible disks on $local_sys->{sys}. Refer to the log file for details");
                    $web->web_script_form("alert",$msg->{msg}) if (Obj::webui());
                    $msg->print;
                    next;
                } else {
                    next if (EDR::cmdexit());
                    next unless ($localdisks);
                    @localdl = split(/\n/, $localdisks);
                    @localdl = grep (/online/, @localdl);
                    @localdl = grep (!/(LVM|error)/, @localdl);
                }
                my ($diskname, $status);
                for my $line (@localdl) {
                    ($diskname, undef, undef, undef, $status) = split(/\s+/m, $line, 5);
                    $availdisks .= $line."    ($local_sys->{sys})\n";

                    # Record the status of local disk to ignore exported disks while init disk
                    $disk_status->{$local_sys->{sys}}{$diskname} = $status;
                    # Record the udid of local disk in order to create dg in master node
                    $localdisk_udid->{$local_sys->{sys}}{$diskname} = $local_sys->cmd("_cmd_vxdisk list $diskname | _cmd_awk '/udid:/ { print \$2}'");
                    # Record the node the disk connected with, in order to init the disk from local node in further step(4)
                    $localdisksys->{$local_sys->{sys}}{$diskname} = $local_sys;
                }
            }
            $cfg->{ocrvotedgoption} = 2;
        }
    ...
    $enable_mirroring = 1;
    if ((!$use_fss_dg && $availdisks) || $index == 2) {
        $ayn = $msg->ayny;
        if ($ayn eq 'N') {
            $enable_mirroring = 0;
        } else {
...
DNA_OPTION_CREATE_DG:
    ...
    if ($index == 1 || $index == 3) {
        ...
        for my $item (@availdisks) {
            ...
            # Filter disks already in disk group
            if ($strlist[3] eq '-'){
                push(@diskstrlist, $item) ;
            }
        }

        if (!@diskstrlist || (scalar(@diskstrlist)<2 && $enable_mirroring)) {
            if (!@diskstrlist) {
                $msg = Msg::new("No eligible disk found");
            } else {
                $msg = Msg::new("No enough disks, at least two disks are needed for mirroring");
            }
            $web->web_script_form("alert", $msg->{msg}) if (Obj::webui());
        ...
        if (Obj::webui()) {
            if ($create_fss_dg) {
                $msg = Msg::new("As you create the FSS disk group, it is recommended that you choose at least one local disk for each node if possible. Choose 2 disks from different nodes is the minimum requirement.");
                $web->web_script_form("alert", $msg->{msg});
            }
            ...
            $msg->print;
            if ($enable_mirroring) {
                $msg = Msg::new("Select the serial numbers of at least two disks separated by spaces for creating storage for OCR and Voting disk, excluding disks used for the Operating System and data:");
            } else {
                $msg = Msg::new("Select the serial numbers of at least a disk for creating storage for OCR and Voting disk, excluding disks used for the Operating System and data:");
            }
            # Option 3&4 are not shown if the padv do not support FSS feature, so it's safe to just judge the option chosen
            $msg = Msg::new("As you create the FSS disk group, it is recommended that you choose at least one local disk for each node if possible. Choose 2 disks from different nodes is the minimum requirement:") if ($create_fss_dg);
            $indices = $msg->menu(\@diskstrlist, $def, $help, 1, 1, 1);
            goto DNA_SELECT_OPTION if (EDR::getmsgkey($indices,'back'));
        }
        ...
        $msg->print;
        my $dgnamelist = '';
        $dgnamelist = $master->cmd("_cmd_vxdg list | _cmd_grep -w 'shared' | _cmd_awk {'print \$1'}");
        if (EDR::cmdexit()) {
            $msg = Msg::log("Fail to find eligible disks groups on $master->{sys}. Error info: $errstr");
            Msg::prtc();
            if (Obj::webui()) {
                $msg = Msg::new("Fail to find eligible disks groups on $master->{sys}. Refer to the log file for details");
                $web->web_script_form("alert", $msg->{msg});
                goto DNA_SELECT_OPTION ;
            }
            return;
        ...
        if ($prod->{is_fss_enabled}) {
            for my $dg (@dgnamelist) {
                my $dg_attr = $master->cmd("_cmd_vxdg list $dg 2>/dev/null");
                my $fss_status = '';
                $fss_status = $1 if ($dg_attr=~/^fss:\s*(\w+)\s*$/m);
                if ($fss_status eq 'on') {
                    my $src_nodes;
                    my @disk_src;
                    $src_nodes = $1 if ($dg_attr=~/^storage-sources:\s*(.*)$/m);
                    $src_nodes = trim($src_nodes);
                    @disk_src = split(/\s+/, $src_nodes);
                    # Filter out disk groups whose disks are from only 1 node
                    if (scalar(@disk_src) <= 1) {
                        Msg::log("Disks of $dg are from only 1 node $src_nodes. Can not be used to create OCR/Voting disk");
                        next;
                    }
                    push(@fssdg, $dg);
                } else {
        ...
        for my $dg (@dgnamelist){
            my $diskname =$master->cmd("_cmd_vxdisk -g $dg list 2>/dev/null | _cmd_awk {'print \$1'} | _cmd_grep -v 'DEVICE'");
            if (EDR::cmdexit()) {
                $msg = Msg::log("Fail to find eligible disks for disk group $dg on $master->{sys}. Error info: $errstr");
                if (Obj::webui()) {
                    $msg = Msg::new("Fail to find eligible disks for disk group $dg on $master->{sys}. Refer to the log file for details");
                    $web->web_script_form("alert", $msg->{msg});
                    goto DNA_SELECT_OPTION ;
                }
                Msg::prtc();
        ...
        # If 1 node 1 disk is not fulfilled, warn the user
        if ($use_fss_dg) {
            my $src_nodes =$master->cmd("_cmd_vxdg list $dgname | _cmd_awk -F: '/storage-sources:/ {print \$2}'");
            $src_nodes = trim($src_nodes);
            @sel_disk_nodes = split(/\s+/, $src_nodes);
            if (@sel_disk_nodes != @sys_names) {
                $msg = Msg::new("The disk group does not fulfill the recommended at least one disk for each node qualification. Do you want to choose another disk group?");
                $ayn = $msg->ayny;
                goto DNA_SELECT_OPTION if ($ayn eq 'Y');
            }
        }
        goto DNA_VOLTYPE;
    }
...
DNA_DISKINDEX_CHECK:
    ...
    # Figure out the nodes the disks seleceted from
    if ($create_fss_dg) {
        @sel_disk_nodes = ();
        for my $disk (@seldisks) {
            my ($diskname, @status, $localnode);
            ($diskname, undef, undef, undef, @status) = split(/\s+/, $disk);
            $localnode = $status[-1];
            $localnode =~ s/[\(\)]//g;
            push (@sel_disk_nodes, $localnode);
        }
        @sel_disk_nodes = @{EDRu::arruniq(@sel_disk_nodes)};
        if (scalar(@sel_disk_nodes) <= 1) {
            $msg = Msg::new("To create FSS disk group, choose at least 2 disks from different nodes");
            if (Obj::webui()) {
                $web->web_script_form("alert", $msg->{msg}) ;
                $index=1;
...
# At least 2 disks needed as mirroring is assumed
    if (($enable_mirroring == 1) && (!$create_fss_dg)) {
        if ($#seldisks + 1 < 2) {
...
DNA_DISKS:
    if (Cfg::opt('responsefile')) {
        if (!$create_fss_dg) {
            for my $disk (@{$cfg->{ocrvotedisks}}) {
                $line = $firstsys->cmd("_cmd_vxdisk list | _cmd_grep -w $disk");
                if (!EDR::cmdexit()) {
                    push (@seldisks, $line);
                } else {
            ...
            if ($use_fss_dg) {
                # TODO
                my $src_nodes =$master->cmd("_cmd_vxdg list $cfg->{ocrvotedgname} | _cmd_awk -F: '/storage-sources:/ {print \$2}'");
                $src_nodes = trim($src_nodes);
                @sel_disk_nodes = split(/\s+/, $src_nodes);
                if (scalar(@sel_disk_nodes) <= 1) {
                    $msg = Msg::new("To create FSS disk group, choose at least 2 disks from different nodes");
            ...
            for my $disk (@{$cfg->{ocrvotedisks}}) {
                my ($sys_name, $diskname) = split(/:/, $disk);
                if ($sys_name eq '' || $diskname eq '') {
                    $msg = Msg::new("Configure item 'ocrvotedisks' error for disk $disk.");
                    $msg->print;
                    return;
                }
                if (!grep(/$sys_name/, @sys_names)) {
                    $msg = Msg::new("Configure item 'ocrvotedisks' error for disk $disk. The node $sys_name is not in the cluster.");
                    $msg->print;
                    return;
                }
                my $tmp_sys;
                if ($Obj::pool{"Sys::$sys_name"}) {
                    ...
                    $tmp_sys = Sys->new($sys_name);
                    if (!$edr->transport_sys($tmp_sys)) {
                        $msg = Msg::new("The CVM node $tmp_sys->{sys} cannot be accessed as root user via either SSH or RSH without entering password from local system. Make sure SSH or RSH communication is configured in password-less mode on $tmp_sys->{sys}.");
                        $msg->print;
                        $web->web_script_form("alert", $msg->{msg}) if (Obj::webui());
                        return;
                    }
                }
                $line = $tmp_sys->cmd("_cmd_vxdisk -o local -o alldgs list | _cmd_grep -w $diskname  | _cmd_grep online | _cmd_grep -v LVM | _cmd_grep -v error");
                if (!EDR::cmdexit()) {
                    my $status;
                    $line .= $line.' ('.$sys_name.')';
                    push (@seldisks, $line);
                    (undef, undef, undef, undef, $status) = split(/\s+/m, $line, 5);
                    $disk_status->{$sys_name}{$diskname} = $status;
                    $localdisksys->{$sys_name}{$diskname} = $tmp_sys;
                    $localdisk_udid->{$sys_name}{$diskname} = $tmp_sys->cmd("_cmd_vxdisk list $diskname | _cmd_awk '/udid:/ { print \$2}'");
                    push(@sel_disk_nodes, $sys_name);
                } else {
                    $msg = Msg::new("$diskname is not eligible or it is not on $sys_name. See logs for more details.");
                    $msg->print;
                    return;
                }
            }
    ...
    while (1) {
        ...
        $ret = $prod->validate_dgname($answer, $master);
        if ($ret == 2) {
            Msg::n();
            return 1 if (Cfg::opt('responsefile'));
            ...
            $msg->print;
            $out = $master->cmd("_cmd_vxprint -g $answer");
            $msg = Msg::new($out);
            $msg->print;
            Msg::n();
            $msg = Msg::new("Input another name. Refer to the installer logs for more information");
    ...
    if (Obj::webui()) {
        ...
        for my $disk (@seldisks) {
            $count++;
            my ($handle, @status, $node);
            if ($create_fss_dg) {
                ($handle, undef, undef, undef, @status) = split(/\s+/m, $disk);
                $node = $status[-1];
                $msg = Msg::new("\t\t${count}. $handle $node");
            } else {
                $handle = (split(/\s+/m, $disk))[0];
...
DNA_CONFIRMSTORE:
    ...
    # Save config for responsefile
    @{$cfg->{ocrvotedisks}} = @initdisks;
    $cfg->{ocrvotedgname} = $dgname;
    $cfg->{enable_mirroring} = $enable_mirroring;

    # 5.
    # Initialize disks (Shared Storage - on Master, FSS - on local node)
    for my $dsk (@initdisks) {
        # By default the command is excecuted from master
        my $tmpdsk = $dsk;
        my $sys = $master;
        $msg = Msg::new("Initializing disk $tmpdsk on Master ($sys->{sys})");
        if ($create_fss_dg) {
            # Ignore local disks already exported
            my $localnode;
            ($localnode, $tmpdsk) = split(/:/, $tmpdsk);
            $sys = $localdisksys->{$localnode}{$tmpdsk};
            next if ($disk_status->{$localnode}{$tmpdsk} =~ /exported/);
            $msg = Msg::new("Initializing local disk $tmpdsk on node ($localnode)");
        }
        $msg->left;
        eval {$sys->cmd("_cmd_vxdisk -f init $tmpdsk type=auto format=cdsdisk");};
        $errstr = $@;
        if ($errstr) {
            Msg::right_failed();
            $msg = Msg::new("Initializing disk $tmpdsk on node ($sys->{sys}) failed. Refer to the installer logs for more information.") if (Obj::webui());
            $web->web_script_form("alert", $msg->{msg}) if (Obj::webui());

            Msg::log("Error info: $errstr");
            Msg::prtc();
            return 1;
        } else {
            if (EDR::cmdexit()) {
                # retrying to initialize the specified disk
                my $type = $sys->cmd("_cmd_vxdisk list $tmpdsk 2>/dev/null | _cmd_grep '^type:'");
                if ( $type !~ /^type:\s*auto\s*$/) {
                    $sys->cmd("_cmd_vxdisk rm $tmpdsk");
                    $sys->cmd("_cmd_vxdisk scandisks");
                }
                $sys->cmd("_cmd_vxdisksetup -if $tmpdsk format=cdsdisk");
                if (EDR::cmdexit()) {
                    Msg::right_failed();
                    $msg = Msg::new("Initializing disk $tmpdsk on node ($sys->{sys}) failed. Refer to the installer logs for more information.") if (Obj::webui());
                    $web->web_script_form("alert", $msg->{msg}) if (Obj::webui());
        ...
        if ($create_fss_dg) {
            $sys->cmd("_cmd_vxdisk export $tmpdsk");
            if (EDR::cmdexit()) {
                Msg::right_failed();
                $msg = Msg::new("Export local disk $tmpdsk on node ($sys->{sys}) failed. Refer to the installer logs for more information.") if (Obj::webui());
                $web->web_script_form("alert", $msg->{msg}) if (Obj::webui());
                Msg::prtc();
                return 1;
            }
    ...
    $msg = Msg::new("Initializing disk group on Master ($master->{sys})");
    $msg->left;

    if ($create_fss_dg) {
        # Local disks' name may changed on master node due to hostprefix, so use udid to create dg
        my $udid_list;
        for my $dsk (@initdisks) {
            my $localnode;
            my $tmpdsk = $dsk;
            ($localnode, $tmpdsk) = split(/:/, $tmpdsk);
            $udid_list .= "udid:".$localdisk_udid->{$localnode}{$tmpdsk}." ";
        }
        $udid_list = trim($udid_list);
        eval {$master->cmd("_cmd_vxdg -s -o fss init $dgname $udid_list");};
    } else {
        eval {$master->cmd("_cmd_vxdg -s init $dgname $initdisks");};
    }
    ...
    if ($errstr) {
        ...
        $msg->log;
        $msg = Msg::new("Initializing disk group on Master ($master->{sys}) failed. Refer to the installer logs for more information.") if (Obj::webui());
        $web->web_script_form("alert", $msg->{msg}) if (Obj::webui());
        Msg::prtc();
        return 1;
    } else {
...
DNA_SELECT_STORAGE:
    ...
    if (Cfg::opt('responsefile')) {
        ...
        if(Obj::webui()) {
            ...
            $webresult=$web->web_script_form("selectstorageoptions");
            if ($web->param("back") eq "back") {
                goto DNA_SELECT_OPTION;
            }
            if ($webresult eq "1") {
...
DNA_CREATE_RAW_VOL:
        ...
        if (Obj::webui()) {
            ...
            $webresult=$web->web_script_form("create_raw_vol");
            if ($web->param("back") eq "back") {
                $onraw = 0 ;
                $oncfs = 0 ;
    ...
    # Volume size for OCR
            if (Cfg::opt('responsefile')) {
                ...
                if (Obj::webui()) {
                    ...
                    $question = Msg::new("Enter the volume size for OCR in MB (Refer to Oracle documentation to set it properly):");
            ...
            if ($prod->validate_vol($ocrvolname, $ocrvolsize, $unavailsize, $dgname, $master, $enable_mirroring, $use_fss_dg, scalar(@sel_disk_nodes))) {
                return 1 if (Cfg::opt('responsefile'));
                if (Obj::webui()) {
                    goto DNA_CREATE_RAW_VOL ;
                }
    ...
    # Volume size for Vote
                ...
                if ($prod->validate_vol($votevolname, $votevolsize, $unavailsize, $dgname, $master , $enable_mirroring, $use_fss_dg, scalar(@sel_disk_nodes))) {
                    return 1 if (Cfg::opt('responsefile'));
                    if (Obj::webui()) {
                        goto DNA_CREATE_RAW_VOL ;
                    }
 ...
 DNA_CREATE_CFS:
        if ($enable_sep_filesys == 0){
            ...
            if (Obj::webui()) {
                ...
                $webresult=$web->web_script_form("create_cfs");
                if ($web->param("back") eq "back") {
                    $onraw = 0 ;
                    $oncfs = 0 ;
                    goto DNA_SELECT_STORAGE;
            ...
            while (1) {
                ...
                if (Cfg::opt('responsefile')) {
                    ...
                    if (Obj::webui()) {
                        ...
                        $question = Msg::new("Enter the volume size for OCR and Voting disk in MB (Refer to Oracle documentation to set it properly):"); 
                        $answer = $question->ask($defvolsize, $help, 1);
                        goto DNA_SELECT_STORAGE if (EDR::getmsgkey($answer,'back'));
                    }
                }
                ...
                if ($prod->validate_vol($volname, $volsize, $unavailsize, $dgname, $master, $enable_mirroring, $use_fss_dg, scalar(@sel_disk_nodes))) {
                    return 1 if (Cfg::opt('responsefile'));
                    if (Obj::webui()) {
                        goto DNA_CREATE_CFS;
                    }
...
DNA_CREATE_OCR_USER_GROUP:
    ...
    if(Obj::webui()) {
        ...
        $webresult=$web->web_script_form("create_ocr_user_group");
        if ($web->param("back") eq "back") {
...
DNA_CONFIRMVOLS:
    ...
    if ($onraw || ($enable_sep_filesys == 1)) {
        return 1 if ($prod->create_ocr_vote_volume($master, $dgname, $ocrvolname, $ocrvolsize, $enable_mirroring, $use_fss_dg, $#sel_disk_nodes, 'OCR'));
        return 1 if ($prod->create_ocr_vote_volume($master, $dgname, $votevolname, $votevolsize, $enable_mirroring, $use_fss_dg, $#sel_disk_nodes, 'Voting'));
    } elsif ($oncfs && ($enable_sep_filesys == 0)) {
        return 1 if ($prod->create_ocr_vote_volume($master, $dgname, $volname, $volsize, $enable_mirroring, $use_fss_dg, $#sel_disk_nodes, 'both'));
    }
    ...
    # Set user/group for volume(s)
        if ($onraw) {
            ...
            $msg->left;
            eval {$master->cmd("_cmd_vxedit -g $dgname set user=$temp_user group=$prod->{oracle_group} mode=660 $ocrvolname");};
            $errstr = $@;
            if ($errstr) {
                Msg::right_failed();
                $msg = Msg::new("Error info: $errstr");
            ...
            $msg->left;
            eval {$master->cmd("_cmd_vxedit -g $dgname set user=$temp_user group=$prod->{oracle_group} mode=660 $votevolname");};
            $errstr = $@;
            if ($errstr) {
                Msg::right_failed();
                $msg = Msg::new("Error info: $errstr");
    ...
    # Start volume(s) on all nodes
        for my $sys (@{CPIC::get('systems')}) {
            ...
            if ($master == $sys) {
                ...
                $msg->left;
                eval {$master->cmd("_cmd_vxvol -g $dgname startall");};
                $errstr = $@;
                if ($errstr) {
                    Msg::right_failed();
                    $msg = Msg::new("Error info: $errstr");
...
DNA_CREATE_CFSMOUNT:
          ...
          if(Obj::webui()) {
              ...
              if ($enable_sep_filesys == 0) {
                  $webresult=$web->web_script_form("create_cfsmount");
              } elsif ($enable_sep_filesys == 1) {
                  $webresult=$web->web_script_form("create_cfsmount_sep_filesys");
              }
    ...
    # They are PADV specific operations
            ...
            if ($prod->can('make_mount_cfs')) {
                ...
                if ($enable_sep_filesys == 0) {
                    ($ret, $mk, $mkret, %mnt, %mntret) = $prod->make_mount_cfs($dgname, $volname, $cfsmntpt, $master);
                    if ($ret) {
                        Msg::right_failed();
                        Msg::prtc();
                    ...
                    ($ret, $mk, $mkret, %mnt, %mntret) = $prod->make_mount_cfs($dgname, $ocrvolname, $ocrmount, $master);
                    if ($ret) {
                        Msg::right_failed();
                        Msg::prtc();
                    ...
                    ($ret, $mk, $mkret, %mnt, %mntret) = $prod->make_mount_cfs($dgname, $votevolname, $votemount, $master);
                    if ($ret) {
                        Msg::right_failed();
                        Msg::prtc();
...
sub create_ocr_vote_volume{
    my ($prod, $master, $dgname, $volname, $volsize, $enable_mirroring, $use_fss_dg, $sel_disk_nodes, $vol_type) = @_;
    my ($msg, $errstr, $web);
    ...
    $msg = Msg::new("Creating volume '$volname' for $type disk on CVM Master ($master->{sys})");
    $msg->left;
    if ($enable_mirroring == 1 && !$use_fss_dg) {
        eval {$master->cmd("_cmd_vxassist -g $dgname make $volname ${volsize}m nmirror=2 layout=mirror");};
    } elsif (($enable_mirroring == 0 && !$use_fss_dg) || !$sel_disk_nodes) {
        eval {$master->cmd("_cmd_vxassist -g $dgname make $volname ${volsize}m");};
    } elsif ($use_fss_dg && $sel_disk_nodes) {
        my $sel_disk_nodes_num = $sel_disk_nodes + 1;
        eval {$master->cmd("_cmd_vxassist -g $dgname make $volname ${volsize}m nmirror=$sel_disk_nodes_num layout=mirror mirror=host");};
    }
    $errstr = $@;
    if ($errstr) {
        Msg::right_failed();
        ...
        $msg->log;
        Msg::prtc();
        $msg = Msg::new(" Creating volume '$volname' for $type disk on CVM Master ($master->{sys}) failed") if (Obj::webui());
        $web->web_script_form("alert", $msg->{msg}) if (Obj::webui());
        return 1;
    } else {
        if (EDR::cmdexit()) {
            Msg::right_failed();
            Msg::prtc();
            $msg = Msg::new(" Creating volume '$volname' for OCR and Voting disk on CVM Master ($master->{sys}) failed") if (Obj::webui());
            $web->web_script_form("alert", $msg->{msg}) if (Obj::webui());
            return 1;
        }
        Msg::right_done();
...
sub make_online_system {
    ...
    my $ret = 0;
    my $out;
    my $sysname = Prod::VCS62::Common::transform_system_name($sys->{sys});
...
sub begin_enabling_system {
    ...
    $ret = 0;
    $out = $firstsys->cmd("$vcs->{bindir}/hares -modify $cvmresname Enabled 1");
    $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
    if ($oncfs) {
        if ($enable_sep_filesys == 0) {
            $out = $firstsys->cmd("$vcs->{bindir}/hares -modify $cfsresname Enabled 1");
            $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
        } elsif ($enable_sep_filesys == 1) {
            $out = $firstsys->cmd("$vcs->{bindir}/hares -modify $cfsvoteresname Enabled 1");
            $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
            $out = $firstsys->cmd("$vcs->{bindir}/hares -modify $cfsocrresname Enabled 1");
            $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
        }
    }
    $out = $firstsys->cmd("$vcs->{bindir}/haconf -dump -makero");
    $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
...
sub link_parent_child {
    ...
    if ($oncfs) {
        if ($enable_sep_filesys == 0) {
            $out = $firstsys->cmd("$vcs->{bindir}/hares -link $cfsresname $cvmresname");
            $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
            $out = $firstsys->cmd("$vcs->{bindir}/hares -link $cfsresname vxfsckd");
            $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
        } elsif ($enable_sep_filesys == 1) {
            $out = $firstsys->cmd("$vcs->{bindir}/hares -link $cfsvoteresname $cvmresname");
            $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
            $out = $firstsys->cmd("$vcs->{bindir}/hares -link $cfsvoteresname vxfsckd");
            $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
            $out = $firstsys->cmd("$vcs->{bindir}/hares -link $cfsocrresname $cvmresname");
            $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
            $out = $firstsys->cmd("$vcs->{bindir}/hares -link $cfsocrresname vxfsckd");
            $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
        }
    }
    $out = $firstsys->cmd("$vcs->{bindir}/hares -link $cvmresname cvm_clus");
    $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
...
sub change_vcs_config_perm {
    ...
    my $ret;
    my $cpic = Obj::cpic();
    my $firstsys = $cpic->{systems}[0];

    $ret = $firstsys->cmd("$vcs->{bindir}/haconf -makerw");
...
sub set_up_vols {
    ...
    # Set up the volumes
    $ret = 0;
    $out = $firstsys->cmd("$vcs->{bindir}/hares -add $cvmresname CVMVolDg cvm");
    $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
    $out = $firstsys->cmd("$vcs->{bindir}/hares -modify $cvmresname CVMDiskGroup $dgname");
    $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
    $out = $firstsys->cmd("$vcs->{bindir}/hares -modify $cvmresname CVMVolume -add $volname");
    $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
    $out = $firstsys->cmd("$vcs->{bindir}/hares -modify $cvmresname CVMActivation sw");
    $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
    $out = $firstsys->cmd("$vcs->{bindir}/hares -modify $cvmresname CVMVolumeIoTest -add $volname");
...
sub set_up_fs {
    ...
    my $sysname = '';
    for my $sys (@{CPIC::get('systems')}) {
        $sysname = Prod::VCS62::Common::transform_system_name($sys->{sys});
        $syslist = "$syslist $sysname";
    }

    # Set up the file system
    $ret = 0;
    if ($enable_sep_filesys == 0) {
        $out = $firstsys->cmd("$vcs->{bindir}/hares -add $cfsresname CFSMount cvm");
        $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
        $out = $firstsys->cmd("$vcs->{bindir}/hares -modify $cfsresname Critical 0");
        $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
        $out = $firstsys->cmd("$vcs->{bindir}/hares -modify $cfsresname MountPoint $cfsmntpt");
        $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
        $out = $firstsys->cmd("$vcs->{bindir}/hares -modify $cfsresname BlockDevice /dev/vx/dsk/${dgname}/${volname}");
        $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
        $out = $firstsys->cmd("$vcs->{bindir}/hares -modify $cfsresname MountOpt mincache=direct");
        $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
        $out = $firstsys->cmd("$vcs->{bindir}/hares -modify $cfsresname NodeList $syslist");
        $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
    } elsif ($enable_sep_filesys == 1) {
        if ($ocr == 1) {
            $out = $firstsys->cmd("$vcs->{bindir}/hares -add $cfsvoteresname CFSMount cvm");
            $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
            $out = $firstsys->cmd("$vcs->{bindir}/hares -modify $cfsvoteresname Critical 0");
            $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
            $out = $firstsys->cmd("$vcs->{bindir}/hares -modify $cfsvoteresname MountPoint $cfsmntpt");
            $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
            $out = $firstsys->cmd("$vcs->{bindir}/hares -modify $cfsvoteresname BlockDevice /dev/vx/dsk/${dgname}/${volname}");
            $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
            $out = $firstsys->cmd("$vcs->{bindir}/hares -modify $cfsvoteresname MountOpt mincache=direct");
            $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
            $out = $firstsys->cmd("$vcs->{bindir}/hares -modify $cfsvoteresname NodeList $syslist");
            $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
        } elsif ($ocr == 0) {
            $out = $firstsys->cmd("$vcs->{bindir}/hares -add $cfsocrresname CFSMount cvm");
            $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
            $out = $firstsys->cmd("$vcs->{bindir}/hares -modify $cfsocrresname Critical 0");
            $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
            $out = $firstsys->cmd("$vcs->{bindir}/hares -modify $cfsocrresname MountPoint $cfsmntpt");
            $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
            $out = $firstsys->cmd("$vcs->{bindir}/hares -modify $cfsocrresname BlockDevice /dev/vx/dsk/${dgname}/${volname}");
            $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
            $out = $firstsys->cmd("$vcs->{bindir}/hares -modify $cfsocrresname MountOpt mincache=direct");
            $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
            $out = $firstsys->cmd("$vcs->{bindir}/hares -modify $cfsocrresname NodeList $syslist");
            $ret = $ret || (($out =~ /VCS WARNING/m) ? 0 : EDR::cmdexit());
        }
    }
...
sub validate_vol {
    my ($prod, $volname, $volsize, $unavailsize, $dgname, $master, $enable_mirroring, $use_fss_dg, $sel_disk_nodes) = @_;
    my ($msg, $status, $ret, $temp, $availsize,$webmsg);
    my $web = Obj::web();
    $status = 0;
    ...
    if (!EDR::cmdexit()) {
        ...
        if ($use_fss_dg) {
            $ret = $master->cmd("_cmd_vxassist -g $dgname maxsize nmirror=$sel_disk_nodes");
        } elsif (!$enable_mirroring) {
            $ret = $master->cmd("_cmd_vxassist -g $dgname maxsize ");
        } else {
            $ret = $master->cmd("_cmd_vxassist -g $dgname maxsize layout=mirror");
        }
        if (EDR::cmdexit()) {
...
#e3372952:Get the available volume size while the result of vxassist has info message
            $ret =~ /.*Maximum volume size:\s+\d+\s+\((\d+)Mb\).*/is;
            $availsize = $1;
            $msg = Msg::new("Available volume size: $availsize MB");
            $msg->log;
...
sub validate_display {
    ...
    if ($display =~/^(.*):(\d+)(\.\d+)?$/){
        ...
        if ($host eq '' || EDRu::is_port_connectable($host,$port)){
           $sys->cmd("$prod->{xdpyinfo_path} -display $display 2>&1");
           if (EDR::cmdexit()){
               $msg=Msg::new("$display is not authorized");
               $msg->log;
               return 1;
           }
        } else {
           $msg=Msg::new("$display is not a valid X11 server");
           $msg->log;
           return 1;
        }
    } else {
        $msg=Msg::new("$display is not a valid DISPLAY value");
        $msg->log;
        return 1;
    }
    return 0;
...
WEB_ASK_PRIV_RESOURCE:
        while (1) {
            ...
            if (!$isaddnode) {
                ...
                $firstsys->cmd("$vcs->{bindir}/hares -display $res_name");
                if (EDR::cmdexit() == 0) {
...
DNA_CONFIRMNET:
    ...
    $msg->left;
    $firstsys->cmd("$vcs->{bindir}/haconf -dump -makero");
    $firstsys->cmd("$vcs->{bindir}/haconf -makerw");
    if (EDR::cmdexit() != 0) {
        Msg::right_failed();
        Msg::prtc();
        if (Obj::webui()) {
    ...
    $cmdout = $firstsys->cmd("$vcs->{bindir}/hares -display $prod->{priv_res_name} -attribute Device");
    if ($cmdout =~ /global/){
        $device_global = 1;
    }
    for my $sys (@{$nodes_ref}) {
        ...
        for my $inf (@{$systems{$sys->{sys}}{'inf'}}) {
            ...
            unless ($device_global) {
                $firstsys->cmd("$vcs->{bindir}/hares -modify $prod->{priv_res_name} Device -add $inf $pri -sys $sysname");
                if (EDR::cmdexit() != 0) {
                    $err = 1;
                    goto done;
                }
            }
            $firstsys->cmd("$vcs->{bindir}/hares -modify $prod->{priv_res_name} Address $ip -sys $sysname");
            if (EDR::cmdexit() != 0) {
                $err = 1;
                goto done;
            }
    ...
    if (!$isaddnode && $is_ipv4) {
        $firstsys->cmd("$vcs->{bindir}/hares -modify $prod->{priv_res_name} NetMask $priv_mask");
        if (EDR::cmdexit() != 0) {
            $err = 1;
            goto done;
        }
    ...
    $firstsys->cmd("$vcs->{bindir}/hares -modify $prod->{priv_res_name} Enabled 1");
    if (EDR::cmdexit() != 0) {
        $err = 1;
        goto done;
    }
    ...
    if ($err) {
        if (!$isaddnode) {
            $firstsys->cmd("$vcs->{bindir}/hares -delete $prod->{priv_res_name}");
        }
        $firstsys->cmd("$vcs->{bindir}/haconf -dump -makero");
        Msg::right_failed();
        Msg::prtc();
...
sub config_multiprivnic_common {
    ...
    if (!$isaddnode) {
        ...
        if ($#$mpriv_rsrc < 0) {
            ...
            $msg->right;
            if (Obj::webui()) {
                $msg = Msg::new("No MultiPrivNIC resource needs reconfiguration");
                $web->web_script_form("alert",$msg->{msg});
            }
            Msg::prtc();
            return 1;
...
WEB_ASK_MULTI_PRIV_RESOURCE:
        while (1) {
            ...
            if (!$isaddnode) {
                ...
                if (!$tmp_found) {
                    my $discoveredlist = "[".join(' ', @$mpriv_rsrc)."]";
                    $msg = Msg::new("$res_name should be from the above discovered list $discoveredlist. Input again");
                    $msg->print;
                    return 1 if (Cfg::opt('responsefile'));
                    $web->web_script_form("alert",$msg->{msg}) if (Obj::webui());
                    next;
...
DNA_CONFIRMNET:
    ...
    $cmdout = $firstsys->cmd("$vcs->{bindir}/hares -display $prod->{mpriv_res_name} -attribute Device");
    if ($cmdout =~ /global/){
        $device_global = 1;
    }
    ...
    for my $sys (@{$nodes_ref}) {
        ...
        unless ($device_global) {
            for my $inf (@{$systems{$sys->{sys}}{'inf'}}) {
                $firstsys->cmd("$vcs->{bindir}/hares -modify $prod->{mpriv_res_name} Device -add $inf $pri -sys $sysname");
                if (EDR::cmdexit() != 0) {
                    $err = 1;
                    goto done;
                }
        ...
        for my $inf (@{$systems{$sys->{sys}}{'inf'}}) {
            for my $ip (@{$systems{$sys->{sys}}{'ip'}{$inf}}) {
                if ($ip ne '')  {
                    $firstsys->cmd("$vcs->{bindir}/hares -modify $prod->{mpriv_res_name} Address -add $ip $pri -sys $sysname");
                    if (EDR::cmdexit() != 0) {
                        $err = 1;
                        goto done;
                    }
    ...
    if (!$isaddnode && $is_ipv4) {
        $firstsys->cmd("$vcs->{bindir}/hares -modify $prod->{mpriv_res_name} NetMask $mpriv_mask");
        if (EDR::cmdexit() != 0) {
            $err = 1;
            goto done;
        }
    ...
    $firstsys->cmd("$vcs->{bindir}/hares -modify $prod->{mpriv_res_name} Enabled 1");
    if (EDR::cmdexit() != 0) {
        $err = 1;
        goto done;
    }
    ...
    if ($err) {
        if (!$isaddnode) {
            $firstsys->cmd("$vcs->{bindir}/hares -delete $prod->{mpriv_res_name}");
        }
        $firstsys->cmd("$vcs->{bindir}/haconf -dump -makero");
        Msg::right_failed();
        Msg::prtc();
...
sub config_haip{
    ...
    return 0 if (Cfg::opt('responsefile') && $cfg->{config_haip} == 0);
    $cfg->{config_haip} = 1;

    push @{$prod->{post_config_check_items}}, qw(config_file vcsmm);
    $prod->{post_config_check_items} = EDRu::arruniq(@{$prod->{post_config_check_items}});

    $prod->task_title('config_haip');

    $ret = $prod->config_haip_common(0, CPIC::get('systems'), $clusnodes, $newnodes);
    if ($ret && Obj::webui()) {
        $msg = Msg::new("HAIP configuration failed. Refer to the log file for details");
        $web->web_script_form("alert",$msg->{msg});
    }
...
sub config_haip_common {
    my ($prod, $isaddnode, $allnodes_ref, $clusnodes_ref, $newnodes_ref)  = @_;
    my ($mpriv_ips_ref, @mpriv_ips, $mpriv_ip, $mpriv_mask);
    my ($sys, $key, $inf, $nics, $nics_str, $ip, $ips);
    my ($ret, $err, $msg, $question, $ayn, $done, $help, $backopt);
    my ($res_name, $add_ip_alias, $is_ipv4);
    my ($mpriv_aliases_ref, @mpriv_aliases, $def_aliases, $num_aliases);
    my ($nodes_ref, $mpriv_rsrc);
    my ($cmdout, $device_global, $addnode_str);
    my %systems;
    my $vcs = $prod->prod('VCS62');
    my $cfg = Obj::cfg();
    my $web = Obj::web();
    ...
    if (!$isaddnode) {
        $msg = Msg::new("This step will configure private IP addresses for the Oracle HAIP configuration");
        $msg->print;
        $msg = Msg::new("* All LLT links will be used for the HAIP configuration");
        $msg->print;
        $msg = Msg::new("* Ensure that the same NICs are used for private interconnect on each node of your cluster");
        $msg->print;
    } else {
        $msg = Msg::new("This step will re-configure the existing IP addresses for the new nodes being added");
        $msg->print;
        $msg = Msg::new("* All LLT links will be used for the Oracle HAIP configuration on the new nodes");
        $msg->print;
        $msg = Msg::new("* Ensure that the same NICs are used for private interconnect on each old and new node of your cluster");
        $msg->print;
    }
    ...
    $msg = Msg::new("* All IP addresses must be of same type, either all IPv4 or all IPv6");
    $msg->print;
    $addnode_str = $isaddnode ? ". For those private IP addresses already existing on the cluster, you need to add them to the new node manually" : "";
    if ($prod->{hosts_file} eq '/etc/hosts') {
        $msg = Msg::new("* The private IP addresses must be added to /etc/hosts on all nodes before installing Oracle Clusterware. If you choose the Installer to add IP addresses in /etc/hosts then only those IP addresses which do not already exist in this file on any node will be added$addnode_str");
        $msg->print;
    } else {
        $msg = Msg::new("* The private IP addresses must be added to /etc/hosts or $prod->{hosts_file} file on all nodes before installing Oracle Clusterware (IPv4 addresses in /etc/hosts and IPv6 addresses in $prod->{hosts_file}). If you choose the Installer to add IP addresses in /etc/hosts and $prod->{hosts_file} files then only those IP addresses which do not already exist in these files on any node will be added$addnode_str");
        $msg->print;
    }

    $msg = Msg::new("* You must not use the public network interface for an Oracle HAIP configuration");
    $msg->bold;
    $msg = Msg::new("* IP addresses used for a NIC on all nodes of your cluster must be in the same subnet, which must be different from the subnets for the IP addresses on other NICs. Otherwise, Oracle Clusterware/Grid Infrastructure/Oracle Database UDP IPC will not be able to communicate properly across the nodes");
    $msg->bold;
...
WEB_ASK_HAIP:
        ...
        $nics_str = join(' ',@$nics);
        if ($nics_str ne $prod->{haip_nics}) {
            $prod->{haip_data} = {};
            $prod->{haip_nics} = $nics_str;
        }
        ...
        if (Cfg::opt('responsefile')) {
            $add_ip_alias = $cfg->{nic_add_ip_to_files};
            goto DNA_ADDIP;
        }
        $add_ip_alias = 0;
        if ($prod->{hosts_file} eq '/etc/hosts') {
        ...
        $cfg->{nic_add_ip_to_files} = $add_ip_alias if (!Cfg::opt('responsefile'));
        Msg::n();
...
WEB_ASK_HAIP_AGAIN:
                ...
                if (Obj::webui()) {
                        $prod->{config_haip} = 2;
                        $web->web_script_form("config_haip",$sys,$inf,$add_ip_alias);
                        $prod->{config_haip} = 0;
                        if ($web->param("back") eq "back") {
                            if ($priv_inf_index == -1) {
                                if ($priv_sys_index == -1) {
                ...
                while (1) {
                    ...
                    if (exists($prod->{haip_data}{$sys->{sys}}{ip}{$inf})) {
                        $def_ips = $prod->{haip_data}{$sys->{sys}}{ip}{$inf};
                    } else {
                        $def_ips = '';
                    }
                    $mpriv_ips_ref = $prod->ask_haip_ips_sys($sys, $inf, $def_ips);
                    @mpriv_ips = ($mpriv_ips_ref);
                    return 0 if (EDR::getmsgkey($mpriv_ips[0],'back'));
                    if ($mpriv_ips[0] eq 'x') {
                        $prod->{haip_data}{$sys->{sys}}{ip}{$inf} = 'x';
                        $prod->{haip_data}{$sys->{sys}}{alias}{$inf} = '';
                        goto next_inf;
                    }

                    $ret = $prod->check_ips($sys, $inf, $ip_cnt, $is_ipv4, $add_ip_alias, \@mpriv_ips, \%systems, 'HAIP', $allnodes_ref);
                    if (Cfg::opt('responsefile')) {
                        return 1 if ($ret);
                    } else {
                        if(Obj::webui()) {
                                goto WEB_ASK_HAIP_AGAIN if ($ret);
                        }else {
                                next if ($ret);
                        }
                    ...
                    $prod->{haip_data}{$sys->{sys}}{ip}{$inf} = $mpriv_ips_ref;
                    last;
                }#while

                while ($add_ip_alias) {
                    if (exists($prod->{haip_data}{$sys->{sys}}{alias}{$inf})) {
                        $def_aliases = $prod->{haip_data}{$sys->{sys}}{alias}{$inf};
                    } else {
                        $def_aliases = '';
                    }
                    $num_aliases = @mpriv_ips;
                    $mpriv_aliases_ref = $prod->ask_haip_aliases($sys->{sys}, $inf, $def_aliases, $inf_i);
                    if (Obj::webui()) {
                        if ($mpriv_aliases_ref == -1) {
                            $systems{$sys->{sys}}{ip}{$inf} = '';
                            goto WEB_ASK_HAIP_AGAIN;
                    ...
                    if ($mpriv_aliases[0] eq 'x') {
                        $prod->{haip_data}{$sys->{sys}}{alias}{$inf} = 'x';
                        goto next_inf1;
                    }

                    $ret = $prod->check_aliases($sys, $inf, \@mpriv_aliases, \%systems, 'HAIP', $allnodes_ref);
                    if (Cfg::opt('responsefile')) {
                        return 1 if ($ret);
                    } else {
                        if(Obj::webui()) {
                    ...
                    $prod->{haip_data}{$sys->{sys}}{alias}{$inf} = $mpriv_aliases_ref;
                    if ($ip_sys_cnt == 0) {
                        my @alias_parts = split(/-/m, $mpriv_aliases[0]);
                        my $alias_parts_cnt = @alias_parts;
                        if ($alias_parts_cnt > 1) {
                            $prod->{ip_alias_haip} = "-$alias_parts[-1]";
                        }
                    }
...
next_inf:
        ...
        if (!$isaddnode && $is_ipv4) {
            while(1) {
                $prod->{config_haip} = 3 if (Obj::webui());
                $mpriv_mask = $prod->ask_netmask();
                $prod->{config_haip} = 0 if (Obj::webui());
        ...
        for my $sys (@{$nodes_ref}) {
            ...
            $msg_str .= "$msg->{msg}\n";
            for my $inf (@{$systems{$sys->{sys}}{inf}}) {
                my $ips = $systems{$sys->{sys}}{ip}{$inf};
                $msg = Msg::new("\tPrivate IP on $inf:$ips");
                my $aliases = $systems{$sys->{sys}}{alias}{$inf};
                $msg->print;
                $msg_str .= "$msg->{msg}\n";
                if ($add_ip_alias) {
                    $msg = Msg::new("\tAlias for above IP:$aliases");
                    $msg->print;
                    $msg_str .= "$msg->{msg}\n";
                }
            }
...
DNA_CONFIRMNET:
    ...
    $msg = Msg::new("\nSaving network configuration on all nodes:\n");
    $msg->print;

    my $nodes_ref_save=$allnodes_ref;
    $nodes_ref_save=$newnodes_ref if ($isaddnode);
    $ret = $prod->save_haip_configuration($is_ipv4, \%systems,
                         $nodes_ref_save, $allnodes_ref, $add_ip_alias);
    return 1 if $ret;

    if (Obj::webui()) {
        $msg = Msg::new("HAIP configuration was successful");
        $web->web_script_form("alert",$msg->{msg});
    }
...
sub check_ips {
    ...
    for my $mpriv_ip (@mpriv_ips) {
        ...
        if ($ip_cnt == 0) {
                    ...
                    for my $inf_name (@{$systems{$sys_name}{'inf'}}) {
                        for my $ip_addr (@{$systems{$sys_name}{'ip'}{$inf_name}}) {
                            if (($ip_addr eq $uniq_ip) ||
                                ($ip_addr =~ /^$uniq_ip\/.*$/mx)) {
                                $msg = Msg::new("IP address $mpriv_ip is duplicated or already used");
                                $msg->print;
                                $is_ip_bad = 1;
                                if (Obj::webui()) {
                                    $msg = Msg::new("IP address $mpriv_ip is duplicated or already used. Input again");
                                    $web->web_script_form("alert",$msg->{msg});
                                    last;
                                }
                                goto next_ip;
            ...
            } elsif ($res_type eq 'HAIP') {
                for my $sys_name (keys (%systems)) {
                    for my $inf_name (@{$systems{$sys_name}{inf}}) {
                        my $ip_addr = $systems{$sys_name}{ip}{$inf_name};
                        if (($ip_addr eq $uniq_ip) ||
                            ($ip_addr =~ /^$uniq_ip\/.*$/mx)) {
                            $msg = Msg::new("IP address $mpriv_ip is duplicated or already used");
                            $msg->print;
        ...
        $is_ip_plumbed = 0;
        $ret = $prod->search_word_sys($prod->localsys, "$tmpdir/plumbed_ips.txt", "$uniq_ip", 'ip');
        if ($ret == 0) {
            $is_ip_plumbed = 1;
        }
        ...
        if($add_ip_addr){
            for my $sys1 (@{$nodes_ref}) {
                $ret = $prod->search_word_sys($sys1, '/etc/hosts', "$uniq_ip", 'ip');
                if ($ret == 0) {
                    $is_ip_in_hfile = 1;
                    last;
                } else {
                    ...
                    $ret = $prod->search_word_sys($sys1, $prod->{hosts_file}, "$uniq_ip", 'ip');
                    if ($ret == 0) {
                        $is_ip_in_hfile = 1;
                        goto last;
                    }
        ...
        if ($res_type eq 'MultiPrivNIC') {
            ...
            $systems{$sys->{sys}}{ip}{$inf} = $mpriv_ip;
        } else {
...
sub check_aliases {
    ...
    for my $alias (@mpriv_aliases) {
        ...
        if ($res_type eq 'MultiPrivNIC') {
            for my $sys_name (keys (%systems)) {
                for my $inf_name (@{$systems{$sys_name}{'inf'}}) {
                    for my $als (@{$systems{$sys_name}{'alias'}{$inf_name}}) {
                        if ($als eq $alias) {
                            $msg = Msg::new("Alias $alias is duplicated or already used,");
                            $msg->print;
                            $is_alias_bad = 1;
                            if (Obj::webui()) {
                                $msg = Msg::new("Alias $alias is duplicated or already used. Input again");
                                $web->web_script_form("alert",$msg->{msg});
                                last;
                            }
                            goto next_alias;
           ...
           for my $sys_name (keys (%systems)) {
                for my $inf_name (@{$systems{$sys_name}{inf}}) {
                    my $als = $systems{$sys_name}{alias}{$inf_name};
                    if ($als eq $alias) {
                        $msg = Msg::new("Alias $alias is duplicated or already used");
                        $msg->print;
                        $is_alias_bad = 1;
        ...
        for my $sys1 (@{$nodes_ref}) {
            $ret = $prod->search_word_sys($sys1, '/etc/hosts', "$alias", 'alias');
            if ($ret == 0) {
                ...
                next if ($prod->{hosts_file} eq '/etc/hosts');
                # Solaris specific checks
                $ret = $prod->search_word_sys($sys1, $prod->{hosts_file}, "$alias", 'alias');
                if ($ret == 0) {
                    push (@used_aliases, $alias);
                    last;
                }
        ...
        if ($res_type eq 'MultiPrivNIC') {
            ...
            $systems{$sys->{sys}}{alias}{$inf} = $alias;
        } else {
...
next_alias:
    ...
    if ($is_alias_bad) {
        ...
        my $used_alias_cnt = @used_aliases;
            my $msg_str = "";
        if ($used_alias_cnt) {
            my $alias_str = Msg::new("alias")->{msg};
            if ($res_type eq 'MultiPrivNIC') {
                $alias_str = Msg::new("alias(es)")->{msg};
            }

            my $used_aliases_str = join(' ', @used_aliases);
            if ($prod->{hosts_file} eq '/etc/hosts') {
                $msg = Msg::new("The $alias_str $used_aliases_str already in use, exists in /etc/hosts file on one or more node(s). Note that the entry related to the private IP you just entered and its alias $used_aliases_str will not be added again later.");
                $msg->print;
                $msg_str .= "$msg->{msg}\n";
            } else {
                $msg = Msg::new("The $alias_str $used_aliases_str already in use, exists in /etc/hosts or $prod->{hosts_file} file on one or more node(s). Note that the entry related to the private IP you just entered and its alias $used_aliases_str will not be added again later.");
                $msg->print;
                $msg_str .= "$msg->{msg}\n";
            }
...
sub check_resource {
    ...
    if (EDR::cmdexit() == 0) {
        ...
        for my $res (@resources) {
            ...
            $output = $firstsys->cmd("$vcs->{bindir}/hares -display $res -attribute Address | _cmd_grep -v '^#'");
            @lines = split(/\n/, $output);
            for my $line (@lines) {
                @words = split(/\s+/m, $line);
            ...
            #Delete the resource
            $firstsys->cmd("$vcs->{bindir}/hares -delete $res");
            if (EDR::cmdexit() != 0) {
                $delete_failed = 1;
                next;
            }
...
sub add_resource {
    ...
    $firstsys->cmd("$vcs->{bindir}/hares -add $res_name $res_type cvm");
    if (EDR::cmdexit() != 0) {
        $err = 1;
        goto done;
    }

    $firstsys->cmd("$vcs->{bindir}/hares -modify $res_name Critical 0");
    if (EDR::cmdexit() != 0) {
        $err = 1;
        goto done;
    }

    $firstsys->cmd("$vcs->{bindir}/hares -local $res_name Device");
    if (EDR::cmdexit() != 0) {
        $err = 1;
        goto done;
    }

    $firstsys->cmd("$vcs->{bindir}/hares -local $res_name Address");
    if (EDR::cmdexit() != 0) {
        $err = 1;
        goto done;
    }
    ...
    if ($err) {
        $firstsys->cmd("$vcs->{bindir}/hares -delete $res_name");
        $firstsys->cmd("$vcs->{bindir}/haconf -dump -makero");
        Msg::right_failed();
        Msg::prtc();
        return 1;
...
sub ask_haip_ips_sys {
    my ($prod, $sys, $inf, $def_ip) = @_;
    my ($msg, $question, $help, $backopt, $is_ipv4);
    my $priv_ip = '';
    my $cfg = Obj::cfg();
    ...
    if (Cfg::opt('responsefile') || Obj::webui()) {
        $priv_ip = $cfg->{$sys->{sys}}->{$inf}->{haipip};
    } else {
        $question = Msg::new("Enter the IP for $inf on $sys->{sys}:");
        $priv_ip = $question->ask("$def_ip", $help, $backopt);
    }

    $cfg->{$sys->{sys}}->{$inf}->{haipip} = $priv_ip if (!Cfg::opt('responsefile') && !EDR::getmsgkey($priv_ip,'back'));
    return $priv_ip;
}
...
sub ask_privnic_alias {
    ...
    if ($def_alias eq '') {
        my $sysname = Prod::VCS62::Common::transform_system_name($sys_name);
...
sub ask_haip_aliases {
    my ($prod, $sys_name, $inf, $def_alias, $inf_i) = @_;
    my ($msg, $question, $help, $backopt);
    my $alias;
    my $cfg = Obj::cfg();
    ...
    if ($def_alias eq '') {
        my $sysname = Prod::VCS62::Common::transform_system_name($sys_name);
        if ( $inf_i == 0){
            $def_alias = "$sysname"."$prod->{ip_alias_haip}";
        } else {$def_alias = "$sysname"."$prod->{ip_alias_haip}"."-$inf_i";}
    }

    if (Cfg::opt('responsefile') || Obj::webui()) {
        $alias = $cfg->{$sys_name}->{$inf}->{hostname_for_haip};
    } else {
        $question = Msg::new("Enter Hostname alias for the above IP address:");
        $alias =  $question->ask($def_alias, $help, $backopt);
        $cfg->{$sys_name}->{$inf}->{hostname_for_haip} = $alias;
    }
...
sub is_ip_valid {
    ...
    if ($is_ipv4) {
        ...
        if ($is_mask) {
            ...
            return 0 if ($ip =~ /^(255|0[0-9]*)\.|\.0[0-9]+/m) ;
        }
...
sub save_haip_configuration {
    my ($prod, $is_ipv4, $systems_r, $src_nodes_ref, $dst_nodes_ref, $add_ip_alias) = @_;
    my ($sys, $inf, @ip_parts, $ip_entry, $msg, $ret);
    my ($ip, $alias, $atleast_one);
    my %systems = %$systems_r;
    my $tmpdir=EDR::tmpdir();
    my $nicfileaddr = '';
    my $nic_ip_conf = '';
    ...
    for my $sys (@{$src_nodes_ref}) {
        for my $inf (@{$systems{$sys->{sys}}{inf}}) {
            $msg = Msg::new("Saving configuration for $inf  on $sys->{sys}");
            $msg->left;
            $ip = $systems{$sys->{sys}}{ip}{$inf};
            $ret = $sys->configure_static_ip($inf, $ip, $prod->{priv_mask});
            $ret ? Msg::right_done() : Msg::right_failed();
            if (!$ret){
                $msg = Msg::new("Failed to configure static ip on $sys->{sys}");
                $msg->print;
                Msg::prtc();
                return 1;
            }
            next unless ($add_ip_alias);
            $alias = $systems{$sys->{sys}}{alias}{$inf};
            $ip_entry = "$ip $alias";
            $ret = $prod->check_hosts_file($alias, $dst_nodes_ref);
            if ($ret==0){
                EDR::cmd_local("echo $ip_entry >> $tmpdir/tmp_hosts");
    ...
    if ($atleast_one>0){
       return 0 unless ($is_ipv4);
       my @hosts_files=();
       push @hosts_files,'/etc/hosts';
       (push @hosts_files,$prod->{hosts_file}) if ($prod->{hosts_file} ne '/etc/hosts');
       for my $hosts_file (@hosts_files){
           $ret=$prod->save_hosts_configuration($hosts_file,$dst_nodes_ref);
           return 0 if(!$ret);
       }
       return 1;
    }
...
sub check_hosts_file {
    ...
    for my $sys (@{$nodes_ref}) {
        $ret = $prod->search_word_sys($sys, '/etc/hosts', "$alias", 'alias');
        if ($ret == 0) {
            ...
            next if ($prod->{hosts_file} eq '/etc/hosts');
            # Solaris specific checks
            $ret = $prod->search_word_sys($sys, $prod->{hosts_file}, "$alias", 'alias');
            if ($ret == 0) {
...
sub save_hosts_configuration {
    ...
    for my $sys (@{$nodes_ref}) {
        $sys->cmd("_cmd_cp -pf $hosts_file $hosts_file-$TS");
        if (EDR::cmdexit() != 0) {
            goto done;
        }
    }
...
sub get_plumbed_ips {
    my ($prod, $nodes_ref) = @_;
    my ($tmpdir, $output, $msg);

    $tmpdir=EDR::tmpdir();
    EDR::cmd_local("_cmd_rm $tmpdir/plumbed_ips.txt");

    for my $sys (@{$nodes_ref}) {
#        $output = $sys->cmd("_cmd_ifconfig -a | _cmd_grep inet | _cmd_awk \'{print \$2}\' | _cmd_awk -F\'\/\' \'{print \$1}\'");
        $output = $sys->cmd("_cmd_ifconfig -a | _cmd_awk '/inet/ {print \$2}'");
...
sub is_cfsmount {
    ...
    while ($subdir ne '/') {
        $fs = $firstsys->cmd("$prod->{mount_points} | _cmd_grep $subdir");
        if (EDR::cmdexit() != 0) {
    ...
    #
    $out = $firstsys->cmd("/opt/VRTS/bin/fsclustadm mounttype $fs 2>/dev/null");
    if (EDR::cmdexit() != 0) {
...
sub is_cfsmount_sys {
    ...
    $fs = $firstsys->cmd("$prod->{mount_points} | _cmd_grep $dir");
    if (EDR::cmdexit() != 0) {
...
sub verify_vcsmm {
    my ($prod, $systems_ref) = @_;
    my ($issue_found, $summary, $cmd_ret);
    for my $sys (@{$systems_ref}) {
        $cmd_ret = $sys->cmd("_cmd_gabconfig -a 2>/dev/null | _cmd_grep 'Port o' | _cmd_grep 'membership'");
        if ($cmd_ret eq ''){
            $summary = Msg::new("VCSMM is not configured on $sys->{sys}");
            if (Obj::webui()){
                my $web = Obj::web();
                $web->web_script_form('alert', $summary);
            } else {
                $summary->warning;
            }
            $issue_found++;
...
# Compare db, crs versions with 11grx
sub version_compare_with_11g {
   my ($prod,$req)=@_;
   my $version;
   
   $version=$1 if ($req =~ /^(\d+\.\d+)/mx);
   return 1 if ($version == 11.1); 
   return 2 if ($version >= 11.2);
   return 0;
}

# Validate if the Oracle version is supported
sub validate_oracle_support {
   my ($prod,$release)=@_;
   return 1 if ($release =~ /^(10\.2|11\.1|11\.2|12\.1)$/);
   return 0;
}

# Parse oracle versions
# Return the oracle num if idx=1, return the oracle patch num when idx=2
# Return (oracle_num, oracle_patch_num) when idx=0
sub parse_oracle_version {
   my ($prod,$req,$idx)=@_;
   if ($req =~ /^(\d+)\.(\d+)/mx){
      return eval("\$$idx") if $idx>0;
      return ($1,$2);
   }
}
...
sub install_oracle_clusterware {
    my $prod= shift;
    my ($ret, $msg, $ayn, $compare_with_11g);
    my $cfg = Obj::cfg();
    my $web = Obj::web();
    my $firstsys = Obj::cpic()->{systems}[0];
    ...
    foreach my $a ("config_file","vcsmm","lmx","fencing_mode","crs_install","misscount") {
        push @{$prod->{post_config_check_items}}, $a;
    }
    $prod->{post_config_check_items} = EDRu::arruniq(@{$prod->{post_config_check_items}});
    ...
    if ($prod->verify_vcsmm(CPIC::get('systems'))){
       Msg::prtc();
       return 0;
    }
    ...
    my $temp_user;
    $compare_with_11g = $prod->version_compare_with_11g($prod->{crs_release});
    if (Cfg::opt('responsefile')) {
        # Comment >> check for release.
        $temp_user = ($compare_with_11g==2) ? $cfg->{grid_user}:$cfg->{oracle_user};
    } else {
        $temp_user = $prod->{oracle_user};
    }
    ...
    my $tmpdir=EDR::tmpdir();
    unless ($compare_with_11g == 2) {
        $firstsys->cmd("_cmd_cp $prod->{crs_home}/install.crs_responsefile $tmpdir/install.crs_responsefile") if (!Cfg::opt('responsefile'));
    }
    ...
    # So, let's try to pass no attributes and see if we get any results
        ...
        for my $item (@resname) {
            if ($cfsmnt) {
                ...
                $ret = $sys->cmd("$vcs->{bindir}/hares -value $item CVMDiskGroup $sysname | _cmd_grep -w $ocrdg");
                if (!EDR::cmdexit()) {
...
sub get_voteloc {
    ...
    for my $item (@voteloc) {
        if ($prod->version_compare_with_11g($prod->{crs_release})==2) {
            next if($item =~ /^[#-]/m);
            $item = $1 if ($item =~ /\((.*)\)/m);
        } else {
...
sub get_servgrp {
    ...
    $srvgrp = $sys->cmd("$vcs->{bindir}/hares -display $resname | _cmd_grep -w Group");
    return '' if (EDR::cmdexit());
    chomp($srvgrp);
    @srvgrp = split(/\n/, $srvgrp);
    if ($#srvgrp > 0) {
...
sub get_netresname {
    ...
    my ($nicres,$ret,$privhost,$privip, $msg, $type, @privips,%cssdattrib);
    ...
    if ($prod->version_compare_with_11g($prod->{crs_release})==2) {
        ...
        @privips = split(/,/m, $privips);
    } else {
        $privhost = $sys->cmd("$prod->{crs_home}/bin/olsnodes -p");
        return '' if (EDR::cmdexit());
        $privhost = (split(/\s+/m, $privhost))[1];
        return '' if (!$sys->exists('/etc/hosts'));
        for my $host (split(/,/m, $privhost)) {
            $privip = $sys->cmd("_cmd_cat /etc/hosts 2>/dev/null | _cmd_grep -v '^#' 2>/dev/null | _cmd_grep -w $host");
            if (!EDR::cmdexit() && $privip) {
    ...
    $ret = undef;
    for my $privip (@privips) {
        %cssdattrib = ('Address' => $privip);
        $ret = $prod->get_resname('PrivNIC', \%cssdattrib);
        if (defined $ret && @$ret > 0) {
            last;
        }
    }
    if (defined $ret && @$ret == 0) { # PrivNIC failed, try MultiPrivNIC
        $mpnic = 1;
...
sub check_cps_installed_for_quorum {
    my ($prod) = @_;
    my ($failed,$msg,$syslist,$vers,$cpspkg,$web);
    my $firstnode = @{CPIC::get('systems')}[0];
    $cpspkg = $firstnode->pkg('VRTScps62');
    $web = Obj::web();
...
sub config_cssd_agent {
    my $prod = shift;
    my $cfg = Obj::cfg();
    my ($errstr, $ret, $msg, $ayn, $index, $bs1, $msg_type, $msg_resource, $msg_service, $bs, $msg_pip, $msg_ocr, $msg_vd, $cssd_name);
    my ($cssdres, $question, $answer);
    my $web = Obj::web();
    my $vcs = $prod->prod('VCS62');
    my $sys = @{CPIC::get('systems')}[0];
    ...
    foreach my $a ("config_file","vcsmm","lmx","fencing_mode","db_install","crs_install","relink","misscount") {
        push @{$prod->{post_config_check_items}}, $a;
    }
    $prod->{post_config_check_items} = EDRu::arruniq(@{$prod->{post_config_check_items}});
    ...
    my %cssdattrib = ();
    $ret = $prod->get_resname('CSSD', \%cssdattrib);
    @resname = @{$ret};
    ...
    Msg::n();
    while (1) {
        $question = Msg::new("Enter the name of the CSSD resource:");
        $cssd_name = (Cfg::opt('responsefile')) ? $cfg->{cssd_agent_name} : 'cssd';
        $answer = $question->ask($cssd_name,$help,1);
        return 1 if (EDR::getmsgkey($answer,'back'));
        $cssdres = $answer;
        $ret = $prod->validate_resource($cssdres);
        if ($ret) {
            return 1 if (Cfg::opt('responsefile'));
            next;
        } else {
            $cfg->{cssd_agent_name} = $cssdres unless (Cfg::opt('responsefile'));
            last;
        }
    }
    if (Obj::webui()) {
    ...
    if ($prod->is_crs_down(CPIC::get('systems'))) {
    ...
    my ($ocrvolres, $votevolres, @ocrvolres, @votevolres, $i, @mntpts, $mntpts, $failed_get_ocr_vote_res, $failed_get_ocr_vote_servgrp);
    $failed_get_ocr_vote_res = 0;
    $failed_get_ocr_vote_servgrp = 0;
        ...
        if ($item =~ /\/dev\/vx\//mx) {
            ...
            for my $cfsmtptstr (@mntpts) {
             ...
             #  if (substr($item, 0, length ($cfsmtptstr)) eq $cfsmtptstr) {
                if ($item eq $cfsmtptstr or $item =~ /^$cfsmtptstr\//){
                    $cfsmtpt = $cfsmtptstr;
                    last;
                }
            }
    ...
    if ($failed_get_ocr_vote_res == 1) {
        $msg = Msg::new("There are some issues in finding VCS resource(s) for the OCR or Voting disk. Check the installer logs for more information.");
        $web->web_script_form("alert", $msg) if (Obj::webui());
        $msg->warning;
        Msg::prtc();
        return 1;
    ...
    if ($failed_get_ocr_vote_servgrp == 1) {
        $msg = Msg::new("There are some issues in finding the service group of the VCS resource(s) for the OCR or Voting disk. Check the installer logs for more information.");
        $web->web_script_form("alert", $msg) if (Obj::webui());
        $msg->warning;
        Msg::prtc();
        return 1;
...
SKIP_VOTELOC:
    ...
    $ignore_privnic = 0 if ($prod->{crs_release} < 11.2 || ($prod->{crs_release}=~ /11\.2/m && $prod->{crs_patch_level} <= 0.1));
...
# Show all the information for confirmation
    ...
    if (Obj::webui()) {
        ...
        Msg::bold($bs);
        print "=============================================================\n";
        $msg_pip = Msg::new("Private IP")->{msg} if ($ignore_privnic == 0);
        $msg_ocr = Msg::new("OCR")->{msg};
        $msg_vd = Msg::new("Voting disk")->{msg};
        if ($ignore_privnic == 0){
            $bs = Msg::string_sprintf($bs1, $msg_pip, $nicres, $servicegroup[0]);
...
DNA_CONFIRMCSSDRES:
    ...
    $msg->left;
    $sys->cmd("$vcs->{bindir}/hares -add $cssdres CSSD $servicegroup[0]");
    if (EDR::cmdexit()) {
        Msg::right_failed();
        $msg = Msg::new("Failed to set the CSSD resource ($cssdres). Check the installer logs for more information.");
        if (Obj::webui()) {
             $msg = Msg::new("Failed to set the CSSD resource ($cssdres). Check the installer logs for more information.");
             $web->web_script_form("alert", $msg) if (Obj::webui());
        }
    ...
    if (EDR::cmdexit()) {
        Msg::right_failed();
        $msg = Msg::new("(Failed to set Critical attribute to 0 for the CSSD agent. Check the installer logs for more information.)");
        $web->web_script_form("alert", $msg) if (Obj::webui());
        $msg->print;
        Msg::prtc();
        return 1;
    ...
    if (EDR::cmdexit()) {
        Msg::right_failed();
        $msg = Msg::new("(Failed to override OnlineWaitLimit attribute for the CSSD agent. Check the installer logs for more information.)");
        $web->web_script_form("alert", $msg) if (Obj::webui());
        $msg->print;
        Msg::prtc();
        return 1;
    ...
    if (EDR::cmdexit()) {
        Msg::right_failed();
        $msg = Msg::new("(Failed to set OnlineWaitLimit attribute to 5 for the CSSD agent. Check the installer logs for more information.)");
        $web->web_script_form("alert", $msg) if (Obj::webui());
        $msg->print;
        Msg::prtc();
        return 1;
    }
    $sys->cmd("$vcs->{bindir}/hares -override $cssdres OfflineWaitLimit");
    if (EDR::cmdexit()) {
        Msg::right_failed();
        $msg = Msg::new("(Failed to override OfflineWaitLimit attribute for the CSSD agent. Check the installer logs for more information.)");
        $web->web_script_form("alert", $msg) if (Obj::webui());
        $msg->print;
        Msg::prtc();
        return 1;
    }
    $sys->cmd("$vcs->{bindir}/hares -modify $cssdres OfflineWaitLimit 3");
    if (EDR::cmdexit()) {
        Msg::right_failed();
        $msg = Msg::new("(Failed to set OfflineWaitLimit attribute to 3 for the CSSD agent. Check the installer logs for more information.)");
        $web->web_script_form("alert", $msg) if (Obj::webui());
        $msg->print;
        Msg::prtc();
        return 1;
    }
    $sys->cmd("$vcs->{bindir}/hares -modify $cssdres CRSHOME $prod->{crs_home}");
    if (EDR::cmdexit()) {
        Msg::right_failed();
        $msg = Msg::new("(Failed to set CRSHOME attribute to $prod->{crs_home} for the CSSD agent. Check the installer logs for more information.)");
        $web->web_script_form("alert", $msg) if (Obj::webui());
        $msg->print;
        Msg::prtc();
        return 1;
...
DNA_INSTALL_CRS:
    ...
    while (1) {
        ...
        if (Cfg::opt('responsefile') && $prod->{display} eq '') {
            ...
            $prod->{display} = $cfg->{oracle_display};
        } else {
    ...
    if (!Cfg::opt('responsefile')) {
        ...
        $cfg->{oracle_display} = $prod->{display};
    }
...
sub invoke_oui_installer {
    ...
    my ($msg, $prod_str, $oui_args, $run_cmd, $ret, $sys, $exestr, $line, $crs_up, $prtc, $ayn, $crslog, @line);
    my $tmpdir=EDR::tmpdir();
    my $bs=EDR::get2('tput','bs');
    my $be=EDR::get2('tput','be');
    ...
    if (Cfg::opt('responsefile')) {
        ...
        # for releases prior to 11gR2
        unless ($prod->version_compare_with_11g($release)==2) {
            $firstsys->cmd("_cmd_touch $tmpdir/install.${oraprod}_responsefile");
            $firstsys->cmd("_cmd_chown $temp_user:$prod->{oracle_group} $tmpdir/install.${oraprod}_responsefile");
            $firstsys->cmd("_cmd_chmod 777 $tmpdir/install.${oraprod}_responsefile");
    ...
    $run_cmd = $run_cmd."LC_ALL=C;export LC_ALL;";
    $run_cmd = $run_cmd."$instpath/$prod->{oracle_install_script} $oui_args";

    if (!Cfg::opt('responsefile')) {
        $firstsys->cmd("_cmd_rm -f $tmpdir/install.oracle.${oraprod}.log 2>/dev/null");
    }
    $firstsys->cmd("_cmd_touch $tmpdir/install.oracle.${oraprod}.log");
    if (EDR::cmdexit() != 0) {
    ...
    $msg->log;
    $prod->save_term;
    $firstsys->cmd("_cmd_rm -f $tmpdir/prerootsh 2>/dev/null");
    chdir('/tmp');

    $msg=Msg::new("User is: $temp_user");
    $msg->log;
    $firstsys->cmd("_cmd_su $temp_user -c '$run_cmd' > $tmpdir/install.oracle.${oraprod}.log 2>&1; _cmd_touch $tmpdir/prerootsh");
    ...
    if (Cfg::opt('responsefile') && $oraprod eq 'crs') {
        ...
        while (!$firstsys->exists("$tmpdir/prerootsh")) {
            sleep 10;
        }
        if (!$firstsys->exists("$homedir/root.sh")) {
            Msg::log("root.sh cannot be located on $sys->{sys} at $homedir/");
            return 1;
        }

        # Execute orainstRoot.sh file if prompted by the installer
        $crslog = $firstsys->cmd("_cmd_grep 'orainstRoot.sh' $tmpdir/install.oracle.${oraprod}.log 2>/dev/null");
        @line = split(/\n/,$crslog);
        $exestr = '';
        for my $item (@line) {
            my (@strs) = split(/\s+/m, $item);
            for my $str (@strs) {
                $exestr = $1 if($str =~ /(\/.*\.sh)/m);
            }
        }
        if ($exestr && $firstsys->exists($exestr)) {
            for my $sys (@{CPIC::get('systems')}) {
                if (!$sys->exists($exestr)) {
                    $firstsys->copy_to_sys($sys, $exestr);
                }
        ...
        $line = $firstsys->cmd("$homedir/bin/crs_stat -t 2> /dev/null");
        if (!EDR::cmdexit()) {
            $crs_up = 1;
            while (1) {
                $line = $firstsys->cmd("$homedir/bin/crs_stat -t 2> /dev/null | _cmd_grep 'No resources are registered'");
                if (!EDR::cmdexit()) {
        ...
        $crslog = $firstsys->readfile("$tmpdir/install.oracle.${oraprod}.log");
        @line = grep(/configToolAllCommands/, split(/\n/,$crslog));
        $exestr = '';
        for my $item (@line) {
            my @strs = split(/\s+/m, $item);
            for my $str (@strs) {
        ...
        if ($exestr && $firstsys->exists($exestr)) {
            ...
            $msg->left;
            chdir('/tmp');
            Msg::log("User is: $temp_user");
            $firstsys->cmd("_cmd_su $temp_user -c '$exestr'");
            if (EDR::cmdexit()) {
                Msg::right_failed();
    ...
    if (Cfg::opt('responsefile') && $oraprod eq 'db') {
        ...
        if (!$firstsys->exists("$homedir/root.sh")) {
            Msg::log("root.sh cannot be located on $firstsys->{sys} at $homedir/");
            $web->web_script_form("alert", $msg->{msg}) if (Obj::webui());
            return 1;
        }
    }
    ...
    if ($ret != 0) {
        ...
        $firstsys->cmd("_cmd_grep -Ew 'InternalError|OUI-10025|X11|OUI-10026|OUI-10027|xclock|DISPLAY|X11' $tmpdir/install.oracle.${oraprod}.log > /dev/null");
        if (EDR::cmdexit() == 0) {
            Msg::right_failed();
...
sub install_oracle_database {
    ...
    foreach my $a ("config_file","vcsmm","lmx","fencing_mode","db_install","crs_install","relink") {
        push @{$prod->{post_config_check_items}}, $a;
    }
    $prod->{post_config_check_items} = EDRu::arruniq(@{$prod->{post_config_check_items}});
    ...
    if (Cfg::opt('responsefile')) {
        ...
        unless ($prod->version_compare_with_11g($prod->{db_release})==2) {
            # Copying the responsefile created for DB installation
            $firstsys->cmd("_cmd_cp $prod->{db_home}/install.db_responsefile $tmpdir/install.db_responsefile") if (!Cfg::opt('responsefile'));
        }
...
sub find_grid_user {
    my $prod = shift;
    my $preinst = shift;
    my ($answer,$help,$msg,$backopt,$question,$grid_user);
    my $cfg = Obj::cfg();
    my $ret;
    my $uid = '0';
...
sub find_oracle_group {
    ...
    while (1) {
        ...
        if ($ret == 0) {
            ...
            $msg = Msg::new("Oracle group $answer already exists on a strict subset of the nodes. For using this group for an Oracle installation, it should exist on all the nodes. Create the group manually on those nodes where it is not present, and make the Oracle user a part of it, or choose a different group of which the Oracle user is already a part.");
            $msg->print;
            return 1 if (Cfg::opt('responsefile'));
            ...
            $msg = Msg::new("Oracle group $answer already exists on a strict subset of the nodes. For using this group for an Oracle installation, it should exist on all the nodes. Create the group manually on those nodes where it is not present, and make the Oracle user a part of it, or choose a different group of which the Oracle user is already a part. Restart the current task? (Answering 'No' means you will manually create the user on the remaining nodes before the installation of the Oracle Clusterware/Grid Infrastructure or database)") if(Obj::webui());
            my $ayn = $msg->aynn;
            if ($ayn eq 'Y')
            {
                if(Obj::webui())
...
sub find_installpath {
    ...
    while (1) {
        ...
        if (!$firstsys->exists($answer."/$prod->{oracle_install_script}")) {
            $msg = Msg::new("Cannot find $prod->{oracle_install_script} in ${answer} on $firstsys->{sys}. Input again");
            $msg->print;
            return 1 if (Cfg::opt('responsefile'));
            if (Obj::webui()) {
                $web->web_script_form("alert", $msg->{msg}) if (Obj::webui());
...
DNA_ORACLE_VERSION:
    ...
    while (1) {
        if (Cfg::opt('responsefile')) {
            ...
            $question = Msg::new("Enter Oracle version (e.g. 11.2.0.3, 12.1.0.2):");
        ...
        unless ($prod->validate_oracle_support ($release)){
...
sub find_crs_home {
    ...
    while (1) {
        ...
        if ($answer !~ /^\/+/g) {
            $msg = Msg::new("Enter an absolute path. Input again");
            $msg->print;
            if (Obj::webui()) {
                $web->web_script_form("alert", $msg->{msg}) if (Obj::webui());
                return 1;
        ...
        if ($answer =~ /[^\w\/\.]+/g) {
            $msg = Msg::new("Incorrect path. Input again");
            $msg->print;
            if (Obj::webui()) {
                $web->web_script_form("alert", $msg->{msg}) if (Obj::webui());
                return 1;
        ...
        if ($opr eq 'crs_install') {
            ...
            for my $sys (@{CPIC::get('systems')}) {
                ...
                my $dir_name = EDRu::dirname($answer);
                if ($dir_name eq '/') {
...
sub find_db_home {
    my ($prod, $opr) = @_;
    my ($repeat, $sys, $ret, $str, $msg, $answer,$help,$backopt,$question,$ayn);
    my $cfg = Obj::cfg();
    my $web = Obj::web();
    my $warn_user = 0;
...
sub get_oraver_from_xml {
    ...
    if (!$firstsys->exists($prodxml)) {
        Msg::log("Cannot find '$prodxml' on $firstsys->{sys}");
        return 1;
    }

    if ($oraprod eq 'db') {
        $line = $firstsys->cmd("_cmd_grep 'COMP NAME' $prodxml 2>/dev/null | _cmd_grep 'oracle.server' 2>/dev/null");
    } else {
        $line = $firstsys->cmd("_cmd_grep 'COMP NAME' $prodxml 2>/dev/null | _cmd_grep 'oracle.crs' 2>/dev/null");
    }

    if (EDR::cmdexit()) {
        $line = $firstsys->cmd("_cmd_grep 'PATCHSET NAME' $prodxml 2>/dev/null");
    }
...
sub get_oraver_from_opatch {
    ...
    my ($msg,$release,$patch_level,$prodxml,$orauser,$home,$question,$deforauser);
    if ($oraprod eq 'crs') {
        $home = $prod->{crs_home};
    } elsif ($oraprod eq 'db') {
        $home = $prod->{db_home};
...
#   my $optach_script = "$prod->{db_home}/OPatch/opatch";
    ...
    if (!$firstsys->exists($optach_script)) {
        Msg::log("Cannot find '$optach_script' on $firstsys->{sys}");
        return 1;
    }

    my $defval = $firstsys->cmd("_cmd_ls -al $optach_script | _cmd_awk '{print \$3}' 2> /dev/null");
    if (EDR::cmdexit()) {
    ...
    chdir('/tmp');
    my $line = $firstsys->cmd("cd /opt/VRTS/install; "."_cmd_su $orauser -c '_cmd_sh $optach_script lsinventory -oh $home 2> /dev/null' | _cmd_grep '$productstr'");

    if (EDR::cmdexit()) {
        $msg = Msg::new("Problem determining Oracle Version on $firstsys->{sys}");
        $msg->log;
        return 1;
    }
    my @strings=split(/\n+/,$line);
...
sub validate_orauser {
    ...
    for my $sys (@{$systems_ref}) {
        $noorausr{$sys} = 1;
        $passwdfile_usr = $sys->cmd("_cmd_grep '^$orauser:' /etc/passwd 2>/dev/null");
        if ($passwdfile_usr) {
            $noorausr{$sys} = 0;
            $noorausr = 0;
            $nsystems++;
    ...
    if ($difforauid) {
        ...
        if ($nsystems == $csystems) {
            ...
            if ($prod->determine_group($systems_ref->[0], $orauser)) {
                Msg::log("Error in determining the group for the Oracle user $orauser on $systems_ref->[0]{sys}");
                return 1; # As it's equivalent of the case when we return 1 (See above)
            }
            Msg::log("Oracle user $orauser already exists on all the nodes with the group $prod->{oracle_group} (GID: $prod->{oracle_gid})");
...
sub relink_oracle_database {
    ...
    if ($ret == 1) {
        my $dbs_up = join("\n\t",@{$prod->{db_up}});
        $msg = Msg::new("\nThe following Oracle instances are running on one or more node(s) of your cluster.\nStop them before relinking Oracle.\n\t$dbs_up");
        $msg->log;
        $msg->print;
        $web->web_script_form("alert", $msg->{msg}) if (Obj::webui());
        Msg::prtc();
        ...
        my $dbs_unknown = join("\n\t",@{$prod->{db_unknown}});
        $msg = Msg::new("\nFailed to determine the status of the following Oracle Databases. Shutdown these Oracle Databases on all nodes of your cluster if they are up before proceeding further. Check the logs for more information.\n\t$dbs_unknown");
        $msg->print;
        $mesg = $msg->{msg};
    ...
    # 1. Avoid the relink menu to be choiced again while using responsefile 
    # 2. Keep relink the libraries during rolling upgrade phase 1
    $cfg->{relink_oracle_database} = 0 if (Cfg::opt('responsefile') && $rel->{ru_phase1_done});
    $prod->{relink_done_for_subcluster} = 1;

    return 0;
}

# returns the number of systems on which CRS is running.
sub is_crs_up {
    my ($prod, $systems_ref) = @_;
    my @crs_up_systems = $prod->get_crs_up_systems($systems_ref);

    return scalar(@crs_up_systems);
}

# returns the number of systems on which CRS is not running.
sub is_crs_down {
    my ($prod, $systems_ref) = @_;
    my @crs_up_systems = $prod->get_crs_up_systems($systems_ref);

    return (scalar(@{$systems_ref})-scalar(@crs_up_systems));
}

sub get_crs_up_systems {
    my ($prod, $systems_ref) = @_;
    my $msg;
    my @crs_up_systems=();
    for my $sys (@{$systems_ref}) {
        ...
        if (!EDR::cmdexit()) {
            $msg = Msg::new("Running");
            $msg->right;
            push @crs_up_systems, $sys->{sys};
        } else {
            $msg = Msg::new("Not running");
            $msg->right;
        }
    }
    return @crs_up_systems;
}
...
sub verify_libskgxn {
    ...
    my ($sys, @cpnodes, $msg, $crs_skgxn, $opt_skgxn);
    my $failed = 0;

    $crs_skgxn = $crs_home.'/lib/'.$skgxn_name;
    $opt_skgxn = '/opt/ORCLcluster/lib/'.$skgxn_name;
    @cpnodes = @{CPIC::get('systems')};
    for my $sys (@cpnodes) {
        $msg = Msg::new("Verifying skgxn library on $sys->{sys}");
        $msg->left;
        $sys->cmd("_cmd_ls $crs_skgxn");
        if (!EDR::cmdexit()) {
            $sys->cmd("$prod->{cmd_nm_64} $crs_skgxn 2>/dev/null | _cmd_grep vcsmm");
            if (!EDR::cmdexit()) {
                $opt_skgxn = '/etc/ORCLcluster/lib/'.$skgxn_name if (!$sys->exists($opt_skgxn));
                $sys->cmd("_cmd_diff $opt_skgxn $crs_skgxn 2>/dev/null");
                if (!EDR::cmdexit()) {
                    Msg::right_done();
                    next;
                }
        ...
        $msg = Msg::new("Not linked to the Symantec Membership library");
        $msg->right;
        $failed = 1;
    }
    return $failed;
...
sub is_db_up {
    ...
    my ($s, $sys, $crs_up, $result, $dbname, @instance_lines, $i);
    my $web = Obj::web();
    $db_home =~ s/\/$//;;
    ...
    if (!$crs_up) {
        Msg::log("Oracle Clusterware/Grid Infrastructure is down on all nodes, so declaring Oracle Database down");
        #$web->web_script_form("alert", $msg->{msg}) if (Obj::webui());
        Msg::right_done();
        return 0;
    }
    ...
    if (EDR::cmdexit() != 0) {
        ...
        $output = $sys->cmd("$prod->{crs_home}/bin/crs_stat 2> /dev/null | _cmd_grep 'NAME'");
        @lines=split(/\n/, $output);
        for my $item (@lines) {
            ...
            if ($fields[2] eq 'db') {
                ...
                if ($srvctl_output eq $db_home) {
                    ...
                    if (EDR::cmdexit() != 0) {
                        ...
                        push @{$prod->{db_unknown}}, $dbname;
                    }
                    @instance_lines = split(/\n/,$srvctl_output);
                    for my $s (@{CPIC::get('systems')}) {
                        for my $i (@instance_lines) {
                            if ($i =~ /is running on node $s->{sys}/m) {
                                push @{$prod->{db_up}}, $i;
                                last;
                            }
                        }
...
sub addnode_menu_sfrac {
    ...
    my ($backopt, $def, $help, $ret, $msg, $def_resp);
    ...
    $def = 1; # Default option in the menu
    $def_resp  = 1 if (Cfg::opt('responsefile'));
    $help = '';
    $backopt = 0;
    @menuopts = ();
    $menulist = [];
    ...
    push(@menuopts, 'addnode_config_haip');
...
#    push(@menuopts, "addnode_install_oracle_crs_and_db");
    ...
    while(1) {
        if (Obj::webui()) {
            ...
            $msg = Msg::new("Choose option:");
            $def = $def_resp if (Cfg::opt('responsefile'));
            $choice = $msg->menu($menulist, $def, $help, $backopt);
            $choice = @menuopts[(0+$choice) - 1];
            Msg::n();
        ...
        if ($choice eq 'finish') {
            ...
            $prod->addnode_mount_share_dg();
            $prod->addnode_comp_message();
            last;
        } else {
            $ret = $prod->$choice();
            $def_resp++ if (Cfg::opt('responsefile'));
        }
        Msg::title();
    }
    return;
...
sub addnode_start_procs {
    ...
    for my $sys (@{$prod->{newnodes}}) {
        ...
        my $smallsys = Prod::VCS62::Common::transform_system_name($firstnode->{sys});
        my $vcs = $prod->prod('VCS62');
        my $cssdname = $firstnode->cmd("$vcs->{bindir}/hares -list Type=CSSD  2>/dev/null");
        if ($cssdname ne  '') {
...
sub addnode_online_cvm_group {
    my $prod = shift;
    my ($sys, $ret, $firstnode, $msg, $rtn, $cssdres);
    my $vcs = $prod->prod('VCS62');
    my $web = Obj::web();
    ...
    #If the cssd resource is configured as a critical resource,
    #the cvm group will be brought offline on the new node. Modify
    #the configuration to make the cssd resource non-critical and bring the cvm group online.
    #$rtn = $firstnode->cmd("$vcs->{bindir}/haconf -makerw");
    #$rtn = $firstnode->cmd("$vcs->{bindir}/hares -modify $cssdres Critical 0");
    #$rtn = $firstnode->cmd("$vcs->{bindir}/haconf -dump -makero");
...
# Mount share dg in case it's not done because of cvm wasn't online
# A possible reason that cvm is not online is that the resource in cvm group
# were not configured for new nodes when it tried to online cvm
sub addnode_mount_share_dg {
    my $prod = shift;
    my $cfg = Obj::cfg();
    my ($rtn, $firstnode, $sharevol, @sharevols, $sfcfsha, $sys, $system, $cvmgroupstate, $mount_point, $msg);

    $firstnode = @{$prod->{clusnodes}}[0];
    $sfcfsha = $prod->prod('SFCFSHA62');
    $rtn = $sfcfsha->addnode_need_mount_sharedg();
    return $rtn if ($rtn == 1);
    return unless scalar(@{$rtn});
    ...
    for my $sysi (@{$cfg->{newnodes}}) {
        ...
        # $sys->{mount_share_dg} is set in addnode_mount_share_dg subroutine
        if ($cvmgroupstate eq 'ONLINE' && $sys->{mount_share_dg}) {
            for my $mount_info (@sharevols) {
                $sharevol = $mount_info->[0];
                $mount_point = $mount_info->[1];
                next if $sfcfsha->is_ioship_on_and_ldisabled_sys($sys,$sharevol,$mount_info->[2], {});
...
sub addnode_comp_message {
    ...
    my $smallsys = Prod::VCS62::Common::transform_system_name($sys->{sys});
    my $vcs = $prod->prod('VCS62');
    my $cssdname = $sys->cmd("$vcs->{bindir}/hares -list Type=CSSD  2>/dev/null");
    if ($cssdname ne  '') {
...
sub addnode_config_privnic {
    ...
    return 0 if (Cfg::opt('responsefile') && $cfg->{addnode_config_privnic} == 0);
    $cfg->{addnode_config_privnic} = 1;
...
sub addnode_config_multiprivnic {
    ...
    return 0 if (Cfg::opt('responsefile') && $cfg->{addnode_config_multiprivnic} == 0);
    $cfg->{addnode_config_multiprivnic} = 1;
    $prod->config_multiprivnic_common(1, $prod->{allnodes}, $prod->{clusnodes}, $prod->{newnodes});
    return;
}

sub addnode_config_haip {
    my $prod = shift;
    my ($ret, $web, $msg);;
    my $cfg = Obj::cfg();

    $web = Obj::web();
    return 0 if (Cfg::opt('responsefile') && $cfg->{addnode_config_haip} == 0);
    $cfg->{addnode_config_haip} = 1;
    $ret = $prod->config_haip_common(1, $prod->{allnodes}, $prod->{clusnodes}, $prod->{newnodes});
    if ($ret && Obj::webui()) {
        $msg = Msg::new("HAIP configuration failed. Refer to the log file for details");
        $web->web_script_form("alert",$msg->{msg});
    }
...
sub addnode_mount_ocr_and_vote {
    my $prod = shift;
    my ($sys, $msg, $question, $answer, $help, $ayn, $ret, $web);
    ...
    return 1 if ($prod->determine_crs_home('post_crs_install'));
    if ($prod->is_crs_down($prod->{clusnodes})) {
    ...
    $firstsys->cmd("_cmd_ls $prod->{crs_home}/bin/crsctl 2> /dev/null");
    if (EDR::cmdexit() != 0) {
    ...
    $firstsys->cmd("_cmd_ls $prod->{crs_home}/bin/ocrcheck 2> /dev/null");
    if (EDR::cmdexit() != 0) {
    ...
    if (($vote_loc_parts[1] eq 'dev') && ($ocr_loc_parts[1] eq 'dev')) {
        ...
        if ($vote_loc_parts[1] ne 'dev') {
            $vote_mntp = $prod->addnode_get_mntpt($vote_loc, $firstsys);
        }

        if ($ocr_loc_parts[1] ne 'dev') {
            $ocr_mntp = $prod->addnode_get_mntpt($ocr_loc, $firstsys);
        }
    ...
    if ($is_loc_same) {
        if ($vote_loc_parts[1] ne 'dev') {
            $ocrvote_dev = $prod->addnode_get_mntdev($vote_mntp, $firstsys);
            if ($ocrvote_dev eq '') {
        ...
        if ($ocr_loc_parts[1] ne 'dev') {
            $ocr_dev = $prod->addnode_get_mntdev($ocr_mntp, $firstsys);
            if ($ocr_dev eq '') {
        ...
        if ($vote_loc_parts[1] ne 'dev') {
            $vote_dev =$prod->addnode_get_mntdev($vote_mntp, $firstsys);
            if ($vote_dev eq '') {
...
sub addnode_get_mntpt {
    ...
    $mntpt = $firstsys->cmd("$prod->{mount_points} | _cmd_grep $path");
    if (EDR::cmdexit()) {
        return '';
    }
    return $mntpt;
...
sub addnode_online_CFSMount_res {
    ...
    if (EDR::cmdexit() == 0) {
        $cmd = "$vcs->{bindir}/hares -display -type CFSMount | _cmd_awk '/MountPoint/ {print \$4}' | _cmd_grep -w '$mpoint'";
        $sys->cmd("$cmd");
        if (EDR::cmdexit() == 0) {
...
sub addnode_check_resource {
    ...
    if (EDR::cmdexit() == 0) {
        $cmd = "$vcs->{bindir}/hares -display -type $res_type | _cmd_grep Device | _cmd_grep -v DeviceTag | _cmd_awk '{print \$1}' | _cmd_uniq";
        $res_names = $sys->cmd("$cmd");
        chomp($res_names);
        @tmp_rsrcs = split(/\n/, $res_names);

        for my $rsrc (@tmp_rsrcs) {
            $cmd = "$vcs->{bindir}/hares -display $rsrc | _cmd_grep Device | _cmd_grep -v DeviceTag";
            $cmdout = $sys->cmd("$cmd");
            if ($cmdout =~ /global/) {
                next;
            }
            $cmd = "$vcs->{bindir}/hares -display $rsrc | _cmd_grep Device | _cmd_grep -v DeviceTag | _cmd_grep $sysname | _cmd_awk '{print \$4}'";
            $ip_addr = $sys->cmd("$cmd");
            chomp($ip_addr);
            if ($ip_addr eq '') {
                push(@resources, $rsrc);
...
sub addnode_create_oracle_user_group {
    ...
    return 0 if (Cfg::opt('responsefile') && $cfg->{addnode_create_oracle_user_group} == 0);
    $cfg->{addnode_create_oracle_user_group} = 1;
    Msg::title();
    ...
    if ($prod->{oracle_sgroups}) {
        $msg = Msg::new("Do you want to configure secondary group(s) for the Oracle user $prod->{oracle_user} on the new nodes?");
        $ayn= $msg->ayny;
        if ($ret eq 'N') {
            return 0;
        }
...
sub addnode_find_oracle_user_group {
    my ($prod, $nodes_ref) = @_;
    my ($answer, $help, $msg, $backopt, $question, $ret, $cfg);
    my $web = Obj::web();
    ...
    $prod->{oracle_user} = $cfg->{oracle_user} if (Cfg::opt('responsefile'));
    while (1) {
        ...
        if ($ret == 0) {
            ...
            #
            $prod->{oracle_user} = $answer;
            $cfg->{oracle_user} = $answer unless (Cfg::opt('responsefile'));
...
sub addnode_validate_oragroup {
    ...
    my ($msg, $sys, $ret,$groupfile_entry);
    ...
    for my $sys (@{$systems_ref}) {
        ...
        if ($passwdfile_entry) {
            ...
            if (!$sys->is_dir("$homedir")) {
                $basedir = EDRu::dirname($homedir);
                $sys->cmd("_cmd_mkdir -p $basedir");
                ...
                $sys->cmd("_cmd_chown $uid:$gid $homedir");
...
sub addnode_conf_sgroups_on_new_nodes {
    ...
    my ($sys, $firstsys, $ret, $err, $msg);
    my $web = Obj::web();

    $err = 0;
    $firstsys = @{$clusnodes_ref}[0];
    @groups = split(/\s/m, $oragroups);
    for my $grp (@groups) {
        $groupfile_entry = $firstsys->cmd("_cmd_cat /etc/group | _cmd_grep '^$grp:'");
        if ($groupfile_entry) {
    ...
    if (!$err) {
        $msg = Msg::new("Successfully configured all the secondary group(s) for the Oracle user $orauser on the new nodes.");
        $web->web_script_form("alert",$msg->{msg}) if (Obj::webui());
        $msg->print;
    }
...
sub prepucheck {
    ...
    # "Running the product specific checking for Symantec Storage Foundation for Oracle RAC 5.1"
    #
...
#    $web->{completed} = 0;
    ...
    if (Sys::exists($nodes[0], '/etc/llttab')) {
        $devudp = $nodes[0]->cmd("_cmd_awk '/udp/ {print \$3}' /etc/llttab | _cmd_uniq ");
    }
...
#    }
    ...
    if ($dbac_present) {
        ...
        if (!($nodes[0]->linux()||($nodes[0]->sunos_sol11()))) {
    ...
    if($do_oracle_checks == 0){
        ...
        if ($dbac_present){
            unless ($prod->version_compare_with_11g($prod->{crs_release})==2) {
...
SKIPPED_VCSMM:
    ...
    #
    # Perform only if SFRAC is installed.
    goto SKIPPED_LMX if ($is_sfcfsrac || $nodes[0]->linux() || ($nodes[0]->sunos_sol11()));
    if ($dbac_present) {
...
sub get_oracle_info {
    ...
    for my $item (@users) {
        ...
        if ($sys->exists("/etc/profile")) {
            $rc = $sys->cmd("_cmd_awk '/ORACLE_HOME/ {print \$2}' /etc/profile 2>/dev/null");
        }

        if (($rc eq '') && ($sys->exists("$userdir/.profile"))) {
            $rc = $sys->cmd("_cmd_awk '/ORACLE_HOME/ {print \$2}' $userdir/.profile 2>/dev/null");
        }

        if (($rc eq '') && ($sys->exists("$userdir/.bash_profile"))) {
            $rc = $sys->cmd("_cmd_awk '/ORACLE_HOME/ {print \$2}' $userdir/.bash_profile 2>/dev/null");
        }
        ...
        $rc = '';
        if ($sys->exists("/etc/profile")) {
            $rc = $sys->cmd("_cmd_awk '/CRS_HOME/ {print \$2}' /etc/profile 2>/dev/null");
        }

        if (($rc eq '') && ($sys->exists("$userdir/.profile"))) {
            $rc = $sys->cmd("_cmd_awk '/CRS_HOME/ {print \$2}' $userdir/.profile 2>/dev/null");
        }

        if (($rc eq '') && ($sys->exists("$userdir/.bash_profile"))) {
            $rc = $sys->cmd("_cmd_awk '/CRS_HOME/ {print \$2}' $userdir/.bash_profile 2>/dev/null");
        }
...
sub check_time_sync_sys {
    ...
    my ($msg, $ii, $flag, @diff, $timer_diff);
    ...
    for my $sys (@{$syslistref}) {
        eval {$timer_sin = $sys->cmd('_cmd_date -u +%S:%M:%H:%d:%m:%Y');};
        $errstr = $@;
        if ($errstr) {
...
sub check_archtype_sys {
    ...
    my ($item, $desc, $status, $summary, $arrayref);
    my (%arch_type, $para, @diff, $flag, $ii, $mismatch_list, $precheck,$ret);
    my $msg_check;
...
sub check_cpuspeed_sys {
    ...
    for my $sys1 (@{$syslistref}) {
        for my $sys2 (@{$syslistref}) {
            ...
            #fix for e2947846 - make precheck allow slight different cpu speed
            if (abs($cpu_speed{$sys1} - $cpu_speed{$sys2}) > 10){
                $diff[$ii] = "$sys1->{sys}:$sys2->{sys}";
                $ii = $ii + 1;
                $flag = 1;
            }
...
sub check_oradblevel_sys {
    ...
    $msg->log;
    my (%orahome, $orahome, %oraversions, $flag);
    my ($item, $desc, $status, $summary, $ret);
    $status = 1;
    ...
    for my $sys2 (@{$syslistref}) {
        eval {$oraversions{$sys2} = $sys2->cmd("cd /opt/VRTS/install; "."_cmd_su $prod->{orauser} -c '_cmd_sh $orahome{$sys2}/OPatch/opatch lsinventory -oh $orahome{$sys2} 2>/dev/null' | _cmd_grep 'Oracle Database'");};
        $errstr = $@;
        if ($errstr) {
...
sub check_fencing_sys {
    ...
    my $is_cps_fence= $firstsys->cmd(" _cmd_vxfenadm -d | _cmd_grep 'Fencing Mechanism' | _cmd_grep 'cps' ");
    ...
    my $fencps = $firstsys->cmd("vxfenconfig -l | _cmd_grep '\\[' | awk '{print \$1}'");
    my @fencps = split(/\n/,$fencps);

    my $fqdn = hostfqdn();
    my $export_cmd;
    my $sercurity = $firstsys->cmd("vxfenconfig -l | _cmd_grep 'security'");
    if($sercurity ne ''){
    ...
    for my $cps (@fencps) {
        ...
        $firstsys->cmd($command);

        if (EDR::cmdexit()) {
            $msg = Msg::log("Problem executing the command: $command on $firstsys->{sys}");
    ...
    my $fencps_disks = $firstsys->cmd("_cmd_cat $prod->{vxfentab} | _cmd_grep -v '^#' | _cmd_grep '\/dev'");
...
sub check_gabports_sys {
    ...
    my $syslistref = shift;
    my $is_sfcfsrac = shift;
    my ($overall_status, $master, $up_port_cnt, $msg);
    my $msg_check;
    ...
    my $stage = Msg::new("Installation and Configuration Checks");
    eval {$master = $temp_sys->cmd("_cmd_vxdctl -c mode 2>/dev/null | _cmd_awk '/master:/ {print \$2}'");};
    #just get the package information
    for my $sys (@{$syslistref}) {
        $status = 1;
...
sub check_lmx_sys {
    ...
    my $temp_sys = shift; # target host of the checking
    my $syslistref = shift;
    my ($overall_status,$msg, $msg_check);
...
sub check_sfracoracleintegration_sys {
    ...
    my ($overall_status, $vcs, $errstr, $is_sfcfsrac, $dbac_present, $msg, $retval);
    my $msg_check;
    ...
    for my $sys (@{$syslistref}) {
        ...
        # Check init.cssd patch. Not applicable for SFCFSRAC and 11gR2 or later.
        goto OPROCD_CHECK if ($is_sfcfsrac || $prod->version_compare_with_11g($prod->{oraver})==2);
...
CRS_END:
        if ($prod->version_compare_with_11g($prod->{crs_release})==2) {
            $msg = Msg::new("Skipped node ID mismatch check for Oracle 11gR2 and later");
            $msg->log;
...
SFRAC_LIBLINK_CHECK:
        ...
        $msg_check = Msg::new("Checking Tunables for VCSMM on $sys->{sys}");
        $stage->display_left($msg_check);
        ...
        $msg_check = Msg::new("Checking whether cssd resource exists in CVM service group on $sys->{sys}");
        $stage->display_left($msg_check);
        my $smallsys = Prod::VCS62::Common::transform_system_name($sys->{sys});
        my $vcs = $prod->prod('VCS62');
        my $cssdname = $sys->cmd("$vcs->{bindir}/hares -list Type=CSSD  2>/dev/null");
...
sub get_oracle_information {
    ...
    my ($crshome);
    my ($ayn,$aynn);
    my ($done,$compare_with_11g,$oracle_support);
    my ($orauser, $question, $deforauser, $answer, $backopt, $help, $oraprod, @digits, $release, $ret);
    my $ask_web = 0;
    my $web = Obj::web();
    $prod = shift;
    $sys = shift;
    ...
    if ($dbhome ne '') {
        ...
        $msg->log;
        $crshome = $sys->cmd("_cmd_cat $ora_inventory/ContentsXML/inventory.xml 2>/dev/null | _cmd_grep CRS | _cmd_cut -d' ' -f3 | _cmd_cut -d'=' -f2") if (!$crshome);
        $crshome =~ tr/"//d;
        if(Obj::webui()){
...
WEB_ASK_ORA_INFO:
    ...
    while (($crshome eq '') && (!$done)) {
        ...
        $compare_with_11g = $prod->version_compare_with_11g($prod->{oraver});
        # Check if ORA_CRS_HOME is valid on all the cluster nodes.
        for my $sys (@{$syslistref}) {
            if (!$sys->is_dir($crshome)) {
                if ($compare_with_11g == 2) {
    ...
    if ( $prod->{crs_release} eq '' ) {
        ...
        if ($ret) {
            ...
            if (Obj::webui()){
                ...
                $question = Msg::new("Enter Oracle Clusterware Version (e.g. 11.2.0.3, 12.1.0.1):");
                $answer = $question->ask($prod->{$oraprod.'_release'.'.'.$oraprod.'_patch_level'}, $help, $backopt);
            }
            chomp($answer);
...
sub check_tunables_sys {
    ...
    if (!$sys->exists($prod->{vcsmmconfig})) {
        if ($sys->hpux()) {
            ...
            $msg->log;
            $mm_slave_max = $sys->cmd("_cmd_kctune -m vcsmm  2>/dev/null | _cmd_awk '/mm_slave_max/ {print \$3}'");
        } else {
        ...
        $mm_slave_max = $sys->cmd("_cmd_cat $prod->{vcsmmconfig} 2>/dev/null | _cmd_grep '^mm_slave_max=' 2>/dev/null | _cmd_cut -d '=' -f2");
...
sub check_lltprechecks_sys {
    ...
    my ($item,$desc,$cfg);
    my ($overall_status, $prod, $msg, @diff, $ii, $mismatch_list, $status, $summary);
    my $msg_check;
...
sub check_cross_connection_sys {
    ...
    my ($command, $macaddr, $pid, $status, $result, $prod, $msg, $tmp);
...
sub check_lltprivnetwork_sys {
    my ($sys, $syslistref, $ii, $ret, $devudp);
    ...
    my ($overall_status, $vcs, $prod, $data, $plat, $errstr,$msg, $cfg);
    my $msg_check;
    my $firstsys = Obj::cpic()->{systems}[0];
...
LLT_LINK_CHECK_EXIT:
    ...
    $firstsys->cmd('_cmd_rm -rf /tmp/llthosts 2>/dev/null');
    $firstsys->cmd('_cmd_mkdir -p /tmp/llthosts 2>/dev/null');
    for my $sys (@{$syslistref}) {
        if ($sys->exists($vcs->{llthosts})) {
    ...
    for my $sys (@{$syslistref}) {
        Msg::log("Transport used is : $sys->{rcp}");
        $sys->copy_to_sys($firstsys,$vcs->{llthosts},"/tmp/llthosts/$sys->{sys}");        # Copies from remote system $sys to local system.
    }
    ...
    for my $sys1 (@{$syslistref}) {
        for my $sys2 (@{$syslistref}) {
            ...
            #remove blank lines
            $firstsys->cmd("_cmd_awk '/./' /tmp/llthosts/$sys1->{sys} > /tmp/llthosts/$sys1->{sys}.tmp 2> /dev/null");
            $firstsys->cmd("_cmd_awk '/./' /tmp/llthosts/$sys2->{sys} > /tmp/llthosts/$sys2->{sys}.tmp 2> /dev/null");
            $firstsys->cmd("_cmd_mv /tmp/llthosts/$sys1->{sys}.tmp  /tmp/llthosts/$sys1->{sys} 2> /dev/null");
            $firstsys->cmd("_cmd_mv /tmp/llthosts/$sys2->{sys}.tmp  /tmp/llthosts/$sys2->{sys} 2> /dev/null");

            $firstsys->cmd("_cmd_diff -b /tmp/llthosts/$sys1->{sys} /tmp/llthosts/$sys2->{sys} 2>/dev/null");
            if (EDR::cmdexit() == 0) {
...
LLTHOSTS_CHECK_EXIT:
    ...
    if ($flag) {
        ...
        $msg = Msg::new("==========================");
        $msg->log;
        $msg = Msg::new("Cluster-ID differences between:");
        $msg->log;
        for ($ii = 0; $ii <= $#diff ; $ii++ ) {
            $msg = Msg::new($diff[$ii]);
            $msg->log;
...
sub check_user_nobody{
    ...
    my ($prod, $sys, $syslistref,$msg);
    my ($overall_status, $user_nobody, $status, $msg_check);
...
sub check_oracle_perm {
    ...
    my ($ora_user, $oracle_arr, $tmpstr, $summary, $cmd_ret, $ret);
...
package Prod::SFRAC62::HPUX;
@Prod::SFRAC62::HPUX::ISA = qw(Prod::SFRAC62::Common);

sub init_plat {
    my $prod=shift;
    $prod->{allpkgs}=[ qw(VRTSob34 VRTSdbac62 VRTSgms62 VRTSodm62 VRTSdbed62 VRTSvcsea62) ];
    $prod->{minpkgs}=[ qw(VRTSdbac62 VRTSgms62 VRTSodm62 VRTSdbed62 VRTSvcsea62) ];
    $prod->{recpkgs}=[ qw(VRTSob34 VRTSdbac62 VRTSgms62 VRTSodm62 VRTSdbed62 VRTSvcsea62) ];
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        VRTSdbckp VRTSormap VRTScsocw VRTSdbac62 VRTSvbs62 VRTSvcsea62 VRTScfsdc VRTSodm62
        VRTSgms62 VRTScavf62 VRTSglm62 VRTScpi VRTSd2doc VRTSordoc
        VRTSfsnbl VRTSfppm VRTSap VRTStep VRTSspc VRTSspcq
        VRTSfasdc VRTSfasag VRTSfas VRTSgapms VRTSmapro VRTSvail
        VRTSd2gui VRTSorgui VRTSvxmsa VRTSvrdev VRTSdbdoc VRTSdb2ed VRTSdbed62
        ...
        VRTScutil VRTSvcsdc VRTSvcsmn VRTSvcsmg VRTSvcsag62
        VRTScps62 VRTSvcs62 VRTSamf62 VRTSvxfen62 VRTSgab62 VRTSllt62
        ...
        VRTSjre15 VRTSjre VRTSsfcpi62 VRTSvlic32 VRTSperl516 VRTSwl
    ) ];
...
sub determine_group {
    ...
    $ret = $sys->cmd("_cmd_groups -p $user");
    chomp($ret);
    $prod->{oracle_group} = $ret;
    $stat = $stat || EDR::cmdexit();

    $ret = $sys->cmd("_cmd_groups $user");
    chomp($ret);
    $pgroup = $prod->{oracle_group};
    $ret =~ s/$pgroup//g;
    $prod->{oracle_sgroups} = $ret;
...
sub addnode_get_group {
    ...
    $grp = $sys->cmd("_cmd_groups -p $user");
    chomp($grp);
...
sub get_plumbed_ips {
    ...
    EDR::cmd_local("_cmd_rm $tmpdir/plumbed_ips.txt");
    for my $sys (@{$nodes_ref}) {
        $output = $sys->cmd("_cmd_netstat -in | _cmd_grep -v Address | _cmd_grep -v none | _cmd_awk \'{print \$4}\'");
        EDR::cmd_local("echo \'$output\' >> $tmpdir/plumbed_ips.txt");
        if (EDR::cmdexit() != 0) {
            $msg = Msg::new("Failed to save plumbed ips in $tmpdir/plumbed_ips.txt");
            $msg->print;
    ...
    if ((($release eq '10.2') && ($patch_level eq '0.1'))) {
        if ($ENV{'SKIP_ROOTPRE'} eq 'TRUE') {
            ...
            for my $sys (@{CPIC::get('systems')}) {
                ...
                $msg->left;
                $firstsys->copy_to_sys($sys,"$instpath/rootpre","$tmpdir/rootpre");
...
sub relink_oracle {
    ...
    my @tmp_release = $prod->parse_oracle_version($prod->{db_release});
    my $xr = ($tmp_release[0]=="12") ? "cR":"gR";
    my $release = "$tmp_release[0]$xr$tmp_release[1]";
...
sub link_libskgxn {
    my ($prod, $skgxn_name, $crs_home) = @_;
    my ($sys, @cpnodes, $msg, $cmd, $mach_hardware, $so_ext, $nmapidir_64, $output);
    $sys = ${CPIC::get('systems')}[0];
    $mach_hardware = $sys->cmd('_cmd_uname -m 2>/dev/null');
    if ($mach_hardware eq 'ia64') {
        $so_ext = 'so';
    ...
    if ($crsup) {
        $msg = Msg::new("\nOracle Clusterware/Grid Infrastructure should be linked with the Symantec Membership library. To link the Oracle Clusterware/Grid Infrastructure, the Oracle Clusterware/Grid Infrastructure will be stopped.");
        $msg->print;
        $msg = Msg::new("Do you want to continue?");
        my $ayn = $msg->ayny;
        if ($ayn eq 'n' || $ayn eq 'N') {
            $msg = Msg::new("\nExecute the following commands on all the cluster nodes:\n 1. Stop the Oracle Clusterware/Grid Infrastructure\n\t $crs_home/bin/crsctl stop crs \n 2. Link the Symantec Membership library to the Oracle Clusterware/Grid Infrastructure home \n\t ln -s $nmapidir_64/$skgxn_name $prod->{crs_home}/lib/libskgxn2.$so_ext \n 3. Start the Oracle Clusterware/Grid Infrastructure \n\t $crs_home/bin/crsctl start crs");
            $msg->print;
            Msg::prtc();
            return 1;
        }
        ...
        for my $sys (@{CPIC::get('systems')}) {
            ...
            $msg->left;
            $output=$sys->cmd("$cmd");
            if (EDR::cmdexit() && $output!~ $prod->{crs_offline_msg}) {
                Msg::right_failed();
                $failed_systems++;
            } else {
                Msg::right_done();
...
# Link the Symantec Membership library
# 'sub do_skgxn' in CPI
sub link_vcsmm_lib {
    my $prod = shift;
    my $crs_home = shift;
    my ($msg, $sys,$mach_hardware, $so_ext, $nmapidir_64);
    ...
    for my $sys (@{CPIC::get('systems')}) {
        ...
        my $timestamp = `date +%m%d%y_%H%M%S`;
        $sys->cmd("_cmd_mv $crsgxn $crsgxn-$timestamp 2>/dev/null");
...
sub check_disk_labeling {
    my ($prod, @disks) = @_;
    my ($sys, $disk, $msg, $status, $tmpdir,$mach_hardware, $so_ext, $nmapidir_64);
    $tmpdir=EDR::tmpdir();
    my $label_file = "$tmpdir/label_while_formatting";
    ...
    for my $disk (@disks) {
        ...
        if ($label eq 'nolabel' || $label eq 'error') {
            ...
            for my $sys (@{CPIC::get('systems')}) {
                $sys->writefile("label\n",$label_file);
                $sys->cmd("_cmd_format -d $handle -f $label_file");
                if (EDR::cmdexit()) {
...
sub get_archtype_sys {
    my ($prod, $sys, $archtype, $errstr, $msg);
    $prod = shift;
    $sys = shift; # target host of the checking
    $errstr = '';
...
sub plat_plumbdev_sys {
    my ($prod, $syslistref, @link, $sys, $res, $msg);
...
sub check_liblink_sys {
    my ($prod, $sys, $crshome, $orahome, $oraversion) = @_;
    my ($status, $arch, $lib_dir64, $msg);
    my ($lib_skgxp, $lib_odm);
    my ($vcsipc_lib, $cmd_ret, $odm_lib, $oracle_num);

    $oracle_num= $prod->parse_oracle_version($oraversion,1);
    ...
    if ($oracle_num >= 11) {
        $msg = Msg::new("Skipping the IPC library check for Oracle version $oracle_num on $sys->{sys}");
        $msg->log;
        goto SKIP_IPC;
    }
...
sub plat_check_kernelparams_sys {
    ...
    my ($temp, $status, $semmns, $prod, $oraversion,$msg,$cmd);
...
sub check_mac_speed_autoneg_sys {
    ...
    my ($item, $desc, $status_autoneg, $status_speed, $status_mac, $status);
...
package Prod::SFRAC62::Linux;
@Prod::SFRAC62::Linux::ISA = qw(Prod::SFRAC62::Common);
...
sub init_plat {
    ...
    $prod->{zru_releases}=[qw(5.0.30 5.1 6.0 6.1 6.2)];
    $prod->{menu_options}=['Symantec Volume Replicator','Symantec File Replicator Option','Global Cluster Option'];

    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        VRTScsocw VRTSdbac62 VRTSvcswiz62 VRTSvcsvmw62 VRTSvbs62 VRTSvcsea62 VRTScfsdc VRTSodm62
        VRTSodm-platform VRTSodm-common VRTSgms62 VRTScavf62
        VRTSglm62 VRTScpi VRTSd2doc VRTSordoc VRTSfsnbl VRTSfppm
        VRTSap VRTStep VRTSgapms VRTSmapro-common VRTSvail
        VRTSd2gui-common VRTSorgui-common VRTSvxmsa VRTSdbdoc
        VRTSdb2ed-common VRTSdbed62 VRTSdbed-common VRTSdbcom-common
        ...
        VRTSvcsdc VRTSvcsmn VRTSvcsmg VRTSvcsdr62 VRTSvcsag62
        VRTScps62 VRTSvcs62 VRTSamf62 VRTSvxfen62 VRTSgab62 VRTSllt62 VRTSveki62
        VRTSfsmnd VRTSfssdk62 VRTSfsadv62 VRTSfsdoc VRTSfsman VRTSvrdoc
        VRTSvrw VRTSweb VRTSvcsvr VRTSvrpro VRTSalloc VRTSdcli
        VRTSvsvc VRTSvmpro VRTSddlpr VRTSvdid VRTSlvmconv
        VRTSvmdoc VRTSvmman SYMClma VRTSspt62 VRTSaa VRTSmh
        VRTSccg VRTSobgui VRTSfspro VRTSdsa VRTSsfmh61 VRTSob34
        VRTSobc33 VRTSaslapm62 VRTSat50 VRTSatClient50 VRTSsmf
        VRTSpbx VRTSicsco VRTSvxfs62 VRTSvxfs-platform VRTSvxfs-common
        VRTSvxvm62 VRTSvxvm-platform VRTSvxvm-common VRTSjre15
        VRTSjre VRTSsfcpi62 VRTSvlic32 VRTSperl516
    ) ];
    ...
    # Setting SF Oracle RAC/PADV specific variables here
    $prod->{lib_path} = '/etc/ORCLcluster/lib/libskgxn2.so';
    $prod->{bin_path} = '/opt/VRTSvcs/rac/bin/';
    $prod->{initdir} = '/etc/init.d/';
...
sub relink_oracle {
    ...
    $oracle_num= $prod->parse_oracle_version($prod->{db_release},1);

    $orclodm = "$prod->{db_home}/lib/libodm$oracle_num.so";
...
sub link_libskgxn {
    my ($prod, $skgxn_name, $crs_home) = @_;
    my ($sys, @cpnodes, $msg, $cmd, $output);

    my $crsup = $prod->is_crs_up(CPIC::get('systems'));
    if ($crsup) {
        $msg = Msg::new("\nOracle Clusterware/Grid Infrastructure should be linked with Symantec Membership library. To link Oracle Clusterware/Grid Infrastructure, the Oracle Clusterware/Grid Infrastructure will be stopped.");
        $msg->print;
        $msg = Msg::new("Do you want to continue?");
        my $ayn = $msg->ayny;
        if ($ayn eq 'n' || $ayn eq 'N') {
            $msg = Msg::new("\nExecute following commands on all cluster nodes:\n 1. Stop Oracle Clusterware/Grid Infrastructure\n\t $crs_home/bin/crsctl stop crs \n 2. Link the Veritas Membership library to Oracle Clusterware/Grid Infrastructure home \n\t ln -s $prod->{lib_path} $prod->{crs_home}/lib/libskgxn2.so \n 3. Start Oracle Clusterware/Grid Infrastructure \n\t $crs_home/bin/crsctl start crs");
            $msg->print;
            Msg::prtc();
            return 1;
        }
        $cmd = "$crs_home/bin/crsctl stop crs";
        my $failed_systems = 0;
        my @crs_up_systems = $prod->get_crs_up_systems(CPIC::get('systems'));
        for my $sys (@crs_up_systems) {
...
sub link_vcsmm_lib {
    ...
    for my $sys (@{CPIC::get('systems')}) {
        $msg = Msg::new("Linking Symantec skgxn library on $sys->{sys}");
        $msg->left;

        my $timestamp = `date +%m%d%y_%H%M%S`;
        $sys->cmd("_cmd_mv $orclgxn $orclgxn-$timestamp 2>/dev/null");
...
sub set_css_misscount {
    ...
    my @crs_up_systems = $prod->get_crs_up_systems(CPIC::get('systems'));
    if (@crs_up_systems) {
        my $sys = $crs_up_systems[0];
        $sys = ($Obj::pool{"Sys::$sys"}) ? Obj::sys($sys) : Sys->new($sys);
        $cmd = "$crs_home/bin/crsctl get css misscount";
        my $misscount = $sys->cmd("$cmd");
        if (!EDR::cmdexit()) {
            $misscount =$1 if ($misscount =~ /misscount\s+(\d+)\s+/mx);
            if ($misscount >= 600) {
                return;
...
sub plat_prepu_sfrac_defines {
    ...
    my $padv = $prod->padv();
        ...
        $padv->{cmd}{vcsmmconfig} = '/etc/sysconfig/vcsmm';
    $padv->{cmd}{lltstat} = '/sbin/lltstat';
...
# libraries are linked properly with Symantec libraries.
sub check_liblink_sys {
    my ($prod,$sys,$crshome,$orahome,$temp_version,$orauser) = @_;
    my ($found, $ay, $dirpath);
    my ($ayn,$tmpstr, $tmpstr1, $tmpint);
    my (@fileopaths, @libpaths, $fileopath, $libpath);
    my ($skgxplibpath, @oracle_release, $oracle_num, $oracleversion);
    my ($ipc_utility, $ipc_version, $ipc_lib);
    my ($single_line, $status, $msg);
    ...
    @oracle_release= $prod->parse_oracle_version($temp_version);

    # Calculate precise Oracle Version
    my $xr = ($oracle_release[0]=="12") ? "cR":"gR";
    $oracleversion="$oracle_release[0]$xr$oracle_release[1]";

    # Numerical Oracle version to use during calculations
    $oracle_num=$oracle_release[0];
    ...
    for ($tmpint = 0; $tmpint < $#libpaths + 1 ; $tmpint++) {
        $libpath = $libpaths[$tmpint];

        if ($sys->exists($libpath)) {
            $msg = Msg::new("Symantec library file $libpath is present on $sys->{sys}");
            $msg->log;
        } else {
            $msg = Msg::new("Symantec library file $libpath is not present on $sys->{sys}. This may be because $prod->{abbr} is not installed properly.");
            $msg->log;
            $msg->print;
            $found = 0;
            last;
...
sub plat_check_kernelparams_sys {
    ...
    my ($prod, $msg, $temp, $status, $compare_with_11g);
    my ($oraversion,$shmall_value,$shmmni_value,$sh,$filemax,$ip_local_port_range_max,$rmemmax);
    ...
    $compare_with_11g=$prod->version_compare_with_11g($oraversion);
    ...
    # shmall should be at least 2097152 
    $shmall_value=2097152;
    $param = $sys->cmd('_cmd_cat /proc/sys/kernel/shmall');
    if ((0+$param) < $shmall_value) {
    ...
    # shmmni should be at least 2046 for 10.* and 4096 for later
    $shmmni_value = $compare_with_11g ? 4096:2046;
    ...
    if ((0+$param) < $shmmni_value) {
        ...
        $msg->log;
        $param_list->[++$counter] = "shmmni value is $param. It should be equal to or more than $shmall_value";
        $status = 1;
    }

    # file-max should be at least 6553600 for 11 and later, 4096 for 10*
    $filemax = $compare_with_11g ? 6553600:4096;

    $ip_local_port_range_max = $compare_with_11g ? 65500:65000;
    ...
    # rmem_max should be at least 4194304 for 11* and later, 262144 for 10*
    $rmemmax = $compare_with_11g ? 4194304:262144;
...
sub check_mac_speed_autoneg_sys {
    ...
    for ($ii=0; $ii<2; $ii++) {
        if (!$link[$ii] eq '') {
            $mac[$ii] = $sys->cmd("_cmd_ip link show $link[$ii] 2>/dev/null | _cmd_awk '/^ *link/ { print \$2 }'");
            if ($mac[$ii] eq '') {
                $msg = Msg::new("Could not get MAC address for $link[$ii] on $sys->{sys}");
                $msg->log;
            } else {
...
sub check_llt_link_public_sys {
    ...
    $privipaddrs[0] = $sys->cmd("_cmd_ip -o addr show dev $link1 2>/dev/null | _cmd_awk '/inet / { print \$4 }'");
    $privipaddrs[0] =~ s/\/.*//mx;
    chomp($privipaddrs[0]);
    $privipaddrs[1] = $sys->cmd("_cmd_ip -o addr show dev $link2 2>/dev/null | _cmd_awk '/inet / { print \$4 }'");
    $privipaddrs[1] =~ s/\/.*//mx;
    chomp($privipaddrs[1]);
...
sub get_plumbed_ips {
    ...
    EDR::cmd_local("_cmd_rm $tmpdir/plumbed_ips.txt");
    for my $sys (@{$nodes_ref}) {
        $output = $sys->cmd("_cmd_ifconfig -a | _cmd_awk '/inet/ {print \$2}' | _cmd_awk -F: '{print \$2}'");
        EDR::cmd_local("echo \'$output\' >> $tmpdir/plumbed_ips.txt");
        if (EDR::cmdexit() != 0) {
            $msg = Msg::new("Failed to save plumbed ips in $tmpdir/plumbed_ips.txt");
            $msg->print;
...
package Prod::SFRAC62::SunOS;
@Prod::SFRAC62::SunOS::ISA = qw(Prod::SFRAC62::Common);
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        VRTScsocw VRTSdbac62 VRTSvcswiz62 VRTSvbs62 VRTSvcsea62 VRTScfsdc VRTSodm62
        VRTSgms62 VRTScavf62 VRTSglm62 VRTScpi VRTSd2doc VRTSordoc
        VRTSfsnbl VRTSfppm VRTSap VRTStep VRTSspc VRTSspcq
        VRTSfasdc VRTSfasag VRTSfas VRTSgapms VRTSmapro VRTSvail
        VRTSd2gui VRTSorgui VRTSvxmsa VRTSdbdoc VRTSdb2ed VRTSdbed62
        VRTSdbcom VRTSsydoc VRTSsybed VRTSvcsApache VRTScmc
        VRTSccacm VRTSvcsw VRTScspro VRTSvcsdb VRTSvcsor VRTSvcssy
...
sub relink_oracle {
    ...
    for my $system (@cpnodes) {
        ...
        if (EDR::cmdexit()) {
            ...
            # check 'ldd bin/oracle'
            $system->cmd("_cmd_ldd $prod->{db_home}/bin/oracle 2>/dev/null | _cmd_grep libodm ");
            if (EDR::cmdexit()) {
                # need remove static lib and rebuild rdbms/ioracle
                if ($system->exists("$prod->{db_home}/rdbms/lib/libodm10.a.backup")) {
                    $system->cmd("_cmd_rm -f $prod->{db_home}/rdbms/lib/libodm10.a");
                } elsif($system->exists("$prod->{db_home}/rdbms/lib/libodm10.a")){
                    $system->cmd("_cmd_mv $prod->{db_home}/rdbms/lib/libodm10.a $prod->{db_home}/rdbms/lib/libodm10.a.backup");
                    $system->cmd("_cmd_chown $orauser:$oragrp $prod->{db_home}/rdbms/lib/libodm10.a.backup");
                }
                $system->cmd("su $orauser -c 'cd $prod->{db_home}/rdbms/lib; ORACLE_HOME=$prod->{db_home}; export ORACLE_HOME; /usr/ccs/bin/make -f $prod->{db_home}/rdbms/lib/ins_rdbms.mk ioracle'");
            }
            Msg::right_done();
            $msg->display_right() if (Obj::webui());
        }
...
sub link_libskgxn {
    ...
    if ($crsup) {
        ...
        my $failed_systems = 0;
        for my $sys (@{CPIC::get('systems')}) {
            $msg = Msg::new("Stopping the Oracle Clusterware/Grid Infrastructure on $sys->{sys}");
            $msg->left;
            $output = $sys->cmd("$cmd");
            if (EDR::cmdexit() && $output!~ $prod->{crs_offline_msg}) {
                Msg::right_failed();
...
# Check link speed and Autonegotiation settings
sub check_mac_speed_autoneg_sys {
    my ($sys, $flag, @link, @instance, @autoneg, @speed, @mac, $ii, $summary);
    my ($prod, $msg, $cmd_ret, %autoneg_status, %speed_status, %mac_status);
    my ($item, $desc, $status_autoneg, $status_speed, $status_mac,$status);
    ...
    if ($flag == 1) {
        ...
        $link[0] = shift;
        # Get the linkname
        $link[0] = EDR::cmd_local("echo $link[0] | _cmd_awk 's/[0-9]*//g'");
        # Get the instance number
        $instance[0] = EDR::cmd_local("echo $link[0] | _cmd_awk 's/[a-z A-Z]*//g'");

        # Link name contains instance number as well
        $link[1] = shift;
        # Get the linkname
        $link[1] = EDR::cmd_local("echo $link[1] | _cmd_awk 's/[0-9]*//g'");
        # Get the instance number
        $instance[1] = EDR::cmd_local("echo $link[1] | _cmd_awk 's/[a-z A-Z]*//g'");
        ...
        # Links not provided, get them from /etc/llttab
        if ($sys->exists('/etc/llttab')) {
            # Get the link name and its instance number to be used for "ndd" cmd
            $link[0] = $sys->cmd("_cmd_cat /etc/llttab | _cmd_grep link | _cmd_head -1 | _cmd_awk '{print \$3}' | _cmd_cut -d':' -f1");
            $instance[0] = $sys->cmd("_cmd_cat /etc/llttab | _cmd_grep link | _cmd_head -1 | _cmd_awk '{print \$3}' | _cmd_cut -d':' -f2");

            # Get the link name and its instance number to be used for "ndd" cmd
            $link[1] = $sys->cmd("_cmd_cat /etc/llttab | _cmd_grep link | _cmd_tail -1 | _cmd_awk '{print \$3}' | _cmd_cut -d':' -f1");
            $instance[1] = $sys->cmd("_cmd_cat /etc/llttab | _cmd_grep link | _cmd_tail -1 | _cmd_awk '{print \$3}' | _cmd_cut -d':' -f2");

            $msg = Msg::new("LLT Link1 Complete Name: $link[0]");
            $msg->log;
            $link[0] = EDR::cmd_local("echo $link[0] | cut -d'/' -f3");
            $msg = Msg::new("LLT Link1 Short Name: $link[0]");
            $msg->log;
            $msg = Msg::new("LLT Link1 Instance number: $instance[0]");

            $msg = Msg::new("LLT Link2 Complete Name: $link[1]");
            $link[1] = EDR::cmd_local("echo $link[1] | cut -d'/' -f3");
            $msg = Msg::new("LLT Link2 Short Name: $link[1]");
            $msg->log;
            $msg = Msg::new("LLT Link2 Instance number: $instance[1]");
            $msg->log;
        } else {
    ...
    for ($ii=0; $ii<2; $ii++) {
        if (!$link[$ii] eq '') {
            ...
            if ($mac[$ii] eq '') {
                ...
                if ($sys->exists('/opt/VRTSllt/getmac')) {
                    my $getmacop = $sys->cmd("/opt/VRTSllt/getmac /dev/$link[$ii]$instance[$ii] 2>/dev/null");
                    if (EDR::cmdexit()) {
...
sub check_full_duplex_link_sys {
    ...
    my ($sys, $flag, @link, @duplex, $item, $desc, $status, $summary);
    my ($cmd_ret, $ii, %duplex_status, $prod, $msg, @instance);
...
package Prod::SFRAC62::AIX;
@Prod::SFRAC62::AIX::ISA = qw(Prod::SFRAC62::Common);
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
            VRTScsocw.rte VRTSdbac62 VRTSdbac.rte VRTSvcswiz62 VRTSvbs62 VRTSvcsea62 VRTScfsdc
            VRTSodm62 VRTSgms62 VRTScavf62 VRTSglm62 VRTScpi.rte
            ...
            VRTSvcs.doc VRTSvcs.man VRTSvcs.msg.en_US VRTSvcsag62
            VRTSvcsag.rte VRTScps62 VRTSvcs62 VRTSvcs.rte VRTSamf62 VRTSvxfen62
            VRTSvxfen.rte VRTSgab62 VRTSgab.rte VRTSllt62 VRTSllt.rte
            VRTSfsmnd VRTSfssdk62 VRTSfsadv62 VRTSfsdoc VRTSfsman VRTSvrdoc
            ...
            VRTSicsco VRTSvxfs62 VRTSvxvm62 VRTSveki62 VRTSjre15.rte
            VRTSjre.rte VRTSsfcpi62 VRTSvlic32 VRTSperl516 VRTSperl.rte
        ) ];
    return;
}
...
sub create_orcl_dir {
    ...
    for my $sys (@{CPIC::get('systems')}) {
        ...
        $sys->cmd("_cmd_cp -pf $vcshome/rac/lib/libskgxn2.a $cldir/lib32/libskgxn2.a");
        if (EDR::cmdexit() != 0) {
            Msg::right_failed();
            return 1;
        }

        $sys->cmd("_cmd_cp -pf $vcshome/rac/lib/libskgxp10_ver25_32.so $cldir/lib32/libskgxp10.so");
        $sys->cmd("_cmd_cp -pf $vcshome/rac/lib/libskgxp10_ver25_32.a $cldir/lib32/libskgxp10.a");
        $sys->cmd("_cmd_cp -pf $vcshome/rac/lib/libvcsmm.so $cldir/lib32/libskgxn2.so");
        ...
        $sys->cmd("_cmd_cp -pf $vcshome/rac/lib64/libskgxn2.a $cldir/lib/libskgxn2.a");
        if (EDR::cmdexit() != 0) {
            Msg::right_failed();
            return 1;
        }

        $sys->cmd("_cmd_cp -pf $vcshome/rac/lib/libskgxp10_ver25_64.so $cldir/lib/libskgxp10.so");
        $sys->cmd("_cmd_cp -pf $vcshome/rac/lib/libskgxp10_ver25_64.a $cldir/lib/libskgxp10.a");
        $sys->cmd("_cmd_cp -pf $vcshome/rac/lib64/libvcsmm.so $cldir/lib/libskgxn2.so");
...
sub link_vcsmm_lib {
    ...
    for my $sys (@{CPIC::get('systems')}) {
        ...
        if ($sys->exists($crsgxn)) {
            ...
            $sys->cmd("_cmd_mv $orclgxn $orclgxn-$TS 2>/dev/null");
            $sys->cmd("_cmd_ln -s $vrtsgxn $orclgxn 2>/dev/null");
            if (EDR::cmdexit()) {
                Msg::right_failed();
            } else {
            ...
            $sys->cmd("_cmd_mv $crsgxn $crsgxn-$TS 2>/dev/null");
            $sys->cmd("_cmd_ln -s $orclgxn $crsgxn 2>/dev/null");
            if (EDR::cmdexit()) {
                Msg::right_failed();
            } else {
...
sub copy_libodm {
    ...
    for my $sys (@cpnodes) {
        ...
        if ($prod->copy_and_replace($sys, "$odmhome/lib/libodm64.so",
            "$homedir/lib/libodm$oracle_num.so") != 0) {
            Msg::right_failed();
            return 1;
        }
        $sys->cmd("_cmd_chmod 644 $homedir/lib/libodm$oracle_num.so 2>/dev/null");
...
sub copy_libskgxp_per_ipcutil {
    ...
    #Do nothing for 11g(11.1, 11.2) or later
    return 0 if ($prod->version_compare_with_11g($prod->{db_release})); 
...
sub set_install_env {
    my ($prod, $release, $patch_level, $instpath, $oraprod) = @_;
    my ($sys, $msg, $compare_with_11g);
    my $tmpdir=EDR::tmpdir();
    my $bs=EDR::get2('tput','bs');
    my $be=EDR::get2('tput','be');
    my $localsys=$prod->localsys;
    ...
    # If SKIP_ROOTPRE is not set, invoke the rootpre.sh script.
    #
    $compare_with_11g=$prod->version_compare_with_11g($release); # value=1 for 11.1, value=2 for 11.2 and later.
    ...
    if ((($release eq '10.2') && ($patch_level eq '0.1')) ||
        ...
        if ($ENV{'SKIP_ROOTPRE'} eq 'TRUE') {
            ...
            for my $sys (@{CPIC::get('systems')}) {
                ...
                $msg->left;
                $firstsys->copy_to_sys($sys,"$instpath/rootpre","$tmpdir/rootpre");
                if ($compare_with_11g) {  # Oracle 11.1 or later
                    $firstsys->copy_to_sys($sys,"$instpath/rootpre.sh","$tmpdir/rootpre.sh");
...
DNA_ROOTPRE:
    ...
    my $user = $prod->{oracle_user};
    if (($oraprod eq 'crs') && Cfg::opt('responsefile')) {
        $user = $prod->{grid_user};
    }
    if ((($release eq '10.2') && ($patch_level eq '0.4')) ||
        $compare_with_11g) {  # Oracle 11.1 or later
        for my $sys (@{CPIC::get('systems')}) {
            my $attrstr = $sys->cmd("lsuser -a capabilities $user");
            if (($attrstr !~ /CAP_NUMA_ATTACH/m) ||
                ...
                $msg->left;
                $sys->cmd("chuser capabilities=CAP_BYPASS_RAC_VMM,CAP_PROPAGATE,CAP_NUMA_ATTACH $user");
                if (EDR::cmdexit() != 0) {
                    Msg::right_failed();
...
sub do_relink_oracle {
    my ($prod, $iscfs) = @_;
    my ($out, $sys, @cpnodes, $ret, $msg, $compare_with_11g);
    my $orahome = $prod->{db_home};
    my $orauser = $prod->{oracle_user};
    ...
    $compare_with_11g = $prod->version_compare_with_11g($prod->{db_release});
    if ($compare_with_11g == 2) {
        Msg::log("Skip recompiling Oracle $prod->{db_release}");
        return 0;
    }
    ...
    for my $sys (@cpnodes) {
        ...
        # for version prior to 11.1  
        #

        unless ($compare_with_11g) {
...
sub get_skgxn_lib {
    my ($prod, $crshome, $db_release) = @_;
    my $skgxn_name = '';
    my $firstsys = ${CPIC::get('systems')}[0];

    $skgxn_name = $firstsys->cmd("LIBPATH=$crshome/lib; export LIBPATH; _cmd_ldd $crshome/bin/ocssd.bin | _cmd_grep 'libskgxn2'");
    if ($skgxn_name ne '') {
...
sub check_liblink_sys {
    my ($prod,$sys,$crshome,$orahome,$oraversion) = @_;
    my ($status,$vcshome,$odmhome,$oracle_num,$msg,$ora_rel);
    my ($ext_libs,$ora_skgxplib,$cmd_ret,$vcsmm_lib,$odm_lib);
...
ODM_CHECK:
    ...
    $odm_lib = "$orahome/lib/libodm".$oracle_num.'.so';        # Construct the odm library file name.
...
# for which check failed.
sub plat_check_kernelparams_sys {
    my ($prod, $msg, $sys, $status, $cmd_ret, $counter, $param_list, $maxuprocval, $oraversion);
    ...
    $counter = -1;
    $oraversion = $prod->{oraver};
    $maxuprocval = ($prod->version_compare_with_11g($oraversion)==2)?16384:2048;
    ...
    if ("$cmd_ret" eq '') {
        ...
        $msg->log;
        $param_list->[++$counter] = "maxuproc value is UNKNOWN on $sys->{sys}. It should be 16384 or more for oracle versions 11.2 and later, or 2048 or more for oracle versions less than 11.2.";
        $status = 1;
    } elsif ((0+$cmd_ret) < $maxuprocval) {
        $msg = Msg::new("maxuproc parameter's value is NOT as per the requirement on $sys->{sys}. It should be 16384 or more for oracle versions 11.2 and later, or 2048 or more for oracle versions less than 11.2.");
        $msg->log;
        $param_list->[++$counter] = "maxuproc value is NOT as per the requirement on $sys->{sys}. It should be 16384 or more for oracle versions 11.2 and later, or 2048 or more for oracle versions less than 11.2.";
        $status = 1;
    } else {
...
package Prod::SFRAC62::SolSparc;
@Prod::SFRAC62::SolSparc::ISA = qw(Prod::SFRAC62::SunOS);
...
package Prod::SFRAC62::Sol11sparc;
@Prod::SFRAC62::Sol11sparc::ISA = qw(Prod::SFRAC62::SunOS);
...
sub init_padv {
    ...
    $prod->{minimal_cpu_speed_requirment} = '1 GHz';
    $prod->{allpkgs}=[ qw(VRTSdbac62 VRTSgms62 VRTSodm62 VRTSdbed62 VRTSvcsea62) ];
    $prod->{minpkgs}=[ qw(VRTSdbac62 VRTSgms62 VRTSodm62 VRTSdbed62 VRTSvcsea62) ];
    $prod->{recpkgs}=[ qw(VRTSdbac62 VRTSgms62 VRTSodm62 VRTSdbed62 VRTSvcsea62) ];
    return;
}

package Prod::SFRAC62::Solx64;
@Prod::SFRAC62::Solx64::ISA = qw(Prod::SFRAC62::SunOS);
...
package Prod::SFRAC62::Sol11x64;
@Prod::SFRAC62::Sol11x64::ISA = qw(Prod::SFRAC62::SunOS);
