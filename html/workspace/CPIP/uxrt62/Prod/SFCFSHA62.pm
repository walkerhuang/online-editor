...
package Prod::SFCFSHA62::Common;
@Prod::SFCFSHA62::Common::ISA = qw(Prod);

sub init_common {
    my $prod = shift;
    $prod->{vers}='6.2.0.000';
    $prod->{default_cluster_protocol_version} = 140;

    if ($prod->sfcfsha) {
        $prod->{prod}='SFCFSHA';
        $prod->{abbr}='SFCFSHA';
        $prod->{name}=Msg::new("Symantec Storage Foundation Cluster File System HA")->{msg};
        ...
        $prod->{eula}='EULA_VirtualStore_Ux_6.2.pdf';
        $prod->{mainpkg}='VRTSsvs62';

        $prod->{extra_mainpkgs}=[ qw(VRTSvxvm62 VRTSvxfs62 VRTSvcs62 VRTSllt62 VRTSgab62 VRTSvxfen62 VRTSvcsag62)];
        $prod->{obsoleted_by_merging}=1;

    } elsif ($prod->sfsybasece) {
        $prod->{prod}='SFSYBASECE';
        $prod->{abbr}='SFSYBASECE';
        $prod->{name}=Msg::new("Symantec Storage Foundation for Sybase ASE CE")->{msg};
        $prod->{menu_options}=['Symantec Volume Replicator','Global Cluster Option'];
        $prod->{proddir}='storage_foundation_for_sybase_ce';
        $prod->{eula}='EULA_SF_Sybase_CE_Ux_6.2.pdf';

        $prod->{installscript_prod}='SFSYBASECE62';
    ...
    $prod->{subprods}=[qw(VCS62 SF62 SFHA62)];
    ...
    $padv->{cmd}{odmclusteradm}='/opt/VRTSodm/bin/odmclustadm';
...
sub set_pkgs {
    ...
    #Ensure VRTSodm62 is after VRTSgsm51 in the pkg list.
    $sf1->{allpkgs} = EDRu::arrdel($sf->{allpkgs},'VRTSodm62');
    $sf1->{minpkgs} = $sf->{minpkgs};
    $sf1->{recpkgs} = EDRu::arrdel($sf->{recpkgs},'VRTSodm62');
    ...
    for my $category (@categories) {
        $prod->{$category}=EDRu::arruniq(@{$rel->{$category}},
                                        ...
                                        @{$prod->{$category}});
       if($prod->sfsybasece()){
            $prod->{$category}=EDRu::arrdel($prod->{$category},'VRTSgms62');
            $prod->{$category}=EDRu::arrdel($prod->{$category},'VRTSodm62');
            $prod->{$category}=EDRu::arrdel($prod->{$category},'VRTScps62');
            $prod->{$category}=EDRu::arrdel($prod->{$category},'VRTSdbed62');
       }
    }
...
# return 1 if using installsfcfs/installsvs to configure SFCFSHA
# and SFCFSHA is licensed
sub configure_alternate_prod {
    my ($prod)=@_;
    my $cprod=CPIC::get('prod');
    return 1 if (($cprod =~/(SFCFS|SVS)(\d+)/m) && $prod->sfcfsha );
    return;
}
...
sub cli_prestart_config_questions {
    ...
    if ($prod->svs()) {
        ...
        while ((!$installconfig) || ($prod->{silent_installconfig})) {
            # skip SVS-specific install/config mode
            # due to the merging of SVS into SFCFSHA
            last;
...
sub backup_conf_files_sys {
    ...
    if ($sys->exists($confdir)) {
        ...
        for my $file(split(/\n/,$out)) {
            $sys->cmd("_cmd_cp -p $file $file.cpisave");
        }
    }
...
sub ru_prestop_sys {
    my ($prod,$sys) = @_;
    my ($ver1,$ver2,$ver3,$output,$msg,$had);
    my $vcs=$sys->prod('VCS62');
    ...
    if (Cfg::opt('upgrade_kernelpkgs')) {
        ...
        if ($prod->stop_agents_sys($sys)) {
            # Print Error Message Here
            # Directly exit CPI
            $msg=Msg::new("Failed to stop CVM/CFS agents, contact Symantec technical support or refer to the installation guide for further troubleshooting.");
            $sys->push_error($msg);
            return 1;
        }
        Msg::log("Finished Stopping CVM in rolling upgrade on $sys->{sys}");

        # if ru_prestop_sys failed, return immediately.
        return 1 if ($vcs->ru_prestop_sys($sys));
...
sub upgrade_prestop_sys {
    ...
    if (!Cfg::opt('rootpath')) {
        $prod->stop_agents_sys($sys) unless(Cfg::opt(qw(upgrade_kernelpkgs upgrade_nonkernelpkgs)));
    }
...
sub upgrade_poststart_sys {
    ...
    $sfha->upgrade_poststart_sys($sys) if ($sfha->can('upgrade_poststart_sys'));
...
sub sfcfs_start_agents {
    ...
    for my $sys (@$syslist) {
        if ($vm->vold_status_sys($sys) ne 'enabled') {
            Msg::log('Disabled');
            if (Cfg::opt('start')) {
                $msg=Msg::new("Unable to start CFS agents at this time. vxconfigd is not enabled.");
            } else {
    ...
    for my $sys (@$syslist) {
        if (($sys->exists("$vcs->{configdir}/CVMTypes.cf"))
            ...
            $sys->cmd("_cmd_cp -p $vcs->{configdir}/CVMTypes.cf $vcs->{confdir}/CVMTypes.cf");
        }
        if (($sys->exists("$vcs->{configdir}/CFSTypes.cf"))
            && (!$sys->exists("$vcs->{confdir}/CFSTypes.cf"))) {
            $sys->cmd("_cmd_cp -p $vcs->{configdir}/CFSTypes.cf $vcs->{confdir}/CFSTypes.cf");
        }
    }
    return;
}
...
sub stop_agents_sys {
    ...
    #Etrack 3046613 : merging SVS back to SFCFSHA
    $prod->stop_svsweb_sys($sys);
    $prod->stop_odm_sys($sys);

    # If upgrade or patchupgrade, do not stop VCS.
    # Just stop the CVM and CFS Agents. (Offline group cvm)
    # Otherwise the service group frozen operation will fail in later steps(ET3436664)
    if (Cfg::opt(qw(upgrade patchupgrade))) {
        if ($sys->system1) {
            $failure=$prod->stop_cvm_cfs_agents($sys) if ($sys->{prod_running});
            EDRu::create_flag('stop_cvm_cfs_agents_done');
        } else {
...
sub stop_cvm_cfs_agents {
    my ($prod,$sys) = @_;
    my ($padv,$port_f,$vcs,@deps,@cvmdeps,$syslist,$port_q,$offlinefailuregroups,$offlinefailuregrouplist,$failure);
    $syslist=CPIC::get('systems');
    $vcs = $prod->prod('VCS62');
    $padv=$sys->padv;
    ...
    push(@deps, 'cvm');
    $offlinefailuregroups=$prod->offline_groups($sys,\@deps);
    if (@{$offlinefailuregroups}) {
        # If there're groups depends on CVM and failed to offline
        # No need to go further and attempt to stop CVM/CFS related agents.
        $offlinefailuregrouplist=join(',',@{$offlinefailuregroups});
        Msg::log("The following service groups failed to offline: $offlinefailuregrouplist");
        $failure=1;
    }
    for my $sys (@$syslist) {
        $port_f = $sys->cmd("_cmd_gabconfig -a 2>/dev/null | _cmd_grep -c 'Port f gen' ");
        ...
        $sys->cmd('_cmd_fsclustadm cfsdeinit 2>/dev/null');
        # Stop Qlog for SunOS
        if ($sys->sunos()) {
            $port_q = $sys->cmd("_cmd_gabconfig -a 2>/dev/null | _cmd_grep -c 'Port q gen' ");
            $padv->unload_driver_sys($sys,'qlog') if ($port_q >= 1);
            $port_q = $sys->cmd("_cmd_gabconfig -a 2>/dev/null | _cmd_grep -c 'Port q gen' ");
            if ($port_q >= 1) {
                Msg::log("Failed to unload driver Qlog");
            }
        }
    }
...
sub offline_groups {
    my ($prod,$sys,$deps) = @_;
    my ($grpstate,$sysi,$sys_master,$vxdctl_mode,$state,$vcs,$count,$syslist,@fields,$temp_sys);
    my ($master_node,$master_cvm_state);
    my (@offlinefailuregroups);
    $syslist=CPIC::get('systems');
    $vcs = $prod->prod('VCS62');

    for my $grp (@{$deps}) {
        $grpstate = $sys->cmd("$vcs->{bindir}/hagrp -display $grp -attribute State");
        for my $line (split(/\n/,$grpstate)) {
            next if ($line=~/^#Group/);
            ...
            if ($grp eq 'cvm') {
                $vxdctl_mode = $temp_sys->cmd("_cmd_vxdctl -c mode 2>/dev/null | _cmd_grep 'mode:'");
                if ($vxdctl_mode=~/MASTER/m) {
                    $master_node=$sysi;
                    $master_cvm_state=$state;
                } else {
                    push (@offlinefailuregroups,$grp) if ($prod->offline_group_sys($sys,$sysi,$grp,$state));
                }
            } else {
                push (@offlinefailuregroups,$grp) if ($prod->offline_group_sys($sys,$sysi,$grp,$state));
            }
        ...
        if ($grp eq 'cvm') {
            push (@offlinefailuregroups,$grp) if ($prod->offline_group_sys($sys,$master_node,$grp,$master_cvm_state));
        }
    }
    return \@offlinefailuregroups;
}
...
sub sfcfs_stop_agents_all {
    ...
    for my $sysi (@$syslist) {
        $vxdctl_mode = $sysi->cmd("_cmd_vxdctl -c mode 2>/dev/null | _cmd_grep 'mode:'");
        if ($vxdctl_mode=~/MASTER/m) {
...
sub stop_odm_sys {
    my ($prod,$sys) = @_;
    my ($odmver,$odmpkg,$odm,$gmspkg,$vxgms,$gmsver);
    $odmpkg=$sys->pkg('VRTSodm62');
    if (defined $odmpkg) {
        $odm=$sys->proc('odm62');
        $odmver=$odmpkg->version_sys($sys);
        if ($odmver && ($odm->check_sys($sys,'stop'))) {
            $odm->stop_sys($sys);
            return 1 if ($odm->check_sys($sys,'stop'));
        }
    }
    $gmspkg=$sys->pkg('VRTSgms62');
    if (defined $gmspkg) {
        $vxgms=$sys->proc('vxgms62');
        $gmsver=$gmspkg->version_sys($sys);
        if ($gmsver && ($vxgms->check_sys($sys,'stop'))) {
            $vxgms->stop_sys($sys);
            return 1 if ($vxgms->check_sys($sys,'stop'));
        }
    }
    return 0;
}
...
#Etrack 3046613
#For SVS merging into SFCFSHA, svsweb should be stopped before all the other
#processes of SFCFSHA
sub stop_svsweb_sys {
    my ($prod,$sys) = @_;
    my ($cpic,$svsver,$svspkg,$svs);
    ...
    return unless ($svspkg);
    $svspkg=$sys->pkg($svspkg);
    if (defined $svspkg) {
        $svs=$sys->proc('svsweb62');
        $svsver=$svspkg->version_sys($sys);
        $svs->stop_sys($sys) if ($svsver);
    }
    return;
}
...
sub version_sys {
    ...
    my ($sfcfsha_licensed,$sfsybasece_licensed,$svs_licensed,$svspkg);
    $cpic=Obj::cpic();
    $rel=$cpic->rel;
    ...
    $pkgvers = $prod->version_mapping_sys($sys,$pkgvers) if $prod->can('version_mapping_sys');

    if ($prod->sfsybasece()) {
        # Skip super product license check(last parameter=1)
        # because SFCFSHA has SFSYBASECE, SFRAC as it's super product
        # If SFSYBASECE is licensed, $rel->prod_licensed_sys($sys, 'SFCFSHA62') will return licensed
        $sfcfsha_licensed=$rel->prod_licensed_sys($sys, 'SFCFSHA62', 1);
        $sfsybasece_licensed=$rel->prod_licensed_sys($sys,$prod->{prodi}, 1);
        if ($pkgvers) {
        ...
        # this if block is fix for etrack 3115659
        # aiming at reporting SVS version properly
        $cv=$prod->prod("SFCFSHA62");
        $sfcfsha_licensed=$cv->version_sys($sys);
        $svs_licensed=$rel->prod_licensed_sys($sys,$prod->{prodi}, 1);
        if ($pkgvers) {
            if ($sfcfsha_licensed  && $svs_licensed) {
                $cv=EDRu::compvers($svs_licensed, $sfcfsha_licensed,2);
                if ($cv eq "2") {
                    return $sfcfsha_licensed;
                } else {
                    return ($mpvers || $pkgvers);
                }
            } elsif ($sfcfsha_licensed) {
                return $sfcfsha_licensed;
            } elsif ($svs_licensed) {
                return $pkgvers;
            }
        ...
        $svs_licensed=$rel->prod_licensed_sys($sys, 'SVS62', 1) if ($svspkg);
        if ($pkgvers && $svs_licensed && Cfg::opt(qw(upgrade upgrade_kernelpkgs))) {
            $prod->{upgrade_from}="SVS";
...
sub description {
    ...
    if ($prod->sfcfsha()) {
        $msg=Msg::new("Symantec Storage Foundation Clustered File System HA adds the full functionality of Symantec Cluster Server, and features the power and flexibility to protect everything from a single critical database instance to very large multi-application clusters in networked storage environments. In addition, increased automation and intelligent workload management allow cluster administrators to maximize individual resources by moving beyond reactive recovery to proactive management of availability and performance. SFCFS HA bundle includes support for application monitoring and failover through the Symantec Cluster Server product.");
        $msg->print;
    } elsif ($prod->svs()) {
        $msg=Msg::new("Symantec VirtualStore powered by Symantec Cluster File System serves as a highly scalable, highly available NFS data storage for your VMware virtual machine images. VirtualStore is built on top of Symantec's Cluster File System which provides high availability and linear scalability across the cluster. With integration with VMware Virtual Center, VirtualStore gives you a complete solution for managing your VMware virtual machine images.");
        $msg->print;
    } else {
...
sub reorder_vxglm {
    my ($prod,$ref_procs) = @_;
    my ($v,@a);
    if((EDRu::inarr('vxglm62', @{$ref_procs})) && (EDRu::inarr('had62', @{$ref_procs}))) {
        $ref_procs=EDRu::arrdel($ref_procs, 'vxglm62');
        for my $v (@{$ref_procs}) {
            if ($v eq 'had62') {
                push (@a,'vxglm62',$v);
            } else {
...
sub postcheck_fsproto_sys {
    ...
    if ($sys->exists($sys->padv->{cmd}{fsclustadm})) {
        $supported_protos = $sys->cmd("_cmd_fsclustadm protoversion 2>/dev/null | _cmd_awk '/^local:/ {print \$2}'");
        $current_proto = $sys->cmd("_cmd_fsclustadm protoversion 2>/dev/null | _cmd_awk '/^local:/ {print \$3}'");
...
sub postcheck_odmproto_sys {
    ...
    if ($sys->exists($sys->padv->{cmd}{odmclustadm})) {
        $supported_protos = $sys->cmd("_cmd_odmclustadm protoversion 2>/dev/null | _cmd_awk '/^local:/ {print \$2}'");
        $current_proto = $sys->cmd("_cmd_odmclustadm protoversion 2>/dev/null | _cmd_awk '/^local:/ {print \$3}'");
        if ( $supported_protos && $current_proto && $current_proto ne '-') {
            $max_proto = (reverse sort {$a<=>$b} split(/,/,$supported_protos))[0];
            if ($current_proto != $max_proto) {
                $versmsg = sprintf("\n\t%-21s: %s\n\t%-21s: %s\n\t%-21s: %s", $currentmsg->{msg},$current_proto, $maxmsg->{msg},$max_proto, $supportedmsg->{msg},$supported_protos);
...
sub register_postchecks {
    my $prod=shift;
    my ($sequence_id,$name,$desc,$handler);

    $prod->perform_task('register_postchecks');
...
sub addnode_poststart {
    my ($cfsnfssg,$firstnode,$msg,$errmsg,$out,$proc,$proci,$rtn,$startprocs,$sys,$sysi);
    my $prod = shift;
    my $cpic = Obj::cpic();
    my $cfg = Obj::cfg();
    ...
    if (!$rtn) {
        $msg = Msg::new("Addnode poststart did not complete successfully");
        $sys->push_error($msg);
    }
    return $rtn;
}
...
sub addnode_config_cvm_cfs {
    my ($firstnode,$n,$out,$rtn,$status,$sys,$sysi,$system,$vxfsckd,$vxfsckd_activation,$asymmetry_key,$asymmetry_value,$cvmres,$cvmsg_name,$max_pri);
    my $prod = shift;
    my $cprod=CPIC::get('prod');
    my $cfg = Obj::cfg();
    my $vcs = $prod->prod('VCS62');
    ...
    $firstnode = Obj::sys(${$cfg->{clustersystems}}[0]);
    $cvmsg_name = $firstnode->cmd("_cmd_hares -display -attribute Group -type CVMCluster -localclus 2>/dev/null| _cmd_tail -1 | _cmd_awk '{print \$4}'");
    if (!EDRu::inarr($cvmsg_name, split (/\n/, $firstnode->cmd("$vcs->{bindir}/hagrp -list 2>/dev/null | _cmd_awk '{print \$1}' | _cmd_sort | _cmd_uniq")))) {
    ...
    $out = $firstnode->cmd("$vcs->{bindir}/hares -list Group=$cvmsg_name Type=CFSfsckd 2>/dev/null| _cmd_awk '{print \$1}'");
    $vxfsckd = (split(/\n/,$out))[0] if ($out);
    if ($vxfsckd) {
    ...
    $max_pri = $vcs->get_sg_max_priority($cvmsg_name);
    for my $sysi (@{$cfg->{newnodes}}) {
        $sys = Obj::sys($sysi);
        $system = $sys->{vcs_sysname};
        $n = $firstnode->cmd("$vcs->{bindir}/hasys -value $system LLTNodeId");
        $max_pri+=1;
        $rtn = $firstnode->cmd("$vcs->{bindir}/hagrp -modify $cvmsg_name SystemList -add $system $max_pri");
        if(EDRu::isverror($rtn)) {
            Msg::log("Modify cvm SystemList to add $sysi failed.");
            $status = 0;
        }

        $rtn = $firstnode->cmd("$vcs->{bindir}/hagrp -modify $cvmsg_name AutoStartList -add $system");
        if (EDRu::isverror($rtn)) {
            Msg::log("Modify cvm AutoStartList to add $sysi failed");
            $status = 0;
        }

        $cvmres = $firstnode->cmd("_cmd_hares -list Type=CVMCluster -localclus 2>/dev/null| _cmd_awk '{print \$1}' | _cmd_uniq");
        $rtn = $firstnode->cmd("$vcs->{bindir}/hares -modify $cvmres CVMNodeId -add $system $n");
        if(EDRu::isverror($rtn)) {
            Msg::log("Modify $cvmres CVMNodeId to add $sysi $n failed.");
            $status = 0;
        }
    ...
    if ((!EDRu::isverror($rtn)) && ($rtn =~ /$asymmetry_key\s+(\w+)/)) {
        ...
        for my $sysi (@{$cfg->{newnodes}}) {
            ...
            $rtn = $sys->cmd("_cmd_vxtune $asymmetry_key $asymmetry_value");
...
sub addnode_mount_share_dg {
    ...
    $dgstate = {};
    $failed = 0;
    my $nodename = ${$cfg->{clustersystems}}[0];
    $firstnode = ($Obj::pool{"Sys::$nodename"}) ? Obj::sys($nodename) : Sys->new($nodename);

    my $ret = $prod->addnode_need_mount_sharedg();
    return $ret if ($ret == 1);
    @sharevols = @{$ret};
...
#    $web->web_script_form('alert',$web_str) if (Obj::webui());
    ...
    $master_node = $firstnode->cmd("_cmd_vxdctl -c mode 2>/dev/null |_cmd_grep '^master:'| _cmd_awk '{print \$2}'");
    if( ! $master_node) {
    ...
    for my $sysi (@{$cfg->{newnodes}}) {
        ...
        for my $mount_info (@sharevols) {
            push (@ldisabled_vols,$mount_info->[0]) if $prod->is_ioship_on_and_ldisabled_sys($sys,$mount_info->[0],$mount_info->[2],$dgstate);
        }
        ...
        if(scalar @ldisabled_vols){
            ...
            my $ldisabled_msg = Msg::new(" (Will skip volume(s) which are in $state state: $ldisabled_vols)");
            $msg->{msg}.=$ldisabled_msg->{msg};
        }
        $ayn = $msg->ayny('','',$web_str);
        if( $ayn eq 'N'){
            if ($cprod =~ /^SFRAC\d+/mx && $cvmgroupstate ne 'ONLINE') {
        ...
        $system = $sys->{vcs_sysname};
        # Set flag here for SFRAC to mount shared volumes after cvm is online
        $sys->{mount_share_dg} = 1;
        for my $mount_info (@sharevols) {
            ...
            for my $nodename (@{$cfg->{clustersystems}}) {
                my $node = ($Obj::pool{"Sys::$nodename"}) ? Obj::sys($nodename) : Sys->new($nodename);
                $primary_node = $node->cmd("_cmd_fsclustadm -v showprimary $mount_point 2>&1");
                if (EDRu::isverror($primary_node)) {
                    Msg::log("Failed to show primary mount point $mount_point from node $nodename, try next node");
                    next;
                }
                last;
            }
            if (EDRu::isverror($primary_node)) {
                Msg::log("Failed to show primary mount point $mount_point on $sysi: $primary_node");
                Msg::right_failed();
                $failed++;
                next;
            }
            ...
            $resnames = $sys->cmd("_cmd_hares -list Type=CFSMount MountPoint='$mount_point' 2>/dev/null |_cmd_awk '{print \$1}' 2>/dev/null | _cmd_sort -u ");
            @resnames = split(/\n/,$resnames);
...
# Get the share volumes to be mounted on the added node
# Abstract to a subroutine for it to be called in SFRAC
sub addnode_need_mount_sharedg {
    my $prod = shift;
    my $cfg = Obj::cfg();
    my ($firstnode, $output, @lines, $mount_point, $sharevol, $sharevols, $sharedg, $mounted, $sysi, $sys, $system);
...
package Prod::SFCFSHA62::AIX;
@Prod::SFCFSHA62::AIX::ISA = qw(Prod::SFCFSHA62::Common);

sub init_plat {
    my $prod=shift;
    $prod->{allpkgs}=[ qw(VRTSob34 VRTSglm62 VRTScavf62 VRTSgms62 VRTSodm62) ];
    $prod->{minpkgs}=[ qw(VRTSglm62 VRTScavf62) ];
    $prod->{recpkgs}=[ qw(VRTSob34 VRTSglm62 VRTScavf62 VRTSgms62 VRTSodm62) ];
...
sub version_mapping_sys {
    my ($prod,$sys,$vers)=@_;
    my ($subprod);
    # to handle VRTScavf version is 1.0.4 in 4.0MP4 release.
    $vers='4.0.4' if (EDRu::compvers($vers, '1.0.4', 3)==0);
    # 6.1.1 VRTScavf doesn't have a Patch. Hence need to check its sub-product to distinguish 6.1.1/6.1
    if (EDRu::compvers($vers, '6.1.0.0', 4) == 0) {
        $subprod=$prod->prod('SFHA62');
        $vers=$subprod->version_sys($sys,1);
    }
    return $vers;
}

package Prod::SFCFSHA62::HPUX;
@Prod::SFCFSHA62::HPUX::ISA = qw(Prod::SFCFSHA62::Common);
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSdbed62 VRTSdbcom VRTSsydoc VRTSsybed VRTSvcsApache
        ...
        VRTSvcsag62 VRTScps62 VRTSvcs62 VRTSamf62 VRTSvxfen62 VRTSgab62
        VRTSllt62 VRTSfsmnd VRTSfssdk62 VRTSfsadv62 VRTSfsdoc VRTSfsman
        ...
        VRTSvmman SYMClma VRTSspt62 VRTSaa VRTSmh VRTSccg VRTSobgui
        VRTSfspro VRTSdsa VRTSsfmh61 VRTSob34 VRTSobc33 VRTSaslapm62
...
package Prod::SFCFSHA62::Linux;
@Prod::SFCFSHA62::Linux::ISA = qw(Prod::SFCFSHA62::Common);
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        VRTScsocw VRTSvcswiz62 VRTSvbs62 VRTSvcsvmw62 VRTSvcsea62 VRTScfsdc VRTSodm62 VRTSodm-platform
        VRTSodm-common VRTSgms62 VRTSsvs62 VRTScavf62 VRTSglm62 VRTScpi
        ...
        VRTSdbed62 VRTSdbed-common VRTSdbcom-common VRTSsybed-common
        ...
        VRTSvcsmg VRTSvcsdr62 VRTSvcsag62 VRTScps62 VRTSvcs62 VRTSamf62
        VRTSvxfen62 VRTSgab62 VRTSllt62 VRTSveki62 VRTSfsmnd VRTSfssdk62
        VRTSfsadv62 VRTSfsdoc VRTSfsman VRTSvrdoc VRTSvrw VRTSweb VRTSvcsvr
        VRTSvrpro VRTSalloc VRTSdcli VRTSvsvc VRTSvmpro VRTSddlpr
        VRTSvdid VRTSlvmconv VRTSvmdoc VRTSvmman SYMClma
        VRTSspt62 VRTSaa VRTSmh VRTSccg VRTSobgui VRTSfspro
        VRTSdsa VRTSsfmh61 VRTSob34 VRTSobc33 VRTSaslapm62
    ...
    $prod->{obsolete_scripts} = [ qw(installsfsybasece62 uninstallsfsybasece62) ];
    return;
}

package Prod::SFCFSHA62::RHEL5x8664;
@Prod::SFCFSHA62::RHEL5x8664::ISA = qw(Prod::SFCFSHA62::Linux);

sub init_padv {
    my $prod=shift;
    if ($prod->svs()) {
        $prod->{minpkgs}=[ qw(VRTSsfmh61 VRTSglm62 VRTScavf62 VRTSsvs62) ];
        $prod->{recpkgs}=[ qw(VRTSob34 VRTSglm62 VRTScavf62 VRTSsvs62 VRTSgms62 VRTSodm62) ];
        $prod->{allpkgs}=[ qw(VRTSob34 VRTSglm62 VRTScavf62 VRTSsvs62 VRTSgms62 VRTSodm62) ];
        $prod->{proddir}='virtualstore';
    } elsif ($prod->sfcfsha) {
        $prod->{licsuperprods}=[ qw(SFRAC62) ];
    }
...
package Prod::SFCFSHA62::RHEL6x8664;
@Prod::SFCFSHA62::RHEL6x8664::ISA = qw(Prod::SFCFSHA62::Linux);
...
sub init_padv {
    ...
    if ($prod->svs()) {
        ...
        $prod->{licsuperprods}=[ qw(SFRAC62 SFSYBASECE62) ];
    }
    return ;
}

package Prod::SFCFSHA62::RHEL6ppc64;
@Prod::SFCFSHA62::RHEL6ppc64::ISA = qw(Prod::SFCFSHA62::Linux);

package Prod::SFCFSHA62::RHEL7x8664;
@Prod::SFCFSHA62::RHEL7x8664::ISA = qw(Prod::SFCFSHA62::Linux);
...
package Prod::SFCFSHA62::OL6x8664;
@Prod::SFCFSHA62::OL6x8664::ISA = qw(Prod::SFCFSHA62::RHEL6x8664);

package Prod::SFCFSHA62::OL7x8664;
@Prod::SFCFSHA62::OL7x8664::ISA = qw(Prod::SFCFSHA62::RHEL7x8664);

package Prod::SFCFSHA62::SLES10x8664;
@Prod::SFCFSHA62::SLES10x8664::ISA = qw(Prod::SFCFSHA62::Linux);
...
package Prod::SFCFSHA62::SLES11x8664;
@Prod::SFCFSHA62::SLES11x8664::ISA = qw(Prod::SFCFSHA62::Linux);
...
package Prod::SFCFSHA62::SLES11ppc64;
@Prod::SFCFSHA62::SLES11ppc64::ISA = qw(Prod::SFCFSHA62::Linux);
...
package Prod::SFCFSHA62::SunOS;
@Prod::SFCFSHA62::SunOS::ISA = qw(Prod::SFCFSHA62::Common);
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSgms62 VRTSsvs62 VRTScavf62 VRTSglm62 VRTScpi VRTSd2doc VRTSordoc
...
package Prod::SFCFSHA62::SolSparc;
@Prod::SFCFSHA62::SolSparc::ISA = qw(Prod::SFCFSHA62::SunOS);
...
package Prod::SFCFSHA62::Sol11sparc;
@Prod::SFCFSHA62::Sol11sparc::ISA = qw(Prod::SFCFSHA62::SunOS);
...
sub init_padv {
    ...
    if ($prod->svs()) {
        $prod->{minpkgs}=[ qw(VRTSsfmh61 VRTSglm62 VRTScavf62 VRTSsvs62) ];
        $prod->{recpkgs}=[ qw(VRTSglm62 VRTScavf62 VRTSsvs62 VRTSgms62 VRTSodm62) ];
        $prod->{allpkgs}=[ qw(VRTSglm62 VRTScavf62 VRTSsvs62 VRTSgms62 VRTSodm62) ];
        $prod->{proddir}='virtualstore';
    } else {
        $prod->{minpkgs}=[ qw(VRTSglm62 VRTScavf62) ];
        $prod->{recpkgs}=[ qw(VRTSglm62 VRTScavf62 VRTSgms62 VRTSodm62) ];
        $prod->{allpkgs}=[ qw(VRTSglm62 VRTScavf62 VRTSgms62 VRTSodm62) ];
        $prod->{proddir}='storage_foundation_cluster_file_system_ha';
    }
...
package Prod::SFCFSHA62::Solx64;
@Prod::SFCFSHA62::Solx64::ISA = qw(Prod::SFCFSHA62::SunOS);
...
package Prod::SFCFSHA62::Sol11x64;
@Prod::SFCFSHA62::Sol11x64::ISA = qw(Prod::SFCFSHA62::SunOS);
