...
package Prod::SFSYBASECE62::Common;
@Prod::SFSYBASECE62::Common::ISA = qw(Prod::SFCFSHA62::Common);
    ...
    if ($ase_resource) {
         for my $node ( @{CPIC::get("systems") } ) {
            ...
            if ((EDR::cmdexit() == 0) && ($running + 0) > 0) {
                my $sysname = Prod::VCS62::Common::transform_system_name($node->{sys});
...
sub bring_sybase_online {
    ...
    foreach my $sys1 (@{CPIC::get("systems")}){
        my $sysname = Prod::VCS62::Common::transform_system_name($sys1->{sys});
...
sub wait_vm_enable {
    ...
    if ( $vm_mode !~ /enable/ ) {
        $msg=Msg::new("Configure VxVM before configuring ASE CE in VCS.");
        $msg->warning;
        Msg::prtc();
        return 0;
    }
...
sub add_service_group {
    ...
    for my $nsys ( 0 .. $#{$sysobjs} ) {
        my $sysname = Prod::VCS62::Common::transform_system_name(@{$sysobjs}[$nsys]->{sys});
        $syss    .= "$sysname ";
...
sub addnode_poststart {
    ...
    my $sfcfsha=$prod->prod('SFCFSHA62');
...
sub addnode_ask_sybase_config {
    ...
    $system = Prod::VCS62::Common::transform_system_name($firstnode->{sys});
...
sub addnode_ask_sybase_server {
    ...
    for my $sysi (@{$cfg->{newnodes}}) {
        $system = Prod::VCS62::Common::transform_system_name($sysi);
        while (1){
...
package Prod::SFSYBASECE62::AIX;
@Prod::SFSYBASECE62::AIX::ISA = qw(Prod::SFSYBASECE62::Common);
...
sub init_plat {
    ...
    $prod->{upgradevers}=[qw(4.0.4 5.0 5.1 6.0 6.1 6.2)];
    $prod->{zru_releases}=[qw(5.0.3 5.1 6.0 6.1 6.2)];

    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        VRTScsocw.rte VRTSdbac62 VRTSdbac.rte VRTSvbs62 VRTSvcsea62 VRTScfsdc
...
sub version_mapping_sys {
    my ($prod,$sys,$vers)=@_;
    return Prod::SFCFSHA62::AIX::version_mapping_sys($prod,$sys,$vers);
}

package Prod::SFSYBASECE62::HPUX;
@Prod::SFSYBASECE62::HPUX::ISA = qw(Prod::SFSYBASECE62::Common);
...
package Prod::SFSYBASECE62::Linux;
@Prod::SFSYBASECE62::Linux::ISA = qw(Prod::SFSYBASECE62::Common);
...
sub init_plat {
    ...
    $prod->{recpkgs}=[ qw(VRTSob34 VRTSglm62 VRTScavf62 VRTSgms62 VRTSodm62) ];
    $prod->{proddir}='storage_foundation_for_sybase_ce';
    $prod->{upgradevers}=[qw(4.1.40 5.0 5.1 6.0 6.1 6.2)];
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        VRTScsocw VRTSvcswiz62 VRTSvbs62 VRTSvcsea62 VRTScfsdc VRTSodm62 VRTSodm-platform
        VRTSodm-common VRTSgms62 VRTScavf62 VRTSglm62 VRTScpi
...
package Prod::SFSYBASECE62::SunOS;
@Prod::SFSYBASECE62::SunOS::ISA = qw(Prod::SFSYBASECE62::Common);
...
sub version_mapping_sys {
    my ($prod,$sys,$vers)=@_;
    return Prod::SFCFSHA62::SunOS::version_mapping_sys($prod,$sys,$vers);
}

package Prod::SFSYBASECE62::SolSparc;
@Prod::SFSYBASECE62::SolSparc::ISA = qw(Prod::SFSYBASECE62::SunOS);

sub init_padv {
    my $prod=shift;
    $prod->{upgradevers}=[qw(4.1.2 5.0 5.1 6.0 6.1 6.2)];
    $prod->{zru_releases}=[qw(5.0.3 5.1 6.0 6.1 6.2)];
    return;
}

package Prod::SFSYBASECE62::Sol11sparc;
@Prod::SFSYBASECE62::Sol11sparc::ISA = qw(Prod::SFSYBASECE62::SunOS);
...
package Prod::SFSYBASECE62::Solx64;
@Prod::SFSYBASECE62::Solx64::ISA = qw(Prod::SFSYBASECE62::SunOS);
...
package Prod::SFSYBASECE62::Sol11x64;
@Prod::SFSYBASECE62::Sol11x64::ISA = qw(Prod::SFSYBASECE62::SunOS);
