...
package Prod::VCS62::Common;
@Prod::VCS62::Common::ISA = qw(Prod);
...
sub init_common {
    ...
    $prod->{mainpkg}='VRTSvcs62';
    $prod->{extra_mainpkgs}=[qw(VRTSllt62 VRTSgab62 VRTSvxfen62 VRTSvcsag62)];
    ...
    $prod->{installonupgradepkgs} = [ qw(VRTSamf VRTSsfmh VRTSvbs VRTSvcswiz) ];
    ...
    $prod->{superprods} = [qw(SFRAC62 SVS62 SFSYBASECE62 SFCFSHA62)];

    $prod->{replace_cps_key} = 0;
    
    #VCS support online upgrade which means zero downtime for application.
    $prod->{support_zero_downtime} = 1;

    $prod->{upgrade_backup} = [
        '/opt/VRTSvcs/portal/admin/.xprtlaccess',
        '/opt/VRTSvcs/portal/admin/conf/settings.conf',
...
sub verify_procs_list_sys {
    ...
    if ((defined $state && $state eq 'stop') && (Cfg::opt('upgrade_kernelpkgs') && (!EDRu::inarr('had62',@{$procs})))) {
        push(@{$procs},'had62');
        push(@{$procs},'CmdServer62');
...
sub vcs_enable_sys {
    ...
    if ($cfg->{vcs_allowcomms}) {
        @list = qw(llt62 gab62 had62 amf62);
...
sub vcs_disable_sys {
    ...
    for my $proci (qw(llt62 gab62 had62 vxfen62 amf62)) {
...
sub set_preferredfencing_sys {
    my ($prod,$sys) = @_;
    my ($maincf,$tmpmaincf,$pkg,$vxfenmodefile);

    # in 6.2, we will remove PreferredFencingPolicy when we'd like to change the fencing mode to disabled or majority
    $pkg = $prod->pkg('VRTSvxfen62');
    return 1 if (!$sys->exists($pkg->{vxfenmode_file}));
    $vxfenmodefile = $sys->readfile($pkg->{vxfenmode_file});
    for my $vxfenmode (split(/\n+/,$vxfenmodefile)) {
        if ($vxfenmode =~ /^vxfen_mode=(\s*)$/) {
            return 1 if ($1 !~ /^(disabled|majority)$/im);
        } 
    }

    return 1 if (!$sys->exists($prod->{maincf}));
    $maincf = $sys->readfile($prod->{maincf});
    for my $line (split(/^/m,$maincf)) {
        next if($line =~ /PreferredFencingPolicy/m);
        #e3611032:remove this attributes if it's majority fencing.
        next if($line =~ /FencingWeight/m);
        $tmpmaincf .= $line;
    }
    $sys->writefile($tmpmaincf,$prod->{maincf});
...
sub ask_upgrade_configure {
    ...
    # if before upgrade the prod version is less than 6.1, should ask cps fencing configuration
    if (EDRu::compvers(${$sys->{prodvers}}[0],'6.1.0.000') == 2) {
...
sub upgrade_online_upgrade_check_sys {
    my ($prod, $sys) = @_;
    my ($cprod, $msg);

    return 1 unless (Cfg::opt("online_upgrade"));
        
    $cprod = Obj::cpic()->prod();
    unless ($cprod->{support_zero_downtime}) {
        $msg=Msg::new("$cprod->{prod} is installed on $sys->{sys}, only VCS or ApplicationHA supports online upgrade.\n");
        $sys->push_error($msg);
        return '';
     }
...
sub upgrade_precheck_sys {
    ...
    if ($sys->exists("$rootpath/opt/VRTSvcs/bin/vcsenv") && $sys->padv->file_modified_sys($sys, '/opt/VRTSvcs/bin/vcsenv')) {
        ...
        $sys->cmd('_cmd_cp -p $rootpath/opt/VRTSvcs/bin/vcsenv $rootpath/opt/VRTSvcs/bin/vcsenv.backup');
        if ($rootpath) {
...
sub post_precheck_task {
    ...
    # # if before upgrade the prod version is less than 6.1, prompt users these warning messages
    if ((defined $cfg->{client_vxfen_warning}) && !Cfg::opt(qw(upgrade_nonkernelpkgs)) && EDRu::compvers(${$sys->{prodvers}}[0],'6.1.0.000') == 2) {
    ...
    $prod->upgrade_online_upgrade_check_sys($sys);
    return 1;
}
...
sub phased_upgrade_precheck {
    ...
    if ($csystems < $nsystems) {
        ...
        for my $sysi (split(/\n/,$str)) {
            ...
            unless (EDRu::inarr($sysi, @sysnames)) {
                ...
                if (EDRu::compvers($ver1,$veri,4) == 2) {
                    ...
                    $vcs->set_value('phased_upgrade_2', 1);
                    return '';
                }
            }
        }
        #e3412014:installer should prompt user no matter whether the cluster is security.
        ...
        $sys1->push_note($msg);
        Cfg::set_opt('nostart', 1);
        $vcs->set_value('phased_upgrade_1', 1);
    } else {
...
sub completion_messages {
    my ($prod)=@_;
    my ($cfg,$msg,$rel,$web,$syslist,$sys,$pkg,$msg_printed);
    my ($localsys, $mediapath, $tmpdir, $testVMware);
    $web = Obj::web();
    $rel = Obj::rel();
    ...
    $prod->upgrade_check_old_cps() if (Cfg::opt(qw(upgrade patchupgrade hotfixupgrade/)));
    ...
    return 1 unless (Cfg::opt(qw(configure install addnode upgrade patchupgrade hotfixupgrade)));
    ...
    if(defined($pkg) && $pkg->version_sys($sys)) {
        ...
        } elsif ($localsys->exists("$mediapath/scripts/bin/testVMware")) {
            $testVMware = "$mediapath/scripts/bin/testVMware";
            $localsys->copy_to_sys($sys,$testVMware,"$tmpdir/");
            $testVMware="$tmpdir/testVMware";
        }
        if($sys->exists($testVMware)) {
            $sys->cmd($testVMware);
            if(EDR::cmdexit() eq '0') {
...
sub completion_message_rollingupgrade {
    ...
    @subs = qw(systems set_pkgs upgrade_prestop_config shutdown uninstall install licensing prestart_config startup);
    for my $sysname (@{$rel->{cluster_systems}}) {
        $sys=($Obj::pool{"Sys::$sysname"}) ? Obj::sys($sysname) : Sys->new($sysname);
    ...
    if(@rup1list && @rup2list) {
        if(@{$cpic->{reboot_systems}}){
            ...
            if($sclist) {
                ...
                for my $key (qw/installpkgs uninstallpkgs installpatches uninstallpatches installpkgslast/) {
                    delete $cpic->{$key};
                }
    ...
    if ($conf) {
        if ($sys->system1) {
            ...
            if ( $m < $n) {
                ...
                return 1 unless ($cprod eq 'VCS62' || $cprod eq 'SFHA62');
...
sub vxfen_reuse_precheck {
    ...
    $web = Obj::web();
    $sys = ${CPIC::get('systems')}[0];
    $cpspkg = $prod->pkg('VRTScps62');
    $vxfenpkg = $prod->pkg('VRTSvxfen62');
...
sub vxfen_upgrade_precheck_sys {
    ...
    if ($sys->exists("$rootpath$prod->{vxfenmode}")) {
        ...
        # VCS no longer supports vxfen configuration with "scsi_disk_policy=raw" from 6.2 onwards
        if ($conf->{scsi3_disk_policy} eq 'raw') {
            if ((!$conf->{deprecate_raw_policy}) || ($conf->{deprecate_raw_policy} ne 'override')) {
                $msg=Msg::new("Upgrade is not supported when fencing is configured with raw disk policy. Change the disk interaction policy from scsi3_disk_policy=raw to scsi3_disk_policy=dmp first using vxfenswap utility on $sys->{sys}. Refer to VCS administrator guide for detailed steps.");
                $sys->push_error($msg);
                return 1;
            }    
        }
...
sub get_vxfen_config_sys {
    ...
    my $vrtscps_pkg = $prod->pkg('VRTScps62');
    my $rootpath = Cfg::opt('rootpath') || '';
    ...
    for my $line (split(/^/m,$str)) {
        ...
        } elsif ($line =~ /^\s*deprecate_raw_policy\s*=\s*(\S+)/mx) {
            $conf->{deprecate_raw_policy}=$1;  
        }     
    }
...
sub upgrade_prestop_config {
    my ($prod) = @_;
    my ($cpic,$sys,$secureclus);

    # only ask usrgroup info for regular upgrade and rolling upgrade phase 2
    return 1 if (!Cfg::opt(qw(upgrade)) || Cfg::opt('upgrade_kernelpkgs'));

    $cpic = Obj::cpic();
    $sys = ${$cpic->{systems}}[0];
    $secureclus = $prod->is_secure_cluster($sys);
    return 1 if (!$secureclus);
    
    # only ask question on the first node when doing phase upgrade
    return 1 if ($prod->{phased_upgrade_2});

    # if before upgrade the cluster is secure, ask users to enter the usergoups for which they want to give guest privilege    
    $prod->secureclus_usergroups_config($prod);
    return 1;
}

sub secureclus_usergroups_config {
    my ($prod,$appha) = @_;
    my ($cpic,$sys,$cfg,$backopt,$msg,$ayn,$answer,@secgrps,$help,$rtn,$rereadaccess,$web,$usrgrpspecformat);
    ...
    $web = Obj::web();
    $backopt = 1;
    $usrgrpspecformat = 'usrgrp1@node1';
    Msg::n();
    while (1) {
        $rereadaccess = 0;
        $msg = Msg::new("Do you want to grant read access to everyone?");
        $help= Msg::new("If you choose 'Y', everyone will have read access to the cluster configuration using command line interface.");
        $ayn = $msg->aynn($help);
        Msg::n();
        if ($ayn eq 'N') {
            # ask users if they want to provide usergroups which will get the read access
            $msg = Msg::new("Do you want to provide any usergroups that you would like to grant read access?");
            $help = Msg::new("If you choose 'N', only root and other users/usergroups which are granted explicit privileges on VCS objects will get read access to the cluster.");
            $ayn = $msg->ayny($help);
            Msg::n();
            $cfg->{rootsecusrgrps} = 1 if ($ayn eq 'N');
            return 1 if ($ayn eq 'N');            

            # users will enter the usergroups that they would like to grant read access
            if(Obj::webui()){
                $web->web_script_form('usergroup',$msg);
            } else {
                while (1) {
                    if ($appha && ($appha =~ /AppHA/m)) {
                        $msg = Msg::new("Enter the usergroup names separated by spaces that you would like to grant read access, enter like 'usrgrp1'. If some usergroups are not created yet, create the usergroups after configuration if needed.");
                    } else {
                        $msg = Msg::new("Enter the usergroup names separated by spaces that you would like to grant read access. If you would like to grant read access to a usergroup on a specific node, enter like '$usrgrpspecformat', and if you would like to grant read access to usergroup on any cluster node, enter like 'usrgrp1'. If some usergroups are not created yet, create the usergroups after configuration if needed.");
                    }
                    ...
                    @secgrps = split(/\s+/m,$answer);
                    $rtn = $prod->validate_secclus_usrgrps(@secgrps);
                    next if ($rtn);
                    last;
                }
                next if ($rereadaccess);
            
                # add "" for each element
                for my $secusrgrp (@secgrps) {
                    $secusrgrp = "\"$secusrgrp\"";
                }
                $cfg->{secusrgrps} = \@secgrps;
            }
        }
        last;
    }
...
sub validate_secclus_usrgrps {
    my ($prod,@secgrps) = @_;
    my ($rtn,$msg);
   
    # check the elments in array are unique
    if (!EDRu::arr_isuniq(@secgrps)) {
    ...
    # check if the format of usergroup names is right
    for my $secusrgrp (@secgrps) {
        $rtn = $prod->validate_secclus_usrgrps_name($secusrgrp);
        return 1 if($rtn);
    }
...
sub validate_secclus_usrgrps_name {
    my ($prod,$usrgrpname) = @_;
    my ($cpic,$sys0,$maxlen,$msg,$usrgrpnamelength);

    $cpic=Obj::cpic();
    $sys0 = ${$cpic->{systems}}[0];

    if ($prod->vcs_reservedwords($usrgrpname)) {
        $msg = Msg::new("The usergroup name $usrgrpname is a VCS reserved word. Input again");
        $msg->print;
        Msg::n();
        return 1;
    }

    if ($usrgrpname !~ /^[a-zA-Z|_][\w\.\-\@]*$/mx) {
        $msg = Msg::new("The usergroup name $usrgrpname contains invalid character. Input again");
        $msg->print;
        Msg::n();
        return 1;
    }

    $maxlen = $sys0->cmd("_cmd_getconf LOGIN_NAME_MAX 2>/dev/null");
    chomp($maxlen);
    if ($usrgrpname =~ /(.*)\@/mx) {
        $usrgrpnamelength = length($1);
    } else {
        $usrgrpnamelength = length($usrgrpname);
    }
    if ($maxlen < $usrgrpnamelength) {
        $msg = Msg::new("The usergroup name cannot exceed $maxlen characters. Input again");
        $msg->print;
        Msg::n();
        return 1;
    }
...
sub maincf_upgrade_precheck_sys {
    ...
    if ($sys->system1) {
        ...
        for my $pkgi (qw(VRTSvcsea62 VRTSdbac62 VRTSvcsvmw62)) {
            $pkg = $prod->pkg($pkgi,1);
        ...
        for my $extra_type (@extra_types,@{$prod->{extra_types}}) {
            ...
            if (EDRu::inarr($extra_type_base, @{$sourcefiles})) {
                $sys->cmd("_cmd_cp -p $upgradepath/old_config/$extra_type_base $upgradepath/old_conf")
...
sub padv_maincf_update_netmask_sys {
    ...
    @reslist = Prod::VCS62::Common::dynupgrade_get_resname_list_from_typename_list($treeref, $type_list);
    for my $res (@reslist) {
        $noderef = Prod::VCS62::Common::dynupgrade_get_node_from_tree($treeref, 'RES', $res);
        ...
        $attrref_netmask = Prod::VCS62::Common::dynupgrade_get_node_from_tree($noderef, 'ATTR', 'NetMask');
        if (!$attrref_netmask) {
            $addressref = Prod::VCS62::Common::dynupgrade_get_node_from_tree($noderef, 'ATTR', $address_name);
...
sub upgrade_preremove_sys {
    ...
    for my $file (@{$prod->{upgrade_backup}}) {
        $file = $rootpath.$file if ($rootpath);
        next unless ($sys->exists($file));
        $sys->cmd("_cmd_cp -pf $file $tmpdir/VCS-CFG-BAK/");
    }
...
sub replace_obsolete_cps_key_sys {
    my ($prod,$sys) = (@_);
    my ($cpspkg,$msg,$rootpath,$obsolete_keyfile,$cps_keyfile,$singlenode_licensed,$out,$obsoleted_keys);
    $rootpath = Cfg::opt('rootpath');
    $rootpath ||='';
    # remove the trailing '/'
    $rootpath =~ s/\/*$//g;
    ...
    # Fix for 3554720 - need to replace all previous releases' CPS keys with 6.2 key
    # Fix for ET3067356 - CPI did not upgrade the CPS key when upgrading vcs6.0 to vcs6.1.
    return unless (@{$cpspkg->{obsolete_cpskey}});
    $obsoleted_keys = "";
    for $obsolete_keyfile (@{$cpspkg->{obsolete_cpskey}}) {
        $obsolete_keyfile = "$rootpath/etc/vx/licenses/lic/$obsolete_keyfile\.vxlic";
        ...
        } elsif ($sys->exists($obsolete_keyfile)) {
            $obsoleted_keys .= EDRu::basename($obsolete_keyfile) ."\n";
        }
    }
    return unless ($obsoleted_keys);

    $obsolete_keyfile = join ",", @{$cpspkg->{obsolete_cpskey}};
    if (!$rootpath) {
        ...
        if ($out =~ /\b(successfully|Duplicate)\b/m) {
            ...
            for $obsolete_keyfile (split /\n/, $obsoleted_keys) {
                $obsolete_keyfile = "$rootpath/etc/vx/licenses/lic/$obsolete_keyfile";
                $sys->cmd("_cmd_rmr $obsolete_keyfile");
            }
            # force license step to re-load keys
            $sys->set_value ('donotquerykeys', 0);
        } else {
            Msg::log("Single node cps key did not register successfully on $sys->{sys}, manually replace obsolete cps key(s) $obsolete_keyfile with new key $cpspkg->{cpskey}");
        }
    } else {
        # Cannot register new key on alt disk, prompt to manually replace the old key.
        $msg = Msg::new("Found the following obsolete license key(s) for the coordination point server under the directory $rootpath/etc/vx/licenses/lic/ on $sys->{sys}, manually remove them and register a new coordination point server license key $cpspkg->{cpskey} after an upgrade:\n$obsoleted_keys\n");
        $sys->push_warning($msg);
    }
    return;
}
...
sub backup_vcs_lock_files_sys {
    ...
    if ($sys->cmd("_cmd_ls $lockfolder/*_online_lock 2>/dev/null")){
        ...
        $sys->cmd("_cmd_cp -p $lockfolder/*_online_lock $lockfilepath/");
    }
    ...
    if ($nfsrestart){
        $sys->cmd("_cmd_mkdir -p $lockfilepath");
        $sys->cmd("_cmd_cp -p $lockfolder/.nfsrestart_*_state $lockfilepath/ 2>/dev/null");
        $sys->cmd("_cmd_cp -p $lockfolder/.nfs_lock_* $lockfilepath/ 2>/dev/null");
    }
    # 2. .nfs4_status_file
    ...
    if ($nfs4){
        $sys->cmd("_cmd_mkdir -p $lockfilepath");
        $sys->cmd("_cmd_cp -p $lockfolder/.nfs4_status_file $lockfilepath/ 2>/dev/null");
    }
...
sub restore_vcs_lock_files_sys {
    ...
    for my $oldlockfile (@files) {
        ...
        $sys->cmd("_cmd_cp -p $lockfilepath/$oldlockfile $newfolder/$newlockfile") unless $sys->exists("$newfolder/$newlockfile");
    }
    ...
    @files = split(/\s+/m, $files);
    for my $oldlockfile (@files) {
        $sys->cmd("_cmd_cp -p $lockfilepath/$oldlockfile $newfolder/$oldlockfile") unless $sys->exists("$newfolder/$oldlockfile");
    }

    # e3419772, need copy .nfs4_status_file if NFSv4 is enabled
    $sys->cmd("_cmd_cp -p $lockfilepath/.nfs4_status_file $newfolder/") if $sys->exists("$lockfilepath/.nfs4_status_file");
...
sub startprocs_sys {
    ...
    if ($cfg->{vcs_allowcomms}) {
        if ((!$sys->exists("$prod->{vxfenmode}")) && (!$sys->exists("$prod->{vxfendg}"))) {
            $procs = EDRu::arrdel($procs, 'vxfen62');
        }
        return $procs;
    } else {
        $procs = EDRu::arrdel($procs, 'llt62');
        $procs = EDRu::arrdel($procs, 'gab62');
        $procs = EDRu::arrdel($procs, 'vxfen62');
        return $procs;
    }
}
...
sub register_extra_types {
    ...
    for my $sys (@$syslist) {
        ...
        for my $pkgi (qw(VRTSvcsea62 VRTSdbac62)) {
            $pkg = $prod->pkg($pkgi,1);
            ...
            for my $ext (@{$pkg->{extra_types}}) {
                ...
                if ($sys->exists($ext)) {
                    $sys->cmd("_cmd_cp -p $ext $conf_dir") unless (EDRu::dirname($ext) eq $conf_dir);
                    $sys->cmd("_cmd_cp -p $ext $config_dir");
                } else {
        ...
        if($extypes) {
            ...
            if (!$rootpath && $had->check_sys($sys, 'start')) {
                ...
                for my $extype (@$extypes) {
                    $new_inc .= "\ninclude \"$extype\"";
                    $sys->cmd("_cmd_cp -p $prod->{configdir}/$extype $tmpdir/dir_extypes");
                }
                ...
                $new_inc .= "\n";
                $sys->cmd("_cmd_cp -p $rootpath$prod->{maincf} $rootpath$prod->{maincf}.$uuid");
...
sub default_systemnames {
    ...
    $cpic=Obj::cpic();
    $rel=$cpic->rel;
    return $rel->default_systemnames if(Cfg::opt(qw(online_upgrade rolling_upgrade upgrade_kernelpkgs upgrade_nonkernelpkgs)));
    my $rootpath = Cfg::opt('rootpath')||'';
...
sub responsefile_comments {
    ...
    $cmt=Msg::new("This variable defines the usergroup names granted read access to the cluster");
    $edr->{rfc}{secusrgrps}=[$cmt->{msg},0,1,0];
    $cmt=Msg::new("This variable defines if user chooses to grant read access to the cluster only for root and other users/usergroups which are granted explicit privileges on VCS objects");
    $edr->{rfc}{rootsecusrgrps}=[$cmt->{msg},0,0,0];
    ...
    $cmt=Msg::new("This variable defines a series of sub-cluster division. The index '#' indicates the order to perform Rolling Upgrade phase 1. The index '#' starts from 0. Each item has a list of nodes");
    $edr->{rfc}{'phase1__#'}=[$cmt->{msg},1,1,0];
    $pkg=$prod->pkg('VRTSvxfen62');
    $pkg->responsefile_comments_for_fencing();
    $pkg=$prod->pkg('VRTScps62');
...
sub prestart_config_common_questions {
    ...
    $cfg->{vcs_allowcomms} = 1;
    #if ((($cprod eq 'VCS62')||($cprod eq 'SFHA62')) && (!$cfg->{fencingenabled})) {
    if ($cprod eq 'VCS62'||$cprod eq 'SFHA62'){
        if (scalar(@$syslist) == 1) {
...
sub web_prestart_config_questions {
    my ($rtvalue,$prod,$web,$cfg,$key,$msg,$help,$ayn,$multinode,$rpn,$alertmsg,$nic,$nics);
    $prod = shift;
    ...
    while (1) {
            ...
            if ($cfg->{autocfgllt}) {
                ...
                for my $sys (@{CPIC::get('systems')}) {
                    ...
                    if ($prod->{llt_rdma}) {
                        ...
                        $nics = $padv->systemnics_sys($sys,1,1)
                    }
        ...
        $prod->web_configure_static_ips();

        $rtvalue=$web->web_script_form('vcs_optional');
        next if($rtvalue eq 'back');

        if($cfg->{vcs_eat_security}){
            $prod->secureclus_usergroups_config();
        } else {
            my $webmsg = Msg::new("Symantec recommends that you install the cluster in secure mode. This ensures that communication between cluster components is encrypted and cluster information is visible to specified users only.");
            $msg = Msg::new("\n\nAre you sure that you want to proceed with non-secure installation?");
            my $rtn = $msg->aynn('','',$webmsg);
            next if ($rtn eq 'N'); 
...
sub web_configure_static_ips {
    my ($prod) = @_;
    my $web = Obj::web();
    my ($address, $padv, $nic, $netmask, $sysi);

    for my $n (1..$web->{numOfHeartbeats}) {
        for my $sys (keys %{$web->{rhbn}{"lltlink$n"}}) {
            $sysi = Obj::sys($sys);
            $padv = $sysi->padv;
            $nic = $web->{rhbn}{"lltlink$n"}{$sys};
            if ($web->{rhbn}{"udplink$n".'_address'}) {
                $address = $web->{rhbn}{"udplink$n".'_address'}{$sys};
                $netmask = $web->{rhbn}{"udplink$n".'_netmask'}{$sys};
            } elsif ($web->{rhbn}{"rdmalink$n".'_address'}) {
                $address = $web->{rhbn}{"rdmalink$n".'_address'}{$sys};
                $netmask = $web->{rhbn}{"rdmalink$n".'_netmask'}{$sys};
            } else {
                next;
            }
            $padv->configure_static_ip_sys($sysi, $nic, $address, $netmask);
        }
    }
    for my $n (1..$web->{numOfLowPriority}) { 
        for my $sys (keys %{$web->{rhbn}{"lltlinklowpri$n"}}) {
            $sysi = Obj::sys($sys);
            $padv = $sysi->padv;
            $nic = $web->{rhbn}{"lltlinklowpri$n"}{$sys};
            if ($web->{rhbn}{"udplinklowpri$n".'_address'}) { 
                $address = $web->{rhbn}{"udplinklowpri$n".'_address'}{$sys};
                $netmask = $web->{rhbn}{"udplinklowpri$n".'_netmask'}{$sys};
            } elsif ($web->{rhbn}{"rdmalinklowpri$n".'_address'}) {
                $address = $web->{rhbn}{"rdmalinklowpri$n".'_address'}{$sys};
                $netmask = $web->{rhbn}{"rdmalinklowpri$n".'_netmask'}{$sys};
            } else {
                next;
            }
            $padv->configure_static_ip_sys($sysi, $nic, $address, $netmask);
...
# Check if a NIC supports ipv6 
sub nic_support_ipv6_sys {
    my ($prod,$sys,$nic) = @_;
    my ($padv,$ips);
    $padv=$sys->padv;
    $ips=$padv->nic_ips_sys($sys,$nic);
    for my $ip (@{$ips}) {
        return 1 if(EDRu::ip_is_ipv6($ip));
    }
    return 0;
}
...
sub config_gcoption {
    ...
    while (!$done) {
        ...
        $backopt = 1;
        if ($cfg->{vcs_csgvip}) {
            $vip = $prod->ask_vip($cfg->{vcs_csgvip},1,$prod->{rcsgnic});
            next if (EDR::getmsgkey($vip,'back'));
...
sub ask_netmask {
    my ($prod,$vip,$nic,$def) = @_;
    my ($msg,$help,$question,$answer,$cfg,$sys,$netm,$backopt,$prefix);
...
sub ask_vip {
    my ($prod,$def,$gco,$vipnic) = @_;
    my ($answer,$help,$msg,$backopt,$question,$failure);
    ...
    while(1) {
        ...
        if (!EDRu::isip($answer)) {
            ...
            $msg->print;
        } elsif (EDRu::ip_is_ipv6($answer)) {
            # return $answer if $vipnic is not a hash ref
            return $answer unless ($vipnic && %{$vipnic});
            $failure=0;
            # Check if $nic is plumbed IPv6 on each $sys
            while( my ($sys,$nic) = each %{$vipnic} ) {
                my $sysobj=Obj::sys($sys);
                unless ($prod->nic_support_ipv6_sys($sysobj,$nic)) {
                    $failure=1;
                    $msg = Msg::new("IPv6 on $nic is not plumbed up on $sys");
                    $msg->print;
                }
            }
...
sub get_config_sys {
    ...
    # get service groups
    $conf{groups} = [];
    $str=$sys->cmd("_cmd_grep '^group' $maincf 2>/dev/null| _cmd_awk '{print \$2}'");
...
sub prestart_sys {
    ...
    if (!$cfg->{donotreconfigurevcs}) {
        ...
        if ($cprod =~ /^(SVS|SFCFS|SFRAC|SFSYBASECE)/mx) {
            $sys->cmd("_cmd_cp -p $vxfenmodedisablefile $vxfenmodefile");
        } else {
            # for VCS/SFHA, do not start vxfen initially
            $sys->set_value('donotstartprocs','push','vxfen62');
            $str = EDRu::datetime();
...
sub configure_sys {
    ...
    $sys->cmd("_cmd_cp -p $rootpath$prod->{confdir}/types.cf $rootpath$prod->{configdir}");
...
sub create_uuid_sys {
    ...
    #3534964:Translate IP to hostname for /opt/VRTSvcs/bin/uuidconfig.pl needed.
    if ($sys->{islocal}) {
...
sub is_rdma_supported {
    ...
    #UEK2 don't support LLT over RDMA 
    if ($prod->{padv} =~ /RHEL6/
        && ${CPIC::get('systems')}[0]->{platvers} =~ /uek/m) {
        return 0;
    }
    return $prod->{padv} =~ /RHEL[67]|SLES11/ ? 1 : 0;
}
...
sub setup_rdma_env {
    ...
    if (Cfg::opt(qw/responsefile silent/)) {
...
sub check_rdma_nic_ips_sys {
    ...
    $padv = $sys->padv;
    $all_nics=$padv->systemnics_sys($sys,1,1);
    ...
    for my $sys (@{CPIC::get('systems')}) {
        ...
        if ($all) {
            ...
            if (EDRu::inarr($sys,@{CPIC::get('systems')})) {
                ...
                $msg->left;
                $padv=$sys->padv;
                $all_rsn=$padv->systemnics_sys($sys,1,1);
                $rsn=$all_rsn;
...
sub autocfg_hbnics {
    ...
    for my $sys (@{CPIC::get('systems')}) {
        ...
        $rnics->{$sysi} = $padv->systemnics_sys($sys,1,1);
        $cmd = "_cmd_rmr $tmpdir >/dev/null 2>&1 ;";
        $cmd .= "_cmd_mkdir -p $tmpdir >/dev/null 2>&1 ;";
        $cmd .= "_cmd_cp -p $rootpath/opt/VRTSllt/dlpiping $tmpdir/dlpiping_cpi >/dev/null 2>&1 ";
        $sys->cmd($cmd);
    ...
    for my $n (0..$#{$rnics->{$sysi}}) {
        ...
        if ($nic =~ /sit\d+/m || $nic =~ /vsw\d+/m){ # skip sit# on Linux, vsw# on Solaris
            ...
            if ($sys->linux()) {
                ...
                if ($mac=~/\b([0-9A-Fa-f:]+)\b/x) {
                    $mac=$1;
                } else {
                    Msg::log("Failed to get MAC address of $dev on $sysi.");
...
sub ask_hbnics_addnode {
    ...
    for my $sys (@{CPIC::get('systems')}) {
        $sysi=$sys->{sys};
        $padv=$sys->padv;
        $rsns->{$sysi}=$padv->systemnics_sys($sys,1,1);
...
sub get_next_available_ip {
    my ($prod,$ip) = @_;
    my ($localsys,$subnet,@ips);

    return $ip if (EDRu::ip_is_ipv6($ip));
...
sub ask_nic_ip_sys {
    ...
    my (@valid_ips,$default_ip,$default_netmask,$valid_ip,$is_ipv4,$subnet,$tmp_subnet,$next_id,$rtn,$linktype,$key,$netmask_key,$sys_ip,@used_ips,$localsys);
    ...
    for my $ip (@$ips) {
        ...
        if (!$is_ipv4) {
            # IPv6 address
            # RDMA do not support IPv6 address
            # Not support LINK-LOCAL IPv6 address
            next if ($rdma || (EDRu::iptype_ipv6($ip) =~ m/LINK-LOCAL/));
        }
        push @valid_ips, $ip;
    }
    $ips=\@valid_ips;
    ...
    if (defined $rhbn) {
        ...
        $netmask_key=$linktype.$id.'_netmask';
        if (defined $rhbn->{$key}) {
            ...
            for my $sys (keys %{$rhbn->{$key}}) {
                ...
                if (EDRu::ip_is_ipv4($sys_ip) &&
                    ...
                    $default_netmask||=$rhbn->{$netmask_key}{$sys};
                }
            }
            $next_id++;
            ...
            if ($id=~/^lowpri(\d+)/) {
                ...
                $netmask_key=$linktype.'lowpri'.($1-1).'_netmask';
            } elsif ($id=~/^\d+/) {
                $key=$linktype.($id-1).'_address';
                $netmask_key=$linktype.($id-1).'_netmask';
            }
            if (defined $rhbn->{$key}) {
    ...
    $default_ip=$prod->get_next_available_ip($default_ip);
    ...
    while (1) {
        ...
        if (!EDRu::isip($address)) {
            # not a IP address
            $valid_ip=0;
        } elsif (!$is_ipv4) {
            # IPv6 address
            if ($rdma ||
                (EDRu::iptype_ipv6($address) =~ m/LINK-LOCAL/)) {
                # RDMA do not support IPv6 address
                # Not support LINK-LOCAL IPv6 address
                $valid_ip=0;
            }
        ...
        if (!$valid_ip) {
            ...
            $rtn=1;
            $netmask=$prod->ask_netmask($address,$nic,$default_netmask);
            next if (EDR::getmsgkey($netmask,'back'));
...
sub convert_linkip2nic_sys {
    ...
    return '' if (!$linkip);
    $rnics = $sys->padv()->systemnics_sys($sys,1,1);
    $found = 0;
    for my $nici (@$rnics) {
        next unless $nici;
...
sub update_maincf {
    ...
    # add the usergroups info to main.cf for security
    if ($cfg->{vcs_eat_security}) {
        $maincf .= "\n\tCounterInterval = 5";
        if (!$cfg->{rootsecusrgrps} && !$cfg->{secusrgrps}) {
            $maincf .= "\n\tDefaultGuestAccess = 1\n\t)\n\n";
        } elsif ($cfg->{secusrgrps}) {
            my $secusrgrplist = join (', ', @{$cfg->{secusrgrps}});
            $maincf .= "\n\tGuestGroups = { $secusrgrplist }\n\t)\n\n";
        } else {
            $maincf .= "\n\t)\n\n";
        }
    } else {
...
sub upgrade_configure_sys {
    ...
    $cfg->{vcs_allowcomms} = 1 unless (defined($cfg->{vcs_allowcomms}));
    #e3528533:cleanup the registration on CPS when not starting LLT and GAB.
    if (!$cfg->{vcs_allowcomms}) {
        my $oldvxfenconf = $prod->get_vxfen_config_sys($sys);
        my $vxfenpkg = $prod->pkg('VRTSvxfen62');
        $vxfenpkg->cleanup_from_cps($oldvxfenconf);
        $sys->cmd("_cmd_rmr $rootpath$prod->{vxfenmode} 2>/dev/null");
    }
    ...
    if ($cfg->{vcs_allowcomms}) {
        ...
        #e3528533:reset vxfen configuration when not starting LLT and GAB.
        if (!$initfile_content || !defined($initfile_content->{vxfen}{VXFEN_START})) {
            $prod->set_initconf_variable_sys($sys, 'vxfen', 'VXFEN_START', '0');
        }
        if (!$initfile_content || !defined($initfile_content->{vxfen}{VXFEN_STOP})) {
            $prod->set_initconf_variable_sys($sys, 'vxfen', 'VXFEN_STOP', '0'); 
        }
    }
...
sub ru_prestop_sys {
    ...
    if (Cfg::opt('upgrade_kernelpkgs')) {
        ...
        $cprod=CPIC::get('prod');
        if($cprod=~/^SFRAC\d+/mx){
            $procobj=$sys->proc('vcsmm62');
            $procobj->prestop_sys($sys) if($procobj);
        }
        # Check if had is stopped
        $proc='had62';
        $procobj=$sys->proc($proc);
        # Use had62::Common::check_sys because
        # on had62::SunOS::check_sys, had will be considered stopped when SMF service system/vcs not online
        if (Proc::had62::Common::check_sys($procobj,$sys,'stop')) {
...
#                $sys->cmd("_cmd_cp -p $rootpath/opt/VRTSvcs/bin/sample_triggers/nfs_postoffline $rootpath/opt/VRTSvcs/bin/triggers/");
#            }
#        }
#    }
...
sub maincf_result_upgrade_sys {
    ...
    # Update vxattachd resource
    $treeref_result = $prod->maincf_result_upgrade_vxattachd_sys($sys, $treeref_result);
    ...
    # Update clus if needed
    $treeref_result = $prod->maincf_result_upgrade_clus_sys($sys, $treeref_result);
...
# e3617218 To add a ProcessOnOnly resource vxattachd in CVM service group
sub maincf_result_upgrade_vxattachd_sys {
    my ($prod,$sys,$treeref) =@_;
    my ($result_lines,$res_ref,$cprod,$rel,$sh);
    $cprod=CPIC::get('prod');
    $rel=Obj::rel();
    return $treeref unless ($rel->has_cvm($cprod));

    # Check if this resource exists
    $res_ref=dynupgrade_get_node_from_tree($treeref,'RES','vxattachd');
    if ($res_ref) {
        # If this resource already exists, it shouldn't be modified
        return $treeref;
    } else {
        if ($sys->{plat} eq 'Linux') {
            $sh="/bin/sh";
        } elsif (($sys->{plat} eq 'SunOS') || ($sys->{plat} eq 'HPUX')) {
            $sh="/sbin/sh";
        } elsif ($sys->{plat} eq 'AIX') {
            $sh="/usr/bin/sh";
        } else {
            Msg::log("Unrecognized platform for updating vxattachd resource");
            return $treeref;
        }
        $result_lines.="hares -add vxattachd ProcessOnOnly cvm\n";
        $result_lines.="hares -modify vxattachd PathName $sh\n";
        $result_lines.="hares -modify vxattachd Arguments \"%- /usr/lib/vxvm/bin/vxattachd root\"\n";
        $result_lines.="hares -modify vxattachd Critical 0\n";
        $result_lines.="hares -override vxattachd RestartLimit\n";
        $result_lines.="hares -modify vxattachd RestartLimit 3\n";
        $result_lines.="hares -modify vxattachd Enabled 1\n";
    }
...
sub maincf_result_upgrade_clus_sys {
    my ($prod,$sys,$treeref) = @_;
    my ($cfg,$result_lines);

    $cfg = Obj::cfg();
    # update the main.cf for guest guestgroups
    #e3620311:if the product is non-security before upgrade, the condition (!$cfg->{rootsecusrgrps} && !$cfg->{secusrgrps}) will be satisfied, but the main.cf shouldn't be added security information now.
    if ($prod->{eat_upgrade_secure}) {
        if (!$cfg->{rootsecusrgrps} && !$cfg->{secusrgrps}) {
            $result_lines .= "haclus -modify DefaultGuestAccess 1\n";
        } elsif ($cfg->{secusrgrps}) {
            my $secusrgrplist = join (' ', @{$cfg->{secusrgrps}}); 
            $result_lines .= "haclus -modify GuestGroups $secusrgrplist\n";
        }
    }
    # if os upgraded, CPI will use hacf of PBE, and following new attr will make the old hacf fail
    # so the cmds will be print to user
...
sub maincf_result_upgrade_TriggersEnabled_sys {
    ...
    # get the sys list that already have TriggersEnabled
    for my $grp_name (@grps) {
        @grp_syslist = Prod::VCS62::Common::dynupgrade_get_systemlist_of_grpname($treeref,$grp_name);
...
sub maincf_result_upgrade_Apache_sys {
    ...
    # if the ResLogLevel attribute is not set to 'TRACE' then no action is to be performed.
    # if the ResLogLevel attribute is set to 'TRACE', then perform the following actions:
    #   a) Override the LogDbg attribute at resource level
    #   b) Set LogDbg attribute to value 'DBG_5'.
    #   c) Set ResLogLevel attribute to value 'INFO'
    for my $res (@reslist) {
        $res_ref = dynupgrade_get_node_from_tree($treeref,'RES',$res);
        $attr_ref = dynupgrade_get_attr_from_node($res_ref,'ResLogLevel');
        $attr_value = $attr_ref->{MODIFY}->{ATTRVAL}->[0] if ($attr_ref);
        next if (!$attr_ref || !$attr_value || $attr_value ne 'TRACE');

        # set the ResLogLevel to INFO
        $result_lines .= "hares -modify $res ResLogLevel INFO\n";

        $attr_ref_in = dynupgrade_get_attr_from_node($res_ref,'LogDbg');
        $result_lines .= "hares -override $res LogDbg\n" if (!$attr_ref_in);
        $result_lines .= "hares -modify $res LogDbg DBG_5\n";
    }
...
sub freeze_groups {
    ...
    #e3587121:remove freeze and unfreeze for setting security
    return if (Cfg::opt('security')); 
...
sub correct_typescf_syntax_errors_sys {
    ...
    my ($content,$need_update);

    $content=$sys->readfile($typescfpath);
    $need_update=0;
    ...
    if($content=~/SCPU=\s*,\s*SMem=\s*}/mx) {
        $content=~s/SCPU=\s*,\s*SMem=\s*}/SCPU="", SMem="" }/g;
        $need_update=1;
    }
    if($need_update) {
...
sub config_vxss {
    ...
    Msg::title();
    $msg=Msg::new("Symantec recommends to run $prod->{name} in secure mode.\n");
    $msg->print;
    ...
    while(1) {
        ...
        if ($ayn eq 'Y') {
            ...
            # if users choose to configure the cluster to secure mode, ask users to enter usergroups for which they want to give guest privilege
            $prod->secureclus_usergroups_config($prod);

            $msg=Msg::new("Configure the cluster in secure mode");
            $one=$msg->{msg};
            $msg=Msg::new("Configure the cluster in secure mode with FIPS");
            $two=$msg->{msg};
            $menuopt=[$one,$two];
            $help=Msg::new("Secure mode with FIPS restricts the security module to use only algorithms specified in Federal Information Processing Standard 140-2. Clusters with FIPS mode may not be interoperable with the clusters without FIPS.");
            $msg=Msg::new("Select the option you would like to perform");
            if ($prod->check_solx64_for_fips) {
                $menu = 1;
            } else {
                $menu=$msg->menu($menuopt,1,$help,1);
                next if (EDR::getmsgkey($menu,'back'));
            }
            $cfg->{vcs_eat_security_fips} = 1 if ($menu==2);
            last;
        } else {
            # if users configure cluster to non-security mode, will prompt users warning messages
            Msg::n();
            $msg = Msg::new("Symantec recommends that you install the cluster in secure mode. This ensures that communication between cluster components is encrypted and cluster information is visible to specified users only.");
            $msg->warning();
            $msg = Msg::new("\n\nAre you sure that you want to proceed with non-secure installation?");
            $rtn = $msg->aynn;
            next if ($rtn eq 'N'); 
...
sub stop_vcs {
    my ($prod,$appha)=@_;
    my ($syslist,$ck,$iter,$maxiters,$msg,$pids,@pids,$stop,$sys,$prodname);
    ...
    $syslist=CPIC::get('systems');
    $sys=$$syslist[0];
    $prodname = ($appha) ? 'ApplicationHA' : $prod->{abbr};
    $msg=Msg::new("Stopping $prodname");
    $prod->haconf_dumpmakero();
...
sub secure_startup {
    my ($prod,$appha)=@_;
    my ($cfg,$syslist,$msg,$nr,$pids,$proc,$rc,$status,$sys,$localsys,$tmpdir,$prodname);
    ...
    $prodname = ($appha) ? 'ApplicationHA' : $prod->{abbr};
    for my $sys (@$syslist) {
        $msg=Msg::new("Starting $prodname on $sys->{sys}");
        $localsys->copy_to_sys($sys,"$tmpdir/main.cf",$prod->{maincf});
        sleep(1);
        $proc=$prod->proc('had62');
        $proc->start_sys($sys);
        sleep(1);
        $msg->display_status();
    }
    ...
    $sys=$$syslist[0];
    $msg=Msg::new("Confirming $prodname startup");
...
sub eat_maincf_secure_sys {
    my ($prod,$sys)=@_;
    my ($maincf,$cfg,$tmpdir,$clustername,$secusrgrplist);
    ...
    if ($maincf=~/SecureClus\s*=\s*0/) {
        ...
        if (!$cfg->{rootsecusrgrps} && !$cfg->{secusrgrps}) {
            ...
            $secusrgrplist = join (', ', @{$cfg->{secusrgrps}});
            $maincf=~s/SecureClus\s*=\s*0/SecureClus = 1\n\tGuestGroups = { $secusrgrplist }/;
        } else {
            $maincf=~s/SecureClus\s*=\s*0/SecureClus = 1/;
        }
        ...
        # add the usergroups info to main.cf for security
        $clustername||=$sys->cmd("_cmd_grep '^cluster' $prod->{maincf} 2>/dev/null | _cmd_awk '{print \$2}'");
        if (!$cfg->{rootsecusrgrps} && !$cfg->{secusrgrps}) {
...
sub eat_maincf_unsecure_sys {
    ...
    $maincf=~s/DefaultGuestAccess\s*=\s*1//;
    $maincf=~s/GuestGroups\S*=\s*{ * }//;
...
sub eat_enable {
    my ($prod,$appha)=@_;
    my ($syslist,$sys,$msg,$nsys,$mode,$sec_or_fips,$prodname);
    ...
    $prodname = ($appha) ? 'ApplicationHA' : $prod->{abbr};
    $msg=Msg::new("Deleting cluster users for $prodname");
    $prod->eat_delete_cluster_users($sys);
    $msg->display_status();

    $msg=Msg::new("Configuring a $sec_or_fips cluster for $prodname");
    ...
    $prod->stop_vcs($appha);
    # modify main.cf
    $msg=Msg::new("Updating $prodname configuration");
    $prod->eat_maincf_secure_sys($sys);
    $prod->eat_update_CmdServer_secure_file();
    $msg->display_status();

    $nsys=$prod->secure_startup($appha);
    $mode=$sys->cmd("$prod->{bindir}/haclus -value SecureClus");
    if (($mode==1) && ($nsys==$#{$syslist}+1)) {
        if ($prod->{fips}) {
            $msg=Msg::new("Security with Fips is successfully enabled\n");
        } else {
            $msg=Msg::new("Security is successfully enabled\n");
        }
        $msg->bold;
    } else {
...
sub eat_disable {
    ...
    if (($nsys==scalar(@$syslist)) && ($mode==0)) {
        if ($prod->{fips}) {
            $msg=Msg::new("Security with Fips is successfully disabled\n");
        } else {
            $msg=Msg::new("Security is successfully disabled\n");
        }
        $msg->print;
    } else {
...
sub securityonenode_menu {
    ...
    $help->{msg}.=$msg->{msg};
    $msg=Msg::new("\tRun '/opt/VRTSvcs/bin/hastop -all -force' on node A.\n\tRun '/opt/VRTSvcs/bin/CmdServer -stop' on all nodes.\n\tModify /etc/VRTSvcs/conf/config/main.cf of node A, if you want to grant read access to everyone, add/modify 'SecureClus = 1' and 'DefaultGuestAccess = 1' to the cluster definition, eg:\n\t\tcluster <cluster name> (\n\t\t\tSecureClus = 1\n\t\t\tDefaultGuestAccess = 1\n\t\t)\n\tor if you want to grant read access only to root and other users/usergroups which are granted explicit privileges on VCS objects, add/modify 'SecureClus = 1' to the cluster definition, eg:\n\t\tcluster <cluster name> (\n\t\t\tSecureClus = 1\n\t\t)\n");
    $help->{msg}.=$msg->{msg};
    my $usrgrpspecformat = '"foo@node1"';
    $msg=Msg::new("\tor if you want to grant read access to a usergroup like 'staff' on any cluster node, add/modify 'SecureClus = 1' and 'GuestGroups = { staff }' to the cluster definition, if you want to grant read access to a usergroup like 'foo' on a specific node, add/modify 'SecureClus = 1' and 'GuestGroups = { $usrgrpspecformat }' to the cluster definition, eg:\n\t\tcluster <cluster name> (\n\t\t\tSecureClus = 1\n\t\t\tGuestGroups = { staff, $usrgrpspecformat }\n\t\t)\n");
    $help->{msg}.=$msg->{msg};
    $msg=Msg::new("\tModify /etc/VRTSvcs/conf/config/main.cf of node A, add '-secure' to the wac application definition if global clustering is configured, eg:\n\t\t$secwac\n\tRun 'touch /etc/VRTSvcs/conf/config/.secure' on all nodes.\n\tRun '/opt/VRTSvcs/bin/hastart' on node A, then on all other nodes.\n\tRun '/opt/VRTSvcs/bin/CmdServer' on all the nodes.\n");
    $help->{msg}.=$msg->{msg};
...
sub security_menu {
    ...
    if ($prod->is_secure_cluster($sys0)) {
        ...
        if ($ayn eq 'Y') {
            ...
            $msg->warning();
            my $webmsg = $msg;
            $msg = Msg::new("\n\nAre you sure that you want to disable $sec_or_fips mode in this $prod->{abbr} cluster?");
            $rtn = $msg->ayny('','',$webmsg);
            if ($rtn eq 'Y') {
                Msg::title();
            ...
            if (Obj::webui()) {
                ...
                do {
                    if (Cfg::opt('configcps')) {
                        ...
                        $msg=Msg::new("Enter the name of any system in the cluster to configure secure mode:");
                    } elsif (Cfg::opt('securitytrust')) {
...
sub check_prod_install_sys {
    ...
    $cprod = ($cprodi)? $cpic->prod($cprodi):$cpic->prod("VCS62");
...
sub check_installed_ha_product_sys {
    ...
    $rel = Obj::rel();
    @prodlist = qw(SFRAC62 SFSYBASECE62 SVS62 SFCFSHA62 SFHA62 VCS62);
    ...
    if ($rtn!~/LLT is running/m || $rtn1=~/jeopardy/) {
        ...
        if($sys->exists('/etc/llttab') && $sys->exists('/etc/llthosts')){
            ...
            if($rtn2==0){
                ...
                $systemnics=$padv->systemnics_sys($sys,1,1);
                foreach my $line (split(/\n/,$rtn)){
                    if($line=~/^link/){
                        (undef,$nic,undef)=split(/\s+/,$line,3);
...
sub postcheck_lltvers_sys {
    my ($prod,$sys)=@_;
    return $prod->postcheck_pkg_vers_sys($sys,"VRTSllt62");
}
...
sub postcheck_gabvers_sys {
    my ($prod,$sys)=@_;
    return $prod->postcheck_pkg_vers_sys($sys,"VRTSgab62");
}

sub postcheck_vxfenvers_sys {
    my ($prod,$sys)=@_;
    return $prod->postcheck_pkg_vers_sys($sys,"VRTSvxfen62");
}

sub postcheck_vcsvers_sys {
    my ($prod,$sys)=@_;
    return $prod->postcheck_pkg_vers_sys($sys,"VRTSvcs62");
}
            ...
            if ($vxfenmode) {
                if ($vxfenmode eq 'scsi3' || $vxfenmode eq 'sybase') {
                    ...
                    if ($diskgroup) {
                        ...
                        if (!$rtn) {
                           $msg=Msg::new("\n\tCoordinator disk defined in file /etc/vxfendg is not found or VxVM is not running on $sys->{sys}.");
                           push(@m,$msg);
                           $sys->push_error(@m);
                           return 0;
                        }
...
sub addnode_compare_systems {
    ...
    my $prod = $cpic->prod();
    my $padv = $cpic->{padv};
    my @prodlist = qw(SFRAC62 SFSYBASECE62 SVS62 SFCFSHA62 SFHA62 VCS62);
    ...
    if ($clus_prod->{prod} ne $prod->{prod}) {
        #e3598604&e3612698:prompt the accurate message for adding node.
        my $upi = lc($clus_prod->{installscript_name} || $clus_prod->{prod}) . $rel->get_local_script_version_suffix();
        Msg::n();
    ...
    if ($installed_prodi ne 'SFHA62' && $installed_prodi ne 'VCS62') {
    ...
    for my $sysi (@{$cfg->{newnodes}}) {
        ...
        if (EDRu::compvers($clus_ver, $node_ver) == 0) {
            ...
            if(scalar(@misspkgs) > 0) {
                ...
                $failed = 1;
                @misspkgs = ();
            } elsif($check_cpspkgver && !EDRu::inarr('VRTScps62', @{$minpkgs}) && $cpspkg && $cpspkg->version_sys($sys) ne $check_cpspkgver ) {
                Msg::right_failed();
                Msg::n();
        ...
        if ( ($check_vr || $check_lic_vr) && $installed_prodi ne 'VCS62') {
        ...
        if ( ($check_vfr || $check_lic_vfr) && $installed_prodi ne 'VCS62') {
...
sub web_addnode_configure_heartbeat {
    ...
    while (!$done) {
        ...
        if ($cfg->{autocfgllt}) {
            ...
            for my $sys (@{CPIC::get('systems')}) {
                my $nics = $sys->padv->systemnics_sys($sys,1,1);
...
sub addnode_start_cluster {
    my ($failures,$firstnode,$had,$msg,$n,$onenode,$pids,@procs,$proci,$proc,$sys,$sysi,$system,@syslist,$errmsg);
    my $prod = shift;
    my $edr = Obj::edr();
    my $cpic = Obj::cpic();
    my $cfg = Obj::cfg();
    ...
    if ($onenode) {
        ...
        $sysi = ${$cfg->{clustersystems}}[0];
        $sys = Obj::sys($sysi);
        for my $proci (qw(vxfen62 had62)) {
            # do not start vxfen for VCS/SFHA
            next if (($proci eq 'vxfen62') && ($cpic->{prod} =~ /(VCS|SFHA)/m));
            $proc = $sys->proc($proci);
            $msg=Msg::new("Starting $proc->{proc} on $sysi");
            if ($cpic->proc_start_sys($sys, $proc)) {
                CPIC::proc_start_passed_sys($sys, $proc);
...
sub addnode_configure_fencing {
    ...
    my $cpic = Obj::cpic();
    my $cfg = Obj::cfg();
    my $vxfen_pkg = $prod->pkg('VRTSvxfen62');

    $firstnode = Obj::sys(${$cfg->{clustersystems}}[0]);
    $service_enabled = 1;
    $vxfen = $firstnode->proc('vxfen62');
    ...
    for my $sysi (@{$cfg->{newnodes}}) {
        $sys = Obj::sys($sysi);
        if ($vxfenmode eq 'scsi3' || (($vxfenmode eq 'customized') && ($vxfen_mechanism eq 'cps')) || $vxfenmode eq 'majority') {
            if ($vxfenmode ne 'scsi3' && $vxfenmode ne 'majority') {
                $lltconf = $prod->get_llt_config_sys($sys);
            ...
            if (($vxfenmode eq 'scsi3') || ($vxfendg ne '')) {
                # check VxVM running state on new node
                my $vm = $prod->prod('VM62');
                if ($vm->vold_status_sys($sys) ne 'enabled') {
                    Msg::log("Starting VxVM before starting fencing on $sysi");
...
#                             Msg::right_failed();
                ...
                if ($vm->vold_status_sys($sys) ne 'enabled') {
                    $msg = Msg::new("Failed to start VxVM processes. Manually configure fencing on $sysi after addnode procedure completes.");
                    $msg->warning();
                    $msg->addWarning($msg->{msg});
                }
            }
        } elsif ($vxfenmode eq 'disabled') {
                $sys->cmd("_cmd_cp -p /etc/vxfen.d/vxfenmode_disabled $prod->{vxfenmode}");
        }
        $proc = $sys->proc('vxfen62');
        $proc->enable_sys($sys);
        if (!$service_enabled) {
...
sub addnode_poststart {
    my ($msg,$errmsg,$proc,$proci,$startprocs,@not_startprocs,$sys,$sysi);
    my $prod = shift;
    my $cpic = Obj::cpic();
    my $cfg = Obj::cfg();

    #e3615597:start process for adding node
    for my $sysi (@{$cfg->{newnodes}}) {
        $sys = Obj::sys($sysi);
        $startprocs = $prod->startprocs_sys($sys);
        @not_startprocs = qw(llt62 gab62 had62 amf62 vxfen62);
        $startprocs = EDRu::arrdel($startprocs,@not_startprocs);
        for my $proci (@{$startprocs}) {
            $proc = $sys->proc($proci);
...
sub addnode_completion {
    ...
    # since the cluster is secure, prompt users to configure usergroups for which they want to give gurest privilege manually
    $sys = ${$cpic->{systems}}[0];
    $secureclus = $prod->is_secure_cluster($sys); 
    if ($secureclus) {
        if (Obj::webui()) {
	    $msg = Msg::new("Since the cluster is in secure mode, check the main.cf whether you need to modify the usergroup that you would like to grant read access. If needed, use the following commands to modify:\\nhaconf -makerw\\nhauser -addpriv &ltusergroup&gt GuestGroup\\nhaconf -dump -makero");
	    $web->web_script_form("alert", $msg->{msg}) if (Obj::webui());
        } else {
	    $msg = Msg::new("Since the cluster is in secure mode, check the main.cf whether you need to modify the usergroup that you would like to grant read access. If needed, use the following commands to modify:\n\thaconf -makerw\n\thauser -addpriv <usergroup> GuestGroup\n\thaconf -dump -makero\n");
	    $msg->print;
        }
    }
...
sub set_vcs_onenode_sys {
    ...
    if($onenode && !$enable_lgf){
        for my $proci (qw(vxfen62 gab62 llt62)) {
            my $proc = $sys->proc($proci);
...
sub cluster_master_sys {
    ...
    eval {$vxdctl_master = $system->cmd("_cmd_vxdctl -c mode 2>/dev/null | _cmd_awk '/master:/ {print \$2}'");};
    my $errstr = $@;
...
package Prod::VCS62::AIX;
@Prod::VCS62::AIX::ISA = qw(Prod::VCS62::Common);

sub init_plat {
    my $prod=shift;
    $prod->{allpkgs}=[ qw(VRTSveki62 VRTSllt62 VRTSgab62 VRTSvxfen62 VRTSamf62 VRTSvcs62 VRTScps62 VRTSvcsag62 VRTSvcsea62 VRTSsfmh61 VRTSvbs62 VRTSvcswiz62) ];
    $prod->{minpkgs}=[ qw(VRTSveki62 VRTSllt62 VRTSgab62 VRTSvxfen62 VRTSamf62 VRTSvcs62 VRTSvcsag62 VRTSsfmh61 VRTSvcswiz62) ];
    $prod->{recpkgs}=[ qw(VRTSveki62 VRTSllt62 VRTSgab62 VRTSvxfen62 VRTSamf62 VRTSvcs62 VRTScps62 VRTSvcsag62 VRTSvcsea62 VRTSsfmh61 VRTSvbs62 VRTSvcswiz62) ];
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTScscm.rte VRTSweb.rte VRTScscw.rte VRTScssim.rte VRTSvcswiz62 VRTSvbs62 VRTSsfmh61
        VRTSvcsea62 VRTScutil VRTScutil.rte VRTSjre15.rte VRTSvcs.doc VRTSvcs.man
        VRTSvcs.msg.en_US VRTSvcsag62 VRTSvcsag.rte VRTScps62
        VRTSvcs62 VRTSvcs.rte VRTSamf62 VRTSvxfen62 VRTSvxfen.rte VRTSgab62
        VRTSgab.rte VRTSllt62 VRTSllt.rte VRTSveki62 VRTScpi.rte
        VRTSvsvc SYMClma VRTSspt62 VRTSat50 VRTSat.server VRTSat.client
        VRTSsmf VRTSpbx VRTSicsco VRTSjre.rte VRTSsfcpi62
        VRTSvlic32 VRTSperl516 VRTSperl.rte
    ) ];
...
package Prod::VCS62::HPUX;
@Prod::VCS62::HPUX::ISA = qw(Prod::VCS62::Common);

sub init_plat {
    my $prod=shift;
    $prod->{allpkgs}=[ qw(VRTSllt62 VRTSgab62 VRTSvxfen62 VRTSamf62 VRTSvcs62 VRTScps62 VRTSvcsag62 VRTSvcsea62 VRTSsfmh61 VRTSvbs62) ];
    $prod->{minpkgs}=[ qw(VRTSllt62 VRTSgab62 VRTSvxfen62 VRTSamf62 VRTSvcs62 VRTSvcsag62) ];
    $prod->{recpkgs}=[ qw(VRTSllt62 VRTSgab62 VRTSvxfen62 VRTSamf62 VRTSvcs62 VRTScps62 VRTSvcsag62 VRTSvcsea62 VRTSsfmh61 VRTSvbs62) ];
    $prod->{upgradevers}=[qw(3.5 4.1 5.0 5.1 6.0 6.1 6.2)];
    $prod->{zru_releases}=[qw(3.5 4.1 5.0 5.1 6.0 6.1 6.2)];
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTScmcs VRTScscm VRTSweb VRTScscw VRTScssim VRTSvbs62 VRTSsfmh61
        VRTSvcsea62 VRTScutil VRTSjre15 VRTSvcsdc VRTSvcsmn
        VRTSvcsmg VRTSvcsdr62 VRTSvcsag62 VRTScps62 VRTSvcs62
        VRTSamf62 VRTSvxfen62 VRTSgab62 VRTSllt62 VRTScpi VRTSvsvc SYMClma
        VRTSspt62 VRTSat50 VRTSsmf VRTSpbx VRTSicsco VRTSjre
        VRTSsfcpi62 VRTSvlic32 VRTSperl516 VRTSwl
    ) ];
...
sub padv_maincf_upgrade_sys{
    ...
    Prod::VCS62::Common::dynupgrade_modify_attr_value($treeref_result,'ProcessOnOnly','PathName','/opt/VRTSat/bin/pa20_64/vxatd','/opt/VRTSat/bin/vxatd');
    return $treeref_result;
}

package Prod::VCS62::Linux;
@Prod::VCS62::Linux::ISA = qw(Prod::VCS62::Common);

sub init_plat {
    my $prod=shift;
    $prod->{allpkgs}=[ qw(VRTSllt62 VRTSgab62 VRTSvxfen62 VRTSamf62 VRTSvcs62 VRTScps62 VRTSvcsag62 VRTSvcsdr62 VRTSvcsea62 VRTSsfmh61 VRTSvbs62 VRTSvcswiz62) ];
    $prod->{minpkgs}=[ qw(VRTSllt62 VRTSgab62 VRTSvxfen62 VRTSamf62 VRTSvcs62 VRTSvcsag62 VRTSsfmh61 VRTSvcswiz62) ];
    $prod->{recpkgs}=[ qw(VRTSllt62 VRTSgab62 VRTSvxfen62 VRTSamf62 VRTSvcs62 VRTScps62 VRTSvcsag62 VRTSvcsdr62 VRTSvcsea62 VRTSsfmh61 VRTSvbs62 VRTSvcswiz62) ];
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        VRTScsocw VRTSvcsApache VRTScmc VRTSccacm VRTSvcsw
        VRTScspro VRTSvcsdb VRTSvcsor VRTSvcssy VRTSvcsvr VRTScmccc
        VRTScmcs VRTScscm VRTSweb VRTScscw VRTScssim VRTSvcswiz62 VRTSvbs62 VRTSsfmh61
        VRTSvcsea62 VRTScutil VRTSjre15 VRTSvcsdc VRTSvcsmn
        VRTSvcsmg VRTSvcsdr62 VRTSvcsag62 VRTScps62 VRTSvcsvmw62 VRTSvcs62
        VRTSamf62 VRTSvxfen62 VRTSgab62 VRTSllt62 VRTSveki62 VRTScpi VRTSvsvc SYMClma
        VRTSspt62 VRTSat50 VRTSatClient50 VRTSsmf VRTSpbx VRTSicsco
...
sub padv_maincf_update_netmask_sys {
    ...
    # For IP resources
    @reslist_ip = Prod::VCS62::Common::dynupgrade_get_resname_list_from_typename($treeref, 'IP');
    for my $res (@reslist_ip) {
        $noderef = Prod::VCS62::Common::dynupgrade_get_node_from_tree($treeref, 'RES', $res);
        $attrref_netmask = Prod::VCS62::Common::dynupgrade_get_node_from_tree($noderef, 'ATTR', 'NetMask');
        if (!$attrref_netmask) {
            $netmask = '255.0.0.0';
            $attrref_options = Prod::VCS62::Common::dynupgrade_get_node_from_tree($noderef, 'ATTR', 'Options');
            if ($attrref_options) {
                $attrref_ipoptions = Prod::VCS62::Common::dynupgrade_get_node_from_tree($noderef, 'ATTR', 'IPOptions');
                $attrref_iprouteoptions = Prod::VCS62::Common::dynupgrade_get_node_from_tree($noderef, 'ATTR', 'IPRouteOptions');
                if (!$attrref_ipoptions && ! $attrref_iprouteoptions) {
                    $addressref = Prod::VCS62::Common::dynupgrade_get_node_from_tree($noderef, 'ATTR', 'Address');
                    $address = $addressref->{MODIFY}->{ATTRVAL}->[0];
                    $address =~ s/.*"(.*)"/$1/m;
                    $netmask = $prod->get_netmask_by_address($address);
                }
    ...
    @reslist_ipmultinic = Prod::VCS62::Common::dynupgrade_get_resname_list_from_typename($treeref, 'IPMultiNIC');
    ...
    @reslist_multinica = Prod::VCS62::Common::dynupgrade_get_resname_list_from_typename($treeref, 'MultiNICA');
    for my $res (@reslist_multinica) {
        ...
        if (!$attrref_netmask) {
            ...
            $attrref_options = Prod::VCS62::Common::dynupgrade_get_node_from_tree($noderef, 'ATTR', 'Options');
            if ($attrref_options) {
                $attrref_linkoptions = Prod::VCS62::Common::dynupgrade_get_node_from_tree($noderef, 'ATTR', 'LinkOptions');
                $attrref_ipv4addroptions = Prod::VCS62::Common::dynupgrade_get_node_from_tree($noderef, 'ATTR', 'IPv4AddrOptions');
                if (!$attrref_linkoptions && ! $attrref_ipv4addroptions) {
                    $addressref = Prod::VCS62::Common::dynupgrade_get_node_from_tree($noderef, 'ATTR', 'Device');
                    $address = $addressref->{MODIFY}->{ATTRVAL}->[0];
                    $address =~ s/.*"(.*)"/$1/m;
                    $netmask = $prod->get_netmask_by_address($address);
                }
...
sub check_ksh_for_vxfen_sys {
    ...
    unless ($rtn) {
        $msg=Msg::new("The '$pkg' rpm is not installed on $sys->{sys}, it is required to install it on the system.");
        $sys->push_error($msg);
    }
    return 1;
}
...
_EOL_
    ...
    return 1 if (!$rdma_service);

    if ($sys->exists("/etc/init.d/$rdma_service")) {
        $sys->cmd("/etc/init.d/$rdma_service start 2>/dev/null");
        $sys->cmd("/sbin/chkconfig --level 235 $rdma_service on 2>/dev/null");
    } elsif ($sys->exists("/usr/lib/systemd/system/${rdma_service}.service")) {
        $sys->cmd("/usr/bin/systemctl enable ${rdma_service}.service 2>/dev/null");
        $sys->cmd("/usr/bin/systemctl start ${rdma_service}.service 2>/dev/null");
    }
...
sub check_rdma_opensm_service_sys {
    ...
    if ($sys->exists('/etc/init.d/opensmd')) {
        ...
        $init_script='init';
    } elsif ($sys->exists("/usr/lib/systemd/system/opensm.service")) {
        $opensm_service='opensm';
        $init_script='systemd';
    }

    if (!$opensm_service) {
         $msg=Msg::new("The opensm service does not exist on $sys->{sys}");
         $sys->push_error($msg);
         $prod->{rdma_fatal_errors}=1;
         return 0;
    }
    ...
    if ($init_script eq 'init') {
        $status=$sys->cmd("/etc/init.d/$opensm_service status 2>/dev/null");
    } elsif ($init_script eq 'systemd') {
        $status=$sys->cmd("/usr/bin/systemctl status ${opensm_service}.service 2>/dev/null | _cmd_grep 'Active:'");
    }
...
sub setup_rdma_opensm_service_sys {
    ...
    if ($opensm_init_script eq 'init') {
        $sys->cmd("/etc/init.d/$opensm_service start 2>/dev/null");
        $sys->cmd("/sbin/chkconfig --level 235 $opensm_service on 2>/dev/null");
    } elsif ($opensm_init_script eq 'systemd') {
        $sys->cmd("/usr/bin/systemctl enable ${opensm_service}.service 2>/dev/null");
        $sys->cmd("/usr/bin/systemctl start ${opensm_service}.service 2>/dev/null");
    }
...
package Prod::VCS62::RHEL6x8664;
@Prod::VCS62::RHEL6x8664::ISA = qw(Prod::VCS62::Linux);
...
package Prod::VCS62::RHEL7x8664;
@Prod::VCS62::RHEL7x8664::ISA = qw(Prod::VCS62::Linux);
...
sub init_padv {
    ...
    $prod->{feature_required_os_pkgs} = {
      'LLT_over_RDMA' => [
         ...
         'libmlx4 1.0.5',
         'librdmacm 1.0.17',
         #'librdmacm-devel 1.0.17',
         'librdmacm-utils 1.0.17',
         'rdma 7.0',
         'opensm 3.3.15',
         'opensm-libs 3.3.15',
         'libibumad 1.3.8',
         #'libibumad-devel 1.3.8',
         'libibverbs 1.1.7',
         #'libibverbs-devel 1.1.7',
         'libibverbs-utils 1.1.7',
         'ibutils 1.5.7',
         'ibutils-libs 1.5.7',
    ...
    $prod->{feature_required_os_pkgs}{LLT_over_RDMA_InfiniBand} = $prod->{feature_required_os_pkgs}{LLT_over_RDMA};
    $prod->{feature_required_os_pkgs}{LLT_over_RDMA_RoCE} = EDRu::arrdel($prod->{feature_required_os_pkgs}{LLT_over_RDMA}, 'opensm 3.3.15', 'opensm-libs 3.3.15');

    return;
}

package Prod::VCS62::OL6x8664;
@Prod::VCS62::OL6x8664::ISA = qw(Prod::VCS62::RHEL6x8664);

package Prod::VCS62::OL7x8664;
@Prod::VCS62::OL7x8664::ISA = qw(Prod::VCS62::RHEL7x8664);

package Prod::VCS62::SLES11x8664;
@Prod::VCS62::SLES11x8664::ISA = qw(Prod::VCS62::Linux);
...
package Prod::VCS62::SunOS;
@Prod::VCS62::SunOS::ISA = qw(Prod::VCS62::Common);

sub init_plat {
    my $prod=shift;
    $prod->{allpkgs}=[ qw(VRTSllt62 VRTSgab62 VRTSvxfen62 VRTSamf62 VRTSvcs62 VRTScps62 VRTSvcsag62 VRTSvcsea62 VRTSsfmh61 VRTSvbs62 VRTSvcswiz62) ];
    $prod->{minpkgs}=[ qw(VRTSllt62 VRTSgab62 VRTSvxfen62 VRTSamf62 VRTSvcs62 VRTSvcsag62 VRTSsfmh61 VRTSvcswiz62) ];
    $prod->{recpkgs}=[ qw(VRTSllt62 VRTSgab62 VRTSvxfen62 VRTSamf62 VRTSvcs62 VRTScps62 VRTSvcsag62 VRTSvcsea62 VRTSsfmh61 VRTSvbs62 VRTSvcswiz62) ];
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSvcswiz62 VRTSvbs62 VRTSsfmh61 VRTSvcsea62 VRTScutil VRTSjre15 VRTSvcsdc VRTSvcsmn
        VRTSvcsmg VRTSvcsag62 VRTScps62 VRTSvcs62 VRTSamf62 VRTSvxfen62
        VRTSgab62 VRTSllt62 VRTScpi VRTSvsvc SYMClma VRTSspt62
        VRTSat50 VRTSsmf VRTSpbx VRTSicsco VRTSjre
...
sub set_onenode_cluster_sys {
    ...
    if ($onenode) {
        if ($rootpath) {
            ...
            $sys->cmd("echo /usr/bin/cp -p /etc/VRTSvcs/conf/$manifest /var/tmp/temp_manifest >> $smfprofile_upgrade");
            ...
            $sys->cmd("_cmd_cp -p /etc/VRTSvcs/conf/$manifest /var/tmp/temp_manifest");
        ...
        if ($rootpath) {
            ...
            $sys->cmd("echo /usr/bin/rm -f /var/tmp/temp_manifest >> $smfprofile_upgrade");
            $sys->cmd("echo /usr/bin/cp -p $orig_manifest /var/tmp/temp_manifest >> $smfprofile_upgrade");
            $sys->cmd("echo /usr/sbin/svccfg import /var/tmp/temp_manifest >> $smfprofile_upgrade");
            $sys->cmd("echo /usr/bin/rm -f /var/tmp/temp_manifest >> $smfprofile_upgrade");
            ...
            $sys->cmd("_cmd_rmr /var/tmp/temp_manifest");
            $sys->cmd("_cmd_cp -p $orig_manifest /var/tmp/temp_manifest");
            $sys->cmd("_cmd_svccfg import /var/tmp/temp_manifest");
            $sys->cmd("_cmd_rmr /var/tmp/temp_manifest");
        }
    }
...
sub padv_maincf_upgrade_precheck_sys {
    ...
    if ($vcsvers && (EDRu::compvers($vcsvers,'5.1') == 2) && $sys->{zone}) {
        ...
        # get group list
        @grps = Prod::VCS62::Common::dynupgrade_get_grplist($treeref);
        for my $grp_name (@grps) {
            next unless ($grp_name);

            # get resources list
            @reses = Prod::VCS62::Common::dynupgrade_get_res_from_grp($treeref, $grp_name);
            next unless (@reses);
            $zone_num=0;
            for my $res_name (@reses) {
                next unless ($res_name);
                $res_type = Prod::VCS62::Common::dynupgrade_get_restype_of_resname($treeref,$res_name);
        ...
        for my $grp_name (@grps) {
            ...
            for my $res_name (@reses) {
                ...
                if ($res_type eq 'Zone') {
                    ...
                    $res_container_names = Prod::VCS62::Common::dynupgrade_get_attrvalue_of_resname_array($treeref, $res_name, 'ZoneName');
                } else {
                    $res_container_type = Prod::VCS62::Common::dynupgrade_get_attrvalue_of_typename($treeref,$res_type,'ContainerType');
                    if ($res_container_type) {
                        $res_container_names = Prod::VCS62::Common::dynupgrade_get_attrvalue_of_resname_array($treeref, $res_name, 'ContainerName');
                    }
                }
...
sub padv_maincf_upgrade_sys {
    ...
    for my $grp_name (sort keys %{$sys->{maincf_res_container_name}}) {
        # get group system list
        @grp_syslist = Prod::VCS62::Common::dynupgrade_get_systemlist_of_grpname($treeref_result,$grp_name);
        next unless (@grp_syslist);

        # get resources list
        @reses = Prod::VCS62::Common::dynupgrade_get_res_from_grp($treeref_result, $grp_name);
        next unless (@reses);
        for my $res_name (@reses) {
            ...
            $res_type = Prod::VCS62::Common::dynupgrade_get_restype_of_resname($treeref_result,$res_name);
            next if ($res_type eq 'Zone');

            $container_opts = Prod::VCS62::Common::dynupgrade_get_attrvalue_of_typename($treeref_newsys,$res_type,'ContainerOpts');
            if($res_container_type) {
    ...
    for my $grp_name (sort keys %{$sys->{maincf_grp_container_name}}) {
        ...
        next unless Prod::VCS62::Common::dynupgrade_get_node_from_tree($treeref_result,'GRP',$grp_name);
        @grp_syslist = Prod::VCS62::Common::dynupgrade_get_systemlist_of_grpname($treeref_result,$grp_name);
        next unless (@grp_syslist);
    ...
    $treeref_result = Prod::VCS62::Common::dynupgrade_import_lines2tree(\@container_settings, $treeref_result);
    return $treeref_result;
}
...
package Prod::VCS62::SolSparc;
@Prod::VCS62::SolSparc::ISA = qw(Prod::VCS62::SunOS);
...
package Prod::VCS62::Sol11sparc;
@Prod::VCS62::Sol11sparc::ISA = qw(Prod::VCS62::SunOS);
...
package Prod::VCS62::Solx64;
@Prod::VCS62::Solx64::ISA = qw(Prod::VCS62::SunOS);
...
package Prod::VCS62::Sol11x64;
@Prod::VCS62::Sol11x64::ISA = qw(Prod::VCS62::Sol11sparc);
