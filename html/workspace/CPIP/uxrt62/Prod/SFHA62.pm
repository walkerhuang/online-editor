...
package Prod::SFHA62::Common;
@Prod::SFHA62::Common::ISA = qw(Prod);
...
sub init_common {
    ...
    $prod->{licsuperprods} = [ qw(SFCFSHA62) ];
...
# Only executed when $sys->system1
sub upgrade_rolling_post_sys {
    ...
    $prod->upgrade_protocol_version();
    return;
}

sub upgrade_protocol_version {
    my ($prod)=@_;
    my ($syslist,$done_upgrade,$msg,$cprod,$rel);
    # Check if the product include CVM
    $syslist=CPIC::get('systems');
    $cprod=CPIC::get('prod');
    $rel=Obj::rel();
    return unless ($rel->has_cvm($cprod));
    for my $sys (@$syslist) {
        $msg=$sys->cmd('_cmd_vxdctl -c mode 2>/dev/null');
        if($msg=~/cluster active - MASTER/m) {
            $sys->cmd('_cmd_vxdctl upgrade');
            $done_upgrade=1;
        }
        $sys->cmd('_cmd_fsclustadm protoclear');
        $sys->cmd('_cmd_fsclustadm protoupgrade');
        $sys->cmd('_cmd_odmclustadm protoclear');
        $sys->cmd('_cmd_odmclustadm protoupgrade');
    }
    unless ($done_upgrade) {
...
sub set_partial_flag_sys {
    ...
    # Only SFHA product need set partial flag
    return 0 unless ($cpic->{prod}=~/SFHA/);
...
sub upgrade_precheck_sys {
    ...
    #e3410588:vcs support zero reboot
...
sub upgrade_poststart_sys {
    ...
    if (!Cfg::opt("rolling_upgrade")) {
        if($sys->system1){
            $prod->upgrade_protocol_version();
            EDRu::create_flag("upgrade_protocol_version");
        } else {
            EDRu::wait_for_flag("upgrade_protocol_version")
        }
    }
    return;
}
...
sub startprocs_sys {
    ...
    if (!$cfg->{vcs_allowcomms}) {
        $ref_procs = EDRu::arrdel($ref_procs, 'llt62', 'gab62', 'vxfen62');
    } elsif ((!$sys->exists("$vcs->{vxfenmode}")) && (!$sys->exists("$vcs->{vxfendg}"))) {
        $ref_procs = EDRu::arrdel($ref_procs, 'vxfen62');
    }
    $sf = $prod->prod('SF62');
...
sub addnode_poststart {
    ...
    # eTrack 3131876
    # To start vxfen62 or not depends on whether source cluster has it configured
    $novxfen = 0;
    $src_sysi = ${$cfg->{clustersystems}}[0];
    $src_sys = Obj::sys($src_sysi);
    $vcs = $src_sys->prod('VCS62');
    if ((!$src_sys->exists("$vcs->{vxfenmode}")) && (!$src_sys->exists("$vcs->{vxfendg}"))) {
        $novxfen = 1;
    }
    $vxfen = $src_sys->proc('vxfen62');
    ...
    for my $sysi (@{$cfg->{newnodes}}) {
        ...
        if ($novxfen) {
            $startprocs = EDRu::arrdel($startprocs, 'vxfen62');
            if ($novxfen == 2) {
...
package Prod::SFHA62::AIX;
@Prod::SFHA62::AIX::ISA = qw(Prod::SFHA62::Common);

sub init_plat {
    my $prod=shift;
    Prod::SF62::AIX::init_plat($prod);

    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        VRTScsocw.rte VRTSdbac62 VRTSdbac.rte VRTSvcswiz62 VRTSvbs62 VRTSvcsea62
        VRTSodm62 VRTSgms62 VRTScavf62 VRTSglm62 VRTScpi.rte VRTSd2doc
        VRTSordoc VRTSfppm VRTSap VRTStep VRTSgapms.VRTSgapms
        VRTSmapro VRTSvail.VRTSvail VRTSd2gui VRTSorgui VRTSvxmsa
        VRTSdbdoc VRTSdb2ed VRTSdbed62 VRTSdbcom VRTSsybed
...
package Prod::SFHA62::HPUX;
@Prod::SFHA62::HPUX::ISA = qw(Prod::SFHA62::Common);

sub init_plat {
    my $prod=shift;
    Prod::SF62::HPUX::init_plat($prod);

    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        VRTSdbckp VRTSormap VRTScsocw VRTSdbac62 VRTScfsdc VRTSvbs62
        VRTSvcsea62 VRTSodm62 VRTSgms62 VRTScavf62 VRTSglm62
        VRTScpi VRTSap VRTStep VRTSgapms VRTSmapro VRTSvail
        VRTSorgui VRTSorweb VRTSvxmsa VRTSvrdev VRTSdbdoc VRTSdbed62
        ...
        VRTSvcsdc VRTSvcsmn VRTSvcsmg VRTSvcsag62 VRTScps62
        VRTSvcs62 VRTSamf62 VRTSvxfen62 VRTSgab62 VRTSllt62 VRTSfsmnd
        VRTSfssdk62 VRTSfsadv62 VRTSfsdoc VRTSfsman VRTSvrdoc VRTSvrw VRTSvrmcsg
...
sub rp_installed_sys {
    ...
    $vcs=$sys->pkg($sys->prod('VCS62')->{mainpkg});
    return ($sys->padv->pkg_has_rp_sys($sys,$vm)?1:($sys->padv->pkg_has_rp_sys($sys,$fs)?1:$sys->padv->pkg_has_rp_sys($sys,$vcs)));
}


package Prod::SFHA62::Linux;
@Prod::SFHA62::Linux::ISA = qw(Prod::SFHA62::Common);

sub init_plat {
    my $prod=shift;
    Prod::SF62::Linux::init_plat($prod);
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSvail VRTSd2gui-common VRTSorgui-common VRTSvcswiz62 VRTSvbs62 VRTSvcsea62
        ...
        VRTScutil VRTSvcsdc VRTSvcsmn VRTSvcsmg VRTSvcsdr62
        VRTSvcsag62 VRTScps62 VRTSvcsvmw62 VRTSvcs62 VRTSamf62 VRTSvxfen62 VRTSgab62
        VRTSllt62 VRTSveki62 VRTSfsmnd VRTSfssdk62 VRTSfsadv62 VRTSfsdoc VRTSfsman
        VRTSvrdoc VRTSvrw VRTSweb VRTSvcsvr VRTSvrpro VRTSalloc
        VRTSdcli VRTSvsvc VRTSvmpro VRTSddlpr VRTSvdid VRTSlvmconv
        VRTSvmdoc VRTSvmman SYMClma VRTSspt62 VRTSaa VRTSmh
        VRTSccg VRTSobgui VRTSfspro VRTSdsa VRTSsfmh61 VRTSob34
        VRTSobc33 VRTSaslapm62 VRTSat50 VRTSatClient50 VRTSsmf VRTSpbx VRTSicsco
        VRTSvxfs62 VRTSvxfs-platform VRTSvxfs-common VRTSvxvm62
...
package Prod::SFHA62::SunOS;
@Prod::SFHA62::SunOS::ISA = qw(Prod::SFHA62::Common);

sub init_plat {
    my $prod=shift;
    Prod::SF62::SunOS::init_plat($prod);

    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        VRTScsocw VRTSdbac62 VRTSvcswiz62 VRTSvbs62 VRTSvcsea62 VRTSodm62 VRTSgms62
        VRTScavf62 VRTSglm62 VRTScpi VRTSd2doc VRTSordoc VRTSfsnbl
        VRTSfppm VRTSap VRTStep VRTSspc VRTSspcq VRTSfasdc
        VRTSfasag VRTSfas VRTSgapms VRTSmapro VRTSvail VRTSd2gui
        VRTSorgui VRTSvxmsa VRTSdbdoc VRTSdb2ed VRTSdbed62
...
package Prod::SFHA62::SolSparc;
@Prod::SFHA62::SolSparc::ISA = qw(Prod::SFHA62::SunOS);

sub init_padv {
    my $prod=shift;
    return Prod::SF62::SolSparc::init_padv($prod);
}

package Prod::SFHA62::Sol11sparc;
@Prod::SFHA62::Sol11sparc::ISA = qw(Prod::SFHA62::SunOS);

sub init_padv {
    my $prod=shift;
    return Prod::SF62::Sol11sparc::init_padv($prod);
}

package Prod::SFHA62::Solx64;
@Prod::SFHA62::Solx64::ISA = qw(Prod::SFHA62::SunOS);

sub init_padv {
    my $prod=shift;
    return Prod::SF62::Solx64::init_padv($prod);
}

package Prod::SFHA62::Sol11x64;
@Prod::SFHA62::Sol11x64::ISA = qw(Prod::SFHA62::SunOS);

sub init_padv {
    my $prod=shift;
    return Prod::SF62::Sol11x64::init_padv($prod);
}
