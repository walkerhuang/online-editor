...
package Proc::had62::Common;
@Proc::had62::Common::ISA = qw(Proc);

sub init_common {
    my $proc=shift;
    $proc->{proc}='had';
    $proc->{name}='Symantec Cluster Server';
    $proc->{desc}='Symantec Cluster Server';
...
sub backup_sys{
    ...
    $rootpath = Cfg::opt('rootpath') || '';
    $file= $rootpath . $proc->{initconf};
    $sys->cmd("_cmd_cp -f $file $edr->{tmpdir}/backup/") if $sys->exists("$file");
}

sub restore_sys{
    my ($proc,$sys) = @_;
    my ($cfg,$file,$conf,$rootpath,$stat,$dir);
    my $edr = Obj::edr();
    $cfg = Obj::cfg();
    $rootpath = Cfg::opt('rootpath') || '';
    $file= $rootpath . $proc->{initconf};
    if($file =~ /(\S*\/)(\S+)/){
        $dir = $1;
        $file = $2;
        $sys->cmd("_cmd_cp -f $edr->{tmpdir}/backup/$file $dir") if $sys->exists("$edr->{tmpdir}/backup/$file");
    }
}
...
sub is_enabled_sys {
    ...
    if ($argu eq 'STOP') {
        $conf = $sys->cmd("unset VCS_STOP 2>/dev/null >/dev/null ; . $initconf 2>/dev/null >/dev/null ; echo \$VCS_STOP");
    } else {
    ...
    if ($conf eq '0'){
        ...
        #e3522125:both 1 and null means enable
        return 1;
    }
}

sub is_onenode_running_sys {
    my ($proc,$sys)=@_;
    my ($had_pids);

    $had_pids=$sys->proc_pids('bin/had -onenode');
    if (@$had_pids) {
        Msg::log("had is running in one node mode on $sys->{sys}");
        return 1;
    }
    return 0;
}

package Proc::had62::AIX;
@Proc::had62::AIX::ISA = qw(Proc::had62::Common);
...
package Proc::had62::HPUX;
@Proc::had62::HPUX::ISA = qw(Proc::had62::Common);
...
package Proc::had62::Linux;
@Proc::had62::Linux::ISA = qw(Proc::had62::Common);
...
package Proc::had62::SunOS;
@Proc::had62::SunOS::ISA = qw(Proc::had62::Common);
...
sub check_sys {
    ...
    if ($state=~/stop/m && !$running) {
        ...
        my $onenode = '';
        if ($sys->exists($proc->{initconf})) {
            $onenode = $sys->cmd("unset ONENODE; . $proc->{initconf} >/dev/null 2>/dev/null ; echo \$ONENODE");
            chomp $onenode;
        }
...
package Proc::had62::Sol11sparc;
@Proc::had62::Sol11sparc::ISA = qw(Proc::had62::SunOS);

sub stop_sys {
    my ($proc,$sys)=@_;
    my ($vcs,$vcspkg,$vcspkg_ver);

    # 60PR1 sol11 stop will hang, so change the stop method. this part should only run during upgrade from 60PR1
    if (Cfg::opt('upgrade')) {
        $vcspkg=Pkg::new_pkg('Pkg', 'VRTSvcs', $sys->{padv});
        $vcspkg_ver=$vcspkg->version_sys($sys);
        if($vcspkg_ver =~ /^6\.0\.10\.0/){
            # need to force stop had in case of frozen service groups.
            $vcs=$proc->prod('VCS62');
            $sys->cmd("$vcs->{bindir}/hastop -local -force");

            $sys->cmd('_cmd_svcadm disable -t system/vcs system/vcs-onenode 2>/dev/null');
            sleep 60;
            $sys->cmd('_cmd_svcadm clear system/vcs system/vcs-onenode 2>/dev/null');
            return 1;
        }
    }

    #call the normal sol stop_sys
    return $proc->SUPER::stop_sys($sys);
}

package Proc::had62::Sol11x64;
@Proc::had62::Sol11x64::ISA = qw(Proc::had62::Sol11sparc);
