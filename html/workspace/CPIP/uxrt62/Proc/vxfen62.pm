...
package Proc::vxfen62::Common;
@Proc::vxfen62::Common::ISA = qw(Proc);

sub init_common {
    my $proc=shift;
    $proc->{proc}='vxfen';
    $proc->{name}='Symantec I/O Fencing';
    $proc->{desc}='Symantec I/O Fencing';
...
sub disable_sys {
    ...
    if(!$postinstall && $proc->{driver_unconfig}=~ /\S/){
        if(!$cfg->{vcs_allowcomms} && ($cfg->{systemscfg} && $#{$cfg->{systemscfg}} == 0 || !$cfg->{systemscfg} &&  $#{$cfg->{systems}} == 0)){
...
package Proc::vxfen62::AIX;
@Proc::vxfen62::AIX::ISA = qw(Proc::vxfen62::Common);
sub init_plat {
    my $proc=shift;
    ...
    $proc->{driver_config} = <<'AIX_VXFEN_CONFIG';
odmadd /usr/lib/methods/vxfen.odm
odmadd /usr/lib/methods/vxfend.odm
/etc/methods/vxfenext -start -dvxfen > /dev/null
/etc/methods/vxfenext -start -dvxfend > /dev/null
exit 0
AIX_VXFEN_CONFIG
    $proc->{driver_unconfig} = <<'AIX_VXFEN_UNCONFIG';
/etc/methods/vxfenext -stop -dvxfend > /dev/null 2>&1
...
odmdelete -q"uniquetype='vxfend/node/vxfend'" -o PdAt >/dev/null 2>&1
odmdelete -q"uniquetype='vxfend/node/vxfend'" -o PdDv >/dev/null 2>&1
odmdelete -q"rule='/etc/methods/vxfenext -define -dvxfend'" -o Config_Rules >/dev/null 2>&1
odmdelete -q"name=vxfen" -o CuAt >/dev/null 2>&1
odmdelete -q"name=vxfen" -o CuDv >/dev/null 2>&1
odmdelete -q"uniquetype='vxfen/node/vxfen'" -o PdAt >/dev/null 2>&1
odmdelete -q"uniquetype='vxfen/node/vxfen'" -o PdDv >/dev/null 2>&1
odmdelete -q"rule='/etc/methods/vxfenext -define -dvxfen'" -o Config_Rules >/dev/null 2>&1
exit 0
AIX_VXFEN_UNCONFIG
...
package Proc::vxfen62::HPUX;
@Proc::vxfen62::HPUX::ISA = qw(Proc::vxfen62::Common);
...
package Proc::vxfen62::Linux;
@Proc::vxfen62::Linux::ISA = qw(Proc::vxfen62::Common);
...
package Proc::vxfen62::SunOS;
@Proc::vxfen62::SunOS::ISA = qw(Proc::vxfen62::Common);
...
sub check_service_sys {
    ...
    if ($sys->exists($proc->{smf_manifest})) { # Solaris 10
        $svcinfo_vxfen = $sys->cmd("_cmd_svcs -l system/vxfen 2>/dev/null | _cmd_grep '^enabled' ");
...
package Proc::vxfen62::Sol10sparc;
@Proc::vxfen62::Sol10sparc::ISA = qw(Proc::vxfen62::SunOS);
...
sub init_plat {
    ...
    $proc->{smf_manifest}='/var/svc/manifest/system/vxfen.xml';
    $proc->{initconf}='/etc/default/vxfen';
    $proc->{driver_config} = <<'SOL10_VXFEN_CONFIG';
/usr/sbin/add_drv -v -f -m '* 0600 root sys' vxfen
svccfg import /var/svc/manifest/system/vxfen.xml
SOL10_VXFEN_CONFIG
    $proc->{driver_unconfig} = <<'SOL10_VXFEN_UNCONFIG';
svcadm disable -s system/vxfen
svccfg delete -f system/vxfen
rem_drv vxfen
SOL10_VXFEN_UNCONFIG
    return;
}

package Proc::vxfen62::Sol11sparc;
@Proc::vxfen62::Sol11sparc::ISA = qw(Proc::vxfen62::SunOS);
...
sub init_plat {
    ...
    $proc->{smf_manifest}='/var/svc/manifest/system/vxfen.xml';
    $proc->{initconf}='/etc/default/vxfen';
    $proc->{driver_config} = <<'SOL11_VXFEN_CONFIG';
/usr/sbin/add_drv -v -f -m '* 0600 root sys' vxfen
svccfg -s system/vxfen delcust
svcadm disable system/vxfen
svcadm enable system/vxfen
SOL11_VXFEN_CONFIG
    $proc->{driver_unconfig} = <<'SOL11_VXFEN_UNCONFIG';
svcadm disable system/vxfen
svccfg delete -f system/vxfen
rem_drv vxfen
modinfo | grep -w vxfen | awk '{print $1}' |xargs -i modunload -i {}
SOL11_VXFEN_UNCONFIG
    return;
}
