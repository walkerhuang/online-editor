...
sub init_common {
    ...
    $rel->{vers}='6.0.001.0';
    $rel->{basevers}=[ "6.0.0.0" ]; #Decreasing order necessary
    $rel->{gavers}='6.0';
    $rel->{titlevers}='6.0 RP1';
    $rel->{nopkgs}=1;
    $rel->{type}="RP";
    $rel->{rp}=1;
    $rel->{patchtype}="rppatches";
    $rel->{currentrelease}="UXRT60RP1";
...
sub upgrade_sfha_prods_mix_check {
    ...
    $edr=Obj::edr();
    $cpic=Obj::cpic();
    $relvers = ($rel->{rp}) ? $rel->{basevers}[$#{$rel->{basevers}}] : $rel->{basevers};
    $task_set=0;
...
sub default_systemnames {
    my $rel=shift;
    my ($cpic,$vcs,$dfs,@systems,$cfg,$edr);
    $cpic=Obj::cpic();
    $cfg=Obj::cfg();
    $edr=Obj::edr();
    $vcs=$cpic->prod("VCS60");
...
sub determine_rolling_upgrade_system {
    ...
    my (@systems,$ayn,$clustername,$clusterid,$conf,$done,$failed,$msg,$prod,$sys,$sysname,$syslist,$firstsys);
    my ($web,$cfg,$edr,$localsys,$cpic,$webmsg);
    my ($rulist,$non_rulist,$final_list,$sub,$rutype,$rel,$helpmsg,$def,$prodvers,$cv,$error,$warning,$cprod,$prodlist,$had,$task,$vcs,$ruvers,$namefailed);
    $cfg=Obj::cfg();
    $edr=Obj::edr();
    $cpic=Obj::cpic();
    $rel=$cpic->rel;
    ...
    $failed=0;
    my $remindmsg = Msg::new("Please fix the issues first and try again.");
    while(1){
        if ($failed) {
        ...
        # clean up warning&error messages
        undef($sys->{errors});
        undef($sys->{warnings});
        unless ($rel->check_installed_pkgs_version_sys($sys) && $cpic->prod_version_sys($sys)) {
            $msg->right_failed;
            for my $errmsg (@{$sys->{errors}}) {
                $msg->addError($errmsg);
                Msg::print($errmsg);
        ...
        if($cv==2 || (!EDRu::inarr($cprod,@{$rel->{ru_prod}}))) {
            $msg->right_failed;
            $cprod=~s/\d+//g;
            $error=Msg::new("The system is installed with $cprod($prodvers); but this release only supports rolling upgrade from version $ruvers of the products $prodlist.");
            if(Obj::webui()) {
        ...
        for my $sysname (@systems) {
            ...
            # check if the products are same with first one.
            if ($firstsys->{upgradeprod} && $sys->{upgradeprod}) {
                unless ($firstsys->{upgradeprod}[0] eq $sys->{upgradeprod}[0]) {
                    #different prod
                    $msg->right_failed;
                    $warning=Msg::new("Systems have different products installed. Systems running different product cannot run rolling upgrade.\n");
                    $msg->addError("warning");
                    Msg::print($msg);
                    Msg::print($remindmsg);
                    $done=0;
                    last;
        ...
        if($rulist){
            ...
            $msg=Msg::new("\nIt is recommended to perform rolling upgrade phase 1 on the system(s)$final_list to get minimal downtime.");
            $msg->bold;
            $webmsg .= $msg->{msg};
            $webmsg =~ s/\n/\n\n/g;
            $msg=Msg::new("Would you like to perform rolling upgrade phase 1 on the recommended system(s)?");
...
sub determine_rolling_upgrade_sys{
    my ($rel,$sys)=@_;
    my ($llt,$vcs,$cur_ru,$lltver,$vcsver,$lltmpver,$vcsmpver,$lltpkgver,$vcspkgver,$do,$do1,$cv,$amf,$amfver,$amfmpver,$amfpkgver);
...
sub determine_rolling_upgrade {
    my ($rel,$prod) = @_;
    my ($cfg,$lltmpver,$vcspkgver,$sys,$cur_ru,$padv,$unkernelpkg,$unkernelpkgi,$vcs,$prodver,$prodobj,$lltver,$lltpkgver,$do1,$cv,$vcsmpver,$do_ru2,$diff,$syslist,$sys1_ru,$llt,$do,$vcsver,$amf,$amfver,$amfmpver,$amfpkgver);
    ...
    for my $sys (@$syslist) {
        ...
        $vcsver=$vcsmpver || $vcspkgver;
        $amfver=$amfmpver || $amfpkgver;
        return '' unless($lltver && $vcsver && $amfver);
        $do=$sys->cmd("_cmd_grep -v '^#' /etc/gabtab 2>/dev/null");
        #$do1=$sys->cmd("_cmd_grep -v '^#' /etc/vxfenmode 2>/dev/null");
        #return 0 unless ($do=~/vxfen_protocol_version/);
        $cv=EDRu::compvers($vcsver,$lltver,4);
...
sub prod_menu {
    ...
    for my $mprodi (@{$rel->{menuprods}}) {
        ...
        if (($rel->{type} eq "RP") || (-d "$cpic->{mediapath}/$prod->{proddir}")) {
            push(@mprods,$mprodi);
        }
    }
...
_FINISH_LINES_
    ...
    if ($rel->{type} eq 'RP') {
        $mod_rel =~ s/\d+//m;
        my @vars = split(/\D+/m,$rel->{basevers}[0]);
...
sub ignite {
    ...
    if ($rel->{rp}) {
        $msg = Msg::new("Bundle for RP release is only used to install the patches, ensure base packages of the product are installed before installing this bundle on the systems. Would you like to continue?");
        $ayn = $msg->ayny();
    ...
    Msg::log("Required space for the depots is $pkgspace_required");
...
VRTSamf:61,10429,0,0
VRTScavf:26,1245,0,0
VRTSfsadv:7,35235,0,0
VRTSodm:10,1601,0,0
VRTSsfcpi60:177,4910,0,0
VRTSvbs:5765,62057,0,0
VRTSvcs:0,379881,0,18
VRTSvxfs:1763,46618,0,95
VRTSvxvm:94,268548,0,1880
_PATCHSPACE_
...
sub check_installed_pkgs_version_sys {
    my ($rel,$sys)=@_;
    my ($msg,$vers,$ret);
    $ret = 1;
    if ($sys->{pkgverslist}) {
        for my $pkgi(keys(%{$sys->{pkgverslist}})) {
            if ($#{$sys->{pkgverslist}{$pkgi}}) {
                $vers = join(')(', @{$sys->{pkgverslist}{$pkgi}});
                $msg = Msg::new("Package $pkgi has more than one version ($vers) installed on $sys->{sys}");
                $sys->push_warning($msg);
                $ret = 0;
            }
        }
...
_PKGSPACE_
    ...
    $rel->{patchesspace}=<<"_PATCHSPACE_";
PVCO_03937:36152,298587,0,289794 	   
PVCO_03944:21731,0,0,0 	   
PVCO_03948:6891,0,0,0 	   
PVCO_03950:29740,0,0,3 	   
PVCO_03952:9623,60424,0,61140	   
PVCO_03953:4602,0,0,0 	   
PVCO_03954:811,65,0,7 	   
PVKL_03938:0,22588,0,0 	   
PVKL_03942:0,1028,0,0 	   
PVKL_03949:6949,801,0,4011 	   
PVKL_03951:0,27466,0,0
PVCO_03955:91303,0,0,0
_PATCHSPACE_
    return;
}
...
VRTSamf:29,0,0,0
VRTScavf:22,0,0,0
VRTSfsadv:1879,1,0,9
VRTSfssdk:12,0,0,0
VRTSlvmconv:0,0,0,1
VRTSob:77,0,0,0
VRTSodm:4,0,0,1
VRTSsfcpi60:161,0,0,0
VRTSvbs:0,0,0,0
VRTSvcs:0,47,0,18
VRTSvxfs:434,1583,0,324
VRTSvxvm:11,1905,0,2147
_PATCHSPACE_
    return;
}
...
VRTSamf:33,0,0,0
VRTScavf:21,0,0,0
VRTSfsadv:1793,1,0,6
VRTSfssdk:12,0,0,0
VRTSlvmconv:0,0,0,1
VRTSob:25,0,0,0
VRTSodm:6,0,0,1
VRTSsfcpi60:140,0,0,0
VRTSvbs:0,0,0,0
VRTSvcs:0,37,0,18
VRTSvxfs:462,1623,0,393
VRTSvxvm:20,1897,0,2014
_PATCHSPACE_
    return;
}
...
VRTSvbs:0,0,0,0
VRTSvcs:0,47,0,18
VRTSvxfs:440,1673,0,1038
VRTSvxvm:19,1813,0,2279
_PATCHSPACE_
    return;
}
...
VRTSvbs:0,0,0,0
VRTSvcs:0,35,0,18
VRTSvxfs:451,1877,0,374
VRTSvxvm:15,1870,0,2347
_PATCHSPACE_
    return;
}
...
_PKGSPACE_
    ...
    $rel->{patchesspace}=<<"_PATCHSPACE_";
146917-01:0,40,139086,22
146918-01:167,0,3542,1
147852-01:3280,185,44299,348
147853-02:100,17053,166553,3656
147883-01:17,44,1063,0
147887-01:4791,0,15721,0
147891-01:1,0,1312,0
147893-01:2114,0,2665,0
148452-01:1733,0,7739,1
148453-01:707,0,31747,0
148457-01:22,0,664,0
_PATCHSPACE_
...
_PKGSPACE_
    ...
    $rel->{patchesspace}=<<"_PATCHSPACE_";
147849-01:0,60,231451,22
147850-01:177,0,6340,1
147854-02:20,5255,172254,2856
147855-01:3045,168,34707,339
147886-01:17,31,1002,0
147888-01:10772,0,42596,0
147892-01:4,0,1101,0
148450-01:157,0,2665,0
148454-01:707,0,31744,0
148458-01:22,0,682,0
_PATCHSPACE_
