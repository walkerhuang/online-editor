...
sub init_common {
    ...
    $prod->{migrate_native_file}='/etc/vx/.migrate_native';
...
sub dmp_osn_stop_sys {
    ...
    if ($out =~ /dmp_native_support\s+on/mx) {
        ...
        $sys->cmd("_cmd_touch $prod->{migrate_native_file}") if (Cfg::opt(qw(upgrade patchupgrade stop rollback)));
    }
    return 1;
}
...
sub completion_messages {
    ...
    $rootpath = Cfg::opt('rootpath');
    $web=Obj::web();
    # this message should only be shown after non_kernel package upgraded during rolling upgrade.
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSsfcpi60 VRTSperl512 VRTSperl.rte VRTSvlic32
    ) ];
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSjre VRTSsfcpi60 VRTSperl512 VRTSvlic32 VRTSwl
    ) ];
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSsfcpi60 VRTSperl512 VRTSvlic32
    ) ];
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSicsco VRTSvxvm60 VRTSjre15 VRTSjre VRTSsfcpi60 VRTSperl512
        VRTSvlic32
    ) ];
