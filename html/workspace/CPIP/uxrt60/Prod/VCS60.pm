...
package Prod::VCS60::Common;
@Prod::VCS60::Common::ISA = qw(Prod);
...
sub init_common {
    ...
    $prod->{name}=Msg::new("Veritas Cluster Server")->{msg};
    $prod->{mainpkg}='VRTSvcs60';
    ...
    $prod->{installonupgradepkgs} = [ qw(VRTSamf VRTSsfmh VRTSvbs) ];
...
sub description {
    my $msg=Msg::new("Veritas Cluster Server, the industry's leading open systems clustering solution, is ideal for eliminating both planned and unplanned downtime, facilitating server consolidation, and effectively managing a wide range of applications in heterogeneous environments.");
    $msg->print;
    return;
}
...
sub version_sys {
    ...
    # If ApplicationHA installed, then regard VCS not installed.
    $pkg=Pkg::new_pkg('Pkg', 'VRTSvcsvmw', $sys->{padv});
    $pkgvers=$pkg->version_sys($sys);
    return '' if ($pkgvers);

    return $prod->SUPER::version_sys($sys,$force_flag);
}
...
sub verify_procs_list_sys {
    ...
    if ((defined $state && $state eq 'stop') && (Cfg::opt('upgrade_kernelpkgs') && (!EDRu::inarr('had60',@{$procs})))) {
        push(@{$procs},'had60');
        push(@{$procs},'CmdServer60');
...
sub vcs_enable_sys {
    ...
    if ($cfg->{vcs_allowcomms}) {
        @list = qw(llt60 gab60 had60 amf60);
...
sub vcs_disable_sys {
    ...
    for my $proci (qw(llt60 gab60 had60 vxfen60 amf60)) {
...
sub set_vcs_allowcomms_sys {
    ...
    if (@$pids && $nsystems < 2 && ($cprod =~ /^VCS\d+/mx || $cprod =~ /^SFHA\d+/mx)) {
...
sub upgrade_ru_check_sys {
    ...
    unless ($had->check_sys($sys) && $gab->check_sys($sys) && $llt->check_sys($sys)) {
        $msg = Msg::new("To do rolling upgrade, VCS must be running on $sys->{sys}.");
        $sys->push_error($msg);
        return 0;
    }
...
sub verify_responsefile {
    ...
    if (Cfg::opt('configure')||Cfg::opt('upgrade')||Cfg::opt('start')) {
...
sub upgrade_precheck_sys {
    my ($prod,$sys) = @_;
    my ($msg,$syslist,$had,$cpic,$cprod,$rootpath,$vcsinitfile);

    $cpic=Obj::cpic();
    $syslist=CPIC::get('systems');
    $prod->check_timesync_sys($sys, $syslist, 5, 1) if($sys->system1 && (EDR::num_sys_keyset('stop_checks')==0));
    ...
    unless (Cfg::opt('rootpath')) {
        ...
        if($had->check_sys($sys, 'start')) {
            ...
            if ($prod->{upgrade_ignore_conf}) {
                ...
                $msg = Msg::new("$prod->{abbr} is not running before upgrade. Please make sure all the configurations are valid before upgrade.");
                $sys->push_warning($msg);
            }
        }
    }
...
sub patchupgrade_precheck_sys {
    my ($prod,$sys) = @_;
    my ($syslist,$had,$msg,$cpic,$cprod,$vcsinitfile);
    $cpic=Obj::cpic();
    $syslist=CPIC::get('systems');
    $prod->check_timesync_sys($sys, $syslist, 5, 1) if($sys->system1 && (EDR::num_sys_keyset('stop_checks')==0));
    $prod->upgrade_ru_check_sys($sys);
...
sub precheck_task {
    ...
    if (Cfg::opt(qw(patchupgrade upgrade))) {
        $prod->phased_upgrade_precheck() unless (Cfg::opt('upgrade_kernelpkgs'));
...
sub phased_upgrade_precheck {
    my($cprod,$syslist,$csystems,$edr,$msg,$nsystems,$prod,$str,$sys1,$sysi,$sysj,$obj_sysi,@sysnames,$vcs,$ver1,$veri,@no_comm_systems);
    $vcs = shift;
    ...
    if ($csystems < $nsystems) {
        ...
        for my $sysi (split(/\n/,$str)) {
            ...
            unless (EDRu::inarr($sysi, @sysnames)) {
                ...
                if (EDRu::compvers($ver1,$veri,4) == 2) {
                    $msg = Msg::new("You are performing $prod->{prod} phased upgrade (Phased 2) on the systems. The second subcluster will be upgraded.");
                    $sys1->push_note($msg);
        ...
        if (scalar @no_comm_systems && $vcs->is_secure_cluster($sys1)){
...
sub completion_messages {
    ...
    if (Cfg::opt("configure")) {
        ...
        if ($cfg->{fencingenabled} && (!$cfg->{donotreconfigurevcs}) && $prod->{fencing_config_pending}) {
            ...
            my $installscript= 'install' . (lc($cprod->{installscript_name} || $cprod->{prod}));
            $msg = Msg::new("I/O fencing configuration is not done. You have two ways to configure it:\n 1. Run the command '$installscript -fencing'.\n 2. Select the I/O fencing configuration task while running the webinstaller.\n");
            $msg->print;
...
sub completion_message_rollingupgrade {
    my ($prod)=@_;
    my ($sys,$rel,$rulist,$non_rulist,$cpic,$msg,@subs,@systems,$cfg,$ayn,$web,$webmsg);
    $cpic=Obj::cpic();
    $rel=$cpic->rel;
    $cfg=Obj::cfg();
    $web = Obj::web();
    ...
    if(Cfg::opt('upgrade_nonkernelpkgs')){
        $msg=Msg::new("The Rolling Upgrade has been completed on the cluster");
        $msg->bold;
        $web->web_script_form('alert', $msg) if (Obj::webui());
        return 1;
    }
    @subs = qw(systems set_pkgs shutdown uninstall install licensing startup);
    ...
    if($non_rulist){
        if(@{$cpic->{reboot_systems}}){
            $msg=Msg::new("It is recommended to perform rolling upgrade phase 1 on the systems$non_rulist in the next step. Rerun the installer to do this after reboot\n");
            $msg->bold;
            $web->web_script_form('alert', $msg) if (Obj::webui());
            return;
        } else {
           $msg=Msg::new("It is recommended to perform rolling upgrade phase 1 on the systems$non_rulist in the next step.");
           $msg->bold;
           $webmsg = $msg->{msg};
           ...
           if($ayn eq 'Y'){
               $non_rulist=~ s/^\s+|\s+$//g;
               @systems=split(/\s+/,$non_rulist);
               $rel->{nextrusys}=$non_rulist;
               $web->{systemlist}=$non_rulist;
               $cfg->{systems}=\@systems;
               $rel->{noruconfirm}=1;
        ...
        if(@{$cpic->{reboot_systems}}){
           ...
           $msg=Msg::new("It is recommended to perform rolling upgrade phase 2 on all the cluster systems in the next step.");
           $msg->bold;
           $webmsg = $msg->{msg};
           ...
           if($ayn eq 'Y'){
               $rulist=~ s/^\s+|\s+$//g;
               @systems=split(/\s+/,$rulist);
               $rel->{nextrusys}=$rulist;
               $web->{systemlist}=$rulist;
               $rel->{noruconfirm}=1;
               $cpic->set_task("upgrade");
               foreach my $sysn(split(/\s+/,$rulist)){delete $Obj::pool{"Sys::$sysn"};}
...
sub check_timesync_sys {
    ...
    my ($summary, $timer_sin);
    my ($sec,$min, $hour, $mday, $mon, $year, $timer_min, $timer_max);
    my ($msg, $sys, $timer_diff);
    my ($local_time, $time1, $time2);

    Msg::log("Time synchronization check for $prod->{abbr}");
    $prod->{time_async} = 0;
    $prod->set_value('time_async',0);
    EDR::set_value('time_async',0);
    for my $sys (@{$syslistref}) {
        $sys = Obj::sys($sys) if(ref($sys) ne 'Sys');
        eval {
            $timer_sin = $sys->cmd('_cmd_date -u +%S:%M:%H:%d:%m:%Y');
            $local_time = EDR::cmd_local('_cmd_date -u +%S:%M:%H:%d:%m:%Y');
        };
        $errstr = $@;
        if ($errstr) {
            Msg::log("Can't get date and error info: $errstr");
            $msg = Msg::new("Could not complete the Time synchronization check. Failed to get system 'date' information.");
            $sys->push_warning( $msg);
            return 1;
        }

        eval {
            ($sec,$min,$hour,$mday,$mon,$year) = split(/:/m,$timer_sin);
            $time1 = timegm ($sec, $min, $hour, $mday, $mon - 1, $year);

            ($sec,$min,$hour,$mday,$mon,$year) = split(/:/m,$local_time);
            $time2 = timegm ($sec, $min, $hour, $mday, $mon - 1, $year);

            $timer_diff = $time1-$time2;
            unless(defined $timer_max){
                $timer_max=$timer_min=$timer_diff;
            }
            $timer_max = ($timer_diff>$timer_max)?$timer_diff:$timer_max;
            $timer_min = ($timer_diff<$timer_min)?$timer_diff:$timer_min;
        };
        $errstr = $@;
        if ($errstr) {
            Msg::log("Can't change \$timer_ref:$timer_sin and error info: $errstr");
            $msg = Msg::new("Could not complete the Time synchronization check. Failed to perform range checking on the system 'date'.");
            $sys->push_warning( $msg);
            return 1;
        }
    }
    if ($timer_max-$timer_min>$time_sync) {
        if (!$nowarning) {
            $msg = Msg::new("Nodes have difference in clock by more than 5 sec");
            $sys_t->push_warning($msg);
        }
        $prod->{time_async} = 1;
        $prod->set_value('time_async',1);
        EDR::set_value('time_async',1);
        return 1;
    } else {
        $summary = Msg::new("Synchronized time setting on cluster nodes checking of $prod->{abbr} is OK.");
        $summary->log;
    }
...
sub check_cluster_members_sys {
    my ($prod,$sys) = @_;
    my ($m,$cprod,$msg,$sys0,$n,$syslist,$conf);
    $syslist=CPIC::get('systems');
    $cprod=CPIC::get('prod');
    ...
    if ($conf) {
        if ($sys->system1) {
            ...
            if ( $m < $n) {
                ...
                return 1 unless ($cprod eq 'VCS60' || $cprod eq 'SFHA60');
                $msg = Msg::new("Not all systems of the cluster, $conf->{clustername}, have been entered to be upgraded. It is recommended that all cluster systems are upgraded together, except when you plan to perform phased upgrade on the set of systems.");
...
sub vxfen_upgrade_precheck_sys {
    ...
    my ($conf,$msg);
    my $rootpath = Cfg::opt('rootpath') || '';
    return 1 if ((Cfg::opt('patchupgrade')) && (!$prod->check_config()));
    if ($sys->exists("$rootpath$prod->{vxfenmode}")) {
...
sub get_vxfen_config_sys {
    my ($prod,$sys)=@_;
    my ($line,$str);
    my ($conf,$cps_sys1,$cpsvip,$defport,$port,$sys1_flag,$uuid);
    my $vrtscps_pkg = $prod->pkg('VRTScps60');
    my $rootpath = Cfg::opt('rootpath') || '';

    $conf = {};
    $defport = $vrtscps_pkg->{defport};
    $conf->{security} = '-1';
    $conf->{vxfen_mode} = $conf->{vxfen_diskpolicy} = $conf->{vxfen_mechanism} = '';
    $conf->{cps} = [];
    $conf->{cpport}= {};
    ...
    for my $line (split(/^/m,$str)) {
        ...
        next if ((!$line) || ($line=~/^#/m));
        if ($line=~/vxfen_mode\s*=\s*(\w+)/mx) {
            $conf->{vxfen_mode}=$1;
        } elsif ($line=~/scsi3_disk_policy\s*=\s*(\w+)/mx) {
            $conf->{scsi3_disk_policy}=$1;
        } elsif ($line=~/vxfen_mechanism\s*=\s*(\w+)/mx) {
            $conf->{vxfen_mechanism}=$1;
        } elsif ($line =~ /vxfendg\s*=\s*(\w+)/mx) {
            $conf->{vxfendg}=$1;
...
sub maincf_upgrade_precheck_sys {
    my ($prod,$sys) = @_;
    my ($rootpath,$msg,$unfreeze_sg,$grp,@grp_syslist,$cmd_hacf,@grps,$delete_cmc,$treeref_olduser,@extra_types,$attrs,$vxss_autostartlist,$upgradepath,$sfua_delete_SfuaBase,$line,$rtn,$delete_VCSweb,$grp_syslists,$vxss_add_autostartlist);
    ...
    if ($sys->system1) {
        ...
        unless ($prod->dynupgrade_pre_upgrade_sys($sys)) {
            EDRu::create_flag('maincf_upgrade_precheck_done');
            return 0;
        }
        ...
        maincf_check_TriggerResStateChange($sys,$treeref_olduser);

        # Update PreOnline attribute.
        maincf_preonline_precheck_sys($sys,$treeref_olduser);

        # vip will only bu updated for upgrade from less6.0 to 6.0 onwards
        $prod->maincf_precheck_if_update_vip_sys($sys);
        ...
        for my $pkgi (qw(VRTSvcsea60 VRTSdbac60)) {
            $pkg = $Obj::pool{"Pkg\::$pkgi\::$sys->{padv}"};
        ...
        unless ($prod->check_obsolete_types_sys($sys,$treeref_olduser)) {
            EDRu::create_flag('maincf_upgrade_precheck_done');
            return 0;
        }
        ...
        if ($panic_system_on_dg_loss_attr eq '1') {
            ...
            if (@reslist) {
                $msg = Msg::new("The default value of the diskGroup agent attribute, PanicSystemOnDGLoss, will be changed from 1 to 0 after upgrade. Refer to the Veritas Cluster Server Bundled Agents Reference Guide for the implications of this change.");
                $sys->push_warning($msg);
            }
        }

        $prod->upgrade_cvm_cfs_types_sys($sys,$treeref_olduser);
        ...
        $prod->padv_maincf_upgrade_precheck_sys($sys,$treeref_olduser) if ($prod->can('padv_maincf_upgrade_precheck_sys'));
        $prod->padv_update_netmask($sys,$treeref_olduser);
...
sub padv_update_netmask {
    ...
    @reslist = Prod::VCS60::Common::dynupgrade_get_resname_list_from_typename_list($treeref, $type_list);
    for my $res (@reslist) {
        $noderef = Prod::VCS60::Common::dynupgrade_get_node_from_tree($treeref, 'RES', $res);
        ...
        $attrref_netmask = Prod::VCS60::Common::dynupgrade_get_node_from_tree($noderef, 'ATTR', 'NetMask');
        if (!$attrref_netmask) {
            $addressref = Prod::VCS60::Common::dynupgrade_get_node_from_tree($noderef, 'ATTR', $address_name);
...
sub maincf_check_TriggerResStateChange {
    my ($sys,$treeref) = @_;
...
sub maincf_preonline_precheck_sys {
    my ($sys,$treeref) = @_;
    my ($sysi,$msg,@grps,$grpref,$vcsvers,$grp_name,$padv,$haconf,$preonlineref,$preonline_cmd,$vcspkg,@syslist,$preonline,$prod);


    # Check PreOnline setting in main.cf
    $padv = $sys->{padv};
    $vcspkg=$sys->pkg('VRTSvcs60');
    if (defined $vcspkg) {
        $vcsvers=$vcspkg->version_sys($sys);
    }
...
sub maincf_precheck_if_update_vip_sys {
    ...
    $sys->set_value("update_vip_sgs", 1) if ($vcsvers) && (EDRu::compvers($vcsvers,'6.0') == 2);
    $sys->set_value("update_NFSRestart", 1) if ($vcsvers) && (EDRu::compvers($vcsvers,'5.1.100') == 2);

    return '';
}

sub upgrade_cvm_cfs_types_sys {
    my ($prod,$sys,$treeref_olduser) = @_;
...
sub modify_sourcefile_sys {
    ...
    my ($key, $sourcefile, $sourcefilebase, @sourcefiles);
...
sub start_precheck_sys {
    my ($prod,$sys) = @_;
    my ($sysi,$msg,@missing_comms_files,$sysnum,$filename,$hacf_cmd,$comms_required,$onenode,$rtn,$maincf);
    @missing_comms_files = ();
    $sysnum = 0;
    $onenode = 0;
    $comms_required = 1;
...
sub install_precheck_sys {
    ...
    $syslist=CPIC::get('systems');
    if (Cfg::opt("precheck")) {
        $prod->check_timesync_sys($sys, $syslist, 5) if($sys->system1 && (EDR::num_sys_keyset('stop_checks')==0));
    } else {
        $prod->check_timesync_sys($sys, $syslist, 5, 1) if($sys->system1 && (EDR::num_sys_keyset('stop_checks')==0));
    }
...
sub backup_vcs_lock_files_sys {
    my ($prod,$sys) = @_;
    my ($lockfilepath,$oldfolder,$newfolder,$lockfolder,$vcsvers,$rootpath);
...
sub restore_vcs_lock_files_sys {
    ...
    $rootpath = Cfg::opt('rootpath');
    $files = $sys->cmd("cd $lockfilepath; _cmd_ls -a | _cmd_grep online_lock 2>/dev/null");
...
sub startprocs_sys {
    ...
    if ($cfg->{vcs_allowcomms}) {
        if ((!$sys->exists("$prod->{vxfenmode}")) && (!$sys->exists("$prod->{vxfendg}"))) {
            $procs = EDRu::arrdel($procs, 'vxfen60');
        }
        return $procs;
    } else {
        $procs = EDRu::arrdel($procs, 'llt60');
        $procs = EDRu::arrdel($procs, 'gab60');
        $procs = EDRu::arrdel($procs, 'vxfen60');
        return $procs;
    }
}
...
sub default_systemnames {
    ...
    $hosts = EDR::cmd_local("_cmd_cat $rootpath$prod->{llthosts} 2>/dev/null | _cmd_awk '{print \$2}'");
    for my $host (split(/\n/,$hosts)) {
        next unless $host;
        push(@llthosts,$host);
    }
...
sub responsefile_comments {
    ...
    $cmt=Msg::new("This variable defines the name of the system where the root broker is installed");
    $edr->{rfc}{at_rootdomain}=[$cmt->{msg},0,1,0];
    ...
    $cmt=Msg::new("This variable defines whether to reverse the cluster's secure mode");
...
sub prestart_config_common_questions {
    my ($ayn,$cfg,$sys0,$syslist,$cprod,$msg,$prod,$rel,$rtn);
    $prod=shift;
    ...
    $cfg->{vcs_allowcomms} = 1;
    if ((($cprod eq 'VCS60')||($cprod eq 'SFHA60')) && (!$cfg->{fencingenabled})) {
        if (scalar(@$syslist) == 1) {
    ...
    # check uuid, if uuid is not configured on systems, then it means that 5.1SP1 is not configured before.
    if ($prod->check_config() && $prod->check_uuid()) {
...
sub cli_prestart_config_questions {
    ...
    my $cprod=CPIC::get('prod');
    if ($cprod!~/^SFRAC\d+/mx && $cprod!~/^SFSYBASECE\d+/mx) {
...
sub web_autocfg_hbnics {
    ...
    if ($prod->num_hbnics($rhbn)< 1) {
        # auto detect failed
        $msg=Msg::new("Failed to detect and configure LLT heartbeat links. Please configure LLT manually.");
...
sub web_prestart_config_questions {
    my ($rtvalue,$prod,$web,$cfg,$cprod,$key,$msg,$help,$ayn);
    $prod = shift;
...
sub check_config {
    ...
    $cprod=CPIC::get('prod');
    $cfg = Obj::cfg();
    return 0 if (Cfg::opt('responsefile') && !defined($cfg->{donotreconfigurevcs}) );
    $rootpath = Cfg::opt('rootpath') || '';
    for my $sysi (@$syslist) {
    ...
    $sys0 = $$syslist[0];
    return 0 if $prod->is_secure_cluster($sys0) && !$prod->eat_check_secure_clus();
...
sub get_config_sys {
    ...
    return 0 unless ($sys->exists("$rootpath$prod->{maincf}"));
    ...
    return 0 if ($num == 0); # no system defined
    ...
    if ($num > 1) {
        ...
        unless ($sys->exists("$rootpath$prod->{llthosts}")
                && $sys->exists("$rootpath$prod->{llttab}")
                && $sys->exists("$rootpath$prod->{gabtab}")) {
            return 0;
        }
        ...
        return 0 if ($num != @{$lltconf->{systemlist}});
        my $tmp_aref = EDRu::arruniq(@{$conf{systems}}, @{$lltconf->{systemlist}});
        return 0 if ($num != @{$tmp_aref});
...
sub get_llt_config_sys {
    ...
    if ($sys->exists($llthosts_file)) {
        $llthosts = $sys->readfile($llthosts_file);
        for my $line(split(/\n/,$llthosts)) {
            if ($line =~ /^\s*(\d+)\s+(\S+)/) {
                $hostname = $2;
...
sub configure_sys {
    my ($prod,$sys) = @_;
    my ($sysi,$sysname,$msg,$cprod,$cfg,$str,$vxfen,$rtn,$hostname,$tmpdir,$cpic);
    my ($localsys,$rootpath,$vxfenmodedisablefile,$vxfenmodefile,$vxfendgfile);
    $cpic=Obj::cpic();
    $cfg = Obj::cfg();
    $cprod=CPIC::get('prod');
    $vxfen=$sys->proc('vxfen60');
    ...
    if (!$cfg->{donotreconfigurevcs}) {
        ...
        if ($cprod =~ /^(SVS|SFCFS|SFRAC|SFSYBASECE)/mx) {
            ...
            $sys->set_value('donotstartprocs','push','vxfen60');
            $str = EDRu::datetime();
        ...
        if ($multinode) {
            ...
            if (!$cfg->{autocfgllt}) {
                ...
                if ($diff == 2) {
                    ...
                    $msg = Msg::new("$script can not detect media speed for the selected private NICs properly. Consult your Operating System manual for information on how to set the Media Speed.\n");
                    $msg->warning();
                }
                if ($diff) {
...
sub display_config_info {
    ...
    my ($address,$cfg,$lmsg,$msg,$netmask,$key,$port,$smsg,$sys);
    $cfg = Obj::cfg();
...
sub check_uuidconfig_pl {
    my ($localsys,$prod,$result,$mediapath);
    $prod = shift;
    $localsys = $prod->localsys;
...
sub config_uuid {
    ...
    $result=$prod->check_configure_uuid();
    return $result;
}
...
sub create_uuid_sys {
    ...
    my $cmd_uuidconfig = $export_rootpath.$prod->{uuidconfig};
    if($rsh =~/rsh/m) {
...
sub hb_config_option {
    ...
    $cfg = Obj::cfg();
    $msg = Msg::new("Configure heartbeat links using LLT over Ethernet");
    push(@{$items},$msg->{msg});
    $msg = Msg::new("Configure heartbeat links using LLT over UDP");
    push(@{$items},$msg->{msg});
    ...
    if ($cfg->{lltoverudp}) {
        $defaultopt = '2';
    } elsif ($cfg->{autocfgllt} && !$autocfg_failed) {
        $defaultopt = '3';
    } else {
        $defaultopt = '1';
    }
...
sub ask_hbnics {
    ...
    for my $sys (@{CPIC::get('systems')}) {
        ...
        if ($all) {
            ...
            # link 1
            $hb = $prod->ask_hbnic_sys($sys,1,$rsn,$rpn);
            return $hb if (EDR::getmsgkey($hb,'back'));
            $hbn{lltlink1}{$sysi} = $en[1] = $hb;
            if ($padv->is_bonded_nic_sys($sys,$hb)){
                push(@{$cfg->{$sys}{bonded_nics}},$hb);
            }
        }
        if ($cfg->{lltoverudp}) {
            $ip = $prod->ask_nic_ip_sys($sys,$en[1],1);
        ...
        if ($cfg->{lltoverudp} && ($hb2 eq 'Y')) {
            $ip = $prod->ask_nic_ip_sys($sys,$en[2],2);
            return $ip if (EDR::getmsgkey($ip,'back'));
        ...
        if ($cfg->{lltoverudp} && ($hb3 eq 'Y')) {
            $ip = $prod->ask_nic_ip_sys($sys,$en[3],3);
            return $ip if (EDR::getmsgkey($ip,'back'));
        ...
        if ($cfg->{lltoverudp} && ($hb4 eq 'Y')) {
            $ip = $prod->ask_nic_ip_sys($sys,$en[4],4);
            return $ip if (EDR::getmsgkey($ip,'back'));
        ...
        # link lowpri
        unless ($all) {
            if ($cfg->{fencingenabled} || ($hb2 eq 'Y')) {
        ...
        if ($cfg->{lltoverudp} && ($hbl eq 'Y')) {
            $ip = $prod->ask_nic_ip_sys($sys,$en[$prod->{max_hipri_links}+1],'lowpri');
            return $ip if (EDR::getmsgkey($ip,'back'));
...
sub autocfg_hbnics {
    ...
    for my $sys (@{CPIC::get('systems')}) {
        ...
        $sys->cmd(" ; for i in `_cmd_ls $tmpdir/result-*`;do echo \$i;_cmd_cat \$i;done 2>/dev/null");
    ...
    for my $n (0..$#{$rnics->{$sysi0}}) {
        ...
        if ($all_connected) {
            ...
            for my $sys (@{CPIC::get('systems')}) {
                ...
                if ($selected_speed =~ /\D*(\d+)\s*[Gg]/mx) {
                    $speed_value = $1;
                    $speed_value *= 1000;
                } elsif ($selected_speed =~ /\D*(\d+)\D*/mx) {
                    $speed_value = $1;
                } else {
                    $speed_value = 0;
                }
...
sub ask_hbnics_sys{
    ...
    for my $link (1..$max_links) {
        ...
        if ( $ask_nic ) {
            $selected_nic = $prod->ask_hbnic_sys($sys,$link,$rsns->{$sysi},$rpns->{$sysi},\@configured_nics,$default_nic);
        ...
        if ($udp) {
            ...
            # ask for IP address
            $ip = $prod->ask_nic_ip_sys($sys,$rhbn->{"lltlink$link"}{$sysi},$link);
            return $ip if (EDR::getmsgkey($ip,'back'));
    ...
    if (!$no_lowpri_links) {
        ...
        for my $link (1..$max_links-$prod->num_hbnics($rhbn)) {
            ...
            if ($udp) {
                ...
                # ask for IP address
                $ip = $prod->ask_nic_ip_sys($sys,$rhbn->{"lltlinklowpri$link"}{$sysi},"lowpri$link");
                return $ip if (EDR::getmsgkey($ip,'back'));
...
sub ask_hbnics_addnode {
    ...
    my ($min_hipri_links, $max_links, $is_first_node, $ask_for_nic_names, $is_udp);
    return '' if (Cfg::opt('responsefile'));
    $cfg=Obj::cfg();
    $cprod=CPIC::get('prod');
    if ( ! $sysi0) {
...
sub ask_nic_ip_sys {
    my ($prod,$sys,$nic,$id) = @_;
    my ($script,$sysi,$msg,%ip,$ips,$padv,$answer,$address,$question,$help,@pool,$netmask);
    $padv = $sys->padv;
    $sysi = $sys->{sys};
    while (1) {
        $ips=$padv->nic_ips_sys($sys,$nic);
        ...
        while ($address) {
            if ($id =~ /lowpri/m) {
                if ($id eq 'lowpri' || $id eq 'lowpri1') {
                    $question = Msg::new("Do you want to use address $address for the low-priority heartbeat link on $sysi:");
                } elsif ($id eq 'lowpri2') {
                    $question = Msg::new("Do you want to use address $address for the second low-priority heartbeat link on $sysi:");
                } elsif ($id eq 'lowpri3') {
                    $question = Msg::new("Do you want to use address $address for the third low-priority heartbeat link on $sysi:");
                } elsif ($id eq 'lowpri4') {
                    $question = Msg::new("Do you want to use address $address for the fourth low-priority heartbeat link on $sysi:");
                } elsif ($id eq 'lowpri5') {
                    $question = Msg::new("Do you want to use address $address for the fifth low-priority heartbeat link on $sysi:");
                } elsif ($id eq 'lowpri6') {
                    $question = Msg::new("Do you want to use address $address for the sixth low-priority heartbeat link on $sysi:");
                } elsif ($id eq 'lowpri7') {
                    $question = Msg::new("Do you want to use address $address for the seventh low-priority heartbeat link on $sysi:");
                } elsif ($id eq 'lowpri8') {
                    $question = Msg::new("Do you want to use address $address for the eighth low-priority heartbeat link on $sysi:");
                }
            }elsif ($id == 1) {
                $question = Msg::new("Do you want to use address $address for the first private heartbeat link on $sysi:");
            } elsif ($id == 2) {
                $question = Msg::new("Do you want to use address $address for the second private heartbeat link on $sysi:");
            } elsif ($id == 3) {
                $question = Msg::new("Do you want to use address $address for the third private heartbeat link on $sysi:");
            } elsif ($id == 4) {
                $question = Msg::new("Do you want to use address $address for the fourth private heartbeat link on $sysi:");
            } elsif ($id == 5) {
                $question = Msg::new("Do you want to use address $address for the fifth private heartbeat link on $sysi:");
            } elsif ($id == 6) {
                $question = Msg::new("Do you want to use address $address for the sixth private heartbeat link on $sysi:");
            } elsif ($id == 7) {
                $question = Msg::new("Do you want to use address $address for the seventh private heartbeat link on $sysi:");
            } elsif ($id == 8) {
                $question = Msg::new("Do you want to use address $address for the eighth private heartbeat link on $sysi:");
            }
            $answer = $question->ayny($help,1);
            return $answer if (EDR::getmsgkey($answer,'back'));
            if ($answer eq 'Y') {
                $ip{address} = $address;
                $ip{netmask} = $sys->defaultnetmask($address,$nic);
                return \%ip;
            } else {
                $address = shift(@pool);
            }
        ...
        if (@$ips) {
            $msg = Msg::new("A permanent IP address must be selected for this heartbeat link.");
            $msg->bold();
        } else {
            $script=EDR::get('script');
            $msg = Msg::new("$script did not detect any IP address on $nic");
            $msg->bold();
        }
        $msg = Msg::new("Configuring a heartbeat link using LLT over UDP requires a permanent IP address assigned to the selected heartbeat NIC. Make sure $nic has an permanent IP address. Press y to re-detect, or press n to choose another NIC\n");
        $msg->print();
        $question = Msg::new("Do you want to re-detect $nic on $sysi?");
        $answer = $question->ayny();
        return EDR::get2('msg','back') if ($answer eq 'N');
    }
    return;
}

sub ask_udp_port_sys {
    my ($prod,$sys,$id,$used_port) = @_;
    my ($sysi,$def_port,$msg,$port,$question,$help);
    $sysi = $sys->{sys};
    $help = Msg::new("Please input an available 16-bit integer from the range that follows:\n  Use available ports in the private range 49152 to 65535.\n  Do not use the following ports:\n    Ports from the range of well-known ports, 0 to 1023.\n    Ports from the range of registered ports, 1024 to 49151.\nYou can use the netstat command to list the UDP ports currently in use.");
    if ($id =~ /lowpri/m) {
        if ($id eq 'lowpri' || $id eq 'lowpri1') {
            $question = Msg::new("Enter the UDP port for the low-priority heartbeat link on $sysi:");
            $def_port = '50004'; # for capability, will be increased automatically if it has been used.
        } elsif ($id eq 'lowpri2') {
            $question = Msg::new("Enter the UDP port for the second low-priority heartbeat link on $sysi:");
            $def_port = '50009';
        } elsif ($id eq 'lowpri3') {
            $question = Msg::new("Enter the UDP port for the third low-priority heartbeat link on $sysi:");
            $def_port = '50010';
        } elsif ($id eq 'lowpri4') {
            $question = Msg::new("Enter the UDP port for the fourth low-priority heartbeat link on $sysi:");
            $def_port = '50011';
        } elsif ($id eq 'lowpri5') {
            $question = Msg::new("Enter the UDP port for the fifth low-priority heartbeat link on $sysi:");
            $def_port = '50012';
        } elsif ($id eq 'lowpri6') {
            $question = Msg::new("Enter the UDP port for the sixth low-priority heartbeat link on $sysi:");
            $def_port = '50013';
        } elsif ($id eq 'lowpri7') {
            $question = Msg::new("Enter the UDP port for the seventh low-priority heartbeat link on $sysi:");
            $def_port = '50014';
        } elsif ($id eq 'lowpri8') {
            $question = Msg::new("Enter the UDP port for the eighth low-priority heartbeat link on $sysi:");
            $def_port = '50015';
        }
    } elsif ($id == 1) {
        $question = Msg::new("Enter the UDP port for the first private heartbeat link on $sysi:");
        $def_port = '50000';
    } elsif ($id == 2) {
        $question = Msg::new("Enter the UDP port for the second private heartbeat link on $sysi:");
        $def_port = '50001';
    } elsif ($id == 3) {
        $question = Msg::new("Enter the UDP port for the third private heartbeat link on $sysi:");
        $def_port = '50002';
    } elsif ($id == 4) {
        $question = Msg::new("Enter the UDP port for the fourth private heartbeat link on $sysi:");
        $def_port = '50003';
    } elsif ($id == 5) {
        $question = Msg::new("Enter the UDP port for the fifth private heartbeat link on $sysi:");
        $def_port = '50004';
    } elsif ($id == 6) {
        $question = Msg::new("Enter the UDP port for the sixth private heartbeat link on $sysi:");
        $def_port = '50005';
    } elsif ($id == 7) {
        $question = Msg::new("Enter the UDP port for the seventh private heartbeat link on $sysi:");
        $def_port = '50006';
    } elsif ($id == 8) {
        $question = Msg::new("Enter the UDP port for the eighth private heartbeat link on $sysi:");
        $def_port = '50007';
    }
...
sub ask_hbnic_sys {
    my ($prod,$sysi,$fst,$rsn,$rpn,$ren,$default_nic) = @_;
    my ($sn,$msg,$cfg,$backopt,$sys,$ayn,$padv,$nic,$max_hipri_links,$help,$pn,$en,$def);
    $sys=$sysi->{sys};
    $padv=$sysi->padv();
    $cfg=Obj::cfg();
    $backopt=1;
    ...
    while (!$nic) {
        ...
        if ($fst =~ /lowpri/m) {
            ...
            if ($fst eq 'lowpri' || $fst eq 'lowpri1') {
                $msg=Msg::new("Enter the NIC for the low-priority heartbeat link on $sys:");
                $en=$msg->ask($def,$help,$backopt);
            } elsif ( $fst eq 'lowpri2' ) {
                $msg=Msg::new("Enter the NIC for the second low-priority heartbeat link on $sys:");
                $en=$msg->ask($def,$help,$backopt);
            } elsif ( $fst eq 'lowpri3' ) {
                $msg=Msg::new("Enter the NIC for the third low-priority heartbeat link on $sys:");
                $en=$msg->ask($def,$help,$backopt);
            } elsif ( $fst eq 'lowpri4' ) {
                $msg=Msg::new("Enter the NIC for the fourth low-priority heartbeat link on $sys:");
                $en=$msg->ask($def,$help,$backopt);
            } elsif ( $fst eq 'lowpri5' ) {
                $msg=Msg::new("Enter the NIC for the fifth low-priority heartbeat link on $sys:");
                $en=$msg->ask($def,$help,$backopt);
            } elsif ( $fst eq 'lowpri6' ) {
                $msg=Msg::new("Enter the NIC for the sixth low-priority heartbeat link on $sys:");
                $en=$msg->ask($def,$help,$backopt);
            } elsif ( $fst eq 'lowpri7' ) {
                $msg=Msg::new("Enter the NIC for the seventh low-priority heartbeat link on $sys:");
                $en=$msg->ask($def,$help,$backopt);
            } elsif ( $fst eq 'lowpri8' ) {
                $msg=Msg::new("Enter the NIC for the eighth low-priority heartbeat link on $sys:");
                $en=$msg->ask($def,$help,$backopt);
            }
        } elsif ($fst eq '1') {
            $msg=Msg::new("Enter the NIC for the first private heartbeat link on $sys:");
            if ( Cfg::opt('addnode')) {
            ...
            $msg=Msg::new("Enter the NIC for the second private heartbeat link on $sys:");
            $en=$msg->ask($def,$help,$backopt);
        } elsif ($fst eq '3') {
            $msg=Msg::new("Enter the NIC for the third private heartbeat link on $sys:");
            $en=$msg->ask($def,$help,$backopt);
        } elsif ($fst eq '4') {
            $msg=Msg::new("Enter the NIC for the fourth private heartbeat link on $sys:");
            $en=$msg->ask($def,$help,$backopt);
        } elsif ($fst eq '5') {
            $msg=Msg::new("Enter the NIC for the fifth private heartbeat link on $sys:");
            $en=$msg->ask($def,$help,$backopt);
        } elsif ($fst eq '6') {
            $msg=Msg::new("Enter the NIC for the sixth private heartbeat link on $sys:");
            $en=$msg->ask($def,$help,$backopt);
        } elsif ($fst eq '7') {
            $msg=Msg::new("Enter the NIC for the seventh private heartbeat link on $sys:");
            $en=$msg->ask($def,$help,$backopt);
        } elsif ($fst eq '8') {
            $msg=Msg::new("Enter the NIC for the eighth private heartbeat link on $sys:");
            $en=$msg->ask($def,$help,$backopt);
        }
        return $en if (EDR::getmsgkey($en,'back'));
        $en=EDRu::despace($en);
        ...
        if ($en eq '') {
            ...
            $msg->print;
        } elsif ((($pn>=0) && ($fst !~ /lowpri/m)) && !($cfg->{lltoverudp})||
            (($#$rsn>=0) && ($sn<0))) {
...
sub update_llttab {
    my ($prod,$tmp) = @_;
    my ($sysi,$sys,$cfg,$llttab,$hostname,$n,$tmpdir);
    $cfg = Obj::cfg();
    $tmpdir=EDR::tmpdir();
    for my $sys (@{CPIC::get('systems')}) {
        $sysi = $sys->{sys};
        ...
        if ($cfg->{lltoverudp}) {
            ...
            for my $n (1..$prod->num_hbnics_cfg($cfg)) {
                $llttab.=$prod->convert_nic2lltlink_sys($sys, $cfg->{"vcs_lltlink$n"}{$sysi});
            }
            for my $n (1..$prod->num_lopri_hbnics_cfg($cfg)) {
                $llttab.=$prod->convert_nic2lltlink_sys($sys, $cfg->{"vcs_lltlinklowpri$n"}{$sysi},1);
            }
        }
...
sub update_llthosts {
    my ($prod,$cfg,$llthosts,$n,$sysi,$lltsys,$tmpdir);
    $prod = shift;
    $cfg = Obj::cfg();
...
sub update_gabtab {
    my ($prod,$cfg,$gabtab,$n,$tmpdir);
    $prod = shift;
    $cfg = Obj::cfg();

    $n = scalar @{CPIC::get('systems')};
...
sub update_maincf {
    my ($prod,$sys) = @_;
    my ($admins,$ddev_nwhosts,$sysi,$extype,$cfg,$ddev,$drid,$cdev_nwhosts,$extypes,$opers,$n,$drnd,$cdev,$csl,$users,$sys2,$prefix,$ddev_attr,$maincf,$tmpdir,$recp,$cdev_attr,$cons);
    $cfg = Obj::cfg();
...
sub upgrade_configure_sys {
    my ($prod,$sys) = @_;
    my ($sysi,$msg,$cfg,$uuid,$sysm,$copy_uuid_success,$obj_sysi,$syslist,$rtn,$pkgi,$pkg,$vxfen,$cpic,$out,$amf,$initfile_content,$rootpath,$nsystems,$maincf,$usefence);
    ...
    if ($sys->system1) {
        ...
        if ($prod->{upgrade_ignore_conf}) {
            ...
            for my $sysi (@{$sys->{cluster_systems}}) {
                ...
                next unless ($uuid && $uuid !~ /NO_UUID/m);
                for my $sysm (@$syslist) {
        ...
        $prod->eat_upgrade_configure if $prod->{eat_upgrade_secure};
...
sub ru_prestop_sys {
    my ($prod,$sys)=@_;
    my (@procs,$proc,$procobj,$cprod);
    if (Cfg::opt('upgrade_kernelpkgs')) {
        @procs=qw(gab60 vxfen60);
        for my $proc (@procs) {
...
sub upgrade_prestop_sys {
    ...
    if ($sys->system1) {
        #return 0 unless ($prod->check_obsolete_types_sys($sys));
...
sub dynupgrade_pre_upgrade_sys {
    my ($prod,$sys) = @_;
    my ($rootpath,$msg,$uuid,$upgradepath,$sourcefilebase,$line,$rtn,$sourcefile,$maincf);
...
sub dynupgrade_upgrade_sys {
    my ($prod,$sys) = @_;
    my ($msg,$lines_ref,$syslist,$sourcefilebase,$rtn,$localsys);
    my $rootpath=Cfg::opt('rootpath');
    ...
    if( $sys->exists("$upgradepath/result_config/main.cmd" ) ) {
        ...
        for my $sys (@$syslist) {
            ...
            my $ret= $prod->translate_file_cmd2cf_sys( $sys, "$rootpath$prod->{configdir}");
            unless( $ret) {
...
sub maincf_result_upgrade_cpagent_sys {
    my ($prod,$sys,$treeref) = @_;
    my (@cpgrplist,@grplist,@reslist,$res_group_name,$res_ref,$result_lines);
...
# Update the Sybase resources during upgrade, DetailMonitor is obsoleted
# Override the LevelTwoMonitorFreq if not exist
# set the value as the DetailMonitor
# set the value of DetailMonitor to 0
sub maincf_result_upgrade_Sybase_sys {
    ...
    for my $res (@reslist) {
        ...
        $attr_ref_in = dynupgrade_get_attr_from_node($res_ref,'LevelTwoMonitorFreq');
        if ($attr_ref_in) {
            # do not modify LevelT, just modify the DetailM
            if ( $attr_ref ) {
                $attr_value = $attr_ref->{MODIFY}->{ATTRVAL}->[0];
                $result_lines .= "hares -modify $res DetailMonitor 0\n" unless ($attr_value == 0);
            }
        } else {
            # override and modify LevelT, and modify the DetailM to 0
            $result_lines .= "hares -override $res LevelTwoMonitorFreq\n";
            if ( $attr_ref ) {
                $attr_value = $attr_ref->{MODIFY}->{ATTRVAL}->[0];
                $result_lines .= "hares -modify $res LevelTwoMonitorFreq $attr_value\n";
...
sub maincf_result_upgrade_NFSRestart {
    ...
    my (@share_reslist,$share_res,$share_res_ref);
...
sub freeze_groups {
    my ($syslist,$grp,@grps,$line,$prod,$sys,$rtn,@frozen_grps);
    $prod = shift;
    $syslist=CPIC::get('systems');
    $sys = $$syslist[0];
...
sub unfreeze_groups {
    my ($syslist,$grp,@grps,$line,$prod,$rtn,$sys);
    $prod = shift;
    $syslist=CPIC::get('systems');
    $sys = $$syslist[0];
...
sub translate_file_cmd2cf_sys {
    ...
    my $ret= $sys->cmd("$cmd_hacf -verify $path" );
    return 0 if($ret);
    return 1;
}
...
sub secure_startup {
    my ($prod)=@_;
    my ($cfg,$syslist,$msg,$nr,$pids,$proc,$rc,$status,$sys,$localsys,$vcspkg,$tmpdir);
    ...
    for my $sys (@$syslist) {
        $sys->cmd('/opt/VRTSvcs/bin/CmdServer -stop');
    }
...
sub eat_configure {
    my ($prod)=@_;
    my ($syslist,$cfg,@processes,$domain_name,$sys);
    my ($domain);
...
sub eat_initiate {
    ...
    if ($conf_tag) {
        for my $sys (@$syslist) {
            $sys->cmd("_cmd_mkdir -p $prod->{eat_tempdir}");
            $sys->cmd("_cmd_rm -rf $prod->{eat_data_backup}");
...
sub eat_stop_sys {
    my ($prod,$sys)=@_;
    my ($eat_sh,$eat_exe,$pids,$msg,$port,$eat_exe_orig,$eat_sh_orig);
    ...
    if (EDRu::is_port_connectable($sys->{sys},$port)) {
        $msg=Msg::new("Port $port on $sys->{sys} is being used. Secure cluster could not be configured.");
        $msg->die;
    }
    return;
}
...
sub eat_create_credential_sys {
    ...
    $sys->cmd("$eat_env $vssat setuptrust -b localhost:$prod->{eat_setup}{IPPort} -s high -r $root_hash");
    if (EDR::cmdexit()) {
        $msg=Msg::new("Failed to setuptrust for $pro on system $sys->{sys}");
        $msg->die;
    }
    $sys->cmd("$eat_env $vssat authenticate -d vx:$prod->{eat_domain_name} -p $pro_prpl -s password -b localhost:$prod->{eat_setup}{IPPort}");
    if (EDR::cmdexit()) {
...
sub eat_enable {
    my ($prod)=@_;
    my ($syslist,$sys,$msg,$nsys,$mode);

    $syslist=CPIC::get("systems");
    $sys=$$syslist[0];

    unless($prod->eat_configure_precheck){
        $msg=Msg::new("Failed to pass the precheck for secure cluster configuration.");
        $msg->die;
    }
    ...
    $msg=Msg::new("Configuring a secure cluster for $prod->{abbr}");
    # do not re-install and re-create credentials and so on if they already exist
    if ($prod->eat_check_secure_clus()) {
    ...
    if (($mode==1) && ($nsys==$#{$syslist}+1)) {
        $msg=Msg::new("Succeeded in enabling secure mode\n");
        $msg->print;
    } else {
        $msg=Msg::new("Failed in enabling secure mode\n");
        $msg->print;
    }
    return;
}
...
sub eat_disable {
    ...
    if (($nsys==scalar(@$syslist)) && ($mode==0)) {
        $msg=Msg::new("Succeeded in disabling secure mode\n");
        $msg->print;
    } else {
        $msg=Msg::new("Failed in disabling secure mode\n");
        $msg->print;
    }
    return;
}
...
sub eat_securityonenode_first {
    ...
    $msg=Msg::new("Done");
    $msg->right;
    $msg=Msg::new("\nSecurity configuration files have been saved at $prod->{eat_data_backup}. Copy them to other nodes of the cluster.\n");
    $msg->print;
    return 1;
}
...
sub eat_upgrade_configure {
    ...
    for my $sys (@$syslist) {
        $sys->cmd("_cmd_yes y | $eat_env $eat_vssat setuptrust -b localhost:2821 -s high  2>/dev/null");
        if (EDR::cmdexit()) {
...
sub eat_lu_configure_complete_sys {
    ...
    for my $filename (@eat_files) {
        ...
        # modify the /altroot/opt/VRTS to /opt/VRTS, TODO /altroot/var/VRTS
        $sys->cmd("_cmd_sed 's|$prod->{eat_vcs_bin}|$prod->{eat_vcs_bin_orig}|g' $backfile > $filename");
    }
    return 1;
}
...
sub securityonenode_menu {
    my ($prod)=@_;
    my ($edr,$cfg,$sys,$cpic);
    my ($msg,$one,$two,$rtn,$menuopt,$menu,$help,$done,$ayn);
    ...
    $msg=Msg::new("The securityonenode option is used to configure secure cluster node by node. Manual operations need to be performed during the configuration.");
    $msg->add_summary;
    ...
    $help=Msg::new("The securityonenode option is used to configure a secure cluster node by node. Manual operations also need to be performed during the configuration.\n\n1. Run installvcs -securityonenode and choose 1 on the first node of cluster. Installer will perform configuration and save the security configuration files under $prod->{eat_data_backup}\n\n2. Copy the security configuration files from the first node to other nodes.\n\n3. Run installvcs -securityonenode and choose 2 on other nodes of cluster one by one. Installer will perform security configuration on the node using the security configuration files.\n");
...
sub securitytrust {
    my ($prod)=@_;
    my ($cpic,$edr,$localsys,$sys0,$sys,$msg,$outerRB,$port,$ayn,$ping);
        ...
        if ($ayn eq 'Y') {
            ...
            for my $sys (@{CPIC::get('systems')}) {
                ...
                if (EDR::cmdexit()){
                    ...
                    $msg=Msg::new("Failed to setup trust with broker $outerRB on this cluster");
                    $msg->print();
                    last;
                } else {
                    $msg->display_status();
...
sub security_menu {
    my ($prod)=@_;
    my ($cfg,$cpic,$edr,$msg,$ayn,$sys0,$web,$info);

    $web = Obj::web();

    Cfg::unset_opt('install');
    Cfg::set_opt('security',1);
    $cpic=Obj::cpic();
    $edr=Obj::edr();
    $cfg=Obj::cfg();
    $edr->{savelog} = 1;
    ...
    $msg=Msg::new("The -security option is used to convert a running VCS cluster between secure and non-secure modes");
    $msg->add_summary;
    ...
    $prod->check_timesync_sys($sys0, $cpic->{systems}, 5);
    if($sys0->{warnings}) {
        for my $warningmsg (@{$sys0->{warnings}}) {
            Msg::bold("$warningmsg\n");
        }
        $msg=Msg::new("The secure cluster may not work after convertion if the cluster nodes have time offset.\n");
        $msg->bold();
        undef($sys0->{warnings});
    }
    ...
    if ($prod->is_secure_cluster($sys0)) {
        $msg=Msg::new("Do you want to disable secure mode in this $prod->{abbr} cluster?");
        $ayn=$msg->ayny('','',$info);
        Msg::n();
        if ($ayn eq 'Y') {
            Msg::title();
            $msg=Msg::new("Restarting $prod->{abbr} with secure mode disabled:\n");
            $msg->bold;
            $msg->{msg} =~ s/\n|\://g;
            $web->web_script_form('showstatus',$msg) if (Obj::webui());
            $prod->eat_disable();
        }
    } else {
        $msg=Msg::new("Do you want to enable secure mode in this $prod->{abbr} cluster?");
        $ayn=$msg->ayny('','',$info);
        Msg::n();
        if ($ayn eq 'Y') {
            Msg::title();
            $msg=Msg::new("Restarting $prod->{abbr} with secure mode enabled:\n");
            $msg->bold;
            $msg->{msg} =~ s/\n|\://g;
            $web->web_script_form('showstatus',$msg) if (Obj::webui());
            $prod->{eat_enable} = 1;
    ...
    while(1){
        if ($failed) {
            ...
            if (Obj::webui()) {
                ...
                do {
                    $msg=Msg::new("Enter the name of one system in another $prod->{abbr} cluster that you would like to configure I/O fencing:");
                    $sysname=$msg->ask;
        ...
        $web->web_script_form('precheck',$msg) if (Obj::webui());
...
sub check_prod_install_sys {
    ...
    ($cprodi)? $cprod=$cpic->prod($cprodi):$cprod=$cpic->prod("VCS60");
...
sub web_check_link_speed {
    ...
    if ($diff == 2) {
        $desc = Msg::new("$script can not detect media speed for the selected private NICs properly. Consult your Operating System manual for information on how to set the Media Speed.\n")->{msg};
        $desc .= $web->{nicspeedinfo};
    }
                ...
                if($line =~/^link/){
                    ...
                    if ($nic =~ /sit\d+/m || $nic =~ /vsw\d+/m){ # skip sit# on Linux, vsw# on Solaris
                        ...
                        for my $sap (52001..$last_sap) {
                            ...
                            $sysi->cmd($cmd);
                            if (EDR::cmdexit() != 0) {
                                push(@sys_nics,"$sysi->{sys}#$nic#$sys->{sys}#$sap_mac{$sap}{nic}");
                            }
                        }
                    }
                }
...
sub postcheck_lltconfig_sys {
    ...
    if ($rtn!~/LLT is running/m || $rtn1=~/jeopardy/) {
        ...
        unless ($sys->exists('/etc/llthosts') && $sys->exists('/etc/llttab')){
            $msg=Msg::new("\n\tllt is not configured on $sys->{sys}. Please check LLT configuraton");
            push(@m,$msg);
        }
        if($sys->exists('/etc/llttab') && $sys->exists('/etc/llthosts')){
            ...
            if($rtn2==0){
                ...
                if($wrongnics){
                        $msg=Msg::new("\n\t The NICS$wrongnics can not be found on $sys->{sys}. It is recommended to check the NICs configuration");
                        push(@m,$msg);
                }
                if($list){
                        $msg=Msg::new("\n\t The NICS$list is a part of bonding/agreegation NIC on $sys->{sys}. It can not used for heartbeat link.");
                        push(@m,$msg);
                }
    ...
    $downlinks=$prod->postcheck_lltstat_sys($sys);
    if($downlinks && $sys->system1){
        my $downnics=$prod->postcheck_nics();
        foreach my $line(@{$downnics}){
            my ($sys1,$nic1,$sys2,$nic2)=split(/#/,$line);
           $msg=Msg::new("\n\tThe NIC $nic1 on $sys1 can not communicated with the NIC $nic2 on $sys2. It is recommended to check the configuration of the NIC");
            push(@m,$msg);
        }
    }
    $sys->push_error(@m) if(scalar(@m)>0);
    return 0;
}

sub postcheck_lltvers_sys {
    my ($prod,$sys)=@_;
    return $prod->postcheck_pkg_vers_sys($sys,"VRTSllt60");
}

sub postcheck_pkg_vers_sys {
    my ($prod,$sys,$pkgname)=@_;
    my ($systems,$vers,$pkg,$isru,$msg);
    $systems=CPIC::get("systems");
    $pkg = $sys->pkg($pkgname);
    $vers=$pkg->version_sys($sys);
    $isru=$sys->cmd("_cmd_grep 'V' /etc/gabtab 2>/dev/null");
    if(EDRu::compvers($vers,$pkg->{vers}) != 0 && !$isru){
        $pkgname=~s/\d+//g;
        $msg=Msg::new("The package version of $pkgname on $sys->{sys} is not consistent with the release. Verify the $pkgname package is installed correctly");
        $sys->push_error($msg);
        return 0;
    }
    return 1;
...
sub postcheck_lltenable_sys {
    ...
    if ($rtn!~/LLT is running/m) {
        if($conf !~/\s*LLT_START\s*=\s*1/){
            $report=Msg::new("llt is configured to NOT start on $sys->{sys}. Please check in $file if it is configured correctly");
...
sub postcheck_gab_sys {
    ...
    if($conf !~/\s*GAB_START\s*=\s*1/){
       $msg=Msg::new("\n\tgab is configured to NOT start on $sys->{sys}. Please check in $file if it is configured correctly");
       push(@m,$msg);
    }
...
sub postcheck_gabvers_sys {
    my ($prod,$sys)=@_;
    return $prod->postcheck_pkg_vers_sys($sys,"VRTSgab60");
}

sub postcheck_vxfenvers_sys {
    my ($prod,$sys)=@_;
    return $prod->postcheck_pkg_vers_sys($sys,"VRTSvxfen60");
}

sub postcheck_vcsvers_sys {
    my ($prod,$sys)=@_;
    return $prod->postcheck_pkg_vers_sys($sys,"VRTSvcs60");
}


sub postcheck_vxfen_sys {
    my ($prod,$sys) = @_;
    my ($msg,$line,$rtn,$proc,$conf,$file,$report,$output,$diskgroup,@m);
    $proc=$sys->proc("vxfen60");
    $file=$proc->{initconf};
    ...
    for my $line (split(/\n/,$rtn)) {
        if ($line=~/ERROR/m) {
            ...
            $output=$sys->cmd("_cmd_grep -v '#' /etc/vxfenmode 2>/dev/null");
            if($output =~/vxfen_mode=scsi3|vxfen_mode=sybase/){
                    $diskgroup=$sys->cmd("_cmd_cat /etc/vxfendg 2>/dev/null");
                    if($diskgroup){
                        my $rtn1=$sys->cmd("_cmd_vxdisk -o alldgs list 2>/dev/null | _cmd_grep $diskgroup");
                        unless($rtn1){
            ...
            } elsif ($output !~/vxfen_mode=scsi3/ && $output !~/vxfen_mode=sybase/ && $output !~/vxfen_mode=disabled/ && $output !~/vxfen_mode=customized/) {
            ...
            $conf=$sys->catfile($file);
            if($conf !~/\s*VXFEN_START\s*=\s*1/){
                $msg=Msg::new("\n\tvxfen is configured to NOT start on $sys->{sys}. Please check in $file if it is configured correctly");
                push(@m,$msg);
            }
           $sys->push_error(@m);
            return 0;
...
sub postcheck_had_sys {
    ...
    if($rtn ne $sys->{hostname}){
        $msg=Msg::new("The system name in /etc/VRTSvcs/conf/sysname is not consistent with hostname on $sys->{sys}. It is recommended to check the configuration file.");
        push(@m,$msg);
    }
    unless($prod->check_uuid){
        $msg=Msg::new("\nUUID is not configured on the cluster correctly. Please configure UUID again on the cluster.");
        push(@m,$msg);
        unless($sys->exists($prod->{uuidconfig})){
            $msg=Msg::new("\n\tuuidconfig.pl can not be found on the cluster. Please check if the cluster is installed correctly.\n");
            push(@m,$msg);
        }
    }
    push(@m,$msg);
...
sub postcheck_hastatus_sys {
    my ($prod,$sys) = @_;
    my ($msg,@offline_groups,$state,$group,$onlines,$warning,$line,$rtn,$parallel,@failover_groups,@systems,$systems,$groups,$report,$typesfile,@m);
    ...
    for my $line (split(/\n/,$rtn)) {
        ...
        if ($line !~/RUNNING/m) {
            ...
            if (EDR::cmdexit()!=0||$rtn=~/WARNING/) {
                ...
                $typesfile=$1 if($rtn=~/\/(.*):/);
                if($typesfile){
                    $msg=Msg::new("\n\tVCS configuration is incorrect on $sys->{sys}. It is recommended to check VCS configuration /etc/VRTSvcs/conf/config/$typesfile");
                    push(@m,$msg);
                } else {
                    $msg=Msg::new("\n\tVCS configuration is incorrect on $sys->{sys}. It is recommended to check VCS configuration /etc/VRTSvcs/conf/config/main.cf");
                    push(@m,$msg);
                }
            }
            $rtn=$sys->catfile("/etc/VRTSvcs/conf/sysname");
...
sub register_postchecks_per_system {
    ...
    $sequence_id=321;
    $name='lltpackage';
    $desc=Msg::new("installed VRTSllt");
    $handler=\&postcheck_lltvers_sys;
    $prod->register_postcheck_item_per_system($sequence_id,"$name",$desc,$handler);
    ...
    $sequence_id=331;
    $name='gabpackage';
    $desc=Msg::new("installed VRTSgab");
    $handler=\&postcheck_gabvers_sys;
    $prod->register_postcheck_item_per_system($sequence_id,"$name",$desc,$handler);
    ...
    $sequence_id=336;
    $name='vxfenpackage';
    $desc=Msg::new("installed VRTSvxfen");
    $handler=\&postcheck_vxfenvers_sys;
    $prod->register_postcheck_item_per_system($sequence_id,"$name",$desc,$handler);
    ...
    $sequence_id=341;
    $name='vcspackage';
    $desc=Msg::new("installed VRTSvcs");
    $handler=\&postcheck_vcsvers_sys;
    $prod->register_postcheck_item_per_system($sequence_id,"$name",$desc,$handler);
...
sub register_postchecks_per_cluster {
    ...
    #$name="hastatus";
    #$desc=Msg::new("HAD status");
    #$handler=\&postcheck_hastatus;
    #$prod->tsub("register_postcheck_item_per_cluster",$sequence_id,"$name",$desc,$handler);
...
sub addnode_messages {
    ...
    $msg->print;
    $msg = Msg::new("\nRefer to the $prod->{abbr} Installation Guide for more details\n\n");
    $msg->print;
    Msg::prtc();
    return;
}
...
sub addnode_get_cluster {
    my ($ayn,$clustername,$clusterid,$conf,$done,$had,$llttab,$maincf,$msg,$node,$sys,$sysi,@systems,$syslist);
    my $prod = shift;
    my $edr = Obj::edr();
    my $cfg = Obj::cfg();
    my $cprod=CPIC::get('prod');
        ...
        if (Cfg::opt('responsefile')) {
            ...
            if ($conf) {
                ...
                if ($conf->{onenode}) {
                    # onenode cluster
                    Msg::n();
                    $msg = Msg::new("Cluster $clustername is a single node cluster, and llt and gab is not configured on the node");
                    $msg->print();
                    $msg = Msg::new("To add a new node to single-node cluster, $clustername, a unique cluster ID is required");
                    $msg->print();

                    $clusterid = $prod->ask_clusterid($clusterid);
                    next if (EDR::getmsgkey($clusterid,'back'));
                } else {
                    $clusterid = $conf->{clusterid};
                }
            } else {
...
sub addnode_get_newnode {
    ...
    while (1) {
        ...
        $prod->check_timesync_sys($sys, \@allsystems, 5);
        if($sys->{warnings}) {
            Msg::n();
            for my $warningmsg (@{$sys->{warnings}}) {
                Msg::bold("$warningmsg\n");
            }
            undef($sys->{warnings});
        }
...
sub addnode_compare_systems {
    my ($errmsg,$errmsgobj,$minpkgs,@misspkgs,$n,$msg,$pkgi,$pkg,$prodi,$sysi,$sys);
    ...
    my ($check_gco, $check_vr, $check_vfr);
    my ($check_cpspkgver,$vxfenpkg,$cpspkg);
    my $vcs = shift;
    my $cfg = Obj::cfg();
    my $cpic = Obj::cpic();
    my $rel = $cpic->rel;
    ...
    my $prod = $cpic->prod();
    my $padv = $cpic->{padv};
    my @prodlist = qw(SFRAC60 SFSYBASECE60 SVS60 SFCFSHA60 SFHA60 VCS60);
    ...
    if ($clus_prod->{prod} ne $prod->{prod}) {
        ...
        $msg = Msg::new("$clus_prod->{abbr} is installed on cluster $cfg->{vcs_clustername}, please use install$upi -addnode to add new node(s)");
        $msg->die();
    }

    # check cluster protocol version if cvm is configured
    $check_protocolversion = 0;
    if ($installed_prodi ne 'SFHA60' && $installed_prodi ne 'VCS60') {
    ...
    $vxfenpkg = $vcs->pkg('VRTSvxfen60');
    $cpspkg = $sys->pkg('VRTScps60');
    if ($vxfenpkg && $cpspkg && $vxfenpkg->vxfen_mode_sys($sys) eq 'cps') {
    ...
    for my $sysi (@{$cfg->{newnodes}}) {
        ...
        if (!$node_lic) {
            Msg::right_failed();
            $errmsgobj = Msg::new("$node_prod->{abbr} is installed on $sysi, but does not have valid license");
            $errmsg .= "\n".$errmsgobj->{msg}."\n";
            $failed = 1;
            next;
        }
        ...
        if (EDRu::compvers($clus_ver, $node_ver) == 0) {
            ...
            if(scalar(@misspkgs) > 0) {
                ...
                $failed = 1;
                @misspkgs = ();
            } elsif($check_cpspkgver && !EDRu::inarr('VRTScps60', @{$minpkgs}) && $cpspkg && $cpspkg->version_sys($sys) ne $check_cpspkgver ) {
                Msg::right_failed();
                Msg::n();
        ...
        if ( $check_gco ) {
            ...
            $msg->left;
            if (!$rel->feature_licensed_sys($sys, 'Global Cluster Option')) {
                $errmsgobj = Msg::new("Global Cluster Option (GCO) is configured in cluster $cfg->{vcs_clustername} but it is not licensed on $sysi");
                $errmsg .= "\n".$errmsgobj->{msg}."\n";
                $failed = 1;
                Msg::right_failed();
            } else {
        ...
        if ( $check_vr && $installed_prodi ne 'VCS60') {
            $msg = Msg::new("Checking license for Veritas Volume Replicator on $sysi");
            $msg->left;
            if (!$rel->feature_licensed_sys($sys, 'VVR')) {
                $errmsgobj = Msg::new("Veritas Volume Replicator is configured in cluster $cfg->{vcs_clustername} but it is not licensed on $sysi");
                $errmsg .= "\n".$errmsgobj->{msg}."\n";
                $failed = 1;
                Msg::right_failed();
            } else {
        ...
        if ( $check_vfr && $installed_prodi ne 'VCS60') {
            $msg = Msg::new("Checking license for Veritas File Replicator on $sysi");
            $msg->left;
            if (!$rel->feature_licensed_sys($sys, 'VFR')) {
                $errmsgobj = Msg::new("Veritas File Replicator is configured in cluster $cfg->{vcs_clustername} but it is not licensed on $sysi");
                $errmsg .= "\n".$errmsgobj->{msg}."\n";
                $failed = 1;
                Msg::right_failed();
            } else {
...
sub addnode_configure_heartbeat {
    my ($address,$ayn,$common_speed,$clus_conf,$diff,$key,$lmsg,$msg,$n,$netmask,$nic,$onenode,$port,$rhbn);
    my ($sys,$sysi);
    my $prod = shift;
    my $edr = Obj::edr();
    my $cpic = Obj::cpic();
    ...
    while (1) {
        ...
        if (!$cfg->{autocfgllt}) {
            ...
            if ($diff == 2) {
                Msg::n();
                $msg = Msg::new("$edr->{script} can not detect media speed for the selected private NICs properly. Consult your Operating System manual for information on how to set the Media Speed.\n");
                $msg->warning();
            }
            if ($diff) {
                $msg = Msg::new("Do you want to continue with current heartbeat configuration?");
        ...
        for my $sys (@{CPIC::get('systems')}) {
            ...
            for my $n (1..$prod->num_hbnics($rhbn)) {
                $key="lltlink$n";
                next unless ($rhbn->{$key}{$sysi});
                $lmsg.="\n\t\tlink$n=$rhbn->{$key}{$sysi}";
                if ($cfg->{lltoverudp}) {
                    $address=$rhbn->{"udplink$n".'_address'}{$sysi};
                    $netmask=$rhbn->{"udplink$n".'_netmask'}{$sysi};
                    $port=$rhbn->{"udplink$n".'_port'}{$sysi};
                    if (EDRu::ip_is_ipv6($address)) {
                        $lmsg.=" over UDP6\n\t\t  ip $address prefix $netmask port $port";
                    } else {
                        $lmsg.=" over UDP\n\t\t  ip $address netmask $netmask port $port";
            ...
            $msg=Msg::new("\tPrivate Heartbeat NICs for $sysi: $lmsg");
            $msg->print;
            if ($prod->num_lopri_hbnics($rhbn) > 0) {
                $lmsg = '';
                for my $n (1..$prod->num_lopri_hbnics($rhbn)) {
                    $key="lltlinklowpri$n";
                    $lmsg.="\n\t\tlink-lowpri$n=$rhbn->{$key}{$sysi}";
                    if ($cfg->{lltoverudp}) {
                        $address=$rhbn->{"udplinklowpri$n".'_address'}{$sysi};
                        $netmask=$rhbn->{"udplinklowpri$n".'_netmask'}{$sysi};
                        $port=$rhbn->{"udplinklowpri$n".'_port'}{$sysi};
                        if (EDRu::ip_is_ipv6($address)) {
                            $lmsg.=" over UDP6\n\t\t  ip $address prefix $netmask port $port";
                        } else {
                            $lmsg.=" over UDP\n\t\t  ip $address netmask $netmask port $port";
                ...
                if ($prod->num_lopri_hbnics($rhbn) > 1) {
                    $msg=Msg::new("\tLow-Priority Heartbeat NICs for $sysi: $lmsg");
                } else {
                    $msg=Msg::new("\tLow-Priority Heartbeat NIC for $sysi: $lmsg");
                }
                $msg->print;
            }
        }
...
sub web_addnode_configure_heartbeat {
    ...
    while (!$done) {
        ...
        if ($cfg->{autocfgllt}) {
            ...
            for my $sys (@{CPIC::get('systems')}) {
                my $nics = $sys->padv->systemnics_sys($sys);
...
sub addnode_start_cluster {
    my ($failures,$firstnode,$had,$msg,$n,$onenode,$pids,$proci,$proc,$sys,$sysi,$system,@syslist);
    my $prod = shift;
    my $edr = Obj::edr();
    my $cpic = Obj::cpic();
    my $cfg = Obj::cfg();
    ...
    unless ($#$failures < 0) {
        ...
        $msg->bold();
        $msg = Msg::new("Please check your heartbeat link configurations");
        $msg->die();
    }
    ...
    if ($gabports !~ /^\s*Port\s+a\s+/mx) {
        ...
        if (Obj::webui()){
            $msg = Msg::new("GAB control port seed is not enabled after GAB is started on all systems. Please check your LLT and GAB configurations");
            $msg->die();
        }
        else {
            $msg = Msg::new("GAB control port seed is not enabled after GAB is started on all systems");
            $msg->bold();
            $msg = Msg::new("Please check your LLT and GAB configurations");
            $msg->die();
        }
    }
    ...
    if ($onenode) {
        ...
        $sysi = ${$cfg->{clustersystems}}[0];
        $sys = Obj::sys($sysi);
        for my $proci (qw(vxfen60 had60)) {
            # do not start vxfen for VCS/SFHA
            next if (($proci eq 'vxfen60') && ($cpic->{prod} =~ /(VCS|SFHA)/m));
            $proc = $sys->proc($proci);
            $msg=Msg::new("Starting $proc->{proc} on $sysi");
            if ($cpic->proc_start_sys($sys, $proc)) {
                CPIC::proc_start_passed_sys($sys, $proc);
...
sub addnode_configure_fencing {
    ...
    my($ret,$security,$cps_aref,$ports_href,$conf,$uuid,$node_cnt);
    my ($vxfen_mechanism,$vxfendg,$service_enabled,$vxfen);
    my $prod = shift;
    my $cpic = Obj::cpic();
    my $cfg = Obj::cfg();
    my $vxfen_pkg = $prod->pkg('VRTSvxfen60');

    $firstnode = Obj::sys(${$cfg->{clustersystems}}[0]);
    $service_enabled = 1;
    $vxfen = $firstnode->proc('vxfen60');
    ...
    if ($firstnode->exists($prod->{vxfenmode})) {
        ...
        if ($conf =~ /vxfen_mode\s*=\s*(\w*)/mx) {
            ...
            $msg = Msg::new("Could not get vxfen mode from first node, $firstnode, of the cluster, $cfg->{vcs_clustername}. Fencing will not be started on the newly added node");
            $msg->print();
            return;
        }
    } else {
    ...
    if ($vxfenmode eq 'customized') {
        if ($conf =~ /vxfen_mechanism\s*=\s*cps\s*/mx) {
            ...
            ($ret, $security, $cps_aref, $ports_href) = $vxfen_pkg->addnode_get_cp_client_conf($firstnode);
            if ($ret) {
                $vxfenmode = 'disabled';
                $msg = Msg::new("Failed to read Coordination Point Client configuration from $prod->{vxfenmode} on first node, $firstnode, of cluster, $cfg->{vcs_clustername}. Fencing will be started in disabled mode on the newly added node.");
                $msg->print();
            } else {
                my $cnt = 1;
                my %ports = %$ports_href;
                Msg::log('Existing Coordination Point client side configuration');
                Msg::log("security=$security");
                for my $sys (@$cps_aref) {
    ...
    for my $sysi (@{$cfg->{newnodes}}) {
        ...
        if ($vxfenmode eq 'scsi3' || (($vxfenmode eq 'customized') && ($vxfen_mechanism eq 'cps'))) {
            if ($vxfenmode ne 'scsi3') {
                ...
                $ret = $vxfen_pkg->addnode_configure_cpc_sys($sys, $cfg->{vcs_clustername}, $node_cnt,
                                                                 $uuid, $security, $cps_aref, $ports_href);
                if ($ret) {
...
sub addnode_update_CSSG {
    my ($csgnic,$firstnode,$gconic,$global_csgnic,$global_gconic,$msg,$n,$rtn,$sysi,$system);
    my $prod = shift;
    my $cfg = Obj::cfg();
    ...
    for my $sysi (@{$cfg->{newnodes}}) {
        $n++;
        $system = transform_system_name($sysi);
        $firstnode->cmd("$prod->{bindir}/hagrp -modify ClusterService SystemList -add $system $n");
...
sub config_clusterid {
    ...
    while (1) {
        $defaultid=($cid && !$ret) ? $cid : int(rand(65535));
...
sub set_onenode_cluster_sys {
    my($prod,$sys,$onenode) = @_;
    return $prod->set_vcs_onenode_sys($sys,$onenode);
}

package Prod::VCS60::AIX;
@Prod::VCS60::AIX::ISA = qw(Prod::VCS60::Common);

sub init_plat {
    my $prod=shift;
    $prod->{allpkgs}=[ qw(VRTSveki60 VRTSllt60 VRTSgab60 VRTSvxfen60 VRTSamf60 VRTSvcs60 VRTScps60 VRTSvcsag60 VRTSvcsea60 VRTSsfmh41 VRTSvbs60) ];
    $prod->{minpkgs}=[ qw(VRTSveki60 VRTSllt60 VRTSgab60 VRTSvxfen60 VRTSamf60 VRTSvcs60 VRTSvcsag60) ];
    $prod->{recpkgs}=[ qw(VRTSveki60 VRTSllt60 VRTSgab60 VRTSvxfen60 VRTSamf60 VRTSvcs60 VRTSvcsag60 VRTSvcsea60 VRTSsfmh41 VRTSvbs60) ];
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTScscm.rte VRTSweb.rte VRTScscw.rte VRTScssim.rte VRTSvbs60 VRTSsfmh41
        VRTSvcsea60 VRTScutil VRTScutil.rte VRTSjre15.rte VRTSvcs.doc VRTSvcs.man
        VRTSvcs.msg.en_US VRTSvcsag60 VRTSvcsag.rte VRTScps60
        VRTSvcs60 VRTSvcs.rte VRTSamf60 VRTSvxfen60 VRTSvxfen.rte VRTSgab60
        VRTSgab.rte VRTSllt60 VRTSllt.rte VRTSveki60 VRTScpi.rte
        VRTSvsvc SYMClma VRTSspt60 VRTSat50 VRTSat.server VRTSat.client
        VRTSsmf VRTSpbx VRTSicsco VRTSjre.rte VRTSsfcpi60 VRTSperl512 VRTSperl.rte
        VRTSvlic32
    ) ];
...
package Prod::VCS60::HPUX;
@Prod::VCS60::HPUX::ISA = qw(Prod::VCS60::Common);

sub init_plat {
    my $prod=shift;
    $prod->{allpkgs}=[ qw(VRTSllt60 VRTSgab60 VRTSvxfen60 VRTSamf60 VRTSvcs60 VRTScps60 VRTSvcsag60 VRTSvcsea60 VRTSsfmh41 VRTSvbs60) ];
    $prod->{minpkgs}=[ qw(VRTSllt60 VRTSgab60 VRTSvxfen60 VRTSamf60 VRTSvcs60 VRTSvcsag60) ];
    $prod->{recpkgs}=[ qw(VRTSllt60 VRTSgab60 VRTSvxfen60 VRTSamf60 VRTSvcs60 VRTSvcsag60 VRTSvcsea60 VRTSsfmh41 VRTSvbs60) ];
    $prod->{upgradevers}=[qw(3.5 4.1 5.0 5.1)];
    $prod->{zru_releases}=[qw(3.5 4.1 5.0 5.1)];
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTScmcs VRTScscm VRTSweb VRTScscw VRTScssim VRTSvbs60 VRTSsfmh41
        VRTSvcsea60 VRTScutil VRTSjre15 VRTSvcsdc VRTSvcsmn
        VRTSvcsmg VRTSvcsdr60 VRTSvcsag60 VRTScps60 VRTSvcs60
        VRTSamf60 VRTSvxfen60 VRTSgab60 VRTSllt60 VRTScpi VRTSvsvc SYMClma
        VRTSspt60 VRTSat50 VRTSsmf VRTSpbx VRTSicsco VRTSjre
        VRTSsfcpi60 VRTSperl512 VRTSvlic32 VRTSwl
    ) ];
...
sub padv_maincf_upgrade_sys{
    ...
    Prod::VCS60::Common::dynupgrade_modify_attr_value($treeref_result,'ProcessOnOnly','PathName','/opt/VRTSat/bin/pa20_64/vxatd','/opt/VRTSat/bin/vxatd');
    return $treeref_result;
}

package Prod::VCS60::Linux;
@Prod::VCS60::Linux::ISA = qw(Prod::VCS60::Common);

sub init_plat {
    my $prod=shift;
    $prod->{allpkgs}=[ qw(VRTSllt60 VRTSgab60 VRTSvxfen60 VRTSamf60 VRTSvcs60 VRTScps60 VRTSvcsag60 VRTSvcsdr60 VRTSvcsea60 VRTSsfmh41 VRTSvbs60) ];
    $prod->{minpkgs}=[ qw(VRTSllt60 VRTSgab60 VRTSvxfen60 VRTSamf60 VRTSvcs60 VRTSvcsag60) ];
    $prod->{recpkgs}=[ qw(VRTSllt60 VRTSgab60 VRTSvxfen60 VRTSamf60 VRTSvcs60 VRTSvcsag60 VRTSvcsdr60 VRTSvcsea60 VRTSsfmh41 VRTSvbs60) ];
    $prod->{upgradevers}=[qw(5.0.30 5.1)];
    $prod->{zru_releases}=[qw(4.1.40 5.0 5.1)];
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSvcsmg VRTSvcsdr60 VRTSvcsag60 VRTScps60 VRTSvcs60
        VRTSamf60 VRTSvxfen60 VRTSgab60 VRTSllt60 VRTScpi VRTSvsvc SYMClma
        VRTSspt60 VRTSat50 VRTSatClient50 VRTSsmf VRTSpbx VRTSicsco
        VRTSjre VRTSsfcpi60 VRTSperl512 VRTSvlic32
    ) ];
...
sub padv_update_netmask {
    ...
    # For IP resources
    @reslist_ip = Prod::VCS60::Common::dynupgrade_get_resname_list_from_typename($treeref, 'IP');
    for my $res (@reslist_ip) {
        $noderef = Prod::VCS60::Common::dynupgrade_get_node_from_tree($treeref, 'RES', $res);
        $attrref_netmask = Prod::VCS60::Common::dynupgrade_get_node_from_tree($noderef, 'ATTR', 'NetMask');
        if (!$attrref_netmask) {
            $netmask = '255.0.0.0';
            $attrref_options = Prod::VCS60::Common::dynupgrade_get_node_from_tree($noderef, 'ATTR', 'Options');
            if ($attrref_options) {
                $attrref_ipoptions = Prod::VCS60::Common::dynupgrade_get_node_from_tree($noderef, 'ATTR', 'IPOptions');
                $attrref_iprouteoptions = Prod::VCS60::Common::dynupgrade_get_node_from_tree($noderef, 'ATTR', 'IPRouteOptions');
                if (!$attrref_ipoptions && ! $attrref_iprouteoptions) {
                    $addressref = Prod::VCS60::Common::dynupgrade_get_node_from_tree($noderef, 'ATTR', 'Address');
                    $address = $addressref->{MODIFY}->{ATTRVAL}->[0];
                    $address =~ s/.*"(.*)"/$1/m;
                    $netmask = $prod->get_netmask_by_address($address);
                }
    ...
    @reslist_ipmultinic = Prod::VCS60::Common::dynupgrade_get_resname_list_from_typename($treeref, 'IPMultiNIC');
    ...
    @reslist_multinica = Prod::VCS60::Common::dynupgrade_get_resname_list_from_typename($treeref, 'MultiNICA');
    for my $res (@reslist_multinica) {
        ...
        if (!$attrref_netmask) {
            ...
            $attrref_options = Prod::VCS60::Common::dynupgrade_get_node_from_tree($noderef, 'ATTR', 'Options');
            if ($attrref_options) {
                $attrref_linkoptions = Prod::VCS60::Common::dynupgrade_get_node_from_tree($noderef, 'ATTR', 'LinkOptions');
                $attrref_ipv4addroptions = Prod::VCS60::Common::dynupgrade_get_node_from_tree($noderef, 'ATTR', 'IPv4AddrOptions');
                if (!$attrref_linkoptions && ! $attrref_ipv4addroptions) {
                    $addressref = Prod::VCS60::Common::dynupgrade_get_node_from_tree($noderef, 'ATTR', 'Device');
                    $address = $addressref->{MODIFY}->{ATTRVAL}->[0];
                    $address =~ s/.*"(.*)"/$1/m;
                    $netmask = $prod->get_netmask_by_address($address);
                }
...
package Prod::VCS60::SunOS;
@Prod::VCS60::SunOS::ISA = qw(Prod::VCS60::Common);
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSvbs60 VRTSsfmh41 VRTSvcsea60 VRTScutil VRTSjre15 VRTSvcsdc VRTSvcsmn
        VRTSvcsmg VRTSvcsag60 VRTScps60 VRTSvcs60 VRTSamf60 VRTSvxfen60
        VRTSgab60 VRTSllt60 VRTScpi VRTSvsvc SYMClma VRTSspt60
        VRTSat50 VRTSsmf VRTSpbx VRTSicsco VRTSjre VRTSsfcpi60 VRTSperl512
        VRTSvlic32
    ) ];
    # For UXRT 5.1SP1, VRTSacclib need to be fresh on AIX and Solaris.
    $prod->{obsoleted_but_need_refresh_when_upgrade_pkgs} = [ qw(VRTSacclib52) ];
...
sub set_onenode_cluster_sys {
    ...
    $prod->set_vcs_onenode_sys($sys, $onenode);
    if ($onenode) {
        $sys->cmd("_cmd_cp -rp $rootpath/etc/VRTSvcs/conf/$manifest $rootpath/var/svc/manifest/system/$manifest");
        if ($rootpath) {
            $sys->cmd("echo svcadm disable -s $clustervcs >> $smfprofile_upgrade");
            $sys->cmd("echo svccfg delete $clustervcs >> $smfprofile_upgrade");
            $sys->cmd("echo svccfg import /var/svc/manifest/system/$manifest >> $smfprofile_upgrade");
            ...
            $sys->cmd("_cmd_svccfg import /var/svc/manifest/system/$manifest");
        }
    } else {
        if ($rootpath) {
            $sys->cmd("echo svcadm disable -s $fmri >> $smfprofile_upgrade");
            $sys->cmd("echo svccfg delete $fmri >> $smfprofile_upgrade");
            $sys->cmd("echo svccfg import $orig_manifest >> $smfprofile_upgrade");
            $sys->cmd("echo svcadm enable $clustervcs >> $smfprofile_upgrade");
            $sys->cmd("_cmd_rm -f $rootpath/var/svc/manifest/system/$manifest");
        } else {
            $sys->cmd("_cmd_svcadm disable -s $fmri");
            $sys->cmd("_cmd_svccfg delete $fmri");
            $sys->cmd("_cmd_rm -f /var/svc/manifest/system/$manifest");
            $sys->cmd("_cmd_svccfg import $orig_manifest");
        }
    }
    return 1;
}
...
sub padv_maincf_upgrade_precheck_sys {
    my ($prod,$sys,$treeref) = @_;
    my ($sysi,$grp_container_settings,$rootpath,$msg,$attr,$con_type,$grp_container_name,@grp_syslist,@grps,$vcsvers,$grp_name,$padv,$haconf,@reses,$grp_errors,$rtn,$res_name,$zone_name,$res_type,$container_settings,$vcspkg,$grp_zone_settings,$zone_errors,$container_opts,$grp_zone_name,$grp_container_type);
    ...
    $padv = $sys->{padv};
    $vcspkg=$prod->pkg('VRTSvcs60');
    if (defined $vcspkg) {
        $vcsvers=$vcspkg->version_sys($sys);
    }
    if ($vcsvers && (EDRu::compvers($vcsvers,'5.1') == 2) && $sys->{zone}) {
        Msg::log("Checking the zone setting informations on $sys->{sys}");
        ...
        # get group list
        @grps = Prod::VCS60::Common::dynupgrade_get_grplist($treeref);
        for my $grp_name (@grps) {
            next unless ($grp_name);

            $grp_container_name='';
            $grp_container_type='';
            $grp_container_settings='';
            $grp_errors='';
            ...
            # get resources list
            @reses = Prod::VCS60::Common::dynupgrade_get_res_from_grp($treeref, $grp_name);
            for my $res_name (@reses) {
                next unless ($res_name);

                $res_type = Prod::VCS60::Common::dynupgrade_get_restype_of_resname($treeref,$res_name);
                if ($res_type eq 'Zone') {
                    $zone_name = Prod::VCS60::Common::dynupgrade_get_attrvalue_of_resname($treeref, $res_name, 'ZoneName');
                    next unless ($zone_name);
                    $grp_zone_settings.="    $grp_name   Zone Name $zone_name\n";
                    if ($grp_zone_name && ($grp_zone_name ne $zone_name)) {
                        $zone_errors=1;
                    } else {
                        $grp_zone_name = $zone_name;
                    }
                    next;
                }

                # get resource's attributes
                $attr = Prod::VCS60::Common::dynupgrade_get_attrvalue_of_resname($treeref, $res_name, 'ContainerName');
                $sys->set_value("maincf_res_container_name,$res_name", $attr);
                next unless $attr;
                $grp_container_settings.="    $grp_name   $res_name  ContainerName  $attr\n";
                if ($grp_container_name) {
                    if ($grp_container_name ne $attr) {
                        $grp_errors=1;
                    }
                } else {
                    $grp_container_name = $attr;
                }

                $con_type = Prod::VCS60::Common::dynupgrade_get_attrvalue_of_typename($treeref,$res_type,'ContainerType');
                $sys->set_value("maincf_res_container_type,$res_name", $con_type);
                next unless $con_type;
                $grp_container_settings.="    $grp_name   $res_name  ContainerType  $con_type\n";
                if ($grp_container_type) {
                    if ($grp_container_type ne $con_type) {
                        $grp_errors=1;
                    }
                } else {
                    $grp_container_type = $con_type;
                }
            }

            if (($grp_errors) || ($zone_errors) ){
                if ($grp_errors) {
                ...
                if ($zone_errors) {
                    $msg=Msg::new("In the file '$rootpath$prod->{maincf}' on $sys->{sys}, the group '$grp_name' contains more than 1 Zone :\n$grp_zone_settings");
                    $sys->push_error($msg);
                }
            } elsif ($grp_container_name || $grp_container_type) {
                $grp_container_name||='\"\"';
                $sys->set_value("maincf_grp_container_name,$grp_name", $grp_container_name);
                $sys->set_value("maincf_grp_container_type,$grp_name", $grp_container_type);
            } elsif ($grp_zone_name) {
                $sys->set_value("maincf_grp_container_name,$grp_name", $grp_zone_name);
                $sys->set_value("maincf_grp_container_type,$grp_name", 'Zone');
            }
        }

        if ($container_settings) {
            Msg::log("Container Settings:\n$container_settings\n");
            $sys->set_value('maincf_upgrade,21_container_settings', $container_settings);
        }
    }
...
sub padv_maincf_upgrade_sys{
    my ($prod,$sys,$treeref_result,$treeref_newsys) = @_;
    my ($grp_name,$grp_container_name,$grp_container_type,@grp_syslist,$sysi);
    my (@reses,$res_name,$res_type,$container_opts);
    ...
    for my $grp_name (sort keys %{$sys->{maincf_grp_container_name}}) {
        ...
        next unless Prod::VCS60::Common::dynupgrade_get_node_from_tree($treeref_result,'GRP',$grp_name);
        $grp_container_name = $grp_container_type = '';
        $grp_container_name = $sys->{maincf_grp_container_name}{$grp_name};
        $grp_container_type = $sys->{maincf_grp_container_type}{$grp_name} || 'Zone';
        @grp_syslist = Prod::VCS60::Common::dynupgrade_get_systemlist_of_grpname($treeref_result,$grp_name);
        ...
        # get resources list
        @reses = Prod::VCS60::Common::dynupgrade_get_res_from_grp($treeref_result, $grp_name);
        for my $res_name (@reses) {
            $res_type = $container_opts = '';
            $res_type = Prod::VCS60::Common::dynupgrade_get_restype_of_resname($treeref_result,$res_name);
            next if ($res_type eq 'Zone');
            $container_opts = Prod::VCS60::Common::dynupgrade_get_attrvalue_of_typename($treeref_newsys,$res_type,'ContainerOpts');
            if($sys->{maincf_res_container_name}{$res_name}) {
                if ($sys->{maincf_res_container_type}{$res_name} eq 'Zone') {
    ...
    $treeref_result = Prod::VCS60::Common::dynupgrade_import_lines2tree(\@container_settings, $treeref_result);
    return $treeref_result;
}
...
sub get_netmask_by_address_from_netmasks_sys {
    my ($prod,$sys,$address) = @_;
    my ($addr1,$addr2,$addr3,$addr4,@netmasks_files,$netmask,$netmasks);
    $netmask='';
    ...
    if ($netmasks) {
        if ($netmasks =~ /^\s*$address\s+(\S+)/mxg) {
            $netmask = $1;
        } elsif ($netmasks =~ /^\s*$addr1\.$addr2\.$addr3\.0\s+(\S+)/mxg) {
            $netmask = $1;
        } elsif ($netmasks =~ /^\s*$addr1\.$addr2\.0\.0\s+(\S+)/mxg) {
            $netmask = $1;
        } elsif ($netmasks =~ /^\s*$addr1\.0\.0\.0\s+(\S+)/mxg) {
            $netmask = $1;
        }
    }
    return $netmask;
...
package Prod::VCS60::SolSparc;
@Prod::VCS60::SolSparc::ISA = qw(Prod::VCS60::SunOS);

sub init_padv {
    my $prod=shift;
    $prod->{upgradevers}=[qw(5.0.3 5.1)];
    $prod->{zru_releases}=[qw(4.1.2 5.0 5.1)];
    return;
}

package Prod::VCS60::Solx64;
@Prod::VCS60::Solx64::ISA = qw(Prod::VCS60::SunOS);

sub init_padv {
    my $prod=shift;
    $prod->{upgradevers}=[qw(5.0.3 5.1)];
    $prod->{zru_releases}=[qw(5.0 5.1)];
    return;
}
