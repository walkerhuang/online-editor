...
sub init_common {
    ...
    $prod->{eula}='EULA_SFHA_Ux_6.0.pdf';
    ...
    my $padv=$prod->padv();
    $padv->{cmd}{odmclustadm}='/opt/VRTS/bin/odmclustadm';
...
sub version_sys {
    ...
    if (!EDRu::compvers($vm_vers,$fs_vers,2)) {
        if ($cv) {
            ...
            return ($mpvers || $vcs_vers) if ($vcs_vers && $vm_vers && $fs_vers && ($rel->prod_licensed_sys($sys,$prod->{prodi})) && Cfg::opt('upgrade_nonkernelpkgs'));
            return ($mpvers || $vcs_vers) if ($vm_vers && $rel->prod_licensed_sys($sys,$prod->{prodi}));
            # return ($mpvers || $vcs_vers) if ((!EDRu::compvers($vm_vers,$vcs_vers,2)
            #                         && ($rel->prod_licensed_sys($sys,$prod->{prodi}))));
        } else {
            return ($mpvers || $vcs_vers) if ($vm_vers);
            # return ($mpvers || $vcs_vers) if (!EDRu::compvers($vm_vers,$vcs_vers,2));
        }
    }
    return '';
