...
sub wait_vm_enable {
    ...
    unless($vxfenmode){
        $msg=Msg::new("\nI/O Fencing is not running in Sybase Mode. It is recommended to configure I/O Fencing in Sybase Mode to configure Sybase ASE CE.");
        $msg->bold;
        Msg::n();
  ...
  START: while (1) {
        ...
        while (1) {
            ...
            unless ( $sys->is_dir($home ) ) {
                $msg1=Msg::new("The Sybase home directory $home cannot be accessed. This may be due to a file system not being mounted");
                $msg1->warning;
                Msg::n();
                $msg1=Msg::new("Are you sure you want to use $home for Sybase home?");
                my $ayn = $msg1->aynn;
                if ($ayn eq 'N') {
                    next;
                }
        ...
        $msg .= Msg::new("\tSybase UNIX user home directory: $home\n")->{msg};
        ...
        while (1) {
            ...
            unless ( $sys->exists($quorum ) ) {
                ...
                $msg1=Msg::new("The quorum file $quorum cannot be accessed. This may be due to a file system not being mounted");
                $msg1->warning;
                Msg::n();
...
sub init_plat {
    ...
    $prod->{upgradevers}=[qw(4.0.4 5.0 5.1)];
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSjre.rte VRTSsfcpi60 VRTSperl512 VRTSperl.rte VRTSvlic32
    ) ];
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSjre15 VRTSjre VRTSsfcpi60 VRTSperl512 VRTSvlic32 VRTSwl
    ) ];
    return;
}
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSvxvm-common VRTSjre15 VRTSjre VRTSsfcpi60 VRTSperl512 VRTSvlic32
    ) ];
    return;
}
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSdsa VRTSsfmh41 VRTSob34 VRTSobc33 VRTSaslapm60
        VRTSat50 VRTSsmf VRTSpbx VRTSicsco VRTSvxfs60 VRTSvxvm60
        VRTSjre15 VRTSjre VRTSsfcpi60 VRTSperl512 VRTSvlic32
    ) ];
    return;
}
...
sub init_padv {
    my $prod=shift;
    $prod->{upgradevers}=[qw(4.1.2 5.0 5.1)];
    $prod->{zru_releases}=[qw(5.0.3 5.1)];
    return;
}
