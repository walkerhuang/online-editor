...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSpbx VRTSicsco VRTSvxfs60 VRTSveki60 VRTSsfcpi60 VRTSperl512
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSvxfs-common VRTSsfcpi60 VRTSperl512 VRTSvlic32
    ) ];
    my $padv=$prod->padv();
    $prod->{fstab}='/etc/fstab';
    $padv->{cmd}{fstyp}='/opt/VRTS/bin/fstyp';
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSicsco VRTSvxfs60 VRTSsfcpi60 VRTSperl512 VRTSvlic32
    ) ];
...
sub kernel_parameter_sys {
    ...
    if(Cfg::opt('precheck')) {
        ...
        if ($rtn_lwp < 6000 || $rtn_svc < 6000) {
            $msg=Msg::new("To avoid a potential reboot after installation, you should modify the /etc/system file with the appropriate values, and reboot prior to package installation.\n\nAppropriate /etc/system file entries are shown below:\n\tset lwp_default_stksize=0x6000\n\tset rpcmod:svc_default_stksize=0x6000");
            $sys->push_warning($msg);
        }
    }
    return 1;
