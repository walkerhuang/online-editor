...
sub init_common {
    ...
    if($edr && (-d "$edr->{mediapath}/storage_foundation_basic")) {
        ...
        $prod->{eula}='EULA_SF_Basic_Ux_6.0.pdf';
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSvxvm60 VRTSveki60 VRTSjre15.rte VRTSjre.rte VRTSsfcpi60 VRTSperl512
...
sub init_plat {
    ...
    $prod->{obsoleted_bundles}=[ qw(Base-VxTools-50 Base-VxVM-50 B3929FB Base-VxFS-50 Base-VxVM) ];
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSvxfs60 VRTSvxvm60 VRTSjre15 VRTSjre VRTSsfcpi60 VRTSperl512
        VRTSvlic32 VRTSwl
    ) ];
    return;
}
