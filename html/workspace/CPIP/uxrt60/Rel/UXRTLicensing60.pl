...
sub licensing {
    ...
    while(1) {
        ...
        if (Cfg::opt(qw(install license)) || (!$licensed && Cfg::opt(qw(start patchupgrade configure)))) {
            if ($licensed) {
            ...
            $rel->cli_license_option() unless (($prod->{prod} eq 'SF') && (Cfg::opt('prodmode') eq 'SF Basic'));
            $rt = '';
...
sub licensing_status {
    ...
    for my $sys (@{$cpic->{systems}}) {
        ...
        $systems++ if ($pkg->version_sys($sys));
...
sub license_products {
    ...
    my ($cpic,$pkgdir,$msg,$sitelic,$cfg,$lictype,$web,$edr,$sys,$ayn,$plat,$key,$rt,$keytype,$nlsys,$prodname,$pkg,$licupi,$prod);
    $edr=Obj::edr();
    $web=Obj::web();
    $cpic=Obj::cpic();
    if (Obj::webui()) {
    ...
    return '' if (Cfg::opt('rootpath')||$prod->{nolic});
    return '' if (Cfg::opt(qw(precheck uninstall stop start)));
    $cfg=Obj::cfg();
    #Msg::title();
    ...
    unless (Cfg::opt('makeresponsefile')) {
        ...
        if ($cpic->{fromdisk}) {
            ...
            for my $sys (@{$cpic->{systems}}) {
                ...
                $pkg=$sys->pkg(licensing_pkg());
                $plat=$sys->{plat};
                $pkg->{file}||=$plat->media_pkg_file($pkg, $pkgdir);
                $pkg->{vers}||=$plat->media_pkg_version($pkg, $pkgdir);
            }
        ...
        if (Cfg::opt(qw(responsefile precheck))) {
            for my $sys (@{$cpic->{systems}}) {
                ...
                if (!$prod->licensed_sys($sys)) {
                    $msg=Msg::new("$prodname $prod->{name} is not licensed on $sys->{sys}\n");
                    $msg->die;
                }
           }
        } else {
    ...
    $rel->cli_add_additional_license_key($sys);
    $rel->web_add_additional_license_key($sys);
    $prod->update_prod();
    return;
}
...
# web-used adding additional keys sub
sub web_add_additional_license_key {
    my ($rel, $sys) = @_;
    my ($web, $key);

    $web = Obj::web();
    return if (Cfg::opt('responsefile'));
    return if ($web->{bypass_licensing});
    ...
    while (1) {
        ...
        elsif ($web->param('license_mode') eq 'key') {
            delete $web->{submit}{license_mode};
            $key = $rel->web_ask_license_key();
            if ($rel->valid_license_key($sys, $key)) {
                $rel->add_license_key($key);
            } else {
                # key is invalid
                next;
            }
...
sub add_license_key {
    my ($rel,$key) = @_;
    my ($cpic,$msg,$edr,$prod,$sys,$lsys,$license_status,$prodname);
    $edr=Obj::edr();
    $cpic=Obj::cpic();
...
sub license_products_vxkeyless {
    ...
    if (Cfg::opt(qw(install configure patchupgrade start license))) {
        ...
        for my $sys (@{$cpic->{systems}}) {
            last if (Cfg::opt('makeresponsefile'));
            $prod_level=$rel->get_vxkeyless_prod_level($sys, $prod) if (Cfg::opt('license'));
...
sub feature_licensed_sys {
    ...
    for my $lk (sort { $a cmp $b } keys(%{$sys->{keys}})) {
        for my $feat (sort { $a cmp $b } keys(%{$sys->{keys}{$lk}})) {
            ...
            next unless ($feat eq $fm);
            return 1 if (($fval eq 'Enabled') || ($fval eq 'YES'));
            return 1 if (($val) && ($fval eq $val));
        }
    }
    return '';
...
sub prod_licensed_sys {
    ...
    my ($cpic,$msg,$pname,$lname,$temp_name,$ha_status,$vxkeyless,$vxk,$lk,$prod,@prods,$licensed);
    ...
    $prodname||=$prod->{abbr};
    $rel->read_licenses_sys($sys) unless ($sys->{keys});
    if ($prod->{licsuperprods} && @{$prod->{licsuperprods}}) {
    ...
    for my $lk (sort { $a cmp $b } keys(%{$sys->{keys}})) {
        ...
        for my $lname (@{$prod->{lic_names}}) {
            ...
            if ($pname=~/$temp_name/m) {
                ...
                return 1 unless ($prodname=~/SFHA/m);
                # the following line is really just for SFHA
                $ha_status = 1 if ($rel->feature_licensed_sys($sys, 'Mode#VERITAS Cluster Server', 'VCS'));
                $ha_status = 1 if (!$ha_status && ($prodname =~ /SFHA/) && $rel->prod_licensed_sys($sys,'VCS60'));
                return $ha_status;
            }
        }
    }
...
sub vxkeyless_register_sys {
    ...
    my ($cpic,$encap_keyfile,$pdfr,$vxkeyless,$prodname,$pkg,$prod);
    return '' if (Cfg::opt('makeresponsefile'));
    $sys=Obj::sys($sys) unless (ref($sys) eq 'Sys');
    ...
    if ($pkg->version_sys($sys)) {
        ...
        if (EDR::cmdexit()) {
            ...
            Msg::log("$prodname vxkeyless key ($prod_level) failed to register on $sys->{sys}");
        }
    ...
    } elsif (!Cfg::opt(qw(install precheck))) {
        $pdfr=Msg::get('pdfr');
        Msg::log("$pkg->{pkg} $pdfr not installed on $sys->{sys}");
    }
    return;
}
...
sub restore_keyless_keys {
    my ($rel) = @_;
    my ($cpic,$oldkey,$newkey,$edr,$sys,$key);
    ...
    for my $sys (@{$cpic->{systems}}) {
        ...
        if ($oldkey) {
            ...
            $oldkey=~s/_VVR/_VR/mgi;
            if ($oldkey ne $newkey) {
                $oldkey.=",$newkey" if ($newkey);
                $sys->cmd("_cmd_vxkeyless -q set $oldkey 2>/dev/null");
            }
        }
    }
    return;
