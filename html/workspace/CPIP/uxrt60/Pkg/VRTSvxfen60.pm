...
sub configure_cpagent {
    ...
    if (Obj::webui()) {
        ...
        while (1) {
            ...
            $cfg->{fencing_cpagentgrp} = $vxfengrp if (!Cfg::opt('responsefile'));
            last;
        }
    }
    Msg::n();
...
sub configure_fencing {
    ...
    while (1) {
        ...
        if (Cfg::opt('responsefile')) {
            ...
            if (Obj::webui()){
                ...
                if(CPIC::get('prod')=~/SFSYBASECE/m){
                        if(Cfg::opt('fencing')){
                            push (@{$menuopt},$msg_sybase->{msg},$msg_disabled->{msg},$msg_migrate->{msg});
        ...
        $fencing_failed=$pkg->configure_dskfenc($dgtype,$diskpolicyOptions) if ($menu==2);
        if ($menu==4) {
        ...
        if ($fencing_failed eq '__back__') {
            ...
            $tmpmaincf='';
            if($sys->cmd("_cmd_grep UseFence $maincf_dir/main.cf 2> /dev/null")) {
                $msg=Msg::new("Fencing already configured with VCS");
                $msg->print;
            } else {
                $str=EDRu::datetime();
                $sys->cmd("_cmd_cp $maincf_dir/main.cf $maincf_dir/main.cf-$str");
...
sub create_vxfencoorddg {
    ...
    my ($msg,$sys0,$initdisks,$disk,$tmpdsk,$ret,@initdisks);
    $initdisks = '';
    $sys0 = ${CPIC::get('systems')}[0];
    ...
    for my $tmpdsk (@initdisks) {
        $msg = Msg::new("Intializing disk $tmpdsk on $sys0->{sys}");
        $msg->left;
        $msg->display_left($msg) if (Obj::webui());
...
sub get_cps_security {
    ...
    my ($cpsadm,$msg,$security,$security_val,$cpspkg,$web);
    $cpspkg=$pkg->pkg('VRTScps60');
    ...
    for my $server(@{$cps_aref}) {
        ...
        $security = $server->cmd("$cpsadm -s $server->{sys} -a server_security -p ${$ports_href}{$server->{sys}}");
...
sub ask_cps {
    ...
    for ($index = 0; $index < ($ncp - $ndisks); $index++) {
        ...
        if (!Cfg::opt('responsefile')) {
            $question = Msg::new("Enter the total number of Virtual IP addresses or fully qualified host name for the Coordination Point Server #$number:");
            $help = Msg::new("Each Coordination Point Server may have more than one Virtual IP address configured. Input the total number of the Virtual IP addresses or fully qualified host name you would like to use");
            while (1) {
    ...
    while($flag){
        ...
        unless ($diskpolicy eq 'raw') {
            for my $disk (@disks) {
                ...
                $unused = $sys0->cmd("_cmd_vxfenadm -g $devpath$disk");
                if (EDR::cmdexit()) {
...
sub configure_cpc {
    ...
    my ($cpsadm,$cpsat,$vmmode, $msg, $index, $sys, $security, $secmode,$cpserver, $cpspkg);
    ...
    # Check the security status of CP servers
    ($ret,$security) = $pkg->config_cps_cpc_security(\@cps,\%ports);
    return $ret if ($ret);
...
sub config_cps_cpc_security {
    my ($pkg,$cps_aref,$ports_href) = @_;
    my (@cps,%ports,$msg,$secmode,$security,$ret,$vcs,$sys0,$web);
    $vcs = $pkg->prod("VCS60");
    $sys0 = ${CPIC::get('systems')}[0];
    $web = Obj::web();
    $security = $pkg->get_cps_security($cps_aref,$ports_href);
    if ($security eq 'error') {
        ...
        Msg::n();
        $msg = Msg::new("Since the Coordination Point servers are configured in secure mode, installer will configure the client cluster to secure the communication between Coordination Point servers and client cluster.");
        $msg->bold;
        Msg::prtc();
        $web->web_script_form('alert',$msg->{msg})if (Obj::webui());
        $msg = Msg::new("Configure fencing in secure mode");
        $web->web_script_form('showstatus',$msg->{msg}) if(Obj::webui());
        $secmode = $vcs->is_secure_cluster($sys0);
        if ($secmode == 1) {
            $msg = Msg::new("VCS is running in secure mode on $sys0->{sys}");
            $msg->print;
        } else {
            Msg::title();
...
sub check_cps_installed {
    ...
    for my $sys (@$syslist) {
        ...
        if ($vers eq '') {
            $failed = 1;
            $msg = Msg::new("VRTScps doesn't seem to be installed on $sys->{sys}. Please install it first and then try again.");
            $msg->bold;
            $web->web_script_form('alert',$msg->{msg})if (Obj::webui());
            Msg::prtc();
        }
...
sub configure_dskfenc {
    ...
    if ($vmmode !~ /enable/m) {
        $msg=Msg::new("make sure VM is running before configuring disk based fencing");
        $msg->print;
        $web->web_script_form('alert',$msg->{msg}) if (Obj::webui());
        return 1;
    }
    ...
    if($choice==1) {
        ...
        if (Obj::webui()) {
            ...
            return 1 if (!$pkg->create_new_fencing_dg_sys($sys,$dgname,$line));
            EDRu::despace($line);
...
#    $web->tsub("web_script_form","stopprocess");
    ...
    for my $sys (@{$cpic->{systems}}) {
        ...
        $pkg->update_vxfen_sybase_sys($sys) if(CPIC::get('prod')=~/SFSYBASECE/m);
    }
...
sub update_vxfen_sybase_sys {
    ...
    for my $line (split(/\n/,$out)) {
        if($line !~/\s+?#/m && $line=~/vxfen_mode=/mx){
            $line='vxfen_mode=sybase';
        }
        $out1.="$line\n";
    }
...
sub migrate_fencing {
    ...
    my ($dgname,$newconf,$scsi3,$security,$web);
    $cfg = Obj::cfg();
    $web = Obj::web();
    $syslist = CPIC::get('systems');
    $sys0 = $$syslist[0];
    ...
    if ((@rshfail_systems) || ((@sshfail_systems) && (@sshfail_systems != $#{$syslist}))) {
    ...
    if (@{$newconf->{cps}}) {
        ...
        ($ret,$security) = $pkg->config_cps_cpc_security($newconf->{cps},$newconf->{ports});
        return $ret if ($ret);
    }
    ...
    $pkg->ask_configure_cpagent() unless ($pkg->{sybase_mode});
...
sub config_vxfen {
    ...
    # if VCS is not started successfully return error
