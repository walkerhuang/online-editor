...
sub query_pkgdep_sys {
    my ($pkg,$sys)=@_;
    my (@softdeps, $vxvm, $vxfs, $vcs,$dep,$rel);
    $rel=Obj::rel();
    if ($pkg->{softdeps}) {
    ...
    if (Cfg::opt('uninstall')) {
        $vxvm = $sys->pkg('VRTSvxvm60') if (EDRu::inarr($rel->{prods},"VM60"));
        $vxfs = $sys->pkg('VRTSvxfs60') if (EDRu::inarr($rel->{prods},"FS60"));
        $vcs = $sys->pkg('VRTSvcs60') if (EDRu::inarr($rel->{prods},"VCS60"));
