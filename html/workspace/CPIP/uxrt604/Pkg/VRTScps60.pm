...
sub remove_node_from_cps {
    ...
    $out = $cpssys->cmd("$cpsadm -s $cpssys->{sys} -a list_nodes -p $cpport | _cmd_grep $clusname.*${uuid}.*$sysname\\(");
    if ($out =~ /\s+$sysname\((\d+)\)\s+(\d+)/m) {
        $nodeid = $1;
        # 1. unregister from CPS
        if ($2 eq '1') {
            Msg::log("Unregistering $sys->{sys} from CP server $cpssys->{sys}...");
            $out = $cpssys->cmd("$cpsadm -s $cpssys->{sys} -p $cpport -a unreg_node -u $uuid -n $nodeid");
            if (EDR::cmdexit()) {
        ...
        $out = $cpssys->cmd("$cpsadm -s $cpssys->{sys} -p $cpport -a rm_node -u $uuid -c $clusname -n $nodeid");
        ...
        $out = $cpssys->cmd("$cpsadm -s $cpssys->{sys} -p $cpport -a list_nodes -u $uuid");
...
sub unconfigure_cpserver {
    ...
    ($cpssg,$cpservres) = $pkg->get_cpssg_cpservres_sys($sys0);
    if (!$cpssg) {
    ...
    $msg->nprint;
    $ret = $pkg->removefrom_vcs_config($cpssg,$cpservres);
    if ($ret) {
...
sub removefrom_vcs_config {
    ...
    $res_aref = $pkg->unlink_child_resources_of_resource_from_sys($sys0,$cpservres);
    push (@reses, @$res_aref);
    unshift(@reses,$cpservres);
...
sub configure_precheck_sys {
    ...
    $web = Obj::web();
    $vcs = $pkg->prod('VCS60'); 
    ($cpssg,$cpservres) = $pkg->get_cpssg_cpservres_sys($sys);
    if ($cpssg) {
...
sub init_padv {
    my $pkg=shift;
    $pkg->{oslibs}{all}={
        "libcrypt.so.1"  =>  "glibc-2.12-1.107.el6_4.2.i686",
        "libcrypt.so.1(GLIBC_2.0)"  =>  "glibc-2.12-1.107.el6_4.2.i686",
        ...
        "libc.so.6(GLIBC_2.1.2)"  =>  "glibc-2.12-1.107.el6_4.2.i686",
        "libc.so.6(GLIBC_2.1.3)"  =>  "glibc-2.12-1.107.el6_4.2.i686",
        "libc.so.6(GLIBC_2.2)"  =>  "glibc-2.12-1.107.el6_4.2.i686",
        "libc.so.6(GLIBC_2.3)"  =>  "glibc-2.12-1.107.el6_4.2.i686",
        "libdl.so.2"  =>  "glibc-2.12-1.107.el6_4.2.i686",
        "libdl.so.2(GLIBC_2.0)"  =>  "glibc-2.12-1.107.el6_4.2.i686",
        "libdl.so.2(GLIBC_2.1)"  =>  "glibc-2.12-1.107.el6_4.2.i686",
        ...
        "libm.so.6"  =>  "glibc-2.12-1.107.el6_4.2.i686",
        "libm.so.6(GLIBC_2.0)"  =>  "glibc-2.12-1.107.el6_4.2.i686",
        "libnsl.so.1"  =>  "glibc-2.12-1.107.el6_4.2.i686",
        "libpthread.so.0"  =>  "glibc-2.12-1.107.el6_4.2.i686",
        "libpthread.so.0(GLIBC_2.0)"  =>  "glibc-2.12-1.107.el6_4.2.i686",
        "libpthread.so.0(GLIBC_2.1)"  =>  "glibc-2.12-1.107.el6_4.2.i686",
        "libpthread.so.0(GLIBC_2.2)"  =>  "glibc-2.12-1.107.el6_4.2.i686",
        "libpthread.so.0(GLIBC_2.3.2)"  =>  "glibc-2.12-1.107.el6_4.2.i686",
        "libresolv.so.2"  =>  "glibc-2.12-1.107.el6_4.2.i686",
        "librt.so.1"  =>  "glibc-2.12-1.107.el6_4.2.i686",
        "librt.so.1(GLIBC_2.2)"  =>  "glibc-2.12-1.107.el6_4.2.i686",
...
sub init_padv {
    ...
    $pkg->{oslibs}{SLES11SP2}={
        ...
        "libgcc_s.so.1(GCC_4.2.0)"  =>  "libgcc46-32bit-4.6.1_20110701-0.13.9.x86_64",
    ...
    $pkg->{oslibs}{SLES11SP3}={
        ...
        "libgcc_s.so.1(GCC_4.2.0)"  =>  "libgcc_s1-32bit-4.7.2_20130108-0.15.45.x86_64",
