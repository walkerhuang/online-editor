...
package Rel::UXRT604::Common;
@Rel::UXRT604::Common::ISA = qw(Rel);
...
sub init_common {
    ...
    $rel->{upgradeprods} = [ qw(SFRAC60 SFSYBASECE60 SVS60 SFCFSHA60 SFHA60 SF60 VCS60 VM60 DMP60 FS60) ];
    $rel->{allpkgs}=[ qw(VRTSperl514 VRTSvlic32 VRTSsfcpi604 VRTSspt60) ];
    $rel->{minpkgs}=[ qw(VRTSperl514 VRTSvlic32 VRTSsfcpi604) ];
    $rel->{recpkgs}=[ qw(VRTSperl514 VRTSvlic32 VRTSsfcpi604 VRTSspt60) ];
    $rel->{cpipkg}='VRTSsfcpi604';

    $rel->{ru_prod}=[ qw(SFRAC60 SFSYBASECE60 SVS60 SFCFSHA60 SFHA60 VCS60) ];
    ...
    $rel->{installonupgradepkgs}=[ qw(VRTSsfcpi604) ];
...
sub init_args {
    ...
    $rel->{arguments} = $arguments = {
        ...
        'jumpstart' => {
            ...
            'handler' => \&Rel::UXRT604::Common::process_jumpstart_arg,
        },
        'flash_archive' => {
            ...
            'handler' => \&Rel::UXRT604::Common::process_flash_archive_arg,
        },
        'ai' => {
            ...
            'handler' => \&Rel::UXRT604::Common::process_ai_arg,
        },
        'kickstart' => {
            ...
            'handler' => \&Rel::UXRT604::Common::process_kickstart_arg,
        },
        'yumgroupxml' => {
            ...
            'handler' => \&Rel::UXRT604::Common::process_yumgroupxml_arg,
        },
        'nim' => {
            ...
            'handler' => \&Rel::UXRT604::Common::process_nim_arg,
        },
        ...
        'ignite' => {
            ...
            'handler' => \&Rel::UXRT604::Common::process_ignite_arg,
        },
        ...
        'security' => {
            ...
            'handler' => \&Rel::UXRT604::Common::process_security_arg,
        },
        'securityonenode' => {
        ...
        'fencing' => {
            ...
            'handler' => \&Rel::UXRT604::Common::process_fencing_arg,
        },
        'configcps' => {
...
sub ignite {
    ...
    if (EDR::cmdexit()) {
        ...
        # add post_load_cmd = "/opt/VRTS/install/bin/UXRT604/add_install_scripts" in core_media_cfg
        ...
        for my $line (@conts) {
            if ($line=~/sd_software_list/) {
                $filecont.="    ";
                $filecont.="post_load_cmd = \"/opt/VRTS/install/bin/UXRT604/add_install_scripts\"\n";
            }
            $filecont.=$line."\n";
        }
...
@Rel::UXRT604::AIX::ISA = qw(Rel::UXRT604::Common);
...
@Rel::UXRT604::HPUX::ISA = qw(Rel::UXRT604::Common);
...
@Rel::UXRT604::HPUX1131::ISA = qw(Rel::UXRT604::HPUX);
...
@Rel::UXRT604::Linux::ISA = qw(Rel::UXRT604::Common);
...
sub init_plat {
    ...
    $rel->{platreqs} = [ 'SLES11 SP2 (3.0.13)', 'SLES11 SP3 (3.0.76)',  
                         'OEL6 U4 (2.6.39-400.17.1.el6uek)'];
...
sub platvers_sys {
    ...
    if ($distro eq 'RHEL') {
        ...
        $msg=Msg::new("Kernel Release $kstring found on $sys->{sys} is not supported. Only Oracle Linux 6.4 with UEK kernel is supported. Install Oracle Linux 6.4 with UEK kernel in order to install this product");
        $sys->push_error($msg);
        return '';
        ...
        if ($f[0] != 3) {
            # sles11sp3 is using kernel 3.0.76
            $msg=Msg::new("Kernel Release $kstring found on $sys->{sys} is not supported. Only SuSE 11 SP2 and SP3 are supported platform. Upgrade the OS to SLES11 SP2 or SP3 in order to install this product");
            $sys->push_error($msg);
            return '';
        } else {
            #SLES 11 SP2 and SP3 are both supported
            #if (($f[2]<76) || (defined($patchlevel) && ($patchlevel != 3))){
            #    $msg=Msg::new("Kernel Release $kstring found on $sys->{sys} is not supported. Only SuSE 11 SP3 is supported on SuSE11. Upgrade the OS to SLES11 SP3 in order to install this product");
            #    $sys->push_error($msg);
            #    return '';
            #}
        }
        ...
        $msg=Msg::new("Kernel Release $kstring is not supported on $sys->{sys}. Only SLES11 SP3 is supported.");
        $sys->push_error($msg);
        return '';
    } elsif ($distro eq 'OL') {
        ...
        if ($kstring =~ /el6/m) {
            # OL6 check
            # Only UEK 6.4 supported here
            # UEK 6.4: kernel-uek-2.6.39-400.17.1.el6uek
            # UEK 6.3: kernel-uek-2.6.39-200.24.1.el6uek
            # UEK 6.2: kernel-uek-2.6.32-300.3.1.el6uek
            # UEK 6.1: kernel-uek-2.6.32-100.34.1.el6uek
            unless ($kstring =~ /^2\.6\.39-400.*el6uek/m) {
                $msg=Msg::new("Kernel Release $kstring found on $sys->{sys} is not supported. Only Unbreakable Enterprise Kernel 2.6.39-400 is supported on Oracle Linux 6.4. you may configure the operating system to boot with this kernel instead.");
                $sys->push_error($msg);
                return '';
            }
        } elsif ($kstring =~ /el5/m) {
            $msg=Msg::new("Kernel Release $kstring found on $sys->{sys} is not supported. Oracle Linux 5 is not supported.");
            $sys->push_error($msg);
            return '';
        }
    } else {
...
@Rel::UXRT604::RHEL5x8664::ISA = qw(Rel::UXRT604::Linux);
...
@Rel::UXRT604::RHEL5ppc64::ISA = qw(Rel::UXRT604::Linux);
...
@Rel::UXRT604::RHEL6x8664::ISA = qw(Rel::UXRT604::Linux);

sub init_padv {
    my ($rel) = @_;

    $rel->{prods} = [ qw(VCS60) ];
    $rel->{menuprods} = [ qw(VCS60) ];
    $rel->{upgradeprods} = [ qw(VCS60) ];
    $rel->{platreqs} = [ 'OEL6 U4 (2.6.39-400.17.1.el6uek)' ];
...
@Rel::UXRT604::RHEL6ppc64::ISA = qw(Rel::UXRT604::Linux);
...
@Rel::UXRT604::SLES10x8664::ISA = qw(Rel::UXRT604::Linux);
...
@Rel::UXRT604::SLES10ppc64::ISA = qw(Rel::UXRT604::Linux);
...
@Rel::UXRT604::SLES11x8664::ISA = qw(Rel::UXRT604::Linux);
...
@Rel::UXRT604::SLES11ppc64::ISA = qw(Rel::UXRT604::Linux);
...
@Rel::UXRT604::SunOS::ISA = qw(Rel::UXRT604::Common);
...
@Rel::UXRT604::SolSparc::ISA = qw(Rel::UXRT604::SunOS);
...
@Rel::UXRT604::Solx64::ISA = qw(Rel::UXRT604::SunOS);
...
@Rel::UXRT604::Sol11sparc::ISA = qw(Rel::UXRT604::SunOS);
...
@Rel::UXRT604::Sol11x64::ISA = qw(Rel::UXRT604::Sol11sparc);
