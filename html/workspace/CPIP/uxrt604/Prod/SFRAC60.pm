...
sub init_common {
    ...
    $prod->{installonupgradepkgs} = [ qw(VRTSfsadv VRTSamf VRTSsfmh VRTSvbs VRTSvcsvmw) ];
...
sub version_sys {
    ...
    my ($rootpath,$tmppkgvers, $pkgvers,$mpvers,$cpic,$rel,$cv,$pkg,$tmppkg,@mainpkgs);
    $cpic=Obj::cpic();
    $rel=$cpic->rel;
    ...
    if ($pkgvers) {
        @mainpkgs = qw(VRTSvxvm60 VRTSvxfs60 VRTSvcs60);
...
sub completion_messages {
    ...
    if (Cfg::opt(qw(patchupgrade upgrade)) && !Cfg::opt(qw(upgrade_nonkernelpkgs makeresponsefile responsefile rootpath)) && (!Obj::webui())) {
...
sub config_haip_common {
    my ($prod, $isaddnode, $allnodes_ref, $clusnodes_ref, $newnodes_ref)  = @_;     my ($mpriv_ips_ref, @mpriv_ips, $mpriv_ip, $mpriv_mask);
    my ($sys, $sys1, $key, $inf, $nics, $nics_str, $ip, $ips);
    my ($ret, $err, $msg, $question, $ayn, $done, $help, $backopt);
    ...
    $msg = Msg::new("* You must not use the public network interface for Oracle HAIP configuration");
    $msg->bold;
    $msg = Msg::new("* IP addresses used for a NIC on all nodes of your cluster must be in the same subnet, which must be different from the subnets for the IP addresses on other NICs. Otherwise, Oracle Clusterware/Grid Infrastructure/Oracle Database UDP IPC will not be able to communicate properly across the nodes");
    $msg->bold;
...
SKIP_VOTELOC:
    ...
    $ignore_privnic = 1 if (($prod->{crs_release} =~ /12\.1/m) || ($prod->{crs_release}=~ /11\.2/m && $prod->{crs_patch_level}=~ /0\.2|0\.3/m));
    if ($ignore_privnic == 0) {
        if ($nicres eq '') {
...
DNA_ORACLE_VERSION:
    ...
    while (1) {
        ...
        if ($release !~ /^(10\.2|11\.1|11\.2|12\.1)$/){
...
sub check_sfracoracleintegration_sys {
    ...
    for my $sys (@{$syslistref}) {
        ...
        goto OPROCD_CHECK if (($is_sfcfsrac) || ($prod->{oraver} =~ /11\.2|12\.1/m));
...
sub set_css_misscount {
    ...
    my $sys = ${CPIC::get('systems')}[0];
    my @systems = ($sys);
    my $crsup = $prod->is_crs_up(\@systems);
    if ($crsup) {
        $cmd = "$crs_home/bin/crsctl get css misscount";
        my $misscount = $sys->cmd("$cmd");
        if (!EDR::cmdexit()) {
            $misscount =~ s/^.*misscount\s+(\d+)\s+.*$/$1/ if ($misscount !~ /^\d+$/);
            if ($misscount >= 600) {
                return;
...
sub relink_oracle {
    ...
    for my $system (@cpnodes) {
        ...
        if (EDR::cmdexit()) {
            ...
            $system->cmd("_cmd_ldd $prod->{db_home}/bin/oracle 2>/dev/null | _cmd_grep odm ");
            if (EDR::cmdexit()) {
...
sub check_liblink_sys {
    ...
    if (($oraversion =~ /12\.1/m) ||
        ($oraversion =~ /12\.2/m)) { # Keep adding 12.1, 12.2 here as they get realesed
        $oracle_num = '12';
    }
...
sub copy_libskgxp_per_ipcutil {
    ...
    #Do nothing for 11g 12g
    return 0 if ($prod->{db_release} =~ /^(11\.1|11\.2|12\.1)$/);
...
DNA_ROOTPRE:
    ...
    if ((($release eq '10.2') && ($patch_level eq '0.4')) || ($release =~ /^(11\.1|11\.2|12\.1)$/)) {
        for my $sys (@{CPIC::get('systems')}) {
...
sub do_relink_oracle {
    ...
    for my $sys (@cpnodes) {
        ...
        if ($prod->{db_release} !~ /^(11\.1|11\.2|12\.1)$/){
