...
sub init_common {
    ...
    $prod->{eula}='EULA_VCS_Lx_6.0.2.pdf';
    ...
    $prod->{installonupgradepkgs} = [ qw(VRTSamf VRTSsfmh VRTSvbs VRTSvcsvmw) ];
...
sub version_sys {
    ...
    # if $pkgvers<= 6.0.100.000, it's AppHA.
    # if 6.0.100 < $pkgvers< 6.0.XYZ it's TerraNova(VCS).
    # if $pkgvers>= 6.1, it's AppHA
    $pkg=Pkg::new_pkg('Pkg', 'VRTSvcsvmw', $sys->{padv});
    $pkgvers=$pkg->version_sys($sys);
    if (!$pkgvers || (EDRu::compvers($pkgvers, '6.0.100.000', 3) == 1 && EDRu::compvers($pkgvers, '6.1.000.000', 3) == 2)) {
...
sub verify_procs_list_sys {
    ...
    if (EDRu::inarr($rel->{prods}, "VM60")) {
        my $vm = $prod->prod('VM60');
        # adjust sfmh-discovery process
        $procs = $vm->adjust_sfmh_for_procs_sys($sys,$procs,$state);
    } else {
        # since the above $vm call is for deleting sfmhdiscovery60 from $procs for start
        # we need to do the same if VM is not part of the $rel->{prods}
        $procs=EDRu::arrdel($procs, 'sfmhdiscovery60')
            if (EDRu::inarr('sfmhdiscovery60', @{$procs}) && (defined $state && $state eq 'start'));
    }
...
sub update_maincf {
    my ($prod,$sys) = @_;
    my ($admins,$ddev_nwhosts,$sysi,$extype,$cfg,$ddev,$drid,$cdev_nwhosts,$extypes,$opers,$n,$drnd,$cdev,$csl,$users,$sys2,$prefix,$ddev_attr,$maincf,$tmpdir,$recp,$cdev_attr,$cons,$secure);
    $cfg = Obj::cfg();
...
sub addnode_configure_fencing {
    ...
    my($ret,$security,$cps_aref,$ports_href,$conf,$uuid,$max_pri);
    ...
    for my $sysi (@{$cfg->{newnodes}}) {
        ...
        if ($vxfenmode eq 'scsi3' || (($vxfenmode eq 'customized') && ($vxfen_mechanism eq 'cps'))) {
            if ($vxfenmode ne 'scsi3') {
                $ret = $vxfen_pkg->addnode_configure_cpc_sys($sys, $cfg->{vcs_clustername}, $max_pri+1,
            ...
            $ret = $vxfen_pkg->addnode_configure_cp_agent($firstnode, $sys, $max_pri+1);
            if ($ret) {
            ...
            if ((($vxfenmode eq 'scsi3') || ($vxfendg ne '')) && EDRu::inarr("VM60", @{$rel->{prods}})) {
...
sub addnode_update_CSSG {
    ...
    for my $sysi (@{$cfg->{newnodes}}) {
        $system = transform_system_name($sysi);
        $firstnode->cmd("$prod->{bindir}/hagrp -modify ClusterService SystemList -add $system $max_pri+1");
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSsmf VRTSpbx VRTSicsco VRTSjre.rte VRTSsfcpi604 VRTSperl514 VRTSperl.rte
        VRTSvlic32
    ) ];
...
sub init_plat {
    ...
    $prod->{minpkgs}=[ qw(VRTSllt60 VRTSgab60 VRTSvxfen60 VRTSamf60 VRTSvcs60 VRTSvcsag60 VRTSvcsvmw60) ];
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSjre VRTSsfcpi604 VRTSperl514 VRTSvlic32
    ) ];
...
sub appha_postremove_sys{
    ...
    # VRTSvcsvmw is not uninstalled, skip this function
    return if (!EDRu::inarr('VRTSvcsvmw60', @{$sys->{uninstallpkgs}}) || !EDRu::inarr('VRTSvcsvmw60', @{$cpic->{uninstallpkgs}}));
    # VRTSsfmh is uninstalled, skip this function
    return if (EDRu::inarr('VRTSsfmh41', @{$sys->{uninstallpkgs}}) || EDRu::inarr('VRTSsfmh41', @{$cpic->{uninstallpkgs}})); 
...
sub init_plat {
    ...
    $prod->{obsoleted_previous_releases_pkgs} = [ qw(
        ...
        VRTSat50 VRTSsmf VRTSpbx VRTSicsco VRTSjre VRTSsfcpi604 VRTSperl514
        VRTSvlic32
    ) ];
    # For UXRT 5.1SP1, VRTSacclib need to be fresh on AIX and Solaris.
    $prod->{obsoleted_but_need_refresh_when_upgrade_pkgs} = [ qw(VRTSacclib52) ];
