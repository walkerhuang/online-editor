...
"Upgrade from $installedprod->{abbr} $ivers to $prodname $prod->{vers} is not supported on $sys->{sys}");
        ...
        if($vcsagver ne '6.0.110.000' && (Cfg::opt('installallpkgs') || Cfg::opt('installrecpkgs'))){
            ...
            $msg = Msg::new("$prodname version $ivers is already installed on $sys->{sys} VMware enablement");
        }
