...
sub vxfen_reuse_precheck {
    ...
    if ($failed && $usefence) {
        $msg = Msg::new("VCS may not start successfully due to 'UseFence = SCSI3' is set in main.cf. Fix the fencing issues first or reconfigure the cluster.");
        $msg->warning;
        Msg::n();
        $web->web_script_form('alert',$msg) if (Obj::webui);
        return 0;
...
sub upgrade_preremove_sys {
    ...
    #appha_upgrade_preremove_sys($prod,$sys);
...
sub appha_postinstall_sys {
    ...
    if(Cfg::opt('installallpkgs') || Cfg::opt('installrecpkgs')){
        ...
        if(Cfg::opt('upgrade')){
            ...
            #$sys->cmd("_cmd_cp -f $edr->{tmpdir}/backup/appcontrol_config_status.xml /opt/VRTSvcs/portal/world/");
            #$sys->cmd("_cmd_cp -f $prod->{confdir}/*.cf /etc/VRTSvcs/conf/config/");
            #$sys->cmd("_cmd_cp -f /etc/VRTSagents/ha/conf/Oracle/OracleTypes.cf /etc/VRTSvcs/conf/config/");
            $sys->cmd("/opt/VRTSvcs/portal/admin/synchronize_guest_config.pl CreateAppSGMap=Yes");
            #$exitcode = EDR::cmdexit();
            #if ($exitcode) {
            #    $msg = Msg::new("WARNING: synchronize_guest_config.pl failed with exit code $exitcode.");
            #    $sys->push_warning($msg);
            #}
        }
    }
...
sub appha_upgrade_preremove_sys{
    my ($prod, $sys) = @_;
    if(Cfg::opt('installallpkgs') || Cfg::opt('installrecpkgs')){
        my $edr = Obj::edr();
        $sys->cmd("_cmd_mkdir -p $edr->{tmpdir}/backup");
        $sys->cmd("_cmd_mkdir -p $edr->{tmpdir}/backup/config");
        $sys->cmd("_cmd_cp -f /opt/VRTSvcs/portal/admin/plugins/unix/conf/settings.conf $edr->{tmpdir}/backup/");
        $sys->cmd("_cmd_cp -f /opt/VRTSvcs/portal/world/appcontrol_config_status.xml $edr->{tmpdir}/backup/");
...
sub cluster_systems {
    ...
    if($syscfg){
        for my $sys (@systems) {
            my $sysi = transform_system_name($sys->{sys});
            push @sysis,$sysi;
        }
        for my $sysi(@$syscfg){
            next if (EDRu::inarr($sysi,@sysis));
            push @systems,$sysi;
        }
    }
    return \@systems;
...
sub init_plat {
    my $prod=shift;
    $prod->{allpkgs}=[ qw(VRTSllt60 VRTSgab60 VRTSvxfen60 VRTSamf60 VRTSvcs60 VRTScps60 VRTSvcsag60 VRTSvcsdr60 VRTSvcsea60 VRTSsfmh41 VRTSvbs60 VRTSvcsvmw60 VRTSacclib52 VRTSmq651 VRTSsapnw0450) ];
    $prod->{minpkgs}=[ qw(VRTSllt60 VRTSgab60 VRTSvxfen60 VRTSamf60 VRTSvcs60 VRTSvcsag60) ];
    $prod->{recpkgs}=[ qw(VRTSllt60 VRTSgab60 VRTSvxfen60 VRTSamf60 VRTSvcs60 VRTSvcsag60 VRTSvcsdr60 VRTSvcsea60 VRTSsfmh41 VRTSvbs60 VRTSvcsvmw60 VRTSacclib52 VRTSmq651 VRTSsapnw0450) ];
    $prod->{installonupgradepkgs} = [ qw(VRTSamf VRTSsfmh VRTSvbs VRTSvcsvmw VRTSacclib VRTSmq VRTSsapnw04) ];
